...
const D = () => {
  ...
  return {
    ...
    roll掉落： (gd && typeof gd.rollDrop === 'function') ? gd.rollDrop : rollDrop,
...
function load() {
  ...
  if (loaded) {
    ...
    log('✨ 版本更新載入完成！你的進度與物品已完整保留。', 'rarity-legendary');
    if (state.godMode) {
      log('🛡️ [管理員提醒] 此存檔已啟用無敵模式！在聊天輸入 //god 可停用。', 'warning');
    }
...
function resetSave() {
  if (confirm('確定要完全重置角色嗎？所有進度都會歸零，之後可重新選擇種族與職業。')) {
    managerResetState();
    ...
    if (!candidates.length) {
      container.innerHTML = `
        <div style="padding:24px; text-align:center; color:var(--text-muted); font-size:13px; background:rgba(0,0,0,0.4); border:1px solid rgba(212,175,55,0.2); border-radius:8px;">
          ⚠️ 無a opção de evolução disponível para <strong>${currentClassDef?.name || state.class}</strong> na etapa ${targetStage}.
        </div>
      `;
      return;
    }
    ...
    container.innerHTML = `
      <div style="display:flex; align-items:center; gap:14px; background:linear-gradient(135deg, rgba(32,24,18,0.95), rgba(15,12,8,0.98)); border:1px solid var(--border-gilt); border-radius:10px; padding:12px 16px; margin-bottom:12px; box-shadow:inset 0 1px 0 rgba(255,255,255,0.06);">
        ...
        <div style="flex:1; min-width:0;">
          ...
          <p style="margin:3px 0 0 0; font-size:11px; color:#cbd5e1; line-height:1.4;">${clsDef.desc || '亞丁全新高階職業。'}</p>
        </div>
      </div>
      <div style="background:rgba(212,167,68,0.08); border:1px solid rgba(212,167,68,0.25); border-radius:8px; padding:10px 14px; margin-bottom:14px; font-size:12px; color:#e2e8f0; line-height:1.45;">
        <div style="font-weight:bold; color:#fde047; margin-bottom:4px; display:flex; align-items:center; gap:6px;">
          <span>✨</span> <span>最多選擇 2 個技能，轉化為永久血統被動（20% 效果）：</span>
        </div>
        <div style="font-size:11px; color:#94a3b8;">🔄 前一職業的主動技能將被清除，已投入的 SP 會 100% 返還，供新職業重新配置。</div>
      </div>
      ...
      <div style="font-size:11px; color:#a1a1aa; text-align:center; margin-bottom:12px; line-height:1.4;">
        📖 <em>提醒：二轉技能（等級 40+）需要魔法書（1★–4★）與 SP 才能解鎖。</em>
      </div>
...
function depositToWarehouse(uid, amount = 1) {
  ...
  log(`📦 已將 ${formattedName} 存入倉庫。`, 'loot');
  hideItemTooltip();
  updateInventoryUI();
  updateWarehouseUI();
  updateAllUI(); save();
...
function salvageItem(uid) {
  ...
  if (item.equipped) { log('分解前請先卸下物品！', 'system'); return; }
  ...
  if (isHighValueItem(item)) {
    ...
    if (!confirm(`⚠️ 確定要分解高價值物品「${def.name}」[${rarityName}] 嗎？`)) {
      return;
    }
  }
...
function sellSelectedItems() {
  const set = getSelectedSet();
  if (set.size === 0) { log('沒有選取要出售的物品。', 'system'); return; }
  ...
  if (hasHighValue) {
    if (!confirm(`⚠️ 選取內容包含稀有以上物品，確定要出售嗎？`)) {
      return;
    }
  }
  ...
  log(`💰 已出售 ${count} 件選取物品，獲得 ${totalGold.toLocaleString()}g！`, 'loot');
  updateAllUI();
  save();
}
...
function salvageSelectedItems() {
  const set = getSelectedSet();
  if (set.size === 0) { log('沒有選取要分解的物品。', 'system'); return; }
  const toDelete = Array.from(set);
  ...
  if (hasHighValue) {
    if (!confirm(`⚠️ 選取內容包含稀有以上物品，確定要分解嗎？`)) {
      return;
    }
  }
  ...
  if (count > 0) {
    log(`🔨 已分解 ${count} 件裝備，獲得：${summaryStr || '材料'}！`, 'loot');
  } else {
    log('沒有選取可分解的有效裝備。', 'system');
  }
  updateAllUI();
  save();
}
...
function crystallizeSelectedItems() {
  ...
  if (itemsToCrystallize.length === 0) {
    log('沒有可進行結晶化的 D～S 級裝備。', 'system');
    return;
  }

  const hasHighValue = itemsToCrystallize.some(i => isHighValueItem(i));
  if (hasHighValue) {
    if (!confirm(`💎 已選取 ${itemsToCrystallize.length} 件裝備，其中包含稀有以上物品。確定要將它們結晶化為元素水晶嗎？`)) {
      return;
    }
  }
  ...
  if (count > 0) {
    log(`💎 已成功結晶化 ${count} 件裝備，獲得：**${summaryStr}**！`, 'rarity-legendary');
...
function useItem(uid) {
  ...
  // ── Buff Potions ───────────────────────────────────────────────────────────
    ...
    log(`🛡️ 使用 ${def.name}：1 小時內 +25% 防禦、+20% 魔法抗性`, 'heal');
  ...
  // ── Boosts ─────────────────────────────────────────────────────────────────
    ...
    log(`💰 使用 ${def.name}：${fmtDur(dur)} 內金幣 +${Math.round(pct*100)}%`, 'loot');
  ...
  // ── Elixirs ───────────────────────────────────────────────────────────────
    ...
    log(`🧪 使用 ${def.name}：1 小時內金幣 +25%、幸運 +25%`, 'loot');
    ...
    log(`🧪 使用 ${def.name}：1 小時內經驗值 +30%、金幣 +30%！`, 'heal');
    ...
    log(`🎒 背包擴充完成！永久增加 30 格（最大容量：${totalSlots} 格）！`, 'loot');
    ...
    if (typeof floatText === 'function') floatText('🌟 VIP 30 DIAS 啟用中DO!', 'float-jackpot');
  } else if (item.itemId === 'scroll_blessed_weapon' || item.itemId === 'scroll_blessed_armor') {
    const isWpn = item.itemId.includes('weapon');
    log(`📜 已選擇祝福${isWpn ? '武器' : '防具'}強化卷軸！正在開啟鍛造介面，提供完全保護（不會因失敗損失裝備）。`, 'system');
    ...
    log(`👑 稱號 [${titleName}] 已套用至角色資料與聊天。`, 'system');
    updateAllUI(); save();
    return;
...
window.executeRaceClassChange = (scrollUid, newRace, newClass) => {
  ...
  updateAllUI();
  save();
  log(`✨ 種族與職業已成功變更為 ${(raceObj.name || newRace).toUpperCase()} ${(clsObj?.name || newClass).toUpperCase()}！已返還 ${refundedSp} SP，裝備已安全放回背包。`, 'rarity-legendary');
};
...
window.onCharacterCreated = (data) => {
  ...
  log(`🎉 角色建立成功：${data.charName || state.charName}（${data.race} - ${data.className}）！已裝備無等級新手套裝！`, 'rarity-legendary');
};
...
export function sellItem(uid) {
  ...
  if (idx < 0) return;
  const item = state.inventory[idx];
  if (item.equipped) { log('出售前請先卸下物品。', 'system'); return; }
  const def = D()?.ALL_ITEMS?.[item.itemId];
  if (!def) return;

  if (isHighValueItem(item)) {
    const rarityName = D().RARITY?.[item.rarity]?.name || item.rarity;
    if (!confirm(`⚠️ 確定要出售高價值物品「${def.name}」[${rarityName}] 嗎？`)) return;
  }
...
function renderShopClass(list) {
  ...
  hdr.innerHTML = `<h4>🎖️ ${clsName} 專屬裝備與階級晉升</h4><p>大師級裝備與所屬階級徽章。</p>`;
...
function renderShopMystic(list) {
  ...
  hdr.innerHTML = `<h4>✦ 神秘遺物與寶藏 ✦</h4><p>稀有商品與古代強化物品。刷新倒數： <span id="mystic-timer">${fmtCountdown(rot[0]?.msLeft || 0)}</span></p>`;
  list.appendChild(hdr);
...
function toggleSoulshot() {
  ...
  log(`靈魂彈${state.soulshotActive ? '已啟用（消耗靈魂彈以獲得 +100% 傷害）' : '已停用'}。`, 'system');
  save();
}
...
function toggleAutoPotion() {
  ...
  log(`Auto-Poções ${state.autoPotionActive ? `啟用中DAS (Gatilhos: HP < ${hpPct}%, MP < ${mpPct}%)` : 'DES啟用中DAS'}.`, 'system');
  save();
}
...
function toggleCombatState() {
  ...
  log(`自動狩獵**${isActive ? '已恢復 ▶️' : '已暫停 🛑'}**。`, 'system');
  if (typeof window !== 'undefined' && window.floatText) {
    window.floatText(isActive ? '▶️ 已恢復狩獵' : '🛑 狩獵已暫停', 'float-gold');
  }
  save();
}
...
function checkOfflineProgress(lastTime) {
  ...
  if (isReturnPlayer) {
    ...
    log('👑 **[回歸獎勵]** 歡迎回到亞丁！已獲得 2 小時 +50% 經驗祝福與帝國補給！', 'rarity-legendary');
  }
  ...
  if (rewardsEl && modalEl) {
    rewardsEl.innerHTML = `
      ...
      ${fishOfflineResult && fishOfflineResult.totalCaught > 0 ? `
        <div style="color:#38bdf8; margin-top:4px; font-weight:bold;">🎣 離線釣魚收穫： <strong>+${fishOfflineResult.totalCaught} 條魚 (+${fishOfflineResult.xpGained} 釣魚經驗)</strong></div>
      ` : ''}
      ${huntOfflineResult && huntOfflineResult.actualHunts > 0 ? `
        <div style="color:#34d399; margin-top:4px; font-weight:bold;">🐾 離線狩獵毛皮： <strong>+${huntOfflineResult.actualHunts} 獵物 (+${huntOfflineResult.totalXp} 狩獵經驗)</strong></div>
      ` : ''}
      ${gatherOfflineResult && gatherOfflineResult.actualHarvests > 0 ? `
        <div style="color:#a3e635; margin-top:4px; font-weight:bold;">🌿 離線採集藥草與木材： <strong>+${gatherOfflineResult.actualHarvests} 次採集 (+${gatherOfflineResult.totalXp} 採集經驗)</strong></div>
      ` : ''}
      ${mineOfflineResult && mineOfflineResult.actualMines > 0 ? `
        <div style="color:#fbbf24; margin-top:4px; font-weight:bold;">⛏️ 離線採礦礦石與寶石： <strong>+${mineOfflineResult.actualMines} 次開採 (+${mineOfflineResult.totalXp} 採礦經驗)</strong></div>
      ` : ''}
      ${isReturnPlayer ? `
...
function updateEnchantUI() {
  ...
  for (const ws of wsList) {
    ...
    for (const item of equippable) {
      ...
      const safeMsg = enchant < safeLimit ? `100% 安全（最高 +${safeLimit}）` : `成功率：${Math.round(baseProb * 100)}%（${grade} 級）`;
      ...
      card.innerHTML = `
        ...
        <div class="enchant-card-actions" style="display:flex; gap:6px; align-items:center;">
          ...
          <button class="item-action blessed-btn" data-enchant="${item.uid}" data-blessed="true" style="background:linear-gradient(135deg, #7e22ce, #b45309); color:#fff; border:1px solid #f59e0b; font-weight:bold;" ${blessedCount < 1 ? 'disabled title="沒有祝福強化卷軸"' : ''}>✨ Blessed</button>
        </div>
      `;
      ws.appendChild(card);
    }
  ...
  container.innerHTML = `
    ...
    <div class="z-card-body">
      <div class="z-card-sec">
        ...
        <div class="z-mon-list">${monsterHtml || '<p class="z-empty">無 monstro registrado.</p>'}</div>
      </div>
  ...
  window.addEventListener('DOMContentLoaded', () => {
    ...
    window.installPwaApp = async () => {
      if (deferredPrompt) {
        ...
        alert('在手機安裝亞丁競技場：\n\n• Chrome（Android）：點右上角三點選單，選擇「安裝應用程式」\n• Safari（iOS）：點「分享」，再選擇「加入主畫面」📲');
      }
    };
  });
}
...
function updateQuestsUI() {
        ...
        <div style="display:flex; align-items:center; gap:12px;">
          ...
          <div>
            ...
            <div style="font-size:11px; color:#94a3b8; margin:2px 0 4px 0;">
              完成目前的每日狩獵 (${dailyClaimedCount}/${availableDaily.length}) 即可領取終極寶藏。
            </div>
            <div style="font-size:11px; color:#86efac; font-weight:bold;">
              ✦ +500 SP · 💰 +50,000g · 🪔 +2 魔法神燈 · 🎫 +250 通行證經驗
            </div>
          </div>
        </div>
        <div>
          ...
          ` : `
            <button id="claim-grand-daily-btn" class="action-btn action-btn--primary" style="padding:8px 18px; font-weight:bold; font-size:12px; ${allDailyDone ? 'background:linear-gradient(180deg,#d4a744,#8a641c); border:1px solid #ffe699; color:#000; box-shadow:0 0 12px rgba(234,179,8,0.5); cursor:pointer;' : 'opacity:0.5; cursor:not-allowed;'}" ${!allDailyDone ? 'disabled' : ''}>
              ${allDailyDone ? '🎁 Resgatar Baú (+500 SP)' : `${dailyClaimedCount}/${availableDaily.length} 每日`}
            </button>
          `}
        </div>
      </div>
  ...
  if (weeklyContainer) {
    ...
    weeklyContainer.innerHTML = QUEST_DEFS.weekly.map(q => {
      ...
      if (q.reward.gold) rewardsText.push(`💰 +${q.reward.gold.toLocaleString()}g`);
      if (q.reward.sp) rewardsText.push(`✦ +${q.reward.sp} SP`);
      if (q.reward.magicLamps) rewardsText.push(`🪔 +${q.reward.magicLamps} 魔法神燈s`);
      if (q.reward.passXp) rewardsText.push(`🎫 +${q.reward.passXp} XP Passe`);
...
function renderBattlePassUI() {
  ...
  if (statusText) statusText.textContent = state.battlePass.unlockedPremium ? '👑 高級通行證已啟用' : '免費戰鬥通行證';
...
function updateTowerUI() {
  ...
  if (bonusText) bonusText.textContent = `啟用中的被動加成：+${highest}% 攻擊、防禦與魔攻`;
...
function completeFateWhisperQuest() {
  ...
  if (activeMainLevel < 52) {
    log('⚠️ 主職業需要等級 52 才能完成「命運的低語」任務！', 'warning');
    return false;
  }
...
function selectDivineTransformationModal() {
  ...
  log(`👼 TRANSFORMAÇÃO DIVINA **${transList[idx].name.toUpperCase()}** ${state.activeTransformation ? '啟用中DA' : 'DES啟用中DA'}!`, 'rarity-legendary');
...
export function renderSubclassesUI() {
  ...
  mainCard.innerHTML = `
    <div>
      ...
      <div style="font-size:11px; color:var(--text-muted); margin-top:2px;">主要來源 — 所有副職業認證的永久加成都會累積在這裡。</div>
    </div>
  ...
  if (summaryEl) {
    const activeTransStr = state.activeTransformation ? `<div style="margin-top:4px; color:#fde047; font-weight:bold;">👼 神聖變身啟用中： ${state.activeTransformation.toUpperCase()}</div>` : '';
    
    if (certBonuses.totalCertCount === 0) {
      summaryEl.innerHTML = `目前尚未學習任何認證。將副職業提升至 65、70、75、80 級即可累積永久加成！`;
    } else {
      const parts = [];
...
function openCertificationModal(subId, milestoneKey) {
  ...
  body.innerHTML = `
    <div style="margin-bottom:14px;">
      ...
      <p style="margin:4px 0 0 0; font-size:11px; color:var(--text-muted);">副職業： <strong>${subClassDef?.name || sub.classId}</strong> (原型： <span style="text-transform:uppercase; color:#a855f7;">${archetype}</span>)</p>
    </div>
  ...
  if (learnedDivines.length === 0) {
    body.innerHTML = `
      <div style="text-align:center; padding:20px 10px;">
        ...
        <h3 style="color:#fde047; margin:0 0 6px 0;">無a Transformação Divina Desbloqueada</h3>
    ...
    body.innerHTML = `
      <div style="margin-bottom:14px;">
        <h3 style="margin:0; color:#fde047; font-family:'Cinzel',serif; font-size:16px;">👼 可用的神聖變身</h3>
        <p style="margin:4px 0 0 0; font-size:11px; color:var(--text-muted);">啟用神聖形態，在戰鬥與攻城中獲得強力加成。</p>
      </div>
    ...
    classOptionsHtml += `
      <div style="background:rgba(0,0,0,0.4); border:1px solid rgba(255,255,255,0.1); border-radius:8px; padding:10px; display:flex; justify-content:space-between; align-items:center; gap:8px;">
        <div>
          ...
          <div style="font-size:10px; color:#a855f7; text-transform:uppercase;">原型： ${arch}</div>
        </div>
...
export function switchSubclass(targetIndex) {
  ...
  if (resolvedTarget === null) {
    ...
    log(`👑 已切換回主職業（**${getClass(state.class).name}**）！`, 'system');
  } else {
...
function processMonsterDefeat(monster, killingSkill = null) {
  ...
  if (overhitBonusPct > 0) {
    log(`💥 **過量擊殺！** 使用 **${killingSkill.def?.name || killingSkill.name}** 給予致命一擊！獲得 **+${overhitBonusPct}% 經驗／SP** 加成！`, 'rarity-legendary', 'gold_xp');
  ...
  try {
    ...
    console.warn('登錄社群等級上限目標時發生錯誤：', e);
  }
  ...
  if (state.magicLampExp >= 50000) {
    ...
    log(`🪔 累積新的魔法神燈！（總數：${state.magicLamps}）`, 'rarity-legendary');
    if (typeof window !== 'undefined' && window.floatText) {
  ...
  if (gapMods.isGrey) {
    log(`⚠️ [等級懲罰] 怪物等級過低（${gapMods.reason}），未獲得金幣。`, 'warning', 'gold_xp');
  } else if (jackpot) { 
...
export function attackMonster() {
  ...
  if (state.towerCombatActive) {
    ...
    if (elapsed > 60000) {
      ...
      log('⏱️ 副本時間已到（60 秒）！高塔挑戰失敗！', 'warning');
      if (typeof window !== 'undefined' && window.floatText) window.floatText('⏱️ 時間到！', 'float-warning');
      state.activeMonster = null;
  ...
  if (monster._bleedTicks && monster._bleedTicks > 0) {
    ...
    monster.hp -= bDmg;
    log(`🩸 深度出血：${monster.name} 持續受到 ${bDmg} 點傷害！`, 'combat');
  ...
  for(const skill of activeSkills) {
    ...
    if (isBuff) {
        ...
        log(`🗣 ${skill.def.name}! ${skill.def.info || '增益持續 60 秒'}`, 'rarity-rare');
  ...
  if (stats.hasteProc && Math.random() < 0.06) {
    ...
    log(`⚡ **[副職業] 反擊加速啟動！** 10 秒內攻速 +32%！`, 'rarity-legendary');
  ...
  if (stats.critProc && Math.random() < 0.06) {
    ...
    log(`💥 **[副職業] 暴擊機率啟動！** 10 秒內暴擊率 +35、暴擊傷害 +10%！`, 'rarity-legendary');
...
function monsterAttack(monster) {
  ...
  if (monster.skill && (!monster._skillCooldownUntil || monster._skillCooldownUntil <= now)) {
    if (Math.random() < 0.40 || monster.boss || monster.elite) {
      ...
      if (sk.effect === 'stun') {
        ...
        log(`🌿 **${monster.name}** 使用 [${skillName}]，使你陷入纏繞！`, 'warning');
  ...
  if (aiAttack.manaBurnAmt > 0 && state.mp > 0) {
    ...
    log(`🔥 [魔力燃燒] **${monster.name}** 吸取了你 ${aiAttack.manaBurnAmt} MP！`, 'warning');
    stageFloat(`-${aiAttack.manaBurnAmt} MP`, 'sf-hurt', 'left');
  }
  if (aiAttack.appliedDebuff) {
    log(`🔮 [詛咒] **${monster.name}** 施放 ${aiAttack.appliedDebuff.name}！`, 'warning');
  ...
  if (targetMinCp > 0 && currentCp < targetMinCp) {
    ...
    isCrushingHit = true;
    log(`⚠️ [裝備落後] **${monster.name}** 施展重擊！造成 **${damage}** 點傷害（${Math.round(crushMult * 100)}% 衝擊）！`, 'warning');
    if (typeof stageFloat === 'function') stageFloat('CRUSHING!', 'sf-crit', 'left');
  }

  if (state.godMode) {
    log(`🛡️ [GOD MODE 啟用中DO] ${monster.name} causaria ${damage} de dano (anulado por Invencibilidade de Admin)! Digite //god para desativar.`, 'warning');
    damage = 0;
  }
  ...
  if (damage > 0) {
    ...
    if (stats.celestial && Math.random() < 0.05) {
      ...
      log('🌟 **[副職業] 反擊屏障啟動！** 5 秒內完全免疫！', 'rarity-legendary');
    ...
    if (stats.defenceProc && Math.random() < 0.06) {
      ...
      log('🛡️ **[副職業] 反擊防禦啟動！** 10 秒內物防與魔防 +25%！', 'rarity-epic');
    ...
    if (stats.spiritProc && Math.random() < 0.05) {
      ...
      log('👻 **[副職業] 反擊之魂啟動！** 10 秒內物攻、魔攻與攻速 +10%！', 'rarity-epic');
    ...
    if (isSkillCast && skillName) {
      log(`⚡ **${monster.name}** 使用 [${skillName}] 命中你，造成 **${damage}** 點傷害！`, 'combat');
    } else if (!isCrushingHit) {
      log(`${monster.name} 對你造成 ${damage} 點傷害`, 'damage');
    }
    ...
    if (aiAttack.vampiricHeal > 0) {
      ...
      log(`🦇 [吸血] **${monster.name}** 從你身上吸取 ${vHeal} 生命值！`, 'warning');
...
function applyAdminLevelChange(targetLevel) {
  ...
  playSfx('levelUp');
  log(`⚡ [管理員] 等級已改為 ${newLvl}！SP（+${cumulativeSp + 1000}）、HP/MP、史詩任務、狩獵地圖與技能已同步。`, 'rarity-legendary');
...
function handleChatSubmit(inputStr) {
  ...
  if (lower === '//god') {
    ...
    log(`🛡️ [Admin] God Mode (Invencibilidade): ${state.godMode ? '啟用中DO' : 'DES啟用中DO'}`, 'rarity-legendary');
...
function applyServerRatePreset(presetKey) {
  ...
  if (typeof floatText === 'function') floatText(`🚀 PRESET ${presetKey.toUpperCase()} 啟用中DO!`, 'float-jackpot');
...
function populateAdminItemSelect(query = '') {
  ...
  if (list.length === 0) {
    ...
    opt.value = '';
    opt.textContent = `⚠️ 無 item encontrado para "${query}"`;
    opt.disabled = true;
    sel.appendChild(opt);
    return;
  }
...
function addAdminXP(amount) {
  ...
  updateSkillUI();
  updateRaceClassUI();
  log(`🌟 [管理員] 已給予 +${amt.toLocaleString()} 經驗值！（目前等級：${state.level}）`, 'rarity-legendary');
...
function adminCompleteQuest() {
  ...
  if (state.quests && state.quests.length > 0) {
    ...
    log('✅ [管理員] 所有進行中的任務已完成！', 'rarity-legendary');
    floatText('✅ 任務完成', 'float-jackpot');
    updateQuestUI();
    updateAllUI();
    save(true, true);
  } else {
    log('無a missão ativa encontrada para concluir.', 'system');
  }
}
...
function adminMaxCraft() {
  ...
  state.craftPoints = 0;
  log('⚒️ [管理員] 鍛造升至最高等級（50）並獲得 100 次隨機製作能量！', 'rarity-legendary');
  floatText('⚒️ 製作滿級', 'float-jackpot');
  updateAllUI();
  save(true, true);
}
...
function adminKillMonster() {
  ...
  if (monster) {
    ...
    log('無 monstro em combate ativo.', 'system');
  }
}
...
function executeAdminCmd(cmd) {
  ...
  if (cmd === 'setseason1') { setServerSeason(1); }
    ...
    state.godMode = !state.godMode; 
    log(`🛡️ [Admin] Invencibilidade: ${state.godMode ? '啟用中DO' : 'DES啟用中DO'}!`, 'rarity-legendary'); 
    if (typeof floatText === 'function') floatText(`🛡️ GOD MODE: ${state.godMode ? 'ON' : 'OFF'}`, 'float-jackpot');
  }
...
function autoEquipBest() {
  ...
  if (res.success) {
    log(`⚡ 自動裝備：已更新 ${res.appliedChanges} 件裝備！（CP：${proposal.deltas.cpDelta >= 0 ? '+' : ''}${proposal.deltas.cpDelta}）`, 'rarity-legendary');
    updateAllUI();
    save();
  } else {
...
function unequipAll() {
  ...
  if (count > 0) {
    ...
    log('無 item equipado para remover.', 'system');
  }
}
...
function renderMonsterCardsCodex(container, summaryEl) {
    ...
    cardBox.innerHTML = `
      <div>
        ...
        <div style="background:rgba(0,0,0,0.4); border:1px solid rgba(255,255,255,0.08); padding:8px 10px; border-radius:6px; margin:10px 0 8px 0; font-size:11px;">
          <div style="display:flex; justify-content:space-between; margin-bottom:2px;">
            <span style="color:${rStyle.text}; font-weight:bold;">帳號被動加成：</span>
        ...
        <div style="margin-bottom:8px;">
          <div style="display:flex; justify-content:space-between; font-size:11px; margin-bottom:3px;">
            <span style="color:${isAbsorbed ? '#86efac' : '#94a3b8'}; font-weight:bold;">
              ${isAbsorbed ? `⭐ Rank ${current.rank}/5 (${current.count} absorvidas)` : '⚪ 尚未吸收'}
            </span>
            <span style="font-size:10px; color:#94a3b8;">
      ...
      <div style="display:flex; justify-content:space-between; align-items:center; border-top:1px solid rgba(255,255,255,0.08); padding-top:8px; margin-top:4px;">
        <span style="font-size:11px; color:${invCount > 0 ? '#86efac' : '#64748b'}; font-weight:bold;">
          ${invCount > 0 ? `📦 Possui: ${invCount}x` : '背包中沒有卡片'}
        </span>
        <div style="display:flex; gap:6px;">
          ${invCount > 0 ? `
  ...
  if (matchingCardsCount === 0) {
    ...
    emptyBox.textContent = '無a carta de monstro encontrada para os filtros selecionados.';
  ...
  if (summaryEl) {
    summaryEl.innerHTML = `<span style="color:var(--gilt-bright); font-weight:bold;">卡片圖鑑： ${absorbedCards}/${totalCards} 張卡片已吸收</span> · 帳號永久加成已啟用`;
  }
}
            ...
            <div style="display:flex; align-items:center; gap:10px;">
              ...
              <div>
                <h4 style="margin:0; font-family:'Cinzel',serif; color:${isUnlocked ? '#f4d58a' : '#aaa'}; font-size:14px; display:flex; align-items:center; gap:6px;">
                  ${def.name} ${isUnlocked ? `<span style="font-size:10px; background:rgba(52,211,153,0.2); border:1px solid #34d399; color:#34d399; padding:1px 6px; border-radius:4px;">啟用等級：Lv.${maxOwnedLvl}</span>` : '<span style="font-size:10px; background:rgba(239,68,68,0.2); border:1px solid #ef4444; color:#fca5a5; padding:1px 6px; border-radius:4px;">🔒 未鎖定</span>'}
                </h4>
...
function updateMagicLampUI() {
  ...
  if (countLabel) {
    countLabel.textContent = `${state.magicLamps || 0} 盞魔法神燈可用 (${pct}% 距離下一盞)`;
  }
...
function depositAllToWarehouse() {
  ...
  if (unequipped.length === 0) {
    log('無 item desequipado na mochila para guardar.', 'system');
    return;
  }
  let movedCount = 0;
...
function depositSelectedToWarehouse() {
  ...
  if (!selectedSet || selectedSet.size === 0) {
    log('無 item selecionado na mochila. Marque os itens para guardar.', 'system');
    return;
  }
  let movedCount = 0;
...
function depositMaterialsToWarehouse() {
  ...
  if (unequipped.length === 0) {
    log('無 material ou consumível desequipado para guardar.', 'system');
    return;
  }
  let movedCount = 0;
  for (const item of [...unequipped]) {
...
function withdrawSelectedFromWarehouse() {
  const selectedSet = getSelectedSet(state);
  if (!selectedSet || selectedSet.size === 0) {
    log('無 item selecionado no Baú. Marque os itens para retirar.', 'system');
    return;
  }
  let movedCount = 0;
  for (const uid of Array.from(selectedSet)) {
  ...
  if (movedCount > 0) {
    log(`🎒 已從倉庫取出 ${movedCount} 件選取物品。`, 'loot');
    updateAllUI(); save();
  }
}
...
function reincarnateHero() {
  ...
  log(`✨ 古代轉生完成！聲望等級 ${state.prestigeLevel}（${title}），獲得 +${earnedShards} 星界碎片！`, 'rarity-legendary');
...
const CASTLES_DEFS = {
  ...
  giran: { id: 'giran', name: 'Castelo de Giran', reqLevel: 50, taxPerHour: 15000, desc: '+15,000 金幣／小時 & 商店 5% 折扣', enemyName: 'Guarda de Giran (Lv. 50)' },
  goddard: { id: 'goddard', name: 'Castelo de Goddard', reqLevel: 70, taxPerHour: 35000, desc: '+35,000 金幣／小時 & +5% 經驗值', enemyName: 'Guarda de Goddard (Lv. 70)' },
  aden: { id: 'aden', name: 'Castelo Imperial de Aden', reqLevel: 85, taxPerHour: 75000, desc: '+75,000 金幣／小時 & +10% 總傷害', enemyName: 'Guarda Imperial de Aden (Lv. 85)' }
};
...
function buyManorSeed(seedId, qty = 1) {
  ...
  log(`🌾 購買了 ${count}x ${seed.name} 種子！`, 'loot');
  updateAllUI();
  save();
  return true;
}
...
function exchangeManorCrop(seedId, rewardOption = 1) {
  ...
  if (ownedCrops <= 0) {
    log(`⚠️ 你沒有可交付的 ${seed.name} 收成！`, 'warning');
    return false;
  }
  ...
  if (matAmount < 1) {
    log(`⚠️ 收成不足！至少需要 ${ratio} 份收成才能兌換 1 份材料。`, 'warning');
    return false;
  }
...
function conquerCastle(castleId) {
  ...
  if (state.castles[castleId]?.conquered) {
    log(`⚠️ 你已經控制 ${castle.name}！`, 'warning');
    return false;
  }
  ...
  log(`🏰 已攻下 ${castle.name.toUpperCase()}！加成已啟用：${castle.desc}。`, 'rarity-legendary');
...
function startExpedition(destId) {
  ...
  if (activeExp) {
    log(`⚠️ ${dest.name} 已有一個進行中的遠征！`, 'warning');
    return false;
  }

  if ((state.gold || 0) < dest.cost) {
    log(`⚠️ 金幣不足，無法整備遠征！需要 ${dest.cost.toLocaleString()}g。`, 'warning');
    return false;
  }
...
function fuseSoulCrystals(color = 'red', stage = 1) {
  ...
  if (owned < 2) {
    log(`⚠️ 至少需要 2 顆相同階段的靈魂水晶才能融合！`, 'warning');
    return false;
  }
  ...
  log(`✨ 合成成功！靈魂水晶提升至階段 ${stage + 1}！`, 'rarity-legendary');
  updateAllUI();
  save();
  return true;
}
...
function socketSoulCrystalToWeapon(effect = 'focus', stage = 1) {
  ...
  floatText(`SA ${effect.toUpperCase()} 啟用中DO`, 'float-gold');
...
function upgradeItemToMasterwork(itemUid) {
  ...
  if ((state.gold || 0) < req.adena) {
    log(`⚠️ 金幣不足，無法向普希金大師進行操作！需要 ${req.adena.toLocaleString()}g。`, 'warning');
    return false;
  }
...
function applyTattoo(plusStat = 'str', minusStat = 'con', val = 4) {
  ...
  if ((state.gold || 0) < cost) {
    log(`⚠️ 金幣不足，無法套用染料紋身！需要 ${cost.toLocaleString()}g。`, 'warning');
    return false;
  }
...
function addSkillCharge() {
  ...
  log(`⚡ 技能蓄力：**等級 ${state.charges}/8**（+${(state.charges - 1) * 20}% 技能傷害）！`, 'rarity-legendary');
...
function addKamaelSoul() {
  ...
  log(`👻 已吸收闇天使靈魂：**${state.souls}/5**（+${state.souls * 5}% 技能傷害）！`, 'rarity-legendary');
...
function insertAttributeStone(itemUid, elemType = 'fire') {
  ...
  const cost = 250000;
  if ((state.gold || 0) < cost) {
    log(`⚠️ 金幣不足，無法進行元素鑲嵌！需要 ${cost.toLocaleString()}g。`, 'warning');
    return false;
  }
...
function compoundBelts() {
  const cost = 500000;
  if ((state.gold || 0) < cost) {
    log(`⚠️ 金幣不足，無法合成腰帶！需要 ${cost.toLocaleString()}g。`, 'warning');
    return false;
  }
  ...
  if (roll <= 0.70) {
    ...
    log('✨ 腰帶合成成功！打造出祝福頂級腰帶 [S]（+7.2% 防禦／+6% 傷害）！', 'rarity-legendary');
...
function augmentWithLifeStone(itemUid) {
  ...
  const cost = 750000;
  if ((state.gold || 0) < cost) {
    log(`⚠️ 金幣不足，無法進行精煉！需要 ${cost.toLocaleString()}g。`, 'warning');
    return false;
  }
...
function executeCompoundAction(targetUid, ingredientUid) {
  ...
  if ((state.gold || 0) < cost) {
    log(`⚠️ 金幣不足！合成 Lv.${curLv} 需要 ${cost.toLocaleString()}g。`, 'warning');
    return false;
  }
...
function renderDailyRewardModal() {
  ...
  if (statusBadge) {
    if (status.canClaim) {
      ...
      statusBadge.textContent = `✨ 第 ${status.currentDay} 天獎勵可以領取！`;
    } else {
      ...
      statusBadge.textContent = '✓ 今日簽到完成';
    }
  }
...
export function init() {
  try {
        ...
        if (activePet) {
          ...
          activeContainer.innerHTML = `
            <div style="padding:16px; text-align:center; background:rgba(0,0,0,0.4); border:1px dashed rgba(212,167,68,0.3); border-radius:8px; color:#94a3b8; font-size:12px;">
              🐾 無 mascote invocado no momento. Invoque um dos seus companheiros abaixo para lutar ao seu lado!
            </div>
          `;
        }
      }
    ...
    window.submitReferralCodeAction = async () => {
      ...
      if (typeof window !== 'undefined' && window.FirebaseBridge?.getPlayerByName) {
        try {
          ...
          if (!referrerPlayer) {
            log(`⚠️ 在亞丁找不到角色 [${code}]，請確認名稱是否正確。`, 'error');
            return;
          }
      ...
      log(`✨ **綁定成功！** 你的推薦人是 **${code}**！新手獎勵已啟用：**永久 +10% 經驗值** 並獲得 **+1,000 發靈魂彈**！`, 'rarity-legendary');
      if (typeof floatText === 'function') floatText('✨ 推薦綁定已啟用（+10% 經驗值）！', 'float-jackpot');
      updateAllUI();
      save();
      uiOpenReferralModal(state);
    };
    ...
    window.claimReferralRewardsAction = async () => {
      ...
      try {
        ...
        if (result.claimableRewards > 0) {
          ...
          log('ℹ️ 無a recompensa pendente no momento. Quando seus amigos indicados alcançarem o Nível 40, suas 50 AC e 5x Blessed Scrolls estarão disponíveis aqui!', 'info');
          if (typeof floatText === 'function') floatText('無a recompensa pendente', 'float-normal');
        }
      } catch (err) {
      ...
      contentEl.innerHTML = `
        ...
        <div style="background:rgba(0,0,0,0.4); border:1px solid rgba(212,167,68,0.3); border-radius:10px; padding:14px; display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:10px;">
          <div>
            ...
            <div style="font-size:11px; color:#94a3b8;">
              Status atual: <strong>${state.autoRecycle?.enabled ? '🟢 啟用中DO (' + (state.autoRecycle.mode === 'sell' ? 'Auto-Venda' : 'Desmanche em Cristais') + ')' : '⚪ DES啟用中DO'}</strong>. Suporta No-Grade até Grade A com salvaguarda inviolável de itens +enchant e raros.
            </div>
          </div>
    ...
    window.GameData = {
      ...
      roll掉落： (window.GameData && typeof window.GameData.rollDrop === 'function') ? window.GameData.rollDrop : rollDrop,
        ...
        if (typeof window !== 'undefined' && window.location) {
          ...
          if (status === 'approved' || status === 'completed' || payment === 'success' || urlParams.get('unlocked_premium') === '1') {
            if (!state.battlePass?.unlockedPremium) {
              ...
              updateAllUI();
              save();
              log('🎉 付款成功！高級通行證已啟用！', 'rarity-legendary');
            }
          }
        }
      } catch (err) {
    ...
    if (typeof window !== 'undefined') {
      ...
      window.setHuntingDifficulty = (diffId) => {
        ...
        log(`⚡ 狩獵難度已調整為 **${res.difficulty.name}**（${res.difficulty.xpMult}x 經驗／金幣，${res.difficulty.dropMult}x 掉落）！`, 'rarity-epic');
          ...
          <div>
            <div style="display:flex; align-items:center; gap:8px;">
              <span style="font-size:12px; font-weight:bold; background:${status.isActive ? '#ef4444' : '#3b82f6'}; color:#fff; padding:2px 8px; border-radius:4px;">
                ${status.isActive ? '● 入侵目前進行中' : '⏳ 等待甦醒'}
              </span>
              <span style="font-size:13px; color:#cbd5e1; font-weight:bold;">
                ${status.isActive ? '區域剩餘時間：' : '下次突襲：'}
              </span>
            </div>
