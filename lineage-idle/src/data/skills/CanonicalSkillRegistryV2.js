/**
 * CanonicalSkillRegistryV2.js — Single Source of Truth for Skills (Lineage II Essence - Celestial Destiny 3629)
 * 
 * Major Version Update V2 — Fully Scraped from L2Wiki Essence & L2Bandit.
 * Total Canonical Skills: 811 across 46 Lineages
 * Audited: 100%
 */

export const CANONICAL_SKILL_REGISTRY_V2 = Object.freeze({
    "majesty": {
        "id": "majesty",
        "name": "威嚴",
        "slug": "majesty",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0082.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "P. Def. +50Max HP +200P. Evasion -4",
        "canonicalCooldown": "5 sec.",
        "canonicalCooldownMs": 5000,
        "desc": "物理防禦 +50、最大生命值 +200、物理迴避 -4",
        "balance": {
            "mpCost": 36,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_majesty",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "bladeDancer",
            "darkAvenger",
            "elvenKnight",
            "knight",
            "paladin",
            "palusKnight",
            "swordSinger"
        ]
    },
    "haste": {
        "id": "haste",
        "name": "加速術",
        "slug": "haste",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1086.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Haste effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "提升攻擊速度。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_haste",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "abyssWalker",
            "adventurer",
            "arcanaLord",
            "archmage",
            "assassinS1",
            "assassinS2",
            "assassinS3",
            "bishop",
            "bladeDancer",
            "bloodRose",
            "bloodRoseS1",
            "bloodRoseS2",
            "cardinal",
            "cleric",
            "darkAvenger",
            "darkWizard",
            "deathBlade",
            "deathKnight",
            "deathMessenger",
            "divineTemplar",
            "dreadnought",
            "duelist",
            "elementWeaver",
            "elementWeaverS1",
            "elementWeaverS2",
            "elementalMaster",
            "elementalSummoner",
            "elfElder",
            "elfOracle",
            "elfScout",
            "elvenKnight",
            "elvenWizard",
            "evaSaint",
            "evaTemplar",
            "ghostHunter",
            "ghostSentinel",
            "gladiator",
            "hawkeye",
            "hellKnight",
            "hierophant",
            "holyTemplar",
            "knight",
            "lightTemplar",
            "moonlightSentinel",
            "mysticMuse",
            "necromancer",
            "paladin",
            "palusKnight",
            "phantomRanger",
            "phantomSummoner",
            "phoenixKnight",
            "plainsWalker",
            "prophet",
            "rogue",
            "sagittarius",
            "sayhaSeeker",
            "sayhaSeer",
            "shillienElder",
            "shillienKnight",
            "shillienOracle",
            "shillienSaint",
            "shillienTemplar",
            "silverRanger",
            "sorcerer",
            "soultaker",
            "spectralDancer",
            "spectralMaster",
            "spellhowler",
            "spellsinger",
            "stormScreamer",
            "swordMuse",
            "swordSinger",
            "templeKnight",
            "treasureHunter",
            "warg",
            "warlock",
            "warlord",
            "warrior",
            "windRider",
            "windRiderErth",
            "wizard"
        ]
    },
    "acumen": {
        "id": "acumen",
        "name": "靈活思緒",
        "slug": "acumen",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1085.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Acumen effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "提升施法能力。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_acumen",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "abyssWalker",
            "adventurer",
            "arcanaLord",
            "archmage",
            "assassinS1",
            "assassinS2",
            "assassinS3",
            "bishop",
            "bladeDancer",
            "bloodRose",
            "bloodRoseS1",
            "bloodRoseS2",
            "cardinal",
            "cleric",
            "darkAvenger",
            "darkWizard",
            "deathBlade",
            "deathKnight",
            "deathMessenger",
            "divineTemplar",
            "dreadnought",
            "duelist",
            "elementWeaver",
            "elementWeaverS1",
            "elementWeaverS2",
            "elementalMaster",
            "elementalSummoner",
            "elfElder",
            "elfOracle",
            "elfScout",
            "elvenKnight",
            "elvenWizard",
            "evaSaint",
            "evaTemplar",
            "ghostHunter",
            "ghostSentinel",
            "gladiator",
            "hawkeye",
            "hellKnight",
            "hierophant",
            "holyTemplar",
            "knight",
            "lightTemplar",
            "moonlightSentinel",
            "mysticMuse",
            "necromancer",
            "paladin",
            "palusKnight",
            "phantomRanger",
            "phantomSummoner",
            "phoenixKnight",
            "plainsWalker",
            "prophet",
            "rogue",
            "sagittarius",
            "sayhaSeeker",
            "sayhaSeer",
            "shillienElder",
            "shillienKnight",
            "shillienOracle",
            "shillienSaint",
            "shillienTemplar",
            "silverRanger",
            "sorcerer",
            "soultaker",
            "spectralDancer",
            "spectralMaster",
            "spellhowler",
            "spellsinger",
            "stormScreamer",
            "swordMuse",
            "swordSinger",
            "templeKnight",
            "treasureHunter",
            "warg",
            "warlock",
            "warlord",
            "warrior",
            "windRider",
            "windRiderErth",
            "wizard"
        ]
    },
    "hp_recovery": {
        "id": "hp_recovery",
        "name": "生命值恢復",
        "slug": "hp_recovery",
        "type": "passive",
        "rawType": "Passive",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0212.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Recovers HP every 9.9 sec.",
        "canonicalCooldown": "N/A",
        "canonicalCooldownMs": 0,
        "desc": "提高自然生命值恢復速度。",
        "balance": {
            "mpCost": 0,
            "pwr": 0,
            "pveMultiplier": 1,
            "pvpMultiplier": 1
        },
        "vfxId": null,
        "vfxGap": false,
        "sfxId": null,
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "abyssWalker",
            "adventurer",
            "arcanaLord",
            "archmage",
            "artisanDwarf",
            "assassinS1",
            "assassinS2",
            "assassinS3",
            "berserker",
            "bishop",
            "bladeDancer",
            "bloodRose",
            "bloodRoseS1",
            "bloodRoseS2",
            "bountyHunter",
            "cardinal",
            "cleric",
            "darkAvenger",
            "darkWizard",
            "deathBlade",
            "deathKnight",
            "deathMessenger",
            "destroyer",
            "divineTemplar",
            "dominator",
            "doombringer",
            "doomcryer",
            "dragoon",
            "dreadnought",
            "duelist",
            "elementWeaver",
            "elementWeaverS1",
            "elementWeaverS2",
            "elementalMaster",
            "elementalSummoner",
            "elfElder",
            "elfOracle",
            "elfScout",
            "elvenKnight",
            "elvenWizard",
            "ertheiaWarrior",
            "evaSaint",
            "evaTemplar",
            "eviscerator",
            "fortuneSeeker",
            "ghostHunter",
            "ghostSentinel",
            "gladiator",
            "grandKhavatari",
            "grandVanguard",
            "hatamoto",
            "hawkeye",
            "hellKnight",
            "hierophant",
            "holyTemplar",
            "knight",
            "lightTemplar",
            "maestro",
            "marauder",
            "monk",
            "moonlightSentinel",
            "mysticMuse",
            "necromancer",
            "overlord",
            "paladin",
            "palusKnight",
            "phantomRanger",
            "phantomSummoner",
            "phoenixKnight",
            "plainsWalker",
            "prophet",
            "raider",
            "rogue",
            "ronin",
            "sagittarius",
            "samurai",
            "sayhaSeeker",
            "sayhaSeer",
            "scavenger",
            "shaman",
            "sharpshooter",
            "shillienElder",
            "shillienKnight",
            "shillienOracle",
            "shillienSaint",
            "shillienTemplar",
            "shineMakerS1",
            "shineMakerS2",
            "shinemaker",
            "silverRanger",
            "sorcerer",
            "soulBreakerKamael",
            "soulFinder",
            "soulHound",
            "soulRanger",
            "soultaker",
            "spectralDancer",
            "spectralMaster",
            "spellhowler",
            "spellsinger",
            "stormBlaster",
            "stormScreamer",
            "swordMuse",
            "swordSinger",
            "sylphGunner",
            "templeKnight",
            "titan",
            "treasureHunter",
            "trickster",
            "trooper",
            "tyrant",
            "vanguardRider",
            "warcryer",
            "warder",
            "warg",
            "warlock",
            "warlord",
            "warrior",
            "warsmith",
            "windRider",
            "windRiderErth",
            "windSniper",
            "wizard"
        ]
    },
    "berserker_spirit": {
        "id": "berserker_spirit",
        "name": "狂戰士之魂",
        "slug": "berserker_spirit",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1062.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Berserker Spirit effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "激發狂戰士之魂，提升戰鬥能力。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_berserker_spirit",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "abyssWalker",
            "adventurer",
            "arcanaLord",
            "archmage",
            "assassinS1",
            "assassinS2",
            "assassinS3",
            "bishop",
            "bladeDancer",
            "bloodRose",
            "bloodRoseS1",
            "bloodRoseS2",
            "cardinal",
            "cleric",
            "darkAvenger",
            "darkWizard",
            "deathBlade",
            "deathKnight",
            "deathMessenger",
            "divineTemplar",
            "dreadnought",
            "duelist",
            "elementWeaver",
            "elementWeaverS1",
            "elementWeaverS2",
            "elementalMaster",
            "elementalSummoner",
            "elfElder",
            "elfOracle",
            "elfScout",
            "elvenKnight",
            "elvenWizard",
            "evaSaint",
            "evaTemplar",
            "ghostHunter",
            "ghostSentinel",
            "gladiator",
            "hawkeye",
            "hellKnight",
            "hierophant",
            "holyTemplar",
            "knight",
            "lightTemplar",
            "moonlightSentinel",
            "mysticMuse",
            "necromancer",
            "paladin",
            "palusKnight",
            "phantomRanger",
            "phantomSummoner",
            "phoenixKnight",
            "plainsWalker",
            "prophet",
            "rogue",
            "sagittarius",
            "sayhaSeeker",
            "sayhaSeer",
            "shillienElder",
            "shillienKnight",
            "shillienOracle",
            "shillienSaint",
            "shillienTemplar",
            "silverRanger",
            "sorcerer",
            "soultaker",
            "spectralDancer",
            "spectralMaster",
            "spellhowler",
            "spellsinger",
            "stormScreamer",
            "swordMuse",
            "swordSinger",
            "templeKnight",
            "treasureHunter",
            "warg",
            "warlock",
            "warlord",
            "warrior",
            "windRider",
            "windRiderErth",
            "wizard"
        ]
    },
    "wild_magic": {
        "id": "wild_magic",
        "name": "狂野魔法",
        "slug": "wild_magic",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1303.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Wild Magic effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "強化魔法暴擊能力。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_wild_magic",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "abyssWalker",
            "adventurer",
            "arcanaLord",
            "archmage",
            "assassinS1",
            "assassinS2",
            "assassinS3",
            "bishop",
            "bladeDancer",
            "bloodRose",
            "bloodRoseS1",
            "bloodRoseS2",
            "cardinal",
            "cleric",
            "darkAvenger",
            "darkWizard",
            "deathBlade",
            "deathKnight",
            "deathMessenger",
            "divineTemplar",
            "dreadnought",
            "duelist",
            "elementWeaver",
            "elementWeaverS1",
            "elementWeaverS2",
            "elementalMaster",
            "elementalSummoner",
            "elfElder",
            "elfOracle",
            "elfScout",
            "elvenKnight",
            "elvenWizard",
            "evaSaint",
            "evaTemplar",
            "ghostHunter",
            "ghostSentinel",
            "gladiator",
            "hawkeye",
            "hellKnight",
            "hierophant",
            "holyTemplar",
            "knight",
            "lightTemplar",
            "moonlightSentinel",
            "mysticMuse",
            "necromancer",
            "paladin",
            "palusKnight",
            "phantomRanger",
            "phantomSummoner",
            "phoenixKnight",
            "plainsWalker",
            "prophet",
            "rogue",
            "sagittarius",
            "sayhaSeeker",
            "sayhaSeer",
            "shillienElder",
            "shillienKnight",
            "shillienOracle",
            "shillienSaint",
            "shillienTemplar",
            "silverRanger",
            "sorcerer",
            "soultaker",
            "spectralDancer",
            "spectralMaster",
            "spellhowler",
            "spellsinger",
            "stormScreamer",
            "swordMuse",
            "swordSinger",
            "templeKnight",
            "treasureHunter",
            "warg",
            "warlock",
            "warlord",
            "warrior",
            "windRider",
            "windRiderErth",
            "wizard"
        ]
    },
    "shield_stun": {
        "id": "shield_stun",
        "name": "盾牌暈擊",
        "slug": "shield_stun",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0092.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Shield Stun effect",
        "canonicalCooldown": "20 sec.",
        "canonicalCooldownMs": 20000,
        "desc": "以盾牌攻擊並使目標暈眩。",
        "balance": {
            "mpCost": 48,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_shield_stun",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 60,
        "classes": [
            "darkAvenger",
            "elvenKnight",
            "hellKnight",
            "knight",
            "paladin",
            "phoenixKnight"
        ]
    },
    "magic_barrier": {
        "id": "magic_barrier",
        "name": "魔法屏障",
        "slug": "magic_barrier",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1036.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Magic Barrier effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "形成魔法屏障，提高魔法防禦。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_magic_barrier",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "abyssWalker",
            "adventurer",
            "arcanaLord",
            "archmage",
            "assassinS1",
            "assassinS2",
            "assassinS3",
            "bishop",
            "bladeDancer",
            "bloodRose",
            "bloodRoseS1",
            "bloodRoseS2",
            "cardinal",
            "cleric",
            "darkAvenger",
            "darkWizard",
            "deathBlade",
            "deathKnight",
            "deathMessenger",
            "divineTemplar",
            "dreadnought",
            "duelist",
            "elementWeaver",
            "elementWeaverS1",
            "elementWeaverS2",
            "elementalMaster",
            "elementalSummoner",
            "elfElder",
            "elfOracle",
            "elfScout",
            "elvenKnight",
            "elvenWizard",
            "evaSaint",
            "evaTemplar",
            "ghostHunter",
            "ghostSentinel",
            "gladiator",
            "hawkeye",
            "hellKnight",
            "hierophant",
            "holyTemplar",
            "knight",
            "lightTemplar",
            "moonlightSentinel",
            "mysticMuse",
            "necromancer",
            "paladin",
            "palusKnight",
            "phantomRanger",
            "phantomSummoner",
            "phoenixKnight",
            "plainsWalker",
            "prophet",
            "rogue",
            "sagittarius",
            "sayhaSeeker",
            "sayhaSeer",
            "shillienElder",
            "shillienKnight",
            "shillienOracle",
            "shillienSaint",
            "shillienTemplar",
            "silverRanger",
            "sorcerer",
            "soultaker",
            "spectralDancer",
            "spectralMaster",
            "spellhowler",
            "spellsinger",
            "stormScreamer",
            "swordMuse",
            "swordSinger",
            "templeKnight",
            "treasureHunter",
            "warg",
            "warlock",
            "warlord",
            "warrior",
            "windRider",
            "windRiderErth",
            "wizard"
        ]
    },
    "shield_strike": {
        "id": "shield_strike",
        "name": "盾牌打擊",
        "slug": "shield_strike",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill10011.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks the target with a shield provoking them. Power 619.Ignores Shield Defens",
        "canonicalCooldown": "1 sec.",
        "canonicalCooldownMs": 1000,
        "desc": "以盾牌攻擊並挑釁目標。威力 619。無視盾牌防禦，並無視目標 10% 防禦力。",
        "balance": {
            "mpCost": 33,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_shield_strike",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "darkAvenger",
            "elvenKnight",
            "hellKnight",
            "knight",
            "lightTemplar",
            "paladin",
            "palusKnight",
            "phoenixKnight"
        ]
    },
    "holy_strike": {
        "id": "holy_strike",
        "name": "神聖打擊",
        "slug": "holy_strike",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0049.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks the target. Power 638.Deals heavy damage to demons and undead.No critica",
        "canonicalCooldown": "5 sec.",
        "canonicalCooldownMs": 5000,
        "desc": "攻擊目標，威力 638。對惡魔與不死族造成大量傷害，不會暴擊。",
        "balance": {
            "mpCost": 5,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_holy_strike",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "paladin",
            "phoenixKnight"
        ]
    },
    "knight_s_protection": {
        "id": "knight_s_protection",
        "name": "騎士守護",
        "slug": "knight_s_protection",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill3584.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Max HP +300Shield Defense +120",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "最大生命值 +300，盾牌防禦 +120。",
        "balance": {
            "mpCost": 50,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_knight_s_protection",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 50,
        "classes": [
            "darkAvenger",
            "hellKnight",
            "paladin",
            "phoenixKnight"
        ]
    },
    "angelic_archon": {
        "id": "angelic_archon",
        "name": "天使統領",
        "slug": "angelic_archon",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0406.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "You are blessed by the gods.Debuff Resistance +20%Anomaly Resistance +20%Buff Ca",
        "canonicalCooldown": "5 min.",
        "canonicalCooldownMs": 300000,
        "desc": "獲得諸神祝福：弱化抗性 +20%、異常抗性 +20%、增益解除抗性 +20%、受到治療 -40%、物理防禦 +20%、魔法防禦 +20%、速度 +6。使用劍／鈍器時命中 +4、攻速 +10%；使用劍時基礎暴擊率 +20；使用鈍器時基礎暴擊傷害 +10%。",
        "balance": {
            "mpCost": 133,
            "pwr": 2,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_angelic_archon",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 58,
        "classes": [
            "paladin"
        ]
    },
    "mp_recovery": {
        "id": "mp_recovery",
        "name": "魔力恢復",
        "slug": "mp_recovery",
        "type": "passive",
        "rawType": "Passive",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0229.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Recovers MP every 9.9 sec.",
        "canonicalCooldown": "N/A",
        "canonicalCooldownMs": 0,
        "desc": "提高自然魔力恢復速度。",
        "balance": {
            "mpCost": 0,
            "pwr": 0,
            "pveMultiplier": 1,
            "pvpMultiplier": 1
        },
        "vfxId": null,
        "vfxGap": false,
        "sfxId": null,
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "abyssWalker",
            "adventurer",
            "arcanaLord",
            "archmage",
            "artisanDwarf",
            "assassinS1",
            "assassinS2",
            "assassinS3",
            "berserker",
            "bishop",
            "bladeDancer",
            "bloodRose",
            "bloodRoseS1",
            "bloodRoseS2",
            "bountyHunter",
            "cardinal",
            "cleric",
            "darkAvenger",
            "darkWizard",
            "deathBlade",
            "deathKnight",
            "deathMessenger",
            "destroyer",
            "divineTemplar",
            "dominator",
            "doombringer",
            "doomcryer",
            "dragoon",
            "dreadnought",
            "duelist",
            "elementWeaver",
            "elementWeaverS1",
            "elementWeaverS2",
            "elementalMaster",
            "elementalSummoner",
            "elfElder",
            "elfOracle",
            "elfScout",
            "elvenKnight",
            "elvenWizard",
            "ertheiaWarrior",
            "evaSaint",
            "evaTemplar",
            "eviscerator",
            "fortuneSeeker",
            "ghostHunter",
            "ghostSentinel",
            "gladiator",
            "grandKhavatari",
            "grandVanguard",
            "hatamoto",
            "hawkeye",
            "hellKnight",
            "hierophant",
            "holyTemplar",
            "knight",
            "lightTemplar",
            "maestro",
            "marauder",
            "monk",
            "moonlightSentinel",
            "mysticMuse",
            "necromancer",
            "overlord",
            "paladin",
            "palusKnight",
            "phantomRanger",
            "phantomSummoner",
            "phoenixKnight",
            "plainsWalker",
            "prophet",
            "raider",
            "rogue",
            "ronin",
            "sagittarius",
            "samurai",
            "sayhaSeeker",
            "sayhaSeer",
            "scavenger",
            "shaman",
            "sharpshooter",
            "shillienElder",
            "shillienKnight",
            "shillienOracle",
            "shillienSaint",
            "shillienTemplar",
            "shineMakerS1",
            "shineMakerS2",
            "shinemaker",
            "silverRanger",
            "sorcerer",
            "soulBreakerKamael",
            "soulFinder",
            "soulHound",
            "soulRanger",
            "soultaker",
            "spectralDancer",
            "spectralMaster",
            "spellhowler",
            "spellsinger",
            "stormBlaster",
            "stormScreamer",
            "swordMuse",
            "swordSinger",
            "sylphGunner",
            "templeKnight",
            "titan",
            "treasureHunter",
            "trickster",
            "trooper",
            "tyrant",
            "vanguardRider",
            "warcryer",
            "warder",
            "warg",
            "warlock",
            "warlord",
            "warrior",
            "warsmith",
            "windRider",
            "windRiderErth",
            "windSniper",
            "wizard"
        ]
    },
    "ultimate_defense": {
        "id": "ultimate_defense",
        "name": "極限防禦",
        "slug": "ultimate_defense",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0110.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Ultimate Defense effect",
        "canonicalCooldown": "15 min.",
        "canonicalCooldownMs": 900000,
        "desc": "大幅提高防禦能力。",
        "balance": {
            "mpCost": 21,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_ultimate_defense",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 46,
        "classes": [
            "darkAvenger",
            "elvenKnight",
            "evaTemplar",
            "hellKnight",
            "knight",
            "paladin",
            "palusKnight",
            "phoenixKnight",
            "shillienKnight",
            "shillienTemplar",
            "templeKnight"
        ]
    },
    "shield_deflect_magic": {
        "id": "shield_deflect_magic",
        "name": "盾牌魔法反射",
        "slug": "shield_deflect_magic",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0916.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Reflects M. damage (trigger rate: 20%).",
        "canonicalCooldown": "45 sec.",
        "canonicalCooldownMs": 45000,
        "desc": "有 20% 機率反射魔法傷害。",
        "balance": {
            "mpCost": 28,
            "pwr": 2,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_shield_deflect_magic",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 60,
        "classes": [
            "darkAvenger",
            "paladin",
            "shillienKnight",
            "templeKnight"
        ]
    },
    "quick_heal": {
        "id": "quick_heal",
        "name": "快速治療",
        "slug": "quick_heal",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill10255.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Recovers HP. Power 271.",
        "canonicalCooldown": "2 sec.",
        "canonicalCooldownMs": 2000,
        "desc": "恢復生命值，威力 271。",
        "balance": {
            "mpCost": 100,
            "pwr": 271,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_quick_heal",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "darkAvenger",
            "knight",
            "paladin"
        ]
    },
    "sanctuary": {
        "id": "sanctuary",
        "name": "聖域",
        "slug": "sanctuary",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill11581.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Covers an area with a sacred aura that decreases enemies' P. Def.P. Def. -23%PvP",
        "canonicalCooldown": "30 sec.",
        "canonicalCooldownMs": 30000,
        "desc": "以神聖光環覆蓋區域，使敵人物理防禦 -23%。玩家對戰 最多影響 10 個目標；對怪物戰鬥 最多影響 10 個目標。",
        "balance": {
            "mpCost": 50,
            "pwr": 2,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_sanctuary",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 50,
        "classes": [
            "paladin",
            "phoenixKnight"
        ]
    },
    "shackle": {
        "id": "shackle",
        "name": "束縛",
        "slug": "shackle",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0403.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Inflicts Hold on the target.",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "使目標陷入束縛狀態。",
        "balance": {
            "mpCost": 17,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_shackle",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 50,
        "classes": [
            "bladeDancer",
            "evaTemplar",
            "paladin",
            "phoenixKnight",
            "spectralDancer",
            "swordMuse",
            "swordSinger",
            "templeKnight"
        ]
    },
    "remedy": {
        "id": "remedy",
        "name": "治癒術",
        "slug": "remedy",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0044.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Removes infection with a 50% chance.",
        "canonicalCooldown": "2 sec.",
        "canonicalCooldownMs": 2000,
        "desc": "有 50% 機率移除感染狀態。",
        "balance": {
            "mpCost": 35,
            "pwr": 5,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_remedy",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "paladin"
        ]
    },
    "dark_assassin_transformation": {
        "id": "dark_assassin_transformation",
        "name": "黑暗刺客變身",
        "slug": "dark_assassin_transformation",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1801.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Dark Assassin Transformation effect",
        "canonicalCooldown": "2",
        "canonicalCooldownMs": 1000,
        "desc": "變身為黑暗刺客。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_dark_assassin_transformation",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 60,
        "classes": [],
        "availableTo": [],
        "disabled": true,
        "removalReason": "cosmetic_mount_purge"
    },
    "aggression_aura": {
        "id": "aggression_aura",
        "name": "挑釁光環",
        "slug": "aggression_aura",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0018.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Targets around you start attacking you. Power 5920. Provokes the target.",
        "canonicalCooldown": "20 sec.",
        "canonicalCooldownMs": 20000,
        "desc": "使周圍目標轉而攻擊你，威力 5920，並對目標造成挑釁。",
        "balance": {
            "mpCost": 50,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_aggression_aura",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "darkAvenger",
            "evaTemplar",
            "hellKnight",
            "paladin",
            "phoenixKnight",
            "shillienKnight",
            "shillienTemplar",
            "templeKnight"
        ]
    },
    "sacrifice": {
        "id": "sacrifice",
        "name": "犧牲",
        "slug": "sacrifice",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0069.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Consumes your HP to recover HP of the target. Power 350.",
        "canonicalCooldown": "5 sec.",
        "canonicalCooldownMs": 5000,
        "desc": "消耗自身 生命值 為目標恢復生命，威力 350。",
        "balance": {
            "mpCost": 15,
            "pwr": 350,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_sacrifice",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 50,
        "classes": [
            "paladin",
            "phoenixKnight"
        ]
    },
    "aggression": {
        "id": "aggression",
        "name": "挑釁",
        "slug": "aggression",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0028.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Makes the enemy attack you. Power 5920. Provokes the target.",
        "canonicalCooldown": "6 sec.",
        "canonicalCooldownMs": 6000,
        "desc": "迫使敵人攻擊你，威力 5920，並造成挑釁。",
        "balance": {
            "mpCost": 15,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_aggression",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "darkAvenger",
            "elvenKnight",
            "evaTemplar",
            "hellKnight",
            "knight",
            "paladin",
            "palusKnight",
            "phoenixKnight",
            "shillienKnight",
            "shillienTemplar",
            "templeKnight"
        ]
    },
    "blade_strike": {
        "id": "blade_strike",
        "name": "劍刃打擊",
        "slug": "blade_strike",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0817.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks the target. Power 694.For 20 sec., P. Def. -23%.Ignores Shield Defense.",
        "canonicalCooldown": "5 sec.",
        "canonicalCooldownMs": 5000,
        "desc": "攻擊目標，威力 694。20 秒內使物理防禦 -23%，並無視盾牌防禦。",
        "balance": {
            "mpCost": 43,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_blade_strike",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "duelist",
            "gladiator"
        ]
    },
    "blade_storm_dance": {
        "id": "blade_storm_dance",
        "name": "劍刃風暴之舞",
        "slug": "blade_storm_dance",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0007.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks the target and nearby enemies. Power 155.PvP: Attacks up to 5 targets.Pv",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "攻擊目標與附近敵人，威力 155。玩家對戰／對怪物戰鬥最多攻擊 5 個目標。",
        "balance": {
            "mpCost": 57,
            "pwr": 155,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_blade_storm_dance",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 47,
        "classes": [
            "duelist",
            "gladiator"
        ]
    },
    "light_assassin_transformation": {
        "id": "light_assassin_transformation",
        "name": "光明刺客變身",
        "slug": "light_assassin_transformation",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1802.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Light Assassin Transformation effect",
        "canonicalCooldown": "1",
        "canonicalCooldownMs": 1000,
        "desc": "變身為光明刺客。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_light_assassin_transformation",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 70,
        "classes": [],
        "availableTo": [],
        "disabled": true,
        "removalReason": "cosmetic_mount_purge"
    },
    "transformation_pirate": {
        "id": "transformation_pirate",
        "name": "變身：海賊",
        "slug": "transformation_pirate",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1800.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Transformation: Pirate effect",
        "canonicalCooldown": "2",
        "canonicalCooldownMs": 1000,
        "desc": "變身為海賊。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_transformation_pirate",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 45,
        "classes": [],
        "availableTo": [],
        "disabled": true,
        "removalReason": "cosmetic_mount_purge"
    },
    "slashing_blade": {
        "id": "slashing_blade",
        "name": "斬擊之刃",
        "slug": "slashing_blade",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/triple_sonic_slash.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks the target. Power 1481.Ignores Shield Defense.Ignores 10% of the target'",
        "canonicalCooldown": "1 sec.",
        "canonicalCooldownMs": 1000,
        "desc": "攻擊目標，威力 1481。無視盾牌防禦，並無視目標 10% 防禦力。",
        "balance": {
            "mpCost": 36,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_slashing_blade",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "duelist",
            "gladiator"
        ]
    },
    "rush": {
        "id": "rush",
        "name": "突進",
        "slug": "rush",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0493.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "<Rush skill>Charges at the target.Can only be used if the distance to the target",
        "canonicalCooldown": "3 sec.",
        "canonicalCooldownMs": 3000,
        "desc": "突進至目標身邊。只有與目標距離大於 150 時才能使用。",
        "balance": {
            "mpCost": 35,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_rush",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "berserker",
            "bountyHunter",
            "destroyer",
            "doombringer",
            "dreadnought",
            "duelist",
            "ertheiaWarrior",
            "eviscerator",
            "fortuneSeeker",
            "gladiator",
            "grandKhavatari",
            "maestro",
            "shineMakerS2",
            "shinemaker",
            "titan",
            "tyrant",
            "warlord",
            "warsmith"
        ]
    },
    "mass_shackling": {
        "id": "mass_shackling",
        "name": "群體束縛",
        "slug": "mass_shackling",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0404.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Inflicts Hold on targets around the character.",
        "canonicalCooldown": "30 sec.",
        "canonicalCooldownMs": 30000,
        "desc": "使角色周圍的目標陷入束縛。",
        "balance": {
            "mpCost": 20,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_mass_shackling",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 55,
        "classes": [
            "evaTemplar",
            "paladin",
            "phoenixKnight",
            "templeKnight"
        ]
    },
    "blade_movement": {
        "id": "blade_movement",
        "name": "劍刃步伐",
        "slug": "blade_movement",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0451.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Speed +5",
        "canonicalCooldown": "30 sec.",
        "canonicalCooldownMs": 30000,
        "desc": "速度 +5。",
        "balance": {
            "mpCost": 26,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_blade_movement",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 56,
        "classes": [
            "duelist",
            "gladiator"
        ]
    },
    "dual_weapon_defense": {
        "id": "dual_weapon_defense",
        "name": "雙武器防禦",
        "slug": "dual_weapon_defense",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1605.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "P. Skill Evasion +15%M. Skill Evasion +15%",
        "canonicalCooldown": "4 min.",
        "canonicalCooldownMs": 240000,
        "desc": "物理技能迴避 +15%，魔法技能迴避 +15%。",
        "balance": {
            "mpCost": 15,
            "pwr": 2,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_dual_weapon_defense",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 41,
        "classes": [
            "duelist",
            "gladiator"
        ]
    },
    "lionheart": {
        "id": "lionheart",
        "name": "獅子心",
        "slug": "lionheart",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill10040.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "PvE damage bonus +3%Paralysis Resistance +25%Hold Resistance +25%Sleep Resistanc",
        "canonicalCooldown": "2 min.",
        "canonicalCooldownMs": 120000,
        "desc": "對怪物戰鬥傷害 +3%、麻痺抗性 +25%、束縛抗性 +25%、睡眠抗性 +25%、衝擊抗性 +25%、增益解除抗性 +40%。",
        "balance": {
            "mpCost": 22,
            "pwr": 0,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_lionheart",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 49,
        "classes": [
            "artisanDwarf",
            "bountyHunter",
            "destroyer",
            "duelist",
            "gladiator",
            "monk",
            "raider",
            "scavenger",
            "shineMakerS1",
            "shineMakerS2",
            "tyrant",
            "warlord",
            "warrior",
            "warsmith"
        ]
    },
    "battle_roar": {
        "id": "battle_roar",
        "name": "戰鬥咆哮",
        "slug": "battle_roar",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0121.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Max HP +15%Momentarily recovers bonus HP.",
        "canonicalCooldown": "2 min.",
        "canonicalCooldownMs": 120000,
        "desc": "最大生命值 +15%，並立即恢復額外 生命值。",
        "balance": {
            "mpCost": 18,
            "pwr": 2,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_battle_roar",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "destroyer",
            "gladiator",
            "orcFighter",
            "raider",
            "rider",
            "warlord",
            "warrior"
        ]
    },
    "war_cry": {
        "id": "war_cry",
        "name": "戰吼",
        "slug": "war_cry",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill31146.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "War Cry effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "發出戰吼以強化戰鬥能力。",
        "balance": {
            "mpCost": 40,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_war_cry",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 43,
        "classes": [
            "dreadnought",
            "duelist",
            "gladiator",
            "warlord",
            "warrior"
        ]
    },
    "fatal_strike": {
        "id": "fatal_strike",
        "name": "致命打擊",
        "slug": "fatal_strike",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0190.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "A powerful strike.Attacks the target. Power 694.Ignores Shield Defense.Ignores 5",
        "canonicalCooldown": "1 sec.",
        "canonicalCooldownMs": 1000,
        "desc": "以強力一擊攻擊目標，威力 694。無視盾牌防禦，並無視目標 5% 防禦力。",
        "balance": {
            "mpCost": 30,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_fatal_strike",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "bountyHunter",
            "destroyer",
            "dominator",
            "doomcryer",
            "dreadnought",
            "fortuneSeeker",
            "hierophant",
            "maestro",
            "overlord",
            "prophet",
            "shineMakerS2",
            "shinemaker",
            "titan",
            "warcryer",
            "warlord",
            "warsmith"
        ]
    },
    "thunder_storm": {
        "id": "thunder_storm",
        "name": "雷霆風暴",
        "slug": "thunder_storm",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0048.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks nearby targets. Power 462.Stuns for 3 sec.Ignores Shield Defense.PvP: At",
        "canonicalCooldown": "5 sec.",
        "canonicalCooldownMs": 5000,
        "desc": "攻擊附近目標，威力 462，暈眩 3 秒，無視盾牌防禦。玩家對戰／對怪物戰鬥最多攻擊 5 個目標。",
        "balance": {
            "mpCost": 34,
            "pwr": 462,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_thunder_storm",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "dreadnought",
            "warlord"
        ]
    },
    "power_crash": {
        "id": "power_crash",
        "name": "強力衝擊",
        "slug": "power_crash",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill11509.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Character attacks the enemy with 586 power.Stuns for 3 sec.Ignores Shield Defens",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "以威力 586 攻擊敵人，使其暈眩 3 秒，並無視盾牌防禦。",
        "balance": {
            "mpCost": 31,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_power_crash",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "bountyHunter",
            "destroyer",
            "dominator",
            "doomcryer",
            "dreadnought",
            "fortuneSeeker",
            "maestro",
            "overlord",
            "shineMakerS2",
            "shinemaker",
            "titan",
            "warcryer",
            "warlord",
            "warsmith"
        ]
    },
    "purify": {
        "id": "purify",
        "name": "淨化",
        "slug": "purify",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1902.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Purify effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "淨化負面狀態。",
        "balance": {
            "mpCost": 40,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_purify",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "bishop",
            "divineTemplar",
            "elfElder",
            "gladiator",
            "holyTemplar",
            "shillienElder",
            "warlord"
        ]
    },
    "vortex": {
        "id": "vortex",
        "name": "漩渦",
        "slug": "vortex",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0036.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks the target and nearby enemies. Power 485.Ignores Shield Defense.PvP: Att",
        "canonicalCooldown": "3 sec.",
        "canonicalCooldownMs": 3000,
        "desc": "攻擊目標與附近敵人，威力 485，無視盾牌防禦。玩家對戰／對怪物戰鬥最多攻擊 5 個目標。",
        "balance": {
            "mpCost": 34,
            "pwr": 485,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_vortex",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "bountyHunter",
            "destroyer",
            "dreadnought",
            "fortuneSeeker",
            "maestro",
            "shineMakerS2",
            "shinemaker",
            "warlord",
            "warsmith"
        ]
    },
    "bandage": {
        "id": "bandage",
        "name": "包紮",
        "slug": "bandage",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0034.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Removes infection with a 70% chance.",
        "canonicalCooldown": "1 sec.",
        "canonicalCooldownMs": 1000,
        "desc": "有 70% 機率移除感染狀態。",
        "balance": {
            "mpCost": 41,
            "pwr": 7,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_bandage",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 45,
        "classes": [
            "artisanDwarf",
            "assassinS0",
            "berserker",
            "bloodRoseBase",
            "bountyHunter",
            "destroyer",
            "dragoon",
            "dwarfFighter",
            "elfFighter",
            "ertheiaWarrior",
            "gladiator",
            "marauder",
            "monk",
            "raider",
            "samuraiBase",
            "scavenger",
            "shineMakerS1",
            "shineMakerS2",
            "soulBreakerKamael",
            "soulFinder",
            "trooper",
            "tyrant",
            "vanguardRider",
            "warlord",
            "warrior",
            "warsmith"
        ]
    },
    "quick_spear": {
        "id": "quick_spear",
        "name": "迅捷長槍",
        "slug": "quick_spear",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1606.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks the target and nearby enemies. Power 801.Stuns for 3 sec.Ignores Shield ",
        "canonicalCooldown": "1 min.",
        "canonicalCooldownMs": 60000,
        "desc": "攻擊目標與附近敵人，威力 801，暈眩 3 秒，無視盾牌防禦。玩家對戰／對怪物戰鬥最多攻擊 5 個目標。",
        "balance": {
            "mpCost": 51,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_quick_spear",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 42,
        "classes": [
            "dreadnought",
            "warlord"
        ]
    },
    "focus_attack": {
        "id": "focus_attack",
        "name": "集中攻擊",
        "slug": "focus_attack",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0317.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "When triggered:Basic attacks hit one target.P. Accuracy +2Basic Critical Damage ",
        "canonicalCooldown": "1s",
        "canonicalCooldownMs": 1000,
        "desc": "啟動時普通攻擊只攻擊單一目標，物理命中 +2，基礎暴擊傷害 +3%。",
        "balance": {
            "mpCost": 18,
            "pwr": 0,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_focus_attack",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 46,
        "classes": [
            "dreadnought",
            "warlord"
        ]
    },
    "revival": {
        "id": "revival",
        "name": "再生",
        "slug": "revival",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill30511.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Revival effect",
        "canonicalCooldown": "15 min.",
        "canonicalCooldownMs": 900000,
        "desc": "恢復並強化自身生命力。",
        "balance": {
            "mpCost": 25,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_revival",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 55,
        "classes": [
            "dreadnought",
            "warlord"
        ]
    },
    "provoke": {
        "id": "provoke",
        "name": "挑釁怒吼",
        "slug": "provoke",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill10027.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Provokes nearby enemies decreasing their Spear Resistance.The effect lasts for 1",
        "canonicalCooldown": "20 sec.",
        "canonicalCooldownMs": 20000,
        "desc": "挑釁附近敵人並降低其長槍抗性。持續 10 秒，長槍抗性 -5%。",
        "balance": {
            "mpCost": 54,
            "pwr": 1,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_provoke",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "bountyHunter",
            "dreadnought",
            "fortuneSeeker",
            "maestro",
            "shineMakerS2",
            "shinemaker",
            "warlord",
            "warsmith"
        ]
    },
    "howl": {
        "id": "howl",
        "name": "咆哮",
        "slug": "howl",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0116.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "P. Atk. -23%",
        "canonicalCooldown": "2 sec.",
        "canonicalCooldownMs": 2000,
        "desc": "使物理攻擊力降低 23%。",
        "balance": {
            "mpCost": 45,
            "pwr": 2,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_howl",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 43,
        "classes": [
            "dreadnought",
            "warlord"
        ]
    },
    "thrill_fight": {
        "id": "thrill_fight",
        "name": "激戰",
        "slug": "thrill_fight",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0130.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Atk. Spd. +5%Speed -15%",
        "canonicalCooldown": "5 sec.",
        "canonicalCooldownMs": 5000,
        "desc": "攻擊速度 +5%，移動速度 -15%。",
        "balance": {
            "mpCost": 21,
            "pwr": 1,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_thrill_fight",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 46,
        "classes": [
            "dreadnought",
            "warlord"
        ]
    },
    "power_strike": {
        "id": "power_strike",
        "name": "強力打擊",
        "slug": "power_strike",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/skill0003.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Accumulates power to make a devastating blow.Power 286.No critical.",
        "canonicalCooldown": "1 sec.",
        "canonicalCooldownMs": 1000,
        "desc": "蓄積力量發動毀滅打擊，威力 286，不會暴擊。",
        "balance": {
            "mpCost": 15,
            "pwr": 286,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_power_strike",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 20,
        "classes": [
            "artisanDwarf",
            "bloodRoseBase",
            "darkElfFighter",
            "dwarfFighter",
            "elfFighter",
            "elvenKnight",
            "fighter",
            "knight",
            "orcFighter",
            "palusKnight",
            "raider",
            "samuraiBase",
            "scavenger",
            "shineMakerS1",
            "warrior"
        ]
    },
    "wind_walk": {
        "id": "wind_walk",
        "name": "風之疾走",
        "slug": "wind_walk",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/skill1204.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Speed +20",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "速度 +20。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_wind_walk",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 20,
        "classes": [
            "adventurer",
            "arcanaLord",
            "archmage",
            "assassinS1",
            "assassinS3",
            "bloodRose",
            "bloodRoseS1",
            "cardinal",
            "cleric",
            "darkWizard",
            "deathBlade",
            "deathKnight",
            "divineTemplar",
            "dreadnought",
            "duelist",
            "elementWeaver",
            "elementWeaverS1",
            "elementalMaster",
            "elfOracle",
            "elfScout",
            "elvenKnight",
            "elvenWizard",
            "evaSaint",
            "evaTemplar",
            "ghostHunter",
            "ghostSentinel",
            "hellKnight",
            "hierophant",
            "knight",
            "lightTemplar",
            "moonlightSentinel",
            "mysticMuse",
            "palusKnight",
            "phoenixKnight",
            "rogue",
            "sagittarius",
            "sayhaSeeker",
            "sayhaSeer",
            "shillienOracle",
            "shillienSaint",
            "shillienTemplar",
            "soultaker",
            "spectralDancer",
            "spectralMaster",
            "stormScreamer",
            "swordMuse",
            "warg",
            "warrior",
            "windRider",
            "wizard"
        ]
    },
    "wild_sweep": {
        "id": "wild_sweep",
        "name": "狂野橫掃",
        "slug": "wild_sweep",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/skill0245.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks the target at the front and nearby enemies. Power 108.No critical.PvP: A",
        "canonicalCooldown": "3 sec.",
        "canonicalCooldownMs": 3000,
        "desc": "攻擊前方目標與附近敵人，威力 108，不會暴擊。玩家對戰／對怪物戰鬥最多攻擊 5 個目標。",
        "balance": {
            "mpCost": 24,
            "pwr": 108,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_wild_sweep",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 20,
        "classes": [
            "artisanDwarf",
            "raider",
            "scavenger",
            "shineMakerS1",
            "warrior"
        ]
    },
    "detect_weakness": {
        "id": "detect_weakness",
        "name": "弱點偵測",
        "slug": "detect_weakness",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/skill5243.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Detect Weakness effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "偵測敵人的弱點。",
        "balance": {
            "mpCost": 50,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_detect_weakness",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 20,
        "classes": [
            "warrior"
        ]
    },
    "damage_reflection": {
        "id": "damage_reflection",
        "name": "傷害反射",
        "slug": "damage_reflection",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0086.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Reflects 3% of received damage.",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "反射受到傷害的 3%。",
        "balance": {
            "mpCost": 18,
            "pwr": 0,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_damage_reflection",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "darkAvenger"
        ]
    },
    "dark_panther_s_help": {
        "id": "dark_panther_s_help",
        "name": "黑豹援助",
        "slug": "dark_panther_s_help",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill31349.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Dark Panther comes to your aid.Dark Panther attacks the same target as you.Dark ",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "黑豹前來協助你，會攻擊與你相同的目標。黑豹追加攻擊率 5%，單體傷害依你的物理攻擊 計算，威力 1180。",
        "balance": {
            "mpCost": 50,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_dark_panther_s_help",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "darkAvenger",
            "hellKnight"
        ]
    },
    "power_shot": {
        "id": "power_shot",
        "name": "強力射擊",
        "slug": "power_shot",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/skill0056.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Makes a powerful shot at the target.Power 92.No critical.",
        "canonicalCooldown": "1.5 sec.",
        "canonicalCooldownMs": 1500,
        "desc": "對目標發動強力射擊，威力 92，不會暴擊。",
        "balance": {
            "mpCost": 6,
            "pwr": 92,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_power_shot",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 3,
        "classes": [
            "darkElfFighter",
            "elfFighter",
            "elfScout",
            "fighter",
            "rogue"
        ]
    },
    "mortal_blow": {
        "id": "mortal_blow",
        "name": "致命刺擊",
        "slug": "mortal_blow",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/skill0016.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Character attacks the target at a vital point.Power 89",
        "canonicalCooldown": "1.5 sec.",
        "canonicalCooldownMs": 1500,
        "desc": "攻擊目標要害，威力 89。",
        "balance": {
            "mpCost": 5,
            "pwr": 89,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_mortal_blow",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 3,
        "classes": [
            "darkElfFighter",
            "elfFighter",
            "elfScout",
            "fighter",
            "rogue"
        ]
    },
    "dark_strike": {
        "id": "dark_strike",
        "name": "黑暗打擊",
        "slug": "dark_strike",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill4336_new.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks the target. Power 670.No critical.",
        "canonicalCooldown": "5 sec.",
        "canonicalCooldownMs": 5000,
        "desc": "攻擊目標，威力 670，不會暴擊。",
        "balance": {
            "mpCost": 5,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_dark_strike",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "darkAvenger",
            "hellKnight"
        ]
    },
    "powerful_charge": {
        "id": "powerful_charge",
        "name": "強力衝鋒",
        "slug": "powerful_charge",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill19303.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Powerful Charge effect",
        "canonicalCooldown": "3 sec.",
        "canonicalCooldownMs": 3000,
        "desc": "向目標發動強力衝鋒。",
        "balance": {
            "mpCost": 60,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_powerful_charge",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "hellKnight",
            "phoenixKnight"
        ]
    },
    "horror": {
        "id": "horror",
        "name": "恐懼",
        "slug": "horror",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0065.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Frightens the enemy.",
        "canonicalCooldown": "20 sec.",
        "canonicalCooldownMs": 20000,
        "desc": "使敵人陷入恐懼。",
        "balance": {
            "mpCost": 22,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_horror",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 46,
        "classes": [
            "darkAvenger",
            "hellKnight"
        ]
    },
    "hamstring": {
        "id": "hamstring",
        "name": "斷筋",
        "slug": "hamstring",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0127.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "The effect lasts for 30 sec.Speed -30%",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "效果持續 30 秒，使速度降低 30%。",
        "balance": {
            "mpCost": 19,
            "pwr": 3,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_hamstring",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 43,
        "classes": [
            "darkAvenger",
            "hellKnight"
        ]
    },
    "life_scavenge": {
        "id": "life_scavenge",
        "name": "生命汲取",
        "slug": "life_scavenge",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0046.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "HP Recovery from absorbing residual life energy from a corpse.",
        "canonicalCooldown": "2 sec.",
        "canonicalCooldownMs": 2000,
        "desc": "吸收屍體殘留的生命能量來恢復生命值。",
        "balance": {
            "mpCost": 35,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_life_scavenge",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "darkAvenger",
            "hellKnight"
        ]
    },
    "knight_s_assault": {
        "id": "knight_s_assault",
        "name": "騎士突擊",
        "slug": "knight_s_assault",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill31161.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Makes a powerful strike at the target.Attacks the target. Power 4907.Ignores Shi",
        "canonicalCooldown": "1 sec.",
        "canonicalCooldownMs": 1000,
        "desc": "對目標發動強力打擊，威力 4907。無視盾牌防禦，並無視目標 10% 防禦力。",
        "balance": {
            "mpCost": 63,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_knight_s_assault",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "hellKnight",
            "phoenixKnight"
        ]
    },
    "shield_charge": {
        "id": "shield_charge",
        "name": "盾牌衝鋒",
        "slug": "shield_charge",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1898.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Strikes with a shield.Attacks the target at the front and nearby enemies. Power ",
        "canonicalCooldown": "30 sec.",
        "canonicalCooldownMs": 30000,
        "desc": "以盾牌撞擊前方目標與附近敵人，威力 6075，使其暈眩 3 秒，並無視 10% 防禦力。玩家對戰／對怪物戰鬥最多攻擊 6 個目標。",
        "balance": {
            "mpCost": 60,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_shield_charge",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "evaTemplar",
            "phoenixKnight"
        ]
    },
    "power_of_life": {
        "id": "power_of_life",
        "name": "生命之力",
        "slug": "power_of_life",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/stamina.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Fills your body with power and energy.Max HP +12%Max HP +1200P. Def. +300Shield ",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "以力量與能量充滿全身：最大生命值 +12%、最大生命值 +1200、物理防禦 +300、盾牌防禦 +200。",
        "balance": {
            "mpCost": 86,
            "pwr": 1,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_power_of_life",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "evaTemplar",
            "phoenixKnight"
        ]
    },
    "phoenix_strike": {
        "id": "phoenix_strike",
        "name": "鳳凰打擊",
        "slug": "phoenix_strike",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill10097_2.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Phoenix Strike effect",
        "canonicalCooldown": "15 sec.",
        "canonicalCooldownMs": 15000,
        "desc": "發動鳳凰之力的強力打擊。",
        "balance": {
            "mpCost": 85,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_phoenix_strike",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "phoenixKnight"
        ]
    },
    "shield_throwing": {
        "id": "shield_throwing",
        "name": "盾牌投擲",
        "slug": "shield_throwing",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/shield_throwing.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Throws a shield at the target.Deals P. damage to the target and nearby enemies. ",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "向目標投擲盾牌並傷害附近敵人，威力 7751。玩家對戰：10 秒內物理技能暴擊率 -5%、暴擊傷害 -5%；對怪物戰鬥：10 秒內物理防禦 -25%。無視盾牌防禦與目標 20% 防禦力，最多攻擊 6 個目標。",
        "balance": {
            "mpCost": 66,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_shield_throwing",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "hellKnight",
            "phoenixKnight"
        ]
    },
    "battle_will": {
        "id": "battle_will",
        "name": "戰鬥意志",
        "slug": "battle_will",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill10244_a.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Increases combat abilities.P. Atk. +500P. Def. +250Chain Strike → Chain Strike: ",
        "canonicalCooldown": ".Powerful",
        "canonicalCooldownMs": 1000,
        "desc": "提升戰鬥能力：物理攻擊 +500、物理防禦 +250。強化 鎖鏈打擊 與 強力衝鋒，使其獲得更高牽引率、暈眩、較短冷卻、更多目標、更遠範圍與職業效果。",
        "balance": {
            "mpCost": 120,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_battle_will",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "hellKnight",
            "phoenixKnight"
        ]
    },
    "chain_strike": {
        "id": "chain_strike",
        "name": "鎖鏈打擊",
        "slug": "chain_strike",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill10016.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Pulls the target to you.",
        "canonicalCooldown": "15 sec.",
        "canonicalCooldownMs": 15000,
        "desc": "將目標拉到自己身邊。",
        "balance": {
            "mpCost": 50,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_chain_strike",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "hellKnight",
            "phoenixKnight"
        ]
    },
    "mass_chain_strike": {
        "id": "mass_chain_strike",
        "name": "群體鎖鏈打擊",
        "slug": "mass_chain_strike",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill53003.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Pulls target and nearby enemies and puts them in front of the character.PvP: Att",
        "canonicalCooldown": "1 min.",
        "canonicalCooldownMs": 60000,
        "desc": "將目標與附近敵人拉到角色前方。玩家對戰／對怪物戰鬥最多影響 10 個目標。",
        "balance": {
            "mpCost": 80,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_mass_chain_strike",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "evaTemplar",
            "hellKnight",
            "phoenixKnight",
            "shillienTemplar"
        ]
    },
    "white_guardian_transformation": {
        "id": "white_guardian_transformation",
        "name": "白色守護者變身",
        "slug": "white_guardian_transformation",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/white_knight_suit.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "White Guardian Transformation effect",
        "canonicalCooldown": "1",
        "canonicalCooldownMs": 1000,
        "desc": "變身為白色守護者。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_white_guardian_transformation",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 80,
        "classes": [],
        "availableTo": [],
        "disabled": true,
        "removalReason": "cosmetic_mount_purge"
    },
    "shield_of_faith": {
        "id": "shield_of_faith",
        "name": "信念之盾",
        "slug": "shield_of_faith",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill0528.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Shield of Faith effect",
        "canonicalCooldown": "15 min.",
        "canonicalCooldownMs": 900000,
        "desc": "展開信念之盾保護自身與盟友。",
        "balance": {
            "mpCost": 43,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_shield_of_faith",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 79,
        "classes": [
            "hellKnight",
            "phoenixKnight"
        ]
    },
    "phoenix_protection": {
        "id": "phoenix_protection",
        "name": "鳳凰守護",
        "slug": "phoenix_protection",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/skill19201.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Phoenix protects your party members.P. Def. +15%M. Def. +20%Received Healing +20",
        "canonicalCooldown": "7 min.",
        "canonicalCooldownMs": 420000,
        "desc": "鳳凰守護隊伍成員：物理防禦 +15%、魔法防禦 +20%、受到治療 +20%。施法者距離過遠時效果會取消。",
        "balance": {
            "mpCost": 38,
            "pwr": 2,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_phoenix_protection",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 80,
        "classes": [
            "phoenixKnight"
        ]
    },
    "mount_golden_lion": {
        "id": "mount_golden_lion",
        "name": "騎乘黃金獅",
        "slug": "mount_golden_lion",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1833.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Mount Golden Lion effect",
        "canonicalCooldown": "1s",
        "canonicalCooldownMs": 1000,
        "desc": "召喚並騎乘黃金獅。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_mount_golden_lion",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [],
        "availableTo": [],
        "disabled": true,
        "removalReason": "cosmetic_mount_purge"
    },
    "phoenix_shield": {
        "id": "phoenix_shield",
        "name": "鳳凰之盾",
        "slug": "phoenix_shield",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/pheonix_shield.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Phoenix Shield effect",
        "canonicalCooldown": "2 min.",
        "canonicalCooldownMs": 120000,
        "desc": "展開鳳凰之盾。",
        "balance": {
            "mpCost": 101,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_phoenix_shield",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "phoenixKnight"
        ]
    },
    "dragon_slayer_appearance": {
        "id": "dragon_slayer_appearance",
        "name": "屠龍者外觀",
        "slug": "dragon_slayer_appearance",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/dragon_slayer_suit.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Dragon Slayer Appearance effect",
        "canonicalCooldown": "10 min.",
        "canonicalCooldownMs": 600000,
        "desc": "變更為屠龍者外觀。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_dragon_slayer_appearance",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 82,
        "classes": [],
        "availableTo": [],
        "disabled": true,
        "removalReason": "cosmetic_mount_purge"
    },
    "phoenix_power": {
        "id": "phoenix_power",
        "name": "鳳凰之力",
        "slug": "phoenix_power",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill0438.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Phoenix Power effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "獲得鳳凰之力強化。",
        "balance": {
            "mpCost": 150,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_phoenix_power",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 85,
        "classes": [
            "phoenixKnight"
        ]
    },
    "detection": {
        "id": "detection",
        "name": "偵測",
        "slug": "detection",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill19203.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "<Common de-stealthing skill>Removes the Hide effect from nearby enemies and prev",
        "canonicalCooldown": "30 sec.",
        "canonicalCooldownMs": 30000,
        "desc": "移除附近敵人的隱身效果，並使其 2 秒內無法再次隱身。偵測範圍：200。",
        "balance": {
            "mpCost": 70,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_detection",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [],
        "availableTo": [],
        "disabled": true,
        "removalReason": "cosmetic_mount_purge"
    },
    "mount_glorious_steed": {
        "id": "mount_glorious_steed",
        "name": "騎乘榮耀戰馬",
        "slug": "mount_glorious_steed",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/warhorse_vehicle.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Mount Glorious Steed effect",
        "canonicalCooldown": "1s",
        "canonicalCooldownMs": 1000,
        "desc": "召喚並騎乘榮耀戰馬。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_mount_glorious_steed",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 87,
        "classes": [],
        "availableTo": [],
        "disabled": true,
        "removalReason": "cosmetic_mount_purge"
    },
    "glorious_warrior_enhanced_abilities": {
        "id": "glorious_warrior_enhanced_abilities",
        "name": "榮耀戰士：強化能力",
        "slug": "glorious_warrior_enhanced_abilities",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "5★",
        "starRank": 5,
        "icon": "/icons/skill1288.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "CON +1MEN +1",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "體質 +1、精神 +1。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_glorious_warrior_enhanced_abilities",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 90,
        "classes": [
            "adventurer",
            "arcanaLord",
            "archmage",
            "assassinS3",
            "bloodRose",
            "cardinal",
            "deathKnight",
            "divineTemplar",
            "dominator",
            "doombringer",
            "doomcryer",
            "dreadnought",
            "duelist",
            "elementWeaver",
            "elementalMaster",
            "evaSaint",
            "evaTemplar",
            "eviscerator",
            "fortuneSeeker",
            "ghostHunter",
            "ghostSentinel",
            "grandKhavatari",
            "grandVanguard",
            "hellKnight",
            "hierophant",
            "maestro",
            "moonlightSentinel",
            "mysticMuse",
            "phoenixKnight",
            "sagittarius",
            "samurai",
            "sayhaSeeker",
            "shillienSaint",
            "shillienTemplar",
            "shinemaker",
            "soulHound",
            "soultaker",
            "spectralDancer",
            "spectralMaster",
            "stormBlaster",
            "stormScreamer",
            "swordMuse",
            "titan",
            "trickster",
            "warg",
            "windRider"
        ]
    },
    "vengeance": {
        "id": "vengeance",
        "name": "復仇",
        "slug": "vengeance",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill0368.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Vengeance effect",
        "canonicalCooldown": "15 min.",
        "canonicalCooldownMs": 900000,
        "desc": "啟動復仇之力。",
        "balance": {
            "mpCost": 35,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_vengeance",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 77,
        "classes": [
            "hellKnight",
            "phoenixKnight"
        ]
    },
    "touch_of_life": {
        "id": "touch_of_life",
        "name": "生命之觸",
        "slug": "touch_of_life",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill0341.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Touch of Life effect",
        "canonicalCooldown": "5 min.",
        "canonicalCooldownMs": 300000,
        "desc": "以生命之力強化目標。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_touch_of_life",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 78,
        "classes": [
            "evaTemplar",
            "phoenixKnight"
        ]
    },
    "mount_shining_lady": {
        "id": "mount_shining_lady",
        "name": "騎乘光輝淑女",
        "slug": "mount_shining_lady",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/skill8607.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Mount Shining Lady effect",
        "canonicalCooldown": "1s",
        "canonicalCooldownMs": 1000,
        "desc": "召喚並騎乘光輝淑女。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_mount_shining_lady",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 89,
        "classes": [],
        "availableTo": [],
        "disabled": true,
        "removalReason": "cosmetic_mount_purge"
    },
    "blade_assault": {
        "id": "blade_assault",
        "name": "劍刃突襲",
        "slug": "blade_assault",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/blade_assault1.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Blade Assault effect",
        "canonicalCooldown": "30 sec.",
        "canonicalCooldownMs": 30000,
        "desc": "發動劍刃突襲。",
        "balance": {
            "mpCost": 400,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_blade_assault",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "duelist"
        ]
    },
    "blade_rage": {
        "id": "blade_rage",
        "name": "劍刃狂怒",
        "slug": "blade_rage",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill0345.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks the target. Power 195.",
        "canonicalCooldown": "1s",
        "canonicalCooldownMs": 1000,
        "desc": "攻擊目標，威力 195。",
        "balance": {
            "mpCost": 60,
            "pwr": 195,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_blade_rage",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "duelist"
        ]
    },
    "blade_rush": {
        "id": "blade_rush",
        "name": "劍刃突進",
        "slug": "blade_rush",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/sonic_rush.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "<Rush skill>Attacks the target. Power 7000.Ignores Shield Defense.Ignores 10% of",
        "canonicalCooldown": "3 sec.",
        "canonicalCooldownMs": 3000,
        "desc": "突進並攻擊目標，威力 7000。無視盾牌防禦，並無視目標 10% 防禦力。",
        "balance": {
            "mpCost": 45,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_blade_rush",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "duelist"
        ]
    },
    "final_secret": {
        "id": "final_secret",
        "name": "終極祕訣",
        "slug": "final_secret",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill0917.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Final Secret effect",
        "canonicalCooldown": "2 min.",
        "canonicalCooldownMs": 120000,
        "desc": "啟動終極祕訣。",
        "balance": {
            "mpCost": 41,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_final_secret",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "dreadnought",
            "duelist",
            "fortuneSeeker",
            "grandKhavatari",
            "grandVanguard",
            "maestro",
            "shinemaker",
            "titan"
        ]
    },
    "indestructible_seal": {
        "id": "indestructible_seal",
        "name": "不滅封印",
        "slug": "indestructible_seal",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1624.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks the target. Power 1091.With a certain chance, inflicts Imprisonment on t",
        "canonicalCooldown": "1 min.",
        "canonicalCooldownMs": 60000,
        "desc": "攻擊目標，威力 1091。有一定機率使目標遭到禁錮 5 秒；禁錮期間無法移動、攻擊、傳送、使用技能或恢復生命值／魔力／戰鬥點數。禁錮結束後物理防禦 -10%，持續 5 秒。",
        "balance": {
            "mpCost": 61,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_indestructible_seal",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 78,
        "classes": [
            "duelist"
        ]
    },
    "amazing_blade_strike": {
        "id": "amazing_blade_strike",
        "name": "驚世劍擊",
        "slug": "amazing_blade_strike",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/full_rising_attack.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "<Changing skill>The attack type is based on the distance to the target.With a ce",
        "canonicalCooldown": "3 sec.",
        "canonicalCooldownMs": 3000,
        "desc": "依與目標距離改變攻擊方式，並有機率使其物理防禦 -30%，持續 5 秒。近戰威力 7401，無視盾牌防禦與 15% 防禦力；遠程威力 6290，無視盾牌防禦與 10% 防禦力。",
        "balance": {
            "mpCost": 60,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_amazing_blade_strike",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "duelist"
        ]
    },
    "indestructible_blade": {
        "id": "indestructible_blade",
        "name": "不滅之刃",
        "slug": "indestructible_blade",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/unbreakable_sonic.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks the target. Power 8000.With a certain chance, inflicts Imprisonment on t",
        "canonicalCooldown": "30 sec.",
        "canonicalCooldownMs": 30000,
        "desc": "攻擊目標，威力 8000。有一定機率使其禁錮 5 秒；禁錮期間無法移動、攻擊、傳送、使用技能或恢復生命值／魔力／戰鬥點數，且只能被決鬥者攻擊。",
        "balance": {
            "mpCost": 80,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_indestructible_blade",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 85,
        "classes": [
            "duelist"
        ]
    },
    "challenger": {
        "id": "challenger",
        "name": "挑戰者",
        "slug": "challenger",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/s_contender.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "A sword imbued with the will to win, that grants a mighty power.​P. Atk. +25%P. ",
        "canonicalCooldown": "1 min.",
        "canonicalCooldownMs": 60000,
        "desc": "以求勝意志灌注武器：物理攻擊 +25%、物理技能威力 +15%、技能暴擊率 +20%、技能暴擊傷害 +20%、物理技能傷害 +10%、最大生命值 +20% 與 +3000、攻速 +20%、速度 +10、禁錮成功率 +15%。",
        "balance": {
            "mpCost": 250,
            "pwr": 3,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_challenger",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "duelist"
        ]
    },
    "crossed_blades": {
        "id": "crossed_blades",
        "name": "交叉雙刃",
        "slug": "crossed_blades",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_blade_engage_buff.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Crossed Blades effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "啟動交叉雙刃效果。",
        "balance": {
            "mpCost": 100,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_crossed_blades",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "duelist"
        ]
    },
    "blade_wave": {
        "id": "blade_wave",
        "name": "劍刃波動",
        "slug": "blade_wave",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/blade_wave_new.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks the target and nearby enemies. Power 5811.When attacking a single target",
        "canonicalCooldown": "30 sec.",
        "canonicalCooldownMs": 30000,
        "desc": "攻擊目標與附近敵人，威力 5811；僅攻擊單一目標時威力 7554。無視盾牌防禦與 15% 防禦力，並有機率使物理防禦 -30%，持續 10 秒。玩家對戰／對怪物戰鬥最多攻擊 6 個目標。",
        "balance": {
            "mpCost": 91,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_blade_wave",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "duelist"
        ]
    },
    "tough_body": {
        "id": "tough_body",
        "name": "強韌體魄",
        "slug": "tough_body",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill0442.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Tough Body effect",
        "canonicalCooldown": "10 min.",
        "canonicalCooldownMs": 600000,
        "desc": "強化自身體魄。",
        "balance": {
            "mpCost": 50,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_tough_body",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 77,
        "classes": [
            "duelist"
        ]
    },
    "blade_punishment": {
        "id": "blade_punishment",
        "name": "劍刃懲戒",
        "slug": "blade_punishment",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/blade_cutter_new.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks the target and nearby enemies. Power 6789.Ignores Shield Defense.Ignores",
        "canonicalCooldown": "5 sec.",
        "canonicalCooldownMs": 5000,
        "desc": "攻擊目標與附近敵人，威力 6789。無視盾牌防禦與目標 10% 防禦力。玩家對戰／對怪物戰鬥最多攻擊 3 個目標。",
        "balance": {
            "mpCost": 72,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_blade_punishment",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "duelist"
        ]
    },
    "battle_heart": {
        "id": "battle_heart",
        "name": "戰鬥之心",
        "slug": "battle_heart",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_dethrone_fire_source_1.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Battle Heart effect",
        "canonicalCooldown": "2 min.",
        "canonicalCooldownMs": 120000,
        "desc": "啟動戰鬥之心。",
        "balance": {
            "mpCost": 60,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_battle_heart",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 78,
        "classes": [
            "duelist"
        ]
    },
    "gleaming_blade": {
        "id": "gleaming_blade",
        "name": "閃耀之刃",
        "slug": "gleaming_blade",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/s_blade_flare_buff.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Gleaming Blade effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "使劍刃綻放光芒並獲得強化。",
        "balance": {
            "mpCost": 75,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_gleaming_blade",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 85,
        "classes": [
            "duelist"
        ]
    },
    "tough_body_dual_weapons": {
        "id": "tough_body_dual_weapons",
        "name": "強韌體魄：雙武器",
        "slug": "tough_body_dual_weapons",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/ultimate_body_dualist.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Tough Body: Dual Weapons effect",
        "canonicalCooldown": "5 min.",
        "canonicalCooldownMs": 300000,
        "desc": "使用雙武器時強化自身體魄。",
        "balance": {
            "mpCost": 50,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_tough_body_dual_weapons",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 83,
        "classes": [
            "duelist"
        ]
    },
    "final_secret_dual_weapons": {
        "id": "final_secret_dual_weapons",
        "name": "終極祕訣：雙武器",
        "slug": "final_secret_dual_weapons",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/final_secret_dualist.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Final Secret: Dual Weapons effect",
        "canonicalCooldown": "2 min.",
        "canonicalCooldownMs": 120000,
        "desc": "使用雙武器時啟動終極祕訣。",
        "balance": {
            "mpCost": 60,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_final_secret_dual_weapons",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 85,
        "classes": [
            "duelist"
        ]
    },
    "braveheart": {
        "id": "braveheart",
        "name": "勇者之心",
        "slug": "braveheart",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill0440.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Braveheart effect",
        "canonicalCooldown": "2 min.",
        "canonicalCooldownMs": 120000,
        "desc": "激發勇者之心。",
        "balance": {
            "mpCost": 57,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_braveheart",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 78,
        "classes": [
            "dreadnought",
            "duelist",
            "fortuneSeeker",
            "grandKhavatari",
            "grandVanguard",
            "maestro",
            "shinemaker",
            "titan"
        ]
    },
    "shocking_burst": {
        "id": "shocking_burst",
        "name": "震撼爆發",
        "slug": "shocking_burst",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill0361.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks the target and nearby enemies. Power 2486.With a certain chance, stuns f",
        "canonicalCooldown": "1 min.",
        "canonicalCooldownMs": 60000,
        "desc": "攻擊目標與附近敵人，威力 2486。有一定機率暈眩 3 秒，並使物理防禦、魔法防禦 -30%。無視盾牌防禦。玩家對戰／對怪物戰鬥最多攻擊 5 個目標。",
        "balance": {
            "mpCost": 90,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_shocking_burst",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 77,
        "classes": [
            "dreadnought"
        ]
    },
    "great_sweep": {
        "id": "great_sweep",
        "name": "大橫掃",
        "slug": "great_sweep",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_wide_sweep.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Great Sweep effect",
        "canonicalCooldown": "1 min.",
        "canonicalCooldownMs": 60000,
        "desc": "發動強力橫掃攻擊。",
        "balance": {
            "mpCost": 123,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_great_sweep",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "dreadnought"
        ]
    },
    "spear_rumble": {
        "id": "spear_rumble",
        "name": "長槍震盪",
        "slug": "spear_rumble",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/s_spear_stump.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Swings at enemies with a spear.Attacks nearby enemies. Power 4099.For 30 sec., P",
        "canonicalCooldown": "30 sec.",
        "canonicalCooldownMs": 30000,
        "desc": "揮舞長槍攻擊附近敵人，威力 4099。30 秒內使物理攻擊 -30%、物理技能暴擊傷害 -5%。無視盾牌防禦與目標 15% 防禦力，並移除目標的 風之疾走 增益。",
        "balance": {
            "mpCost": 100,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_spear_rumble",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "dreadnought"
        ]
    },
    "spike_thrust": {
        "id": "spike_thrust",
        "name": "尖刺突刺",
        "slug": "spike_thrust",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill0921.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks the target. Power 4907.Ignores Shield Defense.Ignores 10% of the target'",
        "canonicalCooldown": "1 sec.",
        "canonicalCooldownMs": 1000,
        "desc": "攻擊目標，威力 4907。無視盾牌防禦，並無視目標 10% 防禦力。",
        "balance": {
            "mpCost": 60,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_spike_thrust",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "dreadnought"
        ]
    },
    "spear_howl": {
        "id": "spear_howl",
        "name": "長槍咆哮",
        "slug": "spear_howl",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/skill1626.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Spear Howl effect",
        "canonicalCooldown": "8 sec.",
        "canonicalCooldownMs": 8000,
        "desc": "發出長槍戰吼以壓制敵人。",
        "balance": {
            "mpCost": 91,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_spear_howl",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 80,
        "classes": [
            "dreadnought"
        ]
    },
    "throwing_spear": {
        "id": "throwing_spear",
        "name": "投擲長槍",
        "slug": "throwing_spear",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1625.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks the target. Power 801.Holds for 5 sec.Ignores Shield Defense.No critical",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "投擲長槍攻擊目標，威力 801，使其束縛 5 秒。無視盾牌防禦，不會暴擊。",
        "balance": {
            "mpCost": 60,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_throwing_spear",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 78,
        "classes": [
            "dreadnought"
        ]
    },
    "spear_cage": {
        "id": "spear_cage",
        "name": "長槍牢籠",
        "slug": "spear_cage",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/s_spear_prison.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Throws spears to attack the target and nearby enemies. Power 4660.Inflicts Hold ",
        "canonicalCooldown": "1 min.",
        "canonicalCooldownMs": 60000,
        "desc": "投擲長槍攻擊目標與附近敵人，威力 4660。使目標束縛 3 秒，並在 10 秒內使物理防禦／魔法防禦 -30%。無視盾牌防禦與目標 10% 防禦力。",
        "balance": {
            "mpCost": 120,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_spear_cage",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "dreadnought"
        ]
    },
    "unleashed_power": {
        "id": "unleashed_power",
        "name": "解放之力",
        "slug": "unleashed_power",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_surge.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Received damage -3%Shock Atk. Rate +5%P. Skill Power +1%",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "受到傷害 -3%、衝擊成功率 +5%、物理技能威力 +1%。",
        "balance": {
            "mpCost": 60,
            "pwr": 0,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_unleashed_power",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "dreadnought"
        ]
    },
    "gust_shot": {
        "id": "gust_shot",
        "name": "疾風射擊",
        "slug": "gust_shot",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_gust_shot.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks the target and nearby enemies. Power 4824.Ignores 10% of the target's de",
        "canonicalCooldown": "20 sec.",
        "canonicalCooldownMs": 20000,
        "desc": "攻擊目標與附近敵人，威力 4824。無視目標 10% 防禦力。玩家對戰／對怪物戰鬥最多攻擊 3 個目標。",
        "balance": {
            "mpCost": 102,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_gust_shot",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "ghostSentinel",
            "moonlightSentinel",
            "sagittarius"
        ]
    },
    "lethal_shot": {
        "id": "lethal_shot",
        "name": "致命射擊",
        "slug": "lethal_shot",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_lethal_shot_new.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Aims and shoots at a weak point. Power 9479.Ignores 10% of the target's defense.",
        "canonicalCooldown": "1 sec.",
        "canonicalCooldownMs": 1000,
        "desc": "瞄準弱點射擊，威力 9479。無視目標 10% 防禦力。",
        "balance": {
            "mpCost": 64,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_lethal_shot",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "ghostSentinel",
            "moonlightSentinel",
            "sagittarius"
        ]
    },
    "double_shot": {
        "id": "double_shot",
        "name": "雙重射擊",
        "slug": "double_shot",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_double_shot_new.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Shoots 2 arrows one by one.Attacks the target twice. Power 3646.",
        "canonicalCooldown": "1.5 sec.",
        "canonicalCooldownMs": 1500,
        "desc": "連續射出 2 支箭，對目標攻擊兩次，威力 3646。",
        "balance": {
            "mpCost": 68,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_double_shot",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "ghostSentinel",
            "hawkeye",
            "moonlightSentinel",
            "phantomRanger",
            "sagittarius",
            "silverRanger"
        ]
    },
    "incendiary_shot": {
        "id": "incendiary_shot",
        "name": "燃燒射擊",
        "slug": "incendiary_shot",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/s_burning_shot.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Shoots an arrow imbued with fire energy.Attacks the target and nearby enemies. P",
        "canonicalCooldown": "15 sec.",
        "canonicalCooldownMs": 15000,
        "desc": "射出注入火焰能量的箭矢，攻擊目標與附近敵人，威力 8800。附加 點燃，5 秒內使 生命值藥水效果 -5%。無視目標 10% 防禦力，玩家對戰／對怪物戰鬥最多攻擊 8 個目標。",
        "balance": {
            "mpCost": 110,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_incendiary_shot",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "sagittarius"
        ]
    },
    "buff_thief": {
        "id": "buff_thief",
        "name": "增益竊取",
        "slug": "buff_thief",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/skill1440.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "With a certain chance, steals up to 2 of the enemy's buffs.Cannot be used on mon",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "有一定機率偷取敵人最多 2 個增益；無法對怪物使用。每次嘗試會累積華麗竊取：等級 1 物理／魔法攻擊 +100、等級 2 +200、等級 3 +300，持續 5 分鐘。",
        "balance": {
            "mpCost": 70,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_buff_thief",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 77,
        "classes": [
            "dreadnought",
            "fortuneSeeker"
        ]
    },
    "flame_arrow_rain": {
        "id": "flame_arrow_rain",
        "name": "烈焰箭雨",
        "slug": "flame_arrow_rain",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/s_flame_arrow_rain.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Shoots an arrow imbued with fire power.Attacks the target and nearby enemies. Po",
        "canonicalCooldown": "30 sec.",
        "canonicalCooldownMs": 30000,
        "desc": "射出注入火焰力量的箭矢，攻擊目標與附近敵人，威力 10800。無視目標 10% 防禦力，並有機率附加 箭矢破壞：物理攻擊、魔法攻擊、物理防禦、魔法防禦 各 -20%，持續 10 秒。玩家對戰／對怪物戰鬥最多攻擊 15 個目標。",
        "balance": {
            "mpCost": 170,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_flame_arrow_rain",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "sagittarius"
        ]
    },
    "real_target": {
        "id": "real_target",
        "name": "真實目標",
        "slug": "real_target",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill0522.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "When using a bow, P. Atk. +1%Applies Bow Vulnerability to the target on attack.B",
        "canonicalCooldown": ":",
        "canonicalCooldownMs": 1000,
        "desc": "使用弓時物理攻擊 +1%。攻擊時對目標附加弓弱點，使弓抗性 -15%，持續 10 秒；觸發冷卻 10 秒。",
        "balance": {
            "mpCost": 55,
            "pwr": 0,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_real_target",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "ghostSentinel",
            "moonlightSentinel",
            "sagittarius"
        ]
    },
    "steady_aim": {
        "id": "steady_aim",
        "name": "穩定瞄準",
        "slug": "steady_aim",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_accurate.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Accurate shooting increases your lethal attack rate.P. Atk. +1000Atk. Spd. +3%P.",
        "canonicalCooldown": "-3%Speed",
        "canonicalCooldownMs": 1000,
        "desc": "精準射擊提高致命攻擊能力：物理攻擊 +1000、攻速 +3%、物理技能暴擊率 +5%、技能冷卻 -3%、速度 +5。使用 雙重射擊 時有 50% 機率追加一次攻擊。",
        "balance": {
            "mpCost": 80,
            "pwr": 0,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_steady_aim",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "sagittarius"
        ]
    },
    "true_aim": {
        "id": "true_aim",
        "name": "真實瞄準",
        "slug": "true_aim",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_true_target.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Max HP +1000When using a bow, P. Atk. +20%P. Skill Power +1%PvE damage bonus +5%",
        "canonicalCooldown": ":",
        "canonicalCooldownMs": 1000,
        "desc": "最大生命值 +1000。使用弓時物理攻擊 +20%、物理技能威力 +1%、對怪物戰鬥傷害 +5%、牽引成功率 +5%、束縛成功率 +5%。攻擊時使目標弓抗性 -36%，持續 10 秒。並強化 致命射擊，增加攻擊次數。",
        "balance": {
            "mpCost": 67,
            "pwr": 2,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_true_aim",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "ghostSentinel",
            "moonlightSentinel",
            "sagittarius"
        ]
    },
    "vortex_shot": {
        "id": "vortex_shot",
        "name": "漩渦射擊",
        "slug": "vortex_shot",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/s_spinning_shot.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Shoots a spinning arrow.Attacks the target and nearby enemies. Power 4042.Ignore",
        "canonicalCooldown": "15 sec.",
        "canonicalCooldownMs": 15000,
        "desc": "射出旋轉箭矢，攻擊目標與附近敵人，威力 4042。無視目標 5% 防禦力。",
        "balance": {
            "mpCost": 100,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_vortex_shot",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 77,
        "classes": [
            "ghostSentinel",
            "hawkeye",
            "moonlightSentinel",
            "phantomRanger",
            "sagittarius",
            "silverRanger"
        ]
    },
    "ultimate_evasion_archer": {
        "id": "ultimate_evasion_archer",
        "name": "極限迴避：弓手",
        "slug": "ultimate_evasion_archer",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_ultimate_evasion_ac.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Ultimate Evasion: Archer effect",
        "canonicalCooldown": "5 min.",
        "canonicalCooldownMs": 300000,
        "desc": "弓手專用的極限迴避強化。",
        "balance": {
            "mpCost": 35,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_ultimate_evasion_archer",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": []
    },
    "legendary_archer": {
        "id": "legendary_archer",
        "name": "傳說弓手",
        "slug": "legendary_archer",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/s_legendary_archer_hu1.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Gives the power of a legendary archer.Max HP +10%P. Atk. +25%P. Skill Critical R",
        "canonicalCooldown": "1 min.",
        "canonicalCooldownMs": 60000,
        "desc": "獲得傳說弓手之力：最大生命值 +10%、物理攻擊 +25%、物理技能暴擊率 +20%、暴擊傷害 +20% 並額外 +20、技能威力 +2%、對怪物戰鬥傷害 +5%、受到物理技能暴擊率 -2%、近戰武器抗性 +10%。使用 致命射擊 時有 50% 機率追加攻擊。",
        "balance": {
            "mpCost": 200,
            "pwr": 1,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_legendary_archer",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "grade": "LEGENDARY",
        "bookRequirement": {
            "required": true,
            "tier": "LEGENDARY",
            "stars": 4,
            "itemId": "book_4star",
            "quantity": 1,
            "sourceCoupon": "Legendary Spellbook Coupon (103041)"
        },
        "classes": [
            "ghostSentinel",
            "moonlightSentinel",
            "sagittarius",
            "trickster"
        ]
    },
    "hamstring_shot": {
        "id": "hamstring_shot",
        "name": "斷筋射擊",
        "slug": "hamstring_shot",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill0354.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Hamstring Shot effect",
        "canonicalCooldown": "is",
        "canonicalCooldownMs": 1000,
        "desc": "以箭矢限制目標行動速度。",
        "balance": {
            "mpCost": 70,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_hamstring_shot",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "ghostSentinel",
            "moonlightSentinel",
            "sagittarius"
        ]
    },
    "spiral_shot": {
        "id": "spiral_shot",
        "name": "螺旋射擊",
        "slug": "spiral_shot",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/s_spiral_shot.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "<Pull skill>Shoots an arrow imbued with strong wind energy.Pulls nearby enemies ",
        "canonicalCooldown": "20 sec.",
        "canonicalCooldownMs": 20000,
        "desc": "射出注入強風能量的箭矢，將附近敵人拉向受擊目標並攻擊兩次，威力 6600。無視目標 8% 防禦力。玩家對戰／對怪物戰鬥最多攻擊 8 個目標。",
        "balance": {
            "mpCost": 124,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_spiral_shot",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "ghostSentinel",
            "moonlightSentinel",
            "sagittarius"
        ]
    },
    "legendary_shot": {
        "id": "legendary_shot",
        "name": "傳說射擊",
        "slug": "legendary_shot",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_legendary_shot_hu.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Legendary Shot effect",
        "canonicalCooldown": "1 min.",
        "canonicalCooldownMs": 60000,
        "desc": "施展傳說弓術的強力射擊。",
        "balance": {
            "mpCost": 400,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_legendary_shot",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "ghostSentinel",
            "moonlightSentinel",
            "sagittarius"
        ]
    },
    "legendary_cloak": {
        "id": "legendary_cloak",
        "name": "傳說披風",
        "slug": "legendary_cloak",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill11872.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Legendary Cloak effect",
        "canonicalCooldown": "1s",
        "canonicalCooldownMs": 1000,
        "desc": "啟動傳說披風的強化效果。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_legendary_cloak",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "ghostSentinel",
            "moonlightSentinel",
            "sagittarius",
            "trickster"
        ]
    },
    "binding_shot": {
        "id": "binding_shot",
        "name": "束縛射擊",
        "slug": "binding_shot",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_binding_shot.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "P. Atk. +500P. Skill Power +2%Hold Atk. Rate +20%Pull Atk. Rate +10%Triggers Hol",
        "canonicalCooldown": "is",
        "canonicalCooldownMs": 1000,
        "desc": "物理攻擊 +500、物理技能威力 +2%、束縛成功率 +20%、牽引成功率 +10%。攻擊時可觸發束縛，持續 3 秒，觸發冷卻固定為 15 秒。",
        "balance": {
            "mpCost": 72,
            "pwr": 0,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_binding_shot",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "ghostSentinel",
            "moonlightSentinel",
            "sagittarius"
        ]
    },
    "snipe": {
        "id": "snipe",
        "name": "狙擊",
        "slug": "snipe",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill0313.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "P. Accuracy +6P. Atk. +250Basic Critical Rate +20%Speed -10",
        "canonicalCooldown": "30 sec.",
        "canonicalCooldownMs": 30000,
        "desc": "物理命中 +6、物理攻擊 +250、基礎暴擊率 +20%、速度 -10。",
        "balance": {
            "mpCost": 80,
            "pwr": 2,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_snipe",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 80,
        "classes": [
            "hawkeye",
            "sagittarius"
        ]
    },
    "rain_of_fire": {
        "id": "rain_of_fire",
        "name": "火焰之雨",
        "slug": "rain_of_fire",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1296.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Creates a big explosion.Deals M. damage to the target and nearby enemies. Power ",
        "canonicalCooldown": "7 sec.",
        "canonicalCooldownMs": 7000,
        "desc": "引發巨大爆炸，對目標與附近敵人造成魔法傷害，威力 60。玩家對戰／對怪物戰鬥最多攻擊 5 個目標。",
        "balance": {
            "mpCost": 49,
            "pwr": 60,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_rain_of_fire",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 58,
        "classes": [
            "archmage",
            "sorcerer"
        ]
    },
    "target_lock": {
        "id": "target_lock",
        "name": "目標鎖定",
        "slug": "target_lock",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill0759.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Target Lock effect",
        "canonicalCooldown": "1 min.",
        "canonicalCooldownMs": 60000,
        "desc": "鎖定目標以提高射擊穩定性。",
        "balance": {
            "mpCost": 100,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_target_lock",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "ghostSentinel",
            "moonlightSentinel",
            "sagittarius"
        ]
    },
    "prominence": {
        "id": "prominence",
        "name": "烈焰顯現",
        "slug": "prominence",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1230.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Summons pure flame.Deals M. damage to the target. Power 76.",
        "canonicalCooldown": "1 sec.",
        "canonicalCooldownMs": 1000,
        "desc": "召喚純粹火焰，對目標造成魔法傷害，威力 76。",
        "balance": {
            "mpCost": 32,
            "pwr": 76,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_prominence",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "archmage",
            "sorcerer"
        ]
    },
    "detect_darkness": {
        "id": "detect_darkness",
        "name": "黑暗偵測",
        "slug": "detect_darkness",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill53005.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "<Common de-stealthing skill>Removes the Hide effect from nearby enemies and prev",
        "canonicalCooldown": "30 sec.",
        "canonicalCooldownMs": 30000,
        "desc": "移除附近敵人的隱身效果，並使其 2 秒內無法再次隱身。偵測範圍：1000。",
        "balance": {
            "mpCost": 70,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_detect_darkness",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "ghostSentinel",
            "moonlightSentinel",
            "sagittarius",
            "trickster"
        ]
    },
    "mana_regeneration": {
        "id": "mana_regeneration",
        "name": "魔力再生",
        "slug": "mana_regeneration",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1047.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "MP Recovery Rate +1.72",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "魔力 恢復率 +1.72。",
        "balance": {
            "mpCost": 35,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_mana_regeneration",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "archmage",
            "mysticMuse",
            "necromancer",
            "sayhaSeeker",
            "sorcerer",
            "soultaker",
            "spellhowler",
            "spellsinger",
            "stormScreamer",
            "windRiderErth"
        ]
    },
    "blazing_skin": {
        "id": "blazing_skin",
        "name": "烈焰之膚",
        "slug": "blazing_skin",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1232.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Reflects 3% of received damage.",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "反射受到傷害的 3%。",
        "balance": {
            "mpCost": 35,
            "pwr": 0,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_blazing_skin",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "sorcerer"
        ]
    },
    "inferno": {
        "id": "inferno",
        "name": "煉獄",
        "slug": "inferno",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1289.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Inferno effect",
        "canonicalCooldown": "30 sec.",
        "canonicalCooldownMs": 30000,
        "desc": "召喚煉獄之力攻擊敵人。",
        "balance": {
            "mpCost": 100,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_inferno",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 70,
        "classes": [
            "archmage",
            "sorcerer"
        ]
    },
    "sleep": {
        "id": "sleep",
        "name": "睡眠",
        "slug": "sleep",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1069.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Inflicts Sleep on the enemy.",
        "canonicalCooldown": "20 sec.",
        "canonicalCooldownMs": 20000,
        "desc": "使敵人陷入睡眠。",
        "balance": {
            "mpCost": 21,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_sleep",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "archmage",
            "bishop",
            "cardinal",
            "cleric",
            "elfElder",
            "elfOracle",
            "evaSaint",
            "hierophant",
            "mysticMuse",
            "necromancer",
            "prophet",
            "sayhaSeeker",
            "shillienElder",
            "shillienOracle",
            "shillienSaint",
            "sorcerer",
            "soultaker",
            "spellhowler",
            "spellsinger",
            "stormScreamer",
            "windRiderErth"
        ]
    },
    "concentration": {
        "id": "concentration",
        "name": "專注",
        "slug": "concentration",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1078.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Casting Interruption Rate -36",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "施法中斷率 -36。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_concentration",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 44,
        "classes": [
            "bishop",
            "cleric",
            "darkWizard",
            "elfElder",
            "elfOracle",
            "elvenWizard",
            "prophet",
            "sayhaSeer",
            "shillienElder",
            "shillienOracle",
            "sorcerer",
            "wizard"
        ]
    },
    "magic_wound": {
        "id": "magic_wound",
        "name": "魔法創傷",
        "slug": "magic_wound",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill19191.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Decreases the target's M. Def.M. Def. -23%",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "降低目標魔法防禦，魔法防禦 -23%。",
        "balance": {
            "mpCost": 39,
            "pwr": 2,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_magic_wound",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 44,
        "classes": [
            "archmage",
            "mysticMuse",
            "necromancer",
            "sayhaSeeker",
            "sorcerer",
            "soultaker",
            "spellhowler",
            "spellsinger",
            "stormScreamer",
            "windRiderErth"
        ]
    },
    "weakness": {
        "id": "weakness",
        "name": "虛弱",
        "slug": "weakness",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill16872.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Weakens the target.P. Atk. -23%M. Atk. -23%",
        "canonicalCooldown": "2 sec.",
        "canonicalCooldownMs": 2000,
        "desc": "削弱目標，使物理攻擊 -23%、魔法攻擊 -23%。",
        "balance": {
            "mpCost": 35,
            "pwr": 2,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_weakness",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "arcanaLord",
            "archmage",
            "bishop",
            "cardinal",
            "cleric",
            "darkWizard",
            "elementalMaster",
            "elementalSummoner",
            "elfElder",
            "elfMage",
            "elfOracle",
            "elvenWizard",
            "evaSaint",
            "hierophant",
            "mage",
            "mysticMuse",
            "necromancer",
            "prophet",
            "sayhaMageBase",
            "sayhaSeeker",
            "sayhaSeer",
            "sorcerer",
            "soultaker",
            "spellsinger",
            "warlock",
            "windRiderErth",
            "wizard"
        ]
    },
    "hell": {
        "id": "hell",
        "name": "地獄",
        "slug": "hell",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/hell.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Hell effect",
        "canonicalCooldown": "1 min.",
        "canonicalCooldownMs": 60000,
        "desc": "釋放地獄之力。",
        "balance": {
            "mpCost": 82,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_hell",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "hellKnight"
        ]
    },
    "dark_knight_s_break": {
        "id": "dark_knight_s_break",
        "name": "黑暗騎士破擊",
        "slug": "dark_knight_s_break",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/skill1443.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Dark Knight's Break effect",
        "canonicalCooldown": "15 sec.",
        "canonicalCooldownMs": 15000,
        "desc": "施展黑暗騎士的破壞性攻擊。",
        "balance": {
            "mpCost": 85,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_dark_knight_s_break",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "hellKnight"
        ]
    },
    "body_to_mind": {
        "id": "body_to_mind",
        "name": "生命轉魔",
        "slug": "body_to_mind",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1157.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Sacrifices your own HP to regenerate MP with 90 Power.",
        "canonicalCooldown": "5 sec.",
        "canonicalCooldownMs": 5000,
        "desc": "犧牲自身 生命值，以威力 90 恢復魔力。",
        "balance": {
            "mpCost": 15,
            "pwr": 90,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_body_to_mind",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "archmage",
            "darkWizard",
            "elvenWizard",
            "mysticMuse",
            "necromancer",
            "sayhaSeeker",
            "sayhaSeer",
            "sorcerer",
            "soultaker",
            "spellhowler",
            "spellsinger",
            "stormScreamer",
            "windRiderErth",
            "wizard"
        ]
    },
    "hell_smashing": {
        "id": "hell_smashing",
        "name": "地獄粉碎",
        "slug": "hell_smashing",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/skill10330.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Releases powerful energy of Hell.Attacks the target and nearby enemies. Power 60",
        "canonicalCooldown": "30 sec.",
        "canonicalCooldownMs": 30000,
        "desc": "釋放強大的地獄能量，攻擊目標與附近敵人，威力 6075。無視目標 10% 防禦力，有機率移除最多 3 個增益，並施加 60 秒詛咒：最大戰鬥點數 -90%、弱化／異常抗性 -30%、受到治療 -30%。玩家對戰／對怪物戰鬥最多攻擊 6 個目標。",
        "balance": {
            "mpCost": 60,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_hell_smashing",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "hellKnight"
        ]
    },
    "toughness": {
        "id": "toughness",
        "name": "強韌",
        "slug": "toughness",
        "type": "passive",
        "rawType": "Passive",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0134.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Sleep/Hold Resistance +20%",
        "canonicalCooldown": "N/A",
        "canonicalCooldownMs": 0,
        "desc": "提高對睡眠與束縛攻擊的抗性。",
        "balance": {
            "mpCost": 0,
            "pwr": 0,
            "pveMultiplier": 1,
            "pvpMultiplier": 1
        },
        "vfxId": null,
        "vfxGap": false,
        "sfxId": null,
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "hellKnight",
            "shillienTemplar"
        ]
    },
    "dark_might": {
        "id": "dark_might",
        "name": "黑暗力量",
        "slug": "dark_might",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/skill31386.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Dark Might effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "獲得黑暗力量強化。",
        "balance": {
            "mpCost": 150,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_dark_might",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 85,
        "classes": [
            "hellKnight"
        ]
    },
    "touch_of_death": {
        "id": "touch_of_death",
        "name": "死亡之觸",
        "slug": "touch_of_death",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill0342.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Touch of Death effect",
        "canonicalCooldown": "2 min.",
        "canonicalCooldownMs": 120000,
        "desc": "以死亡之力削弱目標。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_touch_of_death",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "hellKnight",
            "shillienTemplar"
        ]
    },
    "backstab": {
        "id": "backstab",
        "name": "背刺",
        "slug": "backstab",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0030.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "<Gap-closing skill>Rushes to the target and attacks them.Strikes at a vital poin",
        "canonicalCooldown": "5 sec.",
        "canonicalCooldownMs": 5000,
        "desc": "瞬間突進至目標身後並攻擊要害，無視目標戰鬥點數，威力 2490。",
        "balance": {
            "mpCost": 35,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_backstab",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "abyssWalker",
            "adventurer",
            "ghostHunter",
            "plainsWalker",
            "treasureHunter",
            "windRider"
        ]
    },
    "deadly_blow": {
        "id": "deadly_blow",
        "name": "致命一擊",
        "slug": "deadly_blow",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0263.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks the target's vital points.Deals P. damage. Power 1423.",
        "canonicalCooldown": "1 sec.",
        "canonicalCooldownMs": 1000,
        "desc": "攻擊目標要害造成物理傷害，威力 1423。",
        "balance": {
            "mpCost": 30,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_deadly_blow",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "abyssWalker",
            "adventurer",
            "ghostHunter",
            "plainsWalker",
            "treasureHunter",
            "windRider"
        ]
    },
    "fake_death": {
        "id": "fake_death",
        "name": "假死",
        "slug": "fake_death",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0060.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Fake Death effect",
        "canonicalCooldown": "1s",
        "canonicalCooldownMs": 1000,
        "desc": "進入假死狀態欺騙敵人。",
        "balance": {
            "mpCost": 200,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_fake_death",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "bountyHunter",
            "plainsWalker",
            "treasureHunter"
        ]
    },
    "silent_move": {
        "id": "silent_move",
        "name": "無聲移動",
        "slug": "silent_move",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0221.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Silent Move effect",
        "canonicalCooldown": "1s",
        "canonicalCooldownMs": 1000,
        "desc": "隱匿氣息並安靜移動。",
        "balance": {
            "mpCost": 7,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_silent_move",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "abyssWalker",
            "plainsWalker",
            "treasureHunter"
        ]
    },
    "open": {
        "id": "open",
        "name": "開鎖",
        "slug": "open",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0027.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Opens doors Lv. 1 with a 100% chance, doors Lv. 2 with a 75% chance, and chests ",
        "canonicalCooldown": "30 sec.",
        "canonicalCooldownMs": 30000,
        "desc": "100% 開啟 等級 1 門、75% 開啟 等級 2 門，並可 100% 開啟寶箱。",
        "balance": {
            "mpCost": 35,
            "pwr": 10,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_open",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "abyssWalker",
            "assassinDE",
            "elfScout",
            "plainsWalker",
            "rogue",
            "treasureHunter"
        ]
    },
    "ultimate_evasion": {
        "id": "ultimate_evasion",
        "name": "極限迴避",
        "slug": "ultimate_evasion",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0111.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Ultimate Evasion effect",
        "canonicalCooldown": "15 min.",
        "canonicalCooldownMs": 900000,
        "desc": "大幅提高迴避能力。",
        "balance": {
            "mpCost": 25,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_ultimate_evasion",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 55,
        "classes": [
            "abyssWalker",
            "assassinDE",
            "assassinS1",
            "bloodRoseS1",
            "elfScout",
            "ghostSentinel",
            "hawkeye",
            "moonlightSentinel",
            "phantomRanger",
            "plainsWalker",
            "rogue",
            "sagittarius",
            "silverRanger",
            "treasureHunter"
        ]
    },
    "shadow_step": {
        "id": "shadow_step",
        "name": "暗影步",
        "slug": "shadow_step",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill10520.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "<Teleport/ suppression skill>Instantly teleports behind target's back.Target's S",
        "canonicalCooldown": "20 sec.",
        "canonicalCooldownMs": 20000,
        "desc": "瞬間傳送至目標背後，並使目標速度 -30%。",
        "balance": {
            "mpCost": 37,
            "pwr": 3,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_shadow_step",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 50,
        "classes": [
            "abyssWalker",
            "adventurer",
            "ghostHunter",
            "plainsWalker",
            "treasureHunter",
            "windRider"
        ]
    },
    "hide": {
        "id": "hide",
        "name": "隱身",
        "slug": "hide",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0922.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Grants invisibility so enemies cannot see the character.The effect is cancelled ",
        "canonicalCooldown": "3 min.",
        "canonicalCooldownMs": 180000,
        "desc": "進入隱形狀態，使敵人無法看見角色。除移動外的任何動作都會取消效果。",
        "balance": {
            "mpCost": 30,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_hide",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 50,
        "classes": [
            "abyssWalker",
            "plainsWalker",
            "treasureHunter"
        ]
    },
    "ice_bolt": {
        "id": "ice_bolt",
        "name": "冰霜箭",
        "slug": "ice_bolt",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/skill1184.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Freezes the air around the target.Deals M. damage. Power 11.For 30 sec., the tar",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "凍結目標周圍空氣，造成魔法傷害，威力 11。30 秒內使目標速度 -20%。",
        "balance": {
            "mpCost": 8,
            "pwr": 11,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_ice_bolt",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 1,
        "classes": [
            "darkElfMage",
            "darkWizard",
            "elfMage",
            "elvenWizard",
            "mage",
            "sayhaMageBase",
            "sayhaSeer",
            "wizard"
        ]
    },
    "fireball": {
        "id": "fireball",
        "name": "火球術",
        "slug": "fireball",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/skill11298.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Creates a fireball.Deals M. damage to the target. Power 40.",
        "canonicalCooldown": "5 sec.",
        "canonicalCooldownMs": 5000,
        "desc": "創造火球並對目標造成魔法傷害，威力 40。",
        "balance": {
            "mpCost": 2,
            "pwr": 40,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_fireball",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 1,
        "classes": [
            "darkElfMage",
            "darkWizard",
            "elfMage",
            "elvenWizard",
            "mage",
            "sayhaMageBase",
            "sayhaSeer",
            "wizard"
        ]
    },
    "battle_heal": {
        "id": "battle_heal",
        "name": "戰鬥治療",
        "slug": "battle_heal",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/skill1015.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Momentarily recovers HP. Power 83.",
        "canonicalCooldown": "1 sec.",
        "canonicalCooldownMs": 1000,
        "desc": "立即恢復生命值，威力 83。",
        "balance": {
            "mpCost": 25,
            "pwr": 83,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_battle_heal",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 10,
        "classes": [
            "cleric",
            "darkElfMage",
            "elfMage",
            "elfOracle",
            "mage",
            "sayhaMageBase",
            "shillienOracle"
        ]
    },
    "self_heal": {
        "id": "self_heal",
        "name": "自我治療",
        "slug": "self_heal",
        "type": "active",
        "rawType": "Recovery",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/skill1216.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Recovers HP. Power 50.",
        "canonicalCooldown": "3 sec.",
        "canonicalCooldownMs": 3000,
        "desc": "恢復自身 生命值，威力 50。",
        "balance": {
            "mpCost": 9,
            "pwr": 50,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_self_heal",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 1,
        "classes": [
            "cleric",
            "darkElfMage",
            "elfMage",
            "elfOracle",
            "mage",
            "sayhaMageBase",
            "shillienOracle"
        ]
    },
    "wind_strike": {
        "id": "wind_strike",
        "name": "風之打擊",
        "slug": "wind_strike",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/skill1177.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Creates a powerful hurricane.Deals M. damage to the target. Power 12.",
        "canonicalCooldown": "1 sec.",
        "canonicalCooldownMs": 1000,
        "desc": "創造強烈颶風，對目標造成魔法傷害，威力 12。",
        "balance": {
            "mpCost": 8,
            "pwr": 12,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_wind_strike",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 1,
        "classes": [
            "darkElfMage",
            "elfMage",
            "mage",
            "sayhaMageBase"
        ]
    },
    "flame_explosion": {
        "id": "flame_explosion",
        "name": "烈焰爆發",
        "slug": "flame_explosion",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/flame_burst.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Makes an explosion imbued with the Fire energy.Deals M. damage to the target. Po",
        "canonicalCooldown": "1 sec.",
        "canonicalCooldownMs": 1000,
        "desc": "引發注入火元素能量的爆炸，對目標造成魔法傷害，威力 152。帶有弱化／異常狀態的敵人會受到更多傷害。",
        "balance": {
            "mpCost": 56,
            "pwr": 152,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_flame_explosion",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "archmage"
        ]
    },
    "fire_vortex": {
        "id": "fire_vortex",
        "name": "火焰漩渦",
        "slug": "fire_vortex",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1339.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Creates a vortex that draws its strength from the Fire dimension.Deals M. damage",
        "canonicalCooldown": "30 sec.",
        "canonicalCooldownMs": 30000,
        "desc": "召喚來自火焰次元的漩渦，對目標造成魔法傷害，威力 158。",
        "balance": {
            "mpCost": 116,
            "pwr": 158,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_fire_vortex",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "archmage"
        ]
    },
    "meteor": {
        "id": "meteor",
        "name": "隕石術",
        "slug": "meteor",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/meteor.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Meteor effect",
        "canonicalCooldown": "30 sec.",
        "canonicalCooldownMs": 30000,
        "desc": "召喚隕石轟擊敵人。",
        "balance": {
            "mpCost": 131,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_meteor",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "archmage",
            "mysticMuse",
            "sayhaSeeker",
            "soultaker",
            "stormScreamer"
        ]
    },
    "fire_spiral": {
        "id": "fire_spiral",
        "name": "火焰螺旋",
        "slug": "fire_spiral",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/s_fire_spiral.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks with a fire spiral.Deals M. damage to the target and nearby enemies. Pow",
        "canonicalCooldown": "30 sec.",
        "canonicalCooldownMs": 30000,
        "desc": "以火焰螺旋攻擊目標與附近敵人，威力 229。5 秒內使 體質／精神 -2。玩家對戰／對怪物戰鬥最多攻擊 6 個目標。",
        "balance": {
            "mpCost": 116,
            "pwr": 229,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_fire_spiral",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "archmage"
        ]
    },
    "magic_focus": {
        "id": "magic_focus",
        "name": "魔力集中",
        "slug": "magic_focus",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill11870.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Magic Focus effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "集中魔力以強化施法能力。",
        "balance": {
            "mpCost": 80,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_magic_focus",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "archmage",
            "soultaker",
            "stormScreamer"
        ]
    },
    "arcane_shield": {
        "id": "arcane_shield",
        "name": "奧術護盾",
        "slug": "arcane_shield",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/skill1556.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Blocks 50% of damage.For the remainder of inflicted damage MP is consumed instea",
        "canonicalCooldown": "5 min.",
        "canonicalCooldownMs": 300000,
        "desc": "阻擋 50% 傷害；其餘傷害改為消耗 魔力 而非 生命值。當 魔力 降至 0 時效果解除。",
        "balance": {
            "mpCost": 110,
            "pwr": 5,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_arcane_shield",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 80,
        "classes": [
            "archmage",
            "mysticMuse",
            "sayhaSeeker",
            "stormScreamer"
        ]
    },
    "archmage_s_shield": {
        "id": "archmage_s_shield",
        "name": "大法師護盾",
        "slug": "archmage_s_shield",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/s_mystic_shield_original.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Archmage's Shield effect",
        "canonicalCooldown": "5 min.",
        "canonicalCooldownMs": 300000,
        "desc": "展開大法師專屬防護屏障。",
        "balance": {
            "mpCost": 125,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_archmage_s_shield",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "archmage"
        ]
    },
    "heavy_sleep": {
        "id": "heavy_sleep",
        "name": "深度睡眠",
        "slug": "heavy_sleep",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_deep_sleep.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Inflicts Sleep on the target.For some time the target cannot wake up even if hit",
        "canonicalCooldown": "15 sec.",
        "canonicalCooldownMs": 15000,
        "desc": "使目標陷入深度睡眠，一段時間內即使受到攻擊也不會醒來。",
        "balance": {
            "mpCost": 74,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_heavy_sleep",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "archmage",
            "soultaker",
            "stormScreamer"
        ]
    },
    "arcane_power": {
        "id": "arcane_power",
        "name": "奧術力量",
        "slug": "arcane_power",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill0337.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "M. Atk. +10%M. Skill MP Consumption +5%",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "魔法攻擊 +10%，魔法技能魔力消耗 +5%。",
        "balance": {
            "mpCost": 36,
            "pwr": 1,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_arcane_power",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 77,
        "classes": [
            "archmage",
            "mysticMuse",
            "sayhaSeeker",
            "stormScreamer"
        ]
    },
    "master_of_flame": {
        "id": "master_of_flame",
        "name": "火焰大師",
        "slug": "master_of_flame",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_flame_expert1.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Master of Flame effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "強化對火焰魔法的掌控。",
        "balance": {
            "mpCost": 120,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_master_of_flame",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "archmage"
        ]
    },
    "exciting_adventure": {
        "id": "exciting_adventure",
        "name": "刺激冒險",
        "slug": "exciting_adventure",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill0768.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Speed +30P. Evasion +15Vital Point Attack Rate +20%P. Skill Evasion +60%Buff Can",
        "canonicalCooldown": "15 min.",
        "canonicalCooldownMs": 900000,
        "desc": "速度 +30、物理迴避 +15、要害攻擊率 +20%、物理技能迴避 +60%、增益解除抗性 +90%。可與 極限迴避 同時使用。",
        "balance": {
            "mpCost": 36,
            "pwr": 2,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_exciting_adventure",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "adventurer"
        ]
    },
    "lethal_blow": {
        "id": "lethal_blow",
        "name": "致命重擊",
        "slug": "lethal_blow",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill0344.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Increases vital point attack power.Attacks the target at their vital point. Powe",
        "canonicalCooldown": "1 sec.",
        "canonicalCooldownMs": 1000,
        "desc": "提高要害攻擊威力，攻擊目標要害，威力 7888。",
        "balance": {
            "mpCost": 60,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_lethal_blow",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "adventurer",
            "ghostHunter",
            "windRider"
        ]
    },
    "flying_dagger": {
        "id": "flying_dagger",
        "name": "飛刀",
        "slug": "flying_dagger",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/flying_dagger.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks the target. Power 7200.Ignores Shield Defense.Ignores 15% of the target'",
        "canonicalCooldown": "5 sec.",
        "canonicalCooldownMs": 5000,
        "desc": "攻擊目標，威力 7200。無視盾牌防禦與目標 15% 防禦力。使攻速 -10%、施法速度 -10%、速度 -20%，並每 0.66 秒損失 146 生命值。",
        "balance": {
            "mpCost": 62,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_flying_dagger",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "adventurer",
            "ghostHunter",
            "windRider"
        ]
    },
    "dark_blow": {
        "id": "dark_blow",
        "name": "暗影重擊",
        "slug": "dark_blow",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill0928_1.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Strikes at the target together with Shadow Figure.Makes 1 attack at the target's",
        "canonicalCooldown": "5 sec.",
        "canonicalCooldownMs": 5000,
        "desc": "與 暗影分身 一同攻擊目標要害，威力 8200。玩家對戰／對怪物戰鬥最多攻擊 2 個目標，並在主要目標背後召喚 暗影分身 追加一次單體攻擊。",
        "balance": {
            "mpCost": 50,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_dark_blow",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "adventurer",
            "ghostHunter",
            "windRider"
        ]
    },
    "critical_assault": {
        "id": "critical_assault",
        "name": "暴擊突襲",
        "slug": "critical_assault",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/skill10508.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Critical Assault effect",
        "canonicalCooldown": "20 sec.",
        "canonicalCooldownMs": 20000,
        "desc": "發動以暴擊為核心的強力突襲。",
        "balance": {
            "mpCost": 100,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_critical_assault",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "adventurer",
            "ghostHunter"
        ]
    },
    "mirage": {
        "id": "mirage",
        "name": "幻影",
        "slug": "mirage",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill0445.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Vital Point Attack Rate +15%All received Critical Damage -10%",
        "canonicalCooldown": "5 min.",
        "canonicalCooldownMs": 300000,
        "desc": "要害攻擊率 +15%，受到的所有暴擊傷害 -10%。",
        "balance": {
            "mpCost": 40,
            "pwr": 2,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_mirage",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 79,
        "classes": [
            "adventurer"
        ]
    },
    "bluff": {
        "id": "bluff",
        "name": "欺詐",
        "slug": "bluff",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill0358.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Inflicts Stupor on the target.With a 80% chance, makes the target turn their bac",
        "canonicalCooldown": "8 sec.",
        "canonicalCooldownMs": 8000,
        "desc": "使目標陷入失神。80% 機率讓目標轉身背對你，並停止攻擊你。",
        "balance": {
            "mpCost": 35,
            "pwr": 8,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_bluff",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "adventurer",
            "ghostHunter",
            "windRider"
        ]
    },
    "focus_power": {
        "id": "focus_power",
        "name": "力量集中",
        "slug": "focus_power",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill0357.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Focus Power effect",
        "canonicalCooldown": "30 sec.",
        "canonicalCooldownMs": 30000,
        "desc": "集中力量以提高攻擊輸出。",
        "balance": {
            "mpCost": 65,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_focus_power",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 78,
        "classes": [
            "adventurer",
            "ghostHunter",
            "windRider"
        ]
    },
    "focus_chance": {
        "id": "focus_chance",
        "name": "機率集中",
        "slug": "focus_chance",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill0356.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Focus Chance effect",
        "canonicalCooldown": "30 sec.",
        "canonicalCooldownMs": 30000,
        "desc": "提高要害攻擊與暴擊觸發機率。",
        "balance": {
            "mpCost": 65,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_focus_chance",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 78,
        "classes": [
            "adventurer",
            "ghostHunter",
            "windRider"
        ]
    },
    "focus_death": {
        "id": "focus_death",
        "name": "死亡集中",
        "slug": "focus_death",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill0355.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Focus Death effect",
        "canonicalCooldown": "30 sec.",
        "canonicalCooldownMs": 30000,
        "desc": "將攻擊集中在致命要害上。",
        "balance": {
            "mpCost": 65,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_focus_death",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 78,
        "classes": [
            "adventurer",
            "ghostHunter",
            "windRider"
        ]
    },
    "seclusion": {
        "id": "seclusion",
        "name": "隱匿",
        "slug": "seclusion",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/skill10517.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Seclusion effect",
        "canonicalCooldown": "3 min.",
        "canonicalCooldownMs": 180000,
        "desc": "進入高度隱匿狀態。",
        "balance": {
            "mpCost": 50,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_seclusion",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "adventurer",
            "ghostHunter",
            "windRider"
        ]
    },
    "critical_wound": {
        "id": "critical_wound",
        "name": "暴擊創傷",
        "slug": "critical_wound",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill0531.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Makes the target receive more damage.Received Basic Critical Damage +10%Received",
        "canonicalCooldown": "8 sec.",
        "canonicalCooldownMs": 8000,
        "desc": "使目標承受更多傷害：受到的基礎暴擊傷害 +10%，受到要害攻擊率 +10%。",
        "balance": {
            "mpCost": 30,
            "pwr": 1,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_critical_wound",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "adventurer",
            "ghostHunter",
            "windRider"
        ]
    },
    "reversed_pull": {
        "id": "reversed_pull",
        "name": "反向牽引",
        "slug": "reversed_pull",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill19557.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Reversed Pull effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "以反向力量牽引目標。",
        "balance": {
            "mpCost": 50,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_reversed_pull",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "adventurer"
        ]
    },
    "blaze": {
        "id": "blaze",
        "name": "燃焰",
        "slug": "blaze",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/skill1220.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "The enemy's body is set aflame.Deals M. damage to the target. Power 31.",
        "canonicalCooldown": "1 sec.",
        "canonicalCooldownMs": 1000,
        "desc": "點燃敵人身體，造成魔法傷害，威力 31。",
        "balance": {
            "mpCost": 18,
            "pwr": 31,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_blaze",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 20,
        "classes": [
            "arcanaLord",
            "warlock",
            "wizard"
        ]
    },
    "clear_movements": {
        "id": "clear_movements",
        "name": "清除動作",
        "slug": "clear_movements",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill10518.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Hide/ Seclusion/ Shadow StepResets cooldowns.",
        "canonicalCooldown": "s.Terms",
        "canonicalCooldownMs": 1000,
        "desc": "重置 隱身、隱匿、暗影步 的冷卻時間。",
        "balance": {
            "mpCost": 60,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_clear_movements",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 77,
        "classes": [
            "adventurer",
            "ghostHunter",
            "windRider"
        ]
    },
    "fatal_crush": {
        "id": "fatal_crush",
        "name": "致命粉碎",
        "slug": "fatal_crush",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/fatal_smash.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "A powerful strike.Attacks the target. Power 6246.If the enemy's HP is lower than",
        "canonicalCooldown": "1 sec.",
        "canonicalCooldownMs": 1000,
        "desc": "發動強力打擊，威力 6246。若敵人 生命值 低於 30%，攻擊次數增加 1。無視盾牌防禦，並無視目標 20% 防禦力。",
        "balance": {
            "mpCost": 60,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_fatal_crush",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "hierophant"
        ]
    },
    "might_of_heaven": {
        "id": "might_of_heaven",
        "name": "天堂之力",
        "slug": "might_of_heaven",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1028.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Strikes with holy light.Deals M. damage to the target. Power 113.Deals heavy dam",
        "canonicalCooldown": "1 sec.",
        "canonicalCooldownMs": 1000,
        "desc": "以神聖光芒攻擊，對目標造成魔法傷害，威力 113。對惡魔與不死族造成大量傷害。",
        "balance": {
            "mpCost": 56,
            "pwr": 113,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_might_of_heaven",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "bishop",
            "cardinal",
            "elfElder",
            "evaSaint",
            "hierophant",
            "prophet",
            "shillienElder",
            "shillienSaint"
        ]
    },
    "divine_beam": {
        "id": "divine_beam",
        "name": "神聖光束",
        "slug": "divine_beam",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill11773.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Purifies with holy light.Deals M. damage to the target. Power 145.Deals heavy da",
        "canonicalCooldown": "1 sec.",
        "canonicalCooldownMs": 1000,
        "desc": "以神聖光芒淨化並對目標造成魔法傷害，威力 145。對惡魔與不死族造成大量傷害。",
        "balance": {
            "mpCost": 56,
            "pwr": 145,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_divine_beam",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "evaSaint",
            "hierophant",
            "shillienSaint"
        ]
    },
    "mana_burn": {
        "id": "mana_burn",
        "name": "魔力燃燒",
        "slug": "mana_burn",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1398.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Deals M. damage. Power 78.Burns 200 of the target's MP.",
        "canonicalCooldown": "15 sec.",
        "canonicalCooldownMs": 15000,
        "desc": "造成魔法傷害，威力 78，並燃燒目標 200 魔力。",
        "balance": {
            "mpCost": 77,
            "pwr": 78,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_mana_burn",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "hierophant"
        ]
    },
    "dryad_root": {
        "id": "dryad_root",
        "name": "樹精之根",
        "slug": "dryad_root",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1201.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Inflicts Hold on the target.",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "使目標陷入束縛狀態。",
        "balance": {
            "mpCost": 86,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_dryad_root",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "bishop",
            "cardinal",
            "cleric",
            "elfElder",
            "elfOracle",
            "evaSaint",
            "hierophant",
            "prophet",
            "shillienElder",
            "shillienOracle",
            "shillienSaint"
        ]
    },
    "word_of_fear": {
        "id": "word_of_fear",
        "name": "恐懼之言",
        "slug": "word_of_fear",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1272.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Inflicts Fear on targets around the character.",
        "canonicalCooldown": "1 min.",
        "canonicalCooldownMs": 60000,
        "desc": "使角色周圍的目標陷入恐懼。",
        "balance": {
            "mpCost": 224,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_word_of_fear",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "hierophant",
            "prophet"
        ]
    },
    "tower_shield": {
        "id": "tower_shield",
        "name": "高塔之盾",
        "slug": "tower_shield",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/tower_shield.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "P. Def. +50Triggers Protective Shield when hit.Protective ShieldBuff: Creates a ",
        "canonicalCooldown": ":",
        "canonicalCooldownMs": 1000,
        "desc": "物理防禦 +50。受到攻擊時有 10% 機率觸發保護護盾，形成 500 生命值 屏障，持續 5 秒；觸發冷卻 20 秒。",
        "balance": {
            "mpCost": 67,
            "pwr": 1,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_tower_shield",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 80,
        "classes": [
            "hierophant"
        ]
    },
    "enlightenment": {
        "id": "enlightenment",
        "name": "啟蒙",
        "slug": "enlightenment",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1532.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "M. Atk. +10%Received Healing +20%Casting Spd. +30%M. Skill Critical Rate +50%M. ",
        "canonicalCooldown": "10 min.",
        "canonicalCooldownMs": 600000,
        "desc": "魔法攻擊 +10%、受到治療 +20%、施法速度 +30%、魔法技能暴擊率 +50%、魔法技能魔力消耗 -70%、速度 +15。",
        "balance": {
            "mpCost": 80,
            "pwr": 1,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_enlightenment",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 78,
        "classes": [
            "cardinal",
            "evaSaint",
            "hierophant",
            "shillienSaint"
        ]
    },
    "prophecy_of_fire": {
        "id": "prophecy_of_fire",
        "name": "火之預言",
        "slug": "prophecy_of_fire",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1356.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Max HP +10%HP Recovery Rate +10%P. Def. +10%M. Def. +10%Atk. Spd. +10%Casting Sp",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "最大生命值 +10%、生命值 恢復率 +10%、物理防禦 +10%、魔法防禦 +10%、攻速 +10%、施法速度 +10%、物理技能暴擊傷害 +15%、魔法技能暴擊率 +15%、對怪物戰鬥傷害 +15%、弱化抗性 +10%、異常抗性 +10%。",
        "balance": {
            "mpCost": 72,
            "pwr": 1,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_prophecy_of_fire",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "hierophant"
        ]
    },
    "perfect_abilities": {
        "id": "perfect_abilities",
        "name": "完美能力",
        "slug": "perfect_abilities",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/protect_critical.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "P. Def. +100Triggers Ultimate Critical Attack when hit.Ultimate Critical AttackB",
        "canonicalCooldown": ":",
        "canonicalCooldownMs": 1000,
        "desc": "物理防禦 +100。受到攻擊時有 70% 機率觸發 極限暴擊：物理技能暴擊傷害 +50、暴擊率 +1%，持續 8 秒；觸發冷卻 15 秒。",
        "balance": {
            "mpCost": 73,
            "pwr": 0,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_perfect_abilities",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "hierophant"
        ]
    },
    "great_attack": {
        "id": "great_attack",
        "name": "強襲",
        "slug": "great_attack",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/higher_attack.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Great Attack effect",
        "canonicalCooldown": "15 sec.",
        "canonicalCooldownMs": 15000,
        "desc": "大幅提升攻擊能力。",
        "balance": {
            "mpCost": 91,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_great_attack",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 80,
        "classes": [
            "hierophant",
            "prophet"
        ]
    },
    "turn_to_stone": {
        "id": "turn_to_stone",
        "name": "石化",
        "slug": "turn_to_stone",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1540.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Turn to Stone effect",
        "canonicalCooldown": "8 min.",
        "canonicalCooldownMs": 480000,
        "desc": "使自身進入石化狀態。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_turn_to_stone",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 79,
        "classes": [
            "hierophant"
        ]
    },
    "improved_sleep": {
        "id": "improved_sleep",
        "name": "強化睡眠",
        "slug": "improved_sleep",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/mount_sleep.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "With a high chance, inflicts Sleep on the target.",
        "canonicalCooldown": "20 sec.",
        "canonicalCooldownMs": 20000,
        "desc": "有高機率使目標陷入睡眠。",
        "balance": {
            "mpCost": 75,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_improved_sleep",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 85,
        "classes": [
            "hierophant"
        ]
    },
    "unique_seal_of_damage": {
        "id": "unique_seal_of_damage",
        "name": "獨特傷害封印",
        "slug": "unique_seal_of_damage",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/unique_silence.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "With a certain chance, blocks additional attack damage from enemies in front of ",
        "canonicalCooldown": "2 min.",
        "canonicalCooldownMs": 120000,
        "desc": "有一定機率阻擋前方敵人的追加攻擊傷害，並可能觸發物理攻擊弱化或吸血弱化。物理攻擊弱化：物理攻擊 -100；吸血弱化：攻擊時 生命值 恢復 -95%。",
        "balance": {
            "mpCost": 91,
            "pwr": 10,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_unique_seal_of_damage",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "hierophant"
        ]
    },
    "crusader": {
        "id": "crusader",
        "name": "十字軍",
        "slug": "crusader",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/transform_priest.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Crusader effect",
        "canonicalCooldown": ":",
        "canonicalCooldownMs": 1000,
        "desc": "進入十字軍戰鬥狀態。",
        "balance": {
            "mpCost": 200,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_crusader",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "hierophant"
        ]
    },
    "summon_mew_the_cat": {
        "id": "summon_mew_the_cat",
        "name": "召喚咪嗚貓",
        "slug": "summon_mew_the_cat",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1225.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Summons Mew the Cat.",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "召喚咪嗚貓。",
        "balance": {
            "mpCost": 39,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_summon_mew_the_cat",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "warlock"
        ]
    },
    "servitor_heal": {
        "id": "servitor_heal",
        "name": "召喚獸治療",
        "slug": "servitor_heal",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1127.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Recovers servitor's HP with 169 power.",
        "canonicalCooldown": "3 sec.",
        "canonicalCooldownMs": 3000,
        "desc": "以威力 169 恢復召喚獸 生命值。",
        "balance": {
            "mpCost": 24,
            "pwr": 169,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_servitor_heal",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "arcanaLord",
            "necromancer",
            "phantomSummoner",
            "spectralMaster",
            "warlock"
        ]
    },
    "servitor_recharge": {
        "id": "servitor_recharge",
        "name": "召喚獸充能",
        "slug": "servitor_recharge",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1126.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Recovers servitor's MP. Power 41.The MP recovery effect depends on servitor's MP",
        "canonicalCooldown": "3 sec.",
        "canonicalCooldownMs": 3000,
        "desc": "恢復召喚獸 魔力，威力 41。實際 魔力 恢復效果取決於召喚獸的 魔力。",
        "balance": {
            "mpCost": 42,
            "pwr": 41,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_servitor_recharge",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "arcanaLord",
            "necromancer",
            "phantomSummoner",
            "spectralMaster",
            "warlock"
        ]
    },
    "summon_kat_the_cat": {
        "id": "summon_kat_the_cat",
        "name": "召喚凱特貓",
        "slug": "summon_kat_the_cat",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1111.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Summons Kat the Cat.",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "召喚凱特貓。",
        "balance": {
            "mpCost": 39,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_summon_kat_the_cat",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "warlock"
        ]
    },
    "transfer_pain": {
        "id": "transfer_pain",
        "name": "轉移痛苦",
        "slug": "transfer_pain",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1262.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Transfers 10% of received damage to a servitor.MP Consumption is based on your l",
        "canonicalCooldown": "1s",
        "canonicalCooldownMs": 1000,
        "desc": "將受到傷害的 10% 轉移給召喚獸。魔力 消耗依角色等級決定。",
        "balance": {
            "mpCost": 7,
            "pwr": 1,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_transfer_pain",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "necromancer",
            "phantomSummoner",
            "warlock"
        ]
    },
    "summon_kai_the_cat": {
        "id": "summon_kai_the_cat",
        "name": "召喚凱伊貓",
        "slug": "summon_kai_the_cat",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1276.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Summons Kai the Cat.",
        "canonicalCooldown": "5 sec.",
        "canonicalCooldownMs": 5000,
        "desc": "召喚凱伊貓。",
        "balance": {
            "mpCost": 70,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_summon_kai_the_cat",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "warlock"
        ]
    },
    "power_supremacy": {
        "id": "power_supremacy",
        "name": "力量支配",
        "slug": "power_supremacy",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1611.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "When activatedP. Atk. +10%Max HP -10%",
        "canonicalCooldown": "1s",
        "canonicalCooldownMs": 1000,
        "desc": "啟動時物理攻擊 +10%、最大生命值 -10%。",
        "balance": {
            "mpCost": 15,
            "pwr": 1,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_power_supremacy",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 48,
        "classes": [
            "arcanaLord",
            "phantomSummoner",
            "spectralMaster",
            "warlock"
        ]
    },
    "servitor_cure": {
        "id": "servitor_cure",
        "name": "召喚獸淨化",
        "slug": "servitor_cure",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1300.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Removes Infection debuffs from a servitor.",
        "canonicalCooldown": "4 sec.",
        "canonicalCooldownMs": 4000,
        "desc": "移除召喚獸身上的感染類弱化效果。",
        "balance": {
            "mpCost": 35,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_servitor_cure",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "phantomSummoner",
            "warlock"
        ]
    },
    "servitor_ultimate_defense": {
        "id": "servitor_ultimate_defense",
        "name": "召喚獸極限防禦",
        "slug": "servitor_ultimate_defense",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1299.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Absolute skillThe effect is applied on servitors.P. Def. +1800M. Def. +1350Buff ",
        "canonicalCooldown": "15 min.",
        "canonicalCooldownMs": 900000,
        "desc": "絕對技能，效果作用於召喚獸：物理防禦 +1800、魔法防禦 +1350、增益解除抗性 +80%。",
        "balance": {
            "mpCost": 48,
            "pwr": 8,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_servitor_ultimate_defense",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 52,
        "classes": [
            "arcanaLord",
            "phantomSummoner",
            "spectralMaster",
            "warlock"
        ]
    },
    "swift_servitor_heal": {
        "id": "swift_servitor_heal",
        "name": "快速召喚獸治療",
        "slug": "swift_servitor_heal",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1127.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Recovers servitor's HP with 160 power.Available when HP < 80%.",
        "canonicalCooldown": "3 sec.",
        "canonicalCooldownMs": 3000,
        "desc": "以威力 160 恢復召喚獸 生命值。僅在 生命值 低於 80% 時可用。",
        "balance": {
            "mpCost": 24,
            "pwr": 160,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_swift_servitor_heal",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "arcanaLord",
            "necromancer",
            "phantomSummoner",
            "spectralMaster",
            "warlock"
        ]
    },
    "servitor_wind_walk": {
        "id": "servitor_wind_walk",
        "name": "召喚獸風之疾走",
        "slug": "servitor_wind_walk",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1144.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "The effect is applied on servitors.P. Def. +8%M. Def. +10%Atk. Spd. +10%Speed +2",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "效果作用於召喚獸：物理防禦 +8%、魔法防禦 +10%、攻速 +10%、速度 +20。",
        "balance": {
            "mpCost": 30,
            "pwr": 1,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_servitor_wind_walk",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 48,
        "classes": [
            "warlock"
        ]
    },
    "blessed_shield": {
        "id": "blessed_shield",
        "name": "祝福之盾",
        "slug": "blessed_shield",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1243.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Shield Defense Rate +5%",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "盾牌防禦率 +5%。",
        "balance": {
            "mpCost": 35,
            "pwr": 1,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_blessed_shield",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "elfElder",
            "prophet"
        ]
    },
    "servitor_blessing": {
        "id": "servitor_blessing",
        "name": "召喚獸祝福",
        "slug": "servitor_blessing",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1301.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Servitor Blessing effect",
        "canonicalCooldown": "4 sec.",
        "canonicalCooldownMs": 4000,
        "desc": "為召喚獸施加祝福強化。",
        "balance": {
            "mpCost": 58,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_servitor_blessing",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 62,
        "classes": [
            "phantomSummoner",
            "warlock"
        ]
    },
    "summon_friend": {
        "id": "summon_friend",
        "name": "召喚好友",
        "slug": "summon_friend",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1403.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Summon Friend effect",
        "canonicalCooldown": "2 min.",
        "canonicalCooldownMs": 120000,
        "desc": "將好友召喚至身邊。",
        "balance": {
            "mpCost": 180,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_summon_friend",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 56,
        "classes": [
            "elementalSummoner",
            "phantomSummoner",
            "warlock"
        ]
    },
    "return": {
        "id": "return",
        "name": "返回",
        "slug": "return",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1050.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Teleports to the nearest village.Cannot be used in special places such as the GM",
        "canonicalCooldown": "1 min.",
        "canonicalCooldownMs": 60000,
        "desc": "傳送至最近的村莊。在 管理員 諮詢服務等特殊地點無法使用。",
        "balance": {
            "mpCost": 105,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_return",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "elfElder",
            "prophet"
        ]
    },
    "change_armor": {
        "id": "change_armor",
        "name": "更換鎧甲",
        "slug": "change_armor",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "1★",
        "starRank": 1,
        "grade": "COMMON",
        "combatSkill": false,
        "roleType": "COSMETIC_VISUAL_ONLY",
        "icon": "/icons/change_death_armor.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Alterna a aparência visual da armadura do Death Knight. Efeito puramente cosmético.",
        "canonicalCooldown": "1s",
        "canonicalCooldownMs": 1000,
        "desc": "死亡騎士 的鎧甲外觀技能，只改變外觀，不造成傷害。",
        "balance": {
            "mpCost": 0,
            "pwr": 0,
            "pveMultiplier": 0,
            "pvpMultiplier": 0
        },
        "vfxId": "vfx_change_armor",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 1,
        "classes": [
            "deathPilgrim",
            "rider"
        ]
    },
    "mystic_resilience": {
        "id": "mystic_resilience",
        "name": "神秘韌性",
        "slug": "mystic_resilience",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1411_a.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "With a certain chance, when hit while the effect is active:Removes a debuff.Debu",
        "canonicalCooldown": "2 min.",
        "canonicalCooldownMs": 120000,
        "desc": "效果啟動期間受到攻擊時，有一定機率移除 1 個弱化效果。弱化抗性 +3%、異常抗性 +3%。",
        "balance": {
            "mpCost": 15,
            "pwr": 0,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_mystic_resilience",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 51,
        "classes": [
            "prophet"
        ]
    },
    "ethereal_strike": {
        "id": "ethereal_strike",
        "name": "乙太打擊",
        "slug": "ethereal_strike",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1558.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Creates a mysterious energy vortex that attacks the target.Deals M. damage to th",
        "canonicalCooldown": "1 sec.",
        "canonicalCooldownMs": 1000,
        "desc": "創造神秘能量漩渦攻擊目標，造成魔法傷害，威力 147。",
        "balance": {
            "mpCost": 56,
            "pwr": 147,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_ethereal_strike",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "arcanaLord",
            "spectralMaster"
        ]
    },
    "advanced_block": {
        "id": "advanced_block",
        "name": "高階格擋",
        "slug": "advanced_block",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1304.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Shield Defense +10%",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "盾牌防禦 +10%。",
        "balance": {
            "mpCost": 44,
            "pwr": 1,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_advanced_block",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 42,
        "classes": [
            "elfElder",
            "hierophant",
            "prophet"
        ]
    },
    "warrior_bane": {
        "id": "warrior_bane",
        "name": "戰士剋星",
        "slug": "warrior_bane",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1350.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Weakens the target.P. Atk. -23%Speed -20",
        "canonicalCooldown": "3 sec.",
        "canonicalCooldownMs": 3000,
        "desc": "削弱目標：物理攻擊 -23%、速度 -20。",
        "balance": {
            "mpCost": 70,
            "pwr": 2,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_warrior_bane",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "arcanaLord",
            "spectralMaster"
        ]
    },
    "ray_of_light": {
        "id": "ray_of_light",
        "name": "光之射線",
        "slug": "ray_of_light",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_luminous_ray.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Emits a light beam with the spirits' help.Deals M. damage to the target. Power 2",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "借助精靈力量發射光束，對目標造成魔法傷害，威力 202。",
        "balance": {
            "mpCost": 82,
            "pwr": 202,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_ray_of_light",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "arcanaLord",
            "spectralMaster"
        ]
    },
    "mage_bane": {
        "id": "mage_bane",
        "name": "法師剋星",
        "slug": "mage_bane",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1351.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Weakens the target.M. Atk. -23%Casting Spd. -20%",
        "canonicalCooldown": "3 sec.",
        "canonicalCooldownMs": 3000,
        "desc": "削弱目標：魔法攻擊 -23%、施法速度 -20%。",
        "balance": {
            "mpCost": 70,
            "pwr": 2,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_mage_bane",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "arcanaLord",
            "spectralMaster"
        ]
    },
    "summon_feline_queen": {
        "id": "summon_feline_queen",
        "name": "召喚貓后",
        "slug": "summon_feline_queen",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1331.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Summons Feline Queen.",
        "canonicalCooldown": "5 sec.",
        "canonicalCooldownMs": 5000,
        "desc": "召喚貓后。",
        "balance": {
            "mpCost": 144,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_summon_feline_queen",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 77,
        "classes": [
            "arcanaLord"
        ]
    },
    "chains_of_pain": {
        "id": "chains_of_pain",
        "name": "痛苦鎖鏈",
        "slug": "chains_of_pain",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_life_link.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Max HP +1000Transfers 55% of received damage to a servitor.MP Consumption is bas",
        "canonicalCooldown": "1s",
        "canonicalCooldownMs": 1000,
        "desc": "最大生命值 +1000。將受到傷害的 55% 轉移給召喚獸，魔力 消耗依角色等級決定。",
        "balance": {
            "mpCost": 13,
            "pwr": 6,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_chains_of_pain",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "arcanaLord",
            "spectralMaster"
        ]
    },
    "servitor_share": {
        "id": "servitor_share",
        "name": "召喚獸共享",
        "slug": "servitor_share",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1557.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Transfers a certain percentage of the summoner's stats to the servitor.P./ M. At",
        "canonicalCooldown": "1 min.",
        "canonicalCooldownMs": 60000,
        "desc": "將召喚師部分屬性轉移給召喚獸：物理／魔法攻擊 50%、物理／魔法防禦 50%、攻速／施法速度 35%、最大生命值／魔力 30%。",
        "balance": {
            "mpCost": 105,
            "pwr": 5,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_servitor_share",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "arcanaLord",
            "phantomSummoner",
            "spectralMaster",
            "warlock"
        ]
    },
    "summon_feline_king": {
        "id": "summon_feline_king",
        "name": "召喚貓王",
        "slug": "summon_feline_king",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1406.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Summons Feline King.",
        "canonicalCooldown": "5 sec.",
        "canonicalCooldownMs": 5000,
        "desc": "召喚貓王。",
        "balance": {
            "mpCost": 144,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_summon_feline_king",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 77,
        "classes": [
            "arcanaLord"
        ]
    },
    "summon_cubic_of_secrets": {
        "id": "summon_cubic_of_secrets",
        "name": "召喚祕密魔方",
        "slug": "summon_cubic_of_secrets",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/skill19565.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Summons Cubic of Secrets.M. Atk. +150When using magic skills (attack/ anomaly/ d",
        "canonicalCooldown": ":",
        "canonicalCooldownMs": 1000,
        "desc": "召喚祕密魔方，魔法攻擊 +150。使用攻擊／異常／弱化魔法時會追加攻擊，由召喚師的幻影攻擊目標；觸發冷卻 15 秒。",
        "balance": {
            "mpCost": 78,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_summon_cubic_of_secrets",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "arcanaLord"
        ]
    },
    "powerful_servitor_share": {
        "id": "powerful_servitor_share",
        "name": "強力召喚獸共享",
        "slug": "powerful_servitor_share",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill19220.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Powerful Servitor Share effect",
        "canonicalCooldown": "30 sec.",
        "canonicalCooldownMs": 30000,
        "desc": "強化召喚師與召喚獸之間的屬性共享。",
        "balance": {
            "mpCost": 117,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_powerful_servitor_share",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "arcanaLord",
            "spectralMaster"
        ]
    },
    "summon_cat_emperor": {
        "id": "summon_cat_emperor",
        "name": "召喚貓皇",
        "slug": "summon_cat_emperor",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/s_emperor_cat.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Summon Cat Emperor effect",
        "canonicalCooldown": ".-",
        "canonicalCooldownMs": 1000,
        "desc": "召喚貓皇。",
        "balance": {
            "mpCost": 186,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_summon_cat_emperor",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "arcanaLord"
        ]
    },
    "warrior_servitor": {
        "id": "warrior_servitor",
        "name": "戰士召喚獸",
        "slug": "warrior_servitor",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1346.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Makes the servitor possessed with an ancient warrior's spirit.Max HP +50%Max MP ",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "讓召喚獸獲得古代戰士之魂：最大生命值 +50%、最大魔力 +20%、物理攻擊 +45%、魔法攻擊 +20%、物理防禦 +45%、魔法防禦 +20%、攻速 +35%、施法速度 +15%、基礎暴擊率 +50 與 +15%、魔法技能暴擊率 +50 與 +15%、基礎／魔法技能暴擊傷害 +15%、弱化／異常抗性 +40%。",
        "balance": {
            "mpCost": 70,
            "pwr": 5,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_warrior_servitor",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 78,
        "classes": [
            "arcanaLord"
        ]
    },
    "dark_burst": {
        "id": "dark_burst",
        "name": "黑暗爆發",
        "slug": "dark_burst",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill11032.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks the target and nearby enemies with a dark energy blast.Deals M. damage. ",
        "canonicalCooldown": "7 sec.",
        "canonicalCooldownMs": 7000,
        "desc": "以黑暗能量爆炸攻擊目標與附近敵人，造成魔法傷害，威力 61。",
        "balance": {
            "mpCost": 28,
            "pwr": 61,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_dark_burst",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 46,
        "classes": [
            "necromancer",
            "soultaker"
        ]
    },
    "curse_fear": {
        "id": "curse_fear",
        "name": "恐懼詛咒",
        "slug": "curse_fear",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1169.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Frightens the enemy.",
        "canonicalCooldown": "20 sec.",
        "canonicalCooldownMs": 20000,
        "desc": "使敵人陷入恐懼。",
        "balance": {
            "mpCost": 35,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_curse_fear",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "necromancer",
            "soultaker"
        ]
    },
    "anchor": {
        "id": "anchor",
        "name": "定身",
        "slug": "anchor",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1170.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "For 5 sec., slowly stiffens the target's body paralyzing them for 3 sec.",
        "canonicalCooldown": "1 min.",
        "canonicalCooldownMs": 60000,
        "desc": "在 5 秒內逐漸僵硬目標身體，之後使其麻痺 3 秒。",
        "balance": {
            "mpCost": 39,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_anchor",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "necromancer",
            "soultaker"
        ]
    },
    "death_spike": {
        "id": "death_spike",
        "name": "死亡尖刺",
        "slug": "death_spike",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1148.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks with the dark energy.Deals M. damage to the target. Power 84.",
        "canonicalCooldown": "1 sec.",
        "canonicalCooldownMs": 1000,
        "desc": "以黑暗能量攻擊目標，造成魔法傷害，威力 84。",
        "balance": {
            "mpCost": 33,
            "pwr": 84,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_death_spike",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "necromancer",
            "soultaker"
        ]
    },
    "summon_reanimated_man": {
        "id": "summon_reanimated_man",
        "name": "召喚復生者",
        "slug": "summon_reanimated_man",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1129.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Summons Reanimated Man.",
        "canonicalCooldown": "5 sec.",
        "canonicalCooldownMs": 5000,
        "desc": "召喚復生者。",
        "balance": {
            "mpCost": 78,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_summon_reanimated_man",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 44,
        "classes": [
            "necromancer"
        ]
    },
    "summon_corrupted_man": {
        "id": "summon_corrupted_man",
        "name": "召喚墮落者",
        "slug": "summon_corrupted_man",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1154.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Summons Corrupted Man.",
        "canonicalCooldown": "5 sec.",
        "canonicalCooldownMs": 5000,
        "desc": "召喚墮落者。",
        "balance": {
            "mpCost": 70,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_summon_corrupted_man",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "necromancer"
        ]
    },
    "shining_beam": {
        "id": "shining_beam",
        "name": "閃耀光束",
        "slug": "shining_beam",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_shining_ray_new2.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks the target with the power of Light granted by the unbreakable oath.Deals",
        "canonicalCooldown": "1 sec.",
        "canonicalCooldownMs": 1000,
        "desc": "以不可動搖誓言所賜予的光之力量攻擊目標，造成魔法傷害，威力 126。帶有弱化／異常狀態的敵人會受到更多傷害，對惡魔與不死族造成大量傷害。",
        "balance": {
            "mpCost": 56,
            "pwr": 126,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_shining_beam",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "cardinal"
        ]
    },
    "mana_effect_boost": {
        "id": "mana_effect_boost",
        "name": "魔力效果強化",
        "slug": "mana_effect_boost",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1013.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Max MP +20%MP Recovery Rate +5.1",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "最大魔力 +20%，魔力 恢復率 +5.1。",
        "balance": {
            "mpCost": 70,
            "pwr": 2,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_mana_effect_boost",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "bishop",
            "cardinal",
            "cleric",
            "elfElder",
            "elfOracle",
            "evaSaint",
            "shillienElder",
            "shillienOracle",
            "shillienSaint"
        ]
    },
    "prophecy_of_light": {
        "id": "prophecy_of_light",
        "name": "光之預言",
        "slug": "prophecy_of_light",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill11784.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Max HP +10%Max MP +10%P. Def. +10%M. Def. +10%PvE damage bonus +20%Healing Power",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "最大生命值 +10%、最大魔力 +10%、物理防禦 +10%、魔法防禦 +10%、對怪物戰鬥傷害 +20%、治療威力 +15、弱化抗性 +10%、異常抗性與 對怪物戰鬥傷害額外提升 10%。",
        "balance": {
            "mpCost": 72,
            "pwr": 1,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_prophecy_of_light",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "cardinal"
        ]
    },
    "paralysis": {
        "id": "paralysis",
        "name": "麻痺",
        "slug": "paralysis",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1042.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Inflicts Paralysis on the target.",
        "canonicalCooldown": "20 sec.",
        "canonicalCooldownMs": 20000,
        "desc": "使目標陷入麻痺。",
        "balance": {
            "mpCost": 69,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_paralysis",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "bishop",
            "cardinal"
        ]
    },
    "favorable_cover_saved_life": {
        "id": "favorable_cover_saved_life",
        "name": "有利掩護：保命",
        "slug": "favorable_cover_saved_life",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill11785_new.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Favorable Cover: Saved Life effect",
        "canonicalCooldown": "5 min.",
        "canonicalCooldownMs": 300000,
        "desc": "啟動有利掩護的保命效果。",
        "balance": {
            "mpCost": 60,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_favorable_cover_saved_life",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "bishop",
            "cardinal",
            "elfElder",
            "evaSaint",
            "shillienElder",
            "shillienSaint"
        ]
    },
    "dark_side": {
        "id": "dark_side",
        "name": "黑暗面",
        "slug": "dark_side",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_eclipse.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "黑暗面 effect",
        "canonicalCooldown": ".Terms",
        "canonicalCooldownMs": 1000,
        "desc": "進入黑暗面狀態。",
        "balance": {
            "mpCost": 200,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_dark_side",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "cardinal"
        ]
    },
    "unleashing": {
        "id": "unleashing",
        "name": "力量解放",
        "slug": "unleashing",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_awaken.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Unleashing effect",
        "canonicalCooldown": "6 min.",
        "canonicalCooldownMs": 360000,
        "desc": "解放潛藏力量。",
        "balance": {
            "mpCost": 80,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_unleashing",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "cardinal",
            "evaSaint",
            "shillienSaint"
        ]
    },
    "greater_heal": {
        "id": "greater_heal",
        "name": "高級治療",
        "slug": "greater_heal",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1217.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Recovers the target's HP with 1030 power.",
        "canonicalCooldown": "3 sec.",
        "canonicalCooldownMs": 3000,
        "desc": "以威力 1030 恢復目標 生命值。",
        "balance": {
            "mpCost": 92,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_greater_heal",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "bishop",
            "cardinal",
            "elfElder",
            "shillienElder"
        ]
    },
    "life_balance": {
        "id": "life_balance",
        "name": "生命平衡",
        "slug": "life_balance",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1335.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Life Balance effect",
        "canonicalCooldown": "30 sec.",
        "canonicalCooldownMs": 30000,
        "desc": "平衡生命力。",
        "balance": {
            "mpCost": 123,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_life_balance",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "cardinal"
        ]
    },
    "greater_battle_heal": {
        "id": "greater_battle_heal",
        "name": "高級戰鬥治療",
        "slug": "greater_battle_heal",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1218.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Momentarily recovers HP. Power 863.",
        "canonicalCooldown": "1 sec.",
        "canonicalCooldownMs": 1000,
        "desc": "立即恢復生命值，威力 863。",
        "balance": {
            "mpCost": 177,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_greater_battle_heal",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "bishop",
            "cardinal",
            "elfElder",
            "shillienElder"
        ]
    },
    "restore_life": {
        "id": "restore_life",
        "name": "生命恢復",
        "slug": "restore_life",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1258.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Restore Life effect",
        "canonicalCooldown": "30 sec.",
        "canonicalCooldownMs": 30000,
        "desc": "恢復生命力。",
        "balance": {
            "mpCost": 177,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_restore_life",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "bishop",
            "cardinal"
        ]
    },
    "magical_backfire": {
        "id": "magical_backfire",
        "name": "魔力反噬",
        "slug": "magical_backfire",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1396.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "The target's Skill MP Consumption +200%",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "使目標技能 魔力 消耗 +200%。",
        "balance": {
            "mpCost": 75,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_magical_backfire",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "bishop",
            "cardinal"
        ]
    },
    "chain_heal": {
        "id": "chain_heal",
        "name": "連鎖治療",
        "slug": "chain_heal",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1553.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Applied on 10 nearby allies including the target.Recovers 30% of Max HP to the m",
        "canonicalCooldown": "7 sec.",
        "canonicalCooldownMs": 7000,
        "desc": "作用於目標與附近最多 10 名盟友。最受傷的盟友恢復最大生命值的 30%，其餘盟友恢復較少 生命值。",
        "balance": {
            "mpCost": 194,
            "pwr": 3,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_chain_heal",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "cardinal"
        ]
    },
    "greater_group_heal": {
        "id": "greater_group_heal",
        "name": "高級群體治療",
        "slug": "greater_group_heal",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1219.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Recovers all party members' HP with 827 power.",
        "canonicalCooldown": "6 sec.",
        "canonicalCooldownMs": 6000,
        "desc": "以威力 827 恢復所有隊伍成員 生命值。",
        "balance": {
            "mpCost": 158,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_greater_group_heal",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "bishop",
            "cardinal",
            "elfElder",
            "shillienElder"
        ]
    },
    "blessing": {
        "id": "blessing",
        "name": "祝福",
        "slug": "blessing",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1271.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Note!If affected by 黑暗面 and 黑暗面: Master, decreases the amount of rec",
        "canonicalCooldown": "15 min.",
        "canonicalCooldownMs": 900000,
        "desc": "注意：受到 黑暗面 與 黑暗面：大師 影響時，恢復量會降低。為一名隊伍成員恢復 100% 生命值，只能在魔力 30% 以下時使用。",
        "balance": {
            "mpCost": 235,
            "pwr": 10,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_blessing",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "bishop",
            "cardinal"
        ]
    },
    "balanced_body": {
        "id": "balanced_body",
        "name": "平衡體魄",
        "slug": "balanced_body",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill14414.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Balanced Body effect",
        "canonicalCooldown": "15 sec.",
        "canonicalCooldownMs": 15000,
        "desc": "使身體狀態保持平衡。",
        "balance": {
            "mpCost": 249,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_balanced_body",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "cardinal"
        ]
    },
    "dark_vortex": {
        "id": "dark_vortex",
        "name": "黑暗漩渦",
        "slug": "dark_vortex",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1343.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Creates a vortex that draws its strength from the Dark dimension.Deals M. damage",
        "canonicalCooldown": "30 sec.",
        "canonicalCooldownMs": 30000,
        "desc": "召喚來自黑暗次元的漩渦，對目標造成魔法傷害，威力 158。",
        "balance": {
            "mpCost": 116,
            "pwr": 158,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_dark_vortex",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "soultaker"
        ]
    },
    "miracle": {
        "id": "miracle",
        "name": "奇蹟",
        "slug": "miracle",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/skill1426.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Miracle effect",
        "canonicalCooldown": "1",
        "canonicalCooldownMs": 1000,
        "desc": "施展奇蹟效果。",
        "balance": {
            "mpCost": 72,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_miracle",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 80,
        "classes": [
            "cardinal"
        ]
    },
    "salvation": {
        "id": "salvation",
        "name": "救贖",
        "slug": "salvation",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1410.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Salvation effect",
        "canonicalCooldown": "1",
        "canonicalCooldownMs": 1000,
        "desc": "施展救贖效果。",
        "balance": {
            "mpCost": 87,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_salvation",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 79,
        "classes": [
            "cardinal"
        ]
    },
    "sensation": {
        "id": "sensation",
        "name": "感應",
        "slug": "sensation",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/s_sensation.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Sensation effect",
        "canonicalCooldown": "15 min.",
        "canonicalCooldownMs": 900000,
        "desc": "啟動感應能力。",
        "balance": {
            "mpCost": 103,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_sensation",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "cardinal"
        ]
    },
    "tranquility": {
        "id": "tranquility",
        "name": "寧靜",
        "slug": "tranquility",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/serenity.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Tranquility effect",
        "canonicalCooldown": "5 min.",
        "canonicalCooldownMs": 300000,
        "desc": "進入寧靜狀態。",
        "balance": {
            "mpCost": 105,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_tranquility",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "cardinal"
        ]
    },
    "cleanse": {
        "id": "cleanse",
        "name": "淨化",
        "slug": "cleanse",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1409.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Cleanse effect",
        "canonicalCooldown": "8 sec.",
        "canonicalCooldownMs": 8000,
        "desc": "清除負面狀態。",
        "balance": {
            "mpCost": 58,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_cleanse",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 77,
        "classes": [
            "cardinal"
        ]
    },
    "divine_whisper": {
        "id": "divine_whisper",
        "name": "神聖低語",
        "slug": "divine_whisper",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/s_divine_whisper.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Divine Whisper effect",
        "canonicalCooldown": "30 sec.",
        "canonicalCooldownMs": 30000,
        "desc": "獲得神聖低語的力量。",
        "balance": {
            "mpCost": 100,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_divine_whisper",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "cardinal"
        ]
    },
    "advanced_cleanse": {
        "id": "advanced_cleanse",
        "name": "高級淨化",
        "slug": "advanced_cleanse",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_erase.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Removes the target's debuffs and anomalies.Debuff/ Anomaly Resistance +10%",
        "canonicalCooldown": "8 sec.",
        "canonicalCooldownMs": 8000,
        "desc": "移除目標的弱化與異常狀態，並使弱化／異常抗性 +10%。",
        "balance": {
            "mpCost": 55,
            "pwr": 1,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_advanced_cleanse",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "cardinal"
        ]
    },
    "divine_power_acquisition": {
        "id": "divine_power_acquisition",
        "name": "神聖力量吸收",
        "slug": "divine_power_acquisition",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/elemental_protection.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Divine Power Acquisition effect",
        "canonicalCooldown": "1s",
        "canonicalCooldownMs": 1000,
        "desc": "吸收並獲得神聖力量。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_divine_power_acquisition",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "cardinal"
        ]
    },
    "void_explosion": {
        "id": "void_explosion",
        "name": "虛空爆發",
        "slug": "void_explosion",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/skill11817.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Makes an explosion imbued with the Dark energy.Deals M. damage to the target. Po",
        "canonicalCooldown": "1 sec.",
        "canonicalCooldownMs": 1000,
        "desc": "引發注入黑暗能量的爆炸，對目標造成魔法傷害，威力 152。帶有弱化／異常狀態的敵人會受到更多傷害。",
        "balance": {
            "mpCost": 56,
            "pwr": 152,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_void_explosion",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "soultaker"
        ]
    },
    "master_of_void": {
        "id": "master_of_void",
        "name": "虛空大師",
        "slug": "master_of_void",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_abyssal_expert1.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Master of Void effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "強化對虛空力量的掌控。",
        "balance": {
            "mpCost": 120,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_master_of_void",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "soultaker"
        ]
    },
    "dark_spiral": {
        "id": "dark_spiral",
        "name": "黑暗螺旋",
        "slug": "dark_spiral",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/s_dark_spiral.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks with a dark spiral.Deals M. damage to the target and nearby enemies. Pow",
        "canonicalCooldown": "30 sec.",
        "canonicalCooldownMs": 30000,
        "desc": "以黑暗螺旋攻擊目標與附近敵人，威力 229。5 秒內使 力量／精神 -2。玩家對戰／對怪物戰鬥最多攻擊 6 個目標。",
        "balance": {
            "mpCost": 116,
            "pwr": 229,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_dark_spiral",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "soultaker"
        ]
    },
    "soul_guardian": {
        "id": "soul_guardian",
        "name": "靈魂守護者",
        "slug": "soul_guardian",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_soul_guardian.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Soul Guardian effect",
        "canonicalCooldown": "5 min.",
        "canonicalCooldownMs": 300000,
        "desc": "獲得靈魂守護力量。",
        "balance": {
            "mpCost": 125,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_soul_guardian",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "soultaker"
        ]
    },
    "divine_strike": {
        "id": "divine_strike",
        "name": "神聖打擊",
        "slug": "divine_strike",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/energy_burst1.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Summons holy light.Deals M. damage to the target. Power 30.Deals heavy damage to",
        "canonicalCooldown": "1 sec.",
        "canonicalCooldownMs": 1000,
        "desc": "召喚神聖光芒，對目標造成魔法傷害，威力 30。對惡魔與不死族造成大量傷害。",
        "balance": {
            "mpCost": 19,
            "pwr": 30,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_divine_strike",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 20,
        "classes": [
            "cleric",
            "elfOracle",
            "shillienOracle"
        ]
    },
    "summon_cursed_man": {
        "id": "summon_cursed_man",
        "name": "召喚詛咒者",
        "slug": "summon_cursed_man",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1334.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Summons Cursed Man.",
        "canonicalCooldown": "5 sec.",
        "canonicalCooldownMs": 5000,
        "desc": "召喚詛咒者。",
        "balance": {
            "mpCost": 151,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_summon_cursed_man",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 79,
        "classes": [
            "soultaker"
        ]
    },
    "heal": {
        "id": "heal",
        "name": "治療術",
        "slug": "heal",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/skill1011.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Recovers the target's HP with 50 power.",
        "canonicalCooldown": "3 sec.",
        "canonicalCooldownMs": 3000,
        "desc": "以威力 50 恢復目標 生命值。",
        "balance": {
            "mpCost": 10,
            "pwr": 50,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_heal",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 20,
        "classes": [
            "cleric",
            "elfOracle",
            "shillienOracle"
        ]
    },
    "group_heal": {
        "id": "group_heal",
        "name": "群體治療",
        "slug": "group_heal",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/skill1027.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Recovers all party members' HP with 66 power.",
        "canonicalCooldown": "6 sec.",
        "canonicalCooldownMs": 6000,
        "desc": "以威力 66 恢復所有隊伍成員 生命值。",
        "balance": {
            "mpCost": 33,
            "pwr": 66,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_group_heal",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 20,
        "classes": [
            "bishop",
            "cleric",
            "elfOracle",
            "shillienOracle"
        ]
    },
    "peace": {
        "id": "peace",
        "name": "和平",
        "slug": "peace",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/skill1075.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Peace effect",
        "canonicalCooldown": "5 sec.",
        "canonicalCooldownMs": 5000,
        "desc": "使目標進入和平狀態。",
        "balance": {
            "mpCost": 30,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_peace",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 35,
        "classes": [
            "bishop",
            "cardinal",
            "cleric"
        ]
    },
    "cure": {
        "id": "cure",
        "name": "治癒",
        "slug": "cure",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/skill11772.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Removes Infection debuffs from the target.",
        "canonicalCooldown": "4 sec.",
        "canonicalCooldownMs": 4000,
        "desc": "移除目標身上的感染類弱化效果。",
        "balance": {
            "mpCost": 10,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_cure",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 35,
        "classes": [
            "bishop",
            "cardinal",
            "cleric",
            "elfElder",
            "elfOracle",
            "evaSaint",
            "shillienElder",
            "shillienOracle"
        ]
    },
    "resurrection": {
        "id": "resurrection",
        "name": "復活術",
        "slug": "resurrection",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/skill1016.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Resurrects a dead party member.XP recovery: 0%",
        "canonicalCooldown": "30 sec.",
        "canonicalCooldownMs": 30000,
        "desc": "復活死亡的隊伍成員。經驗值 恢復：0%。",
        "balance": {
            "mpCost": 59,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_resurrection",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 20,
        "classes": [
            "bishop",
            "cleric",
            "elfElder",
            "elfOracle",
            "shillienOracle"
        ]
    },
    "divine_power": {
        "id": "divine_power",
        "name": "神聖力量",
        "slug": "divine_power",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1459.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Healing Power +20",
        "canonicalCooldown": "5 min.",
        "canonicalCooldownMs": 300000,
        "desc": "治療威力 +20。",
        "balance": {
            "mpCost": 70,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_divine_power",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 51,
        "classes": [
            "bishop"
        ]
    },
    "body_of_avatar": {
        "id": "body_of_avatar",
        "name": "化身之體",
        "slug": "body_of_avatar",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1311.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Note!When using 黑暗面 and 黑暗面: Master, removes the buff and prevents t",
        "canonicalCooldown": "2 min.",
        "canonicalCooldownMs": 120000,
        "desc": "使用 黑暗面 或 黑暗面：大師 時，移除該增益並阻止目標再次使用。恢復最大生命值的 10%，並使最大生命值 +10%。",
        "balance": {
            "mpCost": 39,
            "pwr": 1,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_body_of_avatar",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 48,
        "classes": [
            "bishop"
        ]
    },
    "favorable_cover": {
        "id": "favorable_cover",
        "name": "有利掩護",
        "slug": "favorable_cover",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill11785.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Absolute skillUses magic to change a protective stance.Max HP +50Max HP +10%P. D",
        "canonicalCooldown": "5 min.",
        "canonicalCooldownMs": 300000,
        "desc": "絕對技能。使用魔法切換為防護姿態：最大生命值 +50、最大生命值 +10%、物理防禦 +10%、速度 +5。",
        "balance": {
            "mpCost": 50,
            "pwr": 1,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_favorable_cover",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 58,
        "classes": []
    },
    "requiem": {
        "id": "requiem",
        "name": "安魂曲",
        "slug": "requiem",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1049.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Nearby monsters do not attack first.",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "附近怪物不會主動攻擊。",
        "balance": {
            "mpCost": 33,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_requiem",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "bishop"
        ]
    },
    "quick_dash": {
        "id": "quick_dash",
        "name": "迅捷突進",
        "slug": "quick_dash",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/s_speed_charge.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Quick Dash effect",
        "canonicalCooldown": "3 sec.",
        "canonicalCooldownMs": 3000,
        "desc": "快速向前衝刺。",
        "balance": {
            "mpCost": 35,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_quick_dash",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 20,
        "classes": [
            "warg"
        ]
    },
    "repose": {
        "id": "repose",
        "name": "安撫",
        "slug": "repose",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1034.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Decreases nearby monsters' aggression.",
        "canonicalCooldown": "5 sec.",
        "canonicalCooldownMs": 5000,
        "desc": "降低附近怪物的仇恨。",
        "balance": {
            "mpCost": 33,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_repose",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 44,
        "classes": [
            "bishop"
        ]
    },
    "mass_resurrection": {
        "id": "mass_resurrection",
        "name": "群體復活",
        "slug": "mass_resurrection",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1254.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Resurrects dead clan members.XP recovery: 0%",
        "canonicalCooldown": "10 min.",
        "canonicalCooldownMs": 600000,
        "desc": "復活死亡的血盟成員。經驗值 恢復：0%。",
        "balance": {
            "mpCost": 243,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_mass_resurrection",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "bishop"
        ]
    },
    "vitalize": {
        "id": "vitalize",
        "name": "活力恢復",
        "slug": "vitalize",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1020.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Recovers the target's HP. Power 460.Removes debuffs from the target.P. Atk. redu",
        "canonicalCooldown": "3 sec.",
        "canonicalCooldownMs": 3000,
        "desc": "以威力 460 恢復目標 生命值，並移除降低物理攻擊、魔法攻擊、物理防禦、魔法防禦 的弱化效果。",
        "balance": {
            "mpCost": 84,
            "pwr": 460,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_vitalize",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 46,
        "classes": [
            "bishop",
            "elfElder",
            "shillienElder"
        ]
    },
    "direct_strike": {
        "id": "direct_strike",
        "name": "直接打擊",
        "slug": "direct_strike",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/s_straight.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks the target.Deals P. damage. Power 460.Ignores Shield Defense.Ignores 15%",
        "canonicalCooldown": "0.5 sec.",
        "canonicalCooldownMs": 500,
        "desc": "對目標造成物理傷害，威力 460。無視盾牌防禦與目標 15% 防禦力。等級 21 起攻擊兩次；狼形態時變為雙爪打擊，對單一目標攻擊兩次。",
        "balance": {
            "mpCost": 15,
            "pwr": 460,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_direct_strike",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 25,
        "classes": [
            "warg"
        ]
    },
    "punishment": {
        "id": "punishment",
        "name": "懲戒",
        "slug": "punishment",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/punishment_b.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks the target. Power 403.Ignores Shield Defense.Ignores 15% of the target's",
        "canonicalCooldown": "1 sec.",
        "canonicalCooldownMs": 1000,
        "desc": "攻擊目標，威力 403。無視盾牌防禦與目標 15% 防禦力。",
        "balance": {
            "mpCost": 18,
            "pwr": 403,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_punishment",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 20,
        "classes": [
            "deathBlade",
            "deathKnight",
            "deathMessenger"
        ]
    },
    "blow": {
        "id": "blow",
        "name": "重擊",
        "slug": "blow",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/s_assassin_swoop_attack.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Blow effect",
        "canonicalCooldown": "1 sec.",
        "canonicalCooldownMs": 1000,
        "desc": "發動強力重擊。",
        "balance": {
            "mpCost": 22,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_blow",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 20,
        "classes": [
            "assassinDE",
            "assassinS0",
            "assassinS1",
            "assassinS2",
            "assassinS3"
        ]
    },
    "roar_of_death": {
        "id": "roar_of_death",
        "name": "死亡咆哮",
        "slug": "roar_of_death",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/death_roar.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Roar of Death effect",
        "canonicalCooldown": "2 min.",
        "canonicalCooldownMs": 120000,
        "desc": "發出死亡咆哮。",
        "balance": {
            "mpCost": 50,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_roar_of_death",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 30,
        "classes": [
            "deathBlade",
            "deathKnight",
            "deathMessenger"
        ]
    },
    "forward_move": {
        "id": "forward_move",
        "name": "向前突進",
        "slug": "forward_move",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/s_assassin_front_dash.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "<Rush skill>Moves you forward.",
        "canonicalCooldown": "3 sec.",
        "canonicalCooldownMs": 3000,
        "desc": "向前快速移動。",
        "balance": {
            "mpCost": 45,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_forward_move",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "assassinS2"
        ]
    },
    "shadow_attack": {
        "id": "shadow_attack",
        "name": "暗影攻擊",
        "slug": "shadow_attack",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/s_assassin_charge_attack.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "<Changing skill>A shadow attacks the target.The attack type is based on the dist",
        "canonicalCooldown": "3 sec.",
        "canonicalCooldownMs": 3000,
        "desc": "由暗影攻擊目標，攻擊方式依距離改變；距離過遠時會突進。對目標攻擊兩次，威力 914。無視盾牌防禦與目標 10% 防禦力。玩家對戰／對怪物戰鬥最多攻擊 3 個目標。",
        "balance": {
            "mpCost": 35,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_shadow_attack",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "assassinS1",
            "assassinS2",
            "assassinS3"
        ]
    },
    "shadow_blast": {
        "id": "shadow_blast",
        "name": "暗影爆發",
        "slug": "shadow_blast",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/s_assassin_shadow_explosion.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Shadow Blast effect",
        "canonicalCooldown": "1s",
        "canonicalCooldownMs": 1000,
        "desc": "釋放暗影能量爆發。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_shadow_blast",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "assassinS3"
        ]
    },
    "change_appearance": {
        "id": "change_appearance",
        "name": "變更外觀",
        "slug": "change_appearance",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/s_assassin_armor_change.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Change Appearance effect",
        "canonicalCooldown": "1s",
        "canonicalCooldownMs": 1000,
        "desc": "變更角色外觀。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_change_appearance",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 1,
        "classes": [],
        "availableTo": [],
        "disabled": true,
        "removalReason": "cosmetic_mount_purge"
    },
    "assassination": {
        "id": "assassination",
        "name": "暗殺",
        "slug": "assassination",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_assassinate.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Assassination effect",
        "canonicalCooldown": ".Terms",
        "canonicalCooldownMs": 1000,
        "desc": "發動致命暗殺攻擊。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_assassination",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "assassinS3"
        ]
    },
    "sharp_blade": {
        "id": "sharp_blade",
        "name": "銳利之刃",
        "slug": "sharp_blade",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/s_assassin_acute_buff.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Sharp Blade effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "強化刀刃的鋒利度。",
        "balance": {
            "mpCost": 50,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_sharp_blade",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 50,
        "classes": [
            "assassinS2",
            "assassinS3"
        ]
    },
    "erosion": {
        "id": "erosion",
        "name": "侵蝕",
        "slug": "erosion",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/s_erosion.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Erosion effect",
        "canonicalCooldown": "1s",
        "canonicalCooldownMs": 1000,
        "desc": "以侵蝕效果削弱敵人。",
        "balance": {
            "mpCost": 50,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_erosion",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 60,
        "classes": [
            "assassinS2",
            "assassinS3"
        ]
    },
    "murder_attempt": {
        "id": "murder_attempt",
        "name": "殺意突襲",
        "slug": "murder_attempt",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/s_enroachment.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Murder Attempt effect",
        "canonicalCooldown": "1s",
        "canonicalCooldownMs": 1000,
        "desc": "發動帶有殺意的突襲。",
        "balance": {
            "mpCost": 50,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_murder_attempt",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 60,
        "classes": [
            "assassinS2",
            "assassinS3"
        ]
    },
    "assassin_s_secret_notes_2nd_page": {
        "id": "assassin_s_secret_notes_2nd_page",
        "name": "刺客祕笈－第 2 頁",
        "slug": "assassin_s_secret_notes_2nd_page",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/etc_piece_of_paper_black_i00.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Assassin's Secret Notes - 2nd Page effect",
        "canonicalCooldown": "2",
        "canonicalCooldownMs": 1000,
        "desc": "刺客祕笈的第 2 頁。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_assassin_s_secret_notes_2nd_page",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 60,
        "classes": [
            "assassinS2"
        ]
    },
    "assassin_s_secret_notes_1st_page": {
        "id": "assassin_s_secret_notes_1st_page",
        "name": "刺客祕笈－第 1 頁",
        "slug": "assassin_s_secret_notes_1st_page",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/etc_piece_of_paper_gray_i00.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Assassin's Secret Notes - 1st Page effect",
        "canonicalCooldown": "2",
        "canonicalCooldownMs": 1000,
        "desc": "刺客祕笈的第 1 頁。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_assassin_s_secret_notes_1st_page",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 45,
        "classes": [
            "assassinS2"
        ]
    },
    "assassin_s_secret_notes_3rd_page": {
        "id": "assassin_s_secret_notes_3rd_page",
        "name": "刺客祕笈－第 3 頁",
        "slug": "assassin_s_secret_notes_3rd_page",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/etc_piece_of_paper_white_i00.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Assassin's Secret Notes - 3rd Page effect",
        "canonicalCooldown": "1",
        "canonicalCooldownMs": 1000,
        "desc": "刺客祕笈的第 3 頁。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_assassin_s_secret_notes_3rd_page",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 70,
        "classes": [
            "assassinS2"
        ]
    },
    "resolve_to_kill": {
        "id": "resolve_to_kill",
        "name": "殺意決心",
        "slug": "resolve_to_kill",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_assassin_get_point.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Resolve to Kill effect",
        "canonicalCooldown": "20 min.",
        "canonicalCooldownMs": 1200000,
        "desc": "堅定殺敵的決心。",
        "balance": {
            "mpCost": 100,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_resolve_to_kill",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "assassinS3"
        ]
    },
    "clone_dance": {
        "id": "clone_dance",
        "name": "分身之舞",
        "slug": "clone_dance",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/s_assassin_combo_attack.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "<Changing skill>Creates shadow clones that attack several targets.Attacks the en",
        "canonicalCooldown": "20 sec.",
        "canonicalCooldownMs": 20000,
        "desc": "創造暗影分身攻擊多個目標，連續攻擊敵人 2 次，威力 5500。無視盾牌防禦與目標 15% 防禦力。玩家對戰／對怪物戰鬥最多攻擊 7 個目標。",
        "balance": {
            "mpCost": 99,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_clone_dance",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "assassinS3"
        ]
    },
    "relentless_attack": {
        "id": "relentless_attack",
        "name": "無情連擊",
        "slug": "relentless_attack",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_assassin_triple_attack.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Strikes at the target several times in a row.Attacks the enemy thrice. Power 480",
        "canonicalCooldown": "5 sec.",
        "canonicalCooldownMs": 5000,
        "desc": "連續猛攻目標 3 次，威力 4800。無視盾牌防禦與目標 15% 防禦力。",
        "balance": {
            "mpCost": 77,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_relentless_attack",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "assassinS3"
        ]
    },
    "silver_wolf": {
        "id": "silver_wolf",
        "name": "銀狼",
        "slug": "silver_wolf",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_assassin_riding_wolf.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Silver Wolf effect",
        "canonicalCooldown": "1s",
        "canonicalCooldownMs": 1000,
        "desc": "獲得銀狼之力。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_silver_wolf",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "assassinS3"
        ]
    },
    "wipeout": {
        "id": "wipeout",
        "name": "殲滅",
        "slug": "wipeout",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/brandish_b.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks the target. Power 1569.Ignores Shield Defense.Ignores 20% of the target'",
        "canonicalCooldown": "8 sec.",
        "canonicalCooldownMs": 8000,
        "desc": "攻擊目標，威力 1569。無視盾牌防禦與目標 20% 防禦力。",
        "balance": {
            "mpCost": 33,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_wipeout",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "deathKnight",
            "deathMessenger"
        ]
    },
    "time_distortion": {
        "id": "time_distortion",
        "name": "時間扭曲",
        "slug": "time_distortion",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/s_time_distortion_new.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Distorts time around you.Attacks nearby enemies 3 time(s). Power 20,000.Ignores ",
        "canonicalCooldown": "3 min.",
        "canonicalCooldownMs": 180000,
        "desc": "扭曲周圍時間，對附近敵人攻擊 3 次，威力 20000。無視盾牌防禦與目標 20% 防禦力。玩家對戰／對怪物戰鬥最多攻擊 10 個目標。使用後獲得 時間大師：物理攻擊 +1000、+10%，物理技能威力 +10%、暴擊傷害 +5%、攻速 +30%、速度 +30、技能迴避 +10%，持續 60 秒。",
        "balance": {
            "mpCost": 200,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_time_distortion",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "assassinS3"
        ]
    },
    "deadly_pull": {
        "id": "deadly_pull",
        "name": "致命牽引",
        "slug": "deadly_pull",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1030.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "<Pull attack>Makes an attack. Power 1500. After that, pulls the target and appli",
        "canonicalCooldown": "3 sec.",
        "canonicalCooldownMs": 3000,
        "desc": "發動牽引攻擊，威力 1500，之後拉近目標並附加 致命之觸。無視盾牌防禦與目標 5% 防禦力。致命之觸 持續 3 秒，使受到的暴擊率 +10%、禁錮抗性 -10%。",
        "balance": {
            "mpCost": 15,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_deadly_pull",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "deathKnight",
            "deathMessenger"
        ]
    },
    "constraint": {
        "id": "constraint",
        "name": "束縛限制",
        "slug": "constraint",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/s_suppression.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Constraint effect",
        "canonicalCooldown": "3 min.",
        "canonicalCooldownMs": 180000,
        "desc": "限制目標行動。",
        "balance": {
            "mpCost": 150,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_constraint",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "assassinS3"
        ]
    },
    "stigma_of_death": {
        "id": "stigma_of_death",
        "name": "死亡烙印",
        "slug": "stigma_of_death",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/drain_soul.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks the target and nearby enemies. Power 1371.Ignores Shield Defense.Ignores",
        "canonicalCooldown": "15 sec.",
        "canonicalCooldownMs": 15000,
        "desc": "攻擊目標與附近敵人，威力 1371。無視盾牌防禦與目標 15% 防禦力。玩家對戰 最多 5 個、對怪物戰鬥 最多 5 個目標。攻擊時附加 黑暗印記，使弱化／異常抗性 -5%。",
        "balance": {
            "mpCost": 100,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_stigma_of_death",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "deathMessenger"
        ]
    },
    "fist_of_fury": {
        "id": "fist_of_fury",
        "name": "狂怒之拳",
        "slug": "fist_of_fury",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/fist_of_fury_human.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks the target with a fist imbued with fire energy.Power 2064.With a certain",
        "canonicalCooldown": "1 sec.",
        "canonicalCooldownMs": 1000,
        "desc": "以火焰能量包覆拳頭攻擊目標，威力 2064。攻擊時有機率獲得 1死亡點數。無視盾牌防禦與目標 10% 防禦力，並附加 狂暴爆破，使物理防禦／魔法防禦 -23%。",
        "balance": {
            "mpCost": 50,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_fist_of_fury",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 50,
        "classes": [
            "deathBlade",
            "deathKnight",
            "deathMessenger"
        ]
    },
    "return_motion": {
        "id": "return_motion",
        "name": "返回動作",
        "slug": "return_motion",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_assassin_back_dash.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Return Motion effect",
        "canonicalCooldown": "1 min.",
        "canonicalCooldownMs": 60000,
        "desc": "執行返回動作。",
        "balance": {
            "mpCost": 100,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_return_motion",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "assassinS3"
        ]
    },
    "flaming_body": {
        "id": "flaming_body",
        "name": "燃燒之體",
        "slug": "flaming_body",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill11154.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "DP recovering skillRecovers 1 DP on attack (cooldown: 1 sec.).P. Atk. +20%P. Ski",
        "canonicalCooldown": ":",
        "canonicalCooldownMs": 1000,
        "desc": "死亡點數恢復技能。攻擊時恢復 1死亡點數（冷卻 1 秒）。物理攻擊 +20%、物理技能暴擊率 +2%、暴擊傷害 +1%。並可強化 燃燒領域 與 死亡騎士 火焰劍相關技能。",
        "balance": {
            "mpCost": 15,
            "pwr": 2,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_flaming_body",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 70,
        "classes": [
            "deathKnight",
            "deathMessenger"
        ]
    },
    "bone_cage": {
        "id": "bone_cage",
        "name": "骨牢",
        "slug": "bone_cage",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/flame_bone_prison.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Puts the target into Bone Cage.Imprisoned enemies cannot be attacked.They cannot",
        "canonicalCooldown": "20 sec.",
        "canonicalCooldownMs": 20000,
        "desc": "將目標囚禁於骨牢中。禁錮期間無法被攻擊，也無法移動、攻擊、傳送、鎖定、使用技能或恢復生命值／魔力／戰鬥點數。效果結束時，玩家對戰 使最大生命值／魔力 -10%；對怪物戰鬥 則造成威力 8500 的物理傷害，並引發相當於物理攻擊 200% 的爆炸。",
        "balance": {
            "mpCost": 57,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_bone_cage",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 55,
        "classes": [
            "deathKnight",
            "deathMessenger"
        ]
    },
    "hellfire": {
        "id": "hellfire",
        "name": "地獄烈焰",
        "slug": "hellfire",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/hell_fire_human.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks nearby enemies. Power 4282. Transforms into Death Knight attacking nearb",
        "canonicalCooldown": "30 sec.",
        "canonicalCooldownMs": 30000,
        "desc": "攻擊附近敵人，威力 4282，並化身 死亡騎士 再次攻擊。可附加 殘酷灼燒，使 物理／魔法攻擊 -250、物理／魔法防禦 -40%。若目標帶有 滅亡印記，會追加威力 8000 的攻擊。玩家對戰／對怪物戰鬥最多攻擊 20 個目標。",
        "balance": {
            "mpCost": 15,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_hellfire",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "grade": "UNPROVEN",
        "bookRequirement": {
            "required": null,
            "tier": "UNPROVEN",
            "stars": null,
            "itemId": null
        },
        "classes": [
            "deathKnight"
        ]
    },
    "burning_field": {
        "id": "burning_field",
        "name": "燃燒領域",
        "slug": "burning_field",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_burning_field_new.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks the enemies thrice. Power 7500.Ignores Shield Defense.Ignores 15% of the",
        "canonicalCooldown": "30 sec.",
        "canonicalCooldownMs": 30000,
        "desc": "連續攻擊敵人 3 次，威力 7500，之後追加一次威力 7500 的物理攻擊並施加 燃燒沼澤。無視盾牌防禦與目標 15% 防禦力。燃燒沼澤 持續 5 秒，使突進技能無法使用。",
        "balance": {
            "mpCost": 15,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_burning_field",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "deathKnight"
        ]
    },
    "death_guard": {
        "id": "death_guard",
        "name": "死亡守護",
        "slug": "death_guard",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/shapeshift_npc_shield1.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Death Guard effect",
        "canonicalCooldown": ":",
        "canonicalCooldownMs": 1000,
        "desc": "啟動死亡守護效果。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_death_guard",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "deathKnight",
            "deathMessenger"
        ]
    },
    "call_of_flame": {
        "id": "call_of_flame",
        "name": "火焰召喚",
        "slug": "call_of_flame",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1492.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Call of Flame effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "呼喚火焰之力。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_call_of_flame",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 60,
        "classes": [
            "deathKnight",
            "deathMessenger"
        ]
    },
    "rage_charge": {
        "id": "rage_charge",
        "name": "狂怒衝鋒",
        "slug": "rage_charge",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/fist_of_fury_human_new.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Rush skillRushes to the enemy dealing P. damage. Power 5658. Applies Furious Bla",
        "canonicalCooldown": "3 sec.",
        "canonicalCooldownMs": 3000,
        "desc": "突進至敵人並造成物理傷害，威力 5658。附加 狂暴爆破，使 物理／魔法防禦 -30%、劍／雙手劍抗性 -10%、牽引抗性 -30%。使用時有 30% 機率恢復 2死亡點數。",
        "balance": {
            "mpCost": 80,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_rage_charge",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 80,
        "classes": [
            "deathKnight"
        ]
    },
    "stigma_of_evil": {
        "id": "stigma_of_evil",
        "name": "邪惡烙印",
        "slug": "stigma_of_evil",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/death_stigma.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks enemies. Power 3200. Applies the Mark of Demise debuff on the enemy.Igno",
        "canonicalCooldown": "12 sec.",
        "canonicalCooldownMs": 12000,
        "desc": "攻擊敵人，威力 3200，並施加 滅亡印記。無視盾牌防禦與目標 15% 防禦力。印記持續 5 秒，使弱化／異常抗性 -10%。地獄烈焰 命中印記目標時會觸發追加攻擊；擊殺印記目標後可疊加 靈魂竊取 強化。",
        "balance": {
            "mpCost": 100,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_stigma_of_evil",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 80,
        "classes": [
            "deathKnight"
        ]
    },
    "upward_strike": {
        "id": "upward_strike",
        "name": "上挑打擊",
        "slug": "upward_strike",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/s_uppercut.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks the target.Deals P. damage. Power 992.Ignores Shield Defense.Ignores 15%",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "對目標造成物理傷害，威力 992。無視盾牌防禦與目標 15% 防禦力。狼形態時變為 爪擊橫掃；學會 狼之精通 後會增加攻擊次數。",
        "balance": {
            "mpCost": 40,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_upward_strike",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "warg"
        ]
    },
    "devour": {
        "id": "devour",
        "name": "吞噬",
        "slug": "devour",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/consume.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Recovers 500 DP.Available only if DP are 500 or lower.",
        "canonicalCooldown": "30 sec.",
        "canonicalCooldownMs": 30000,
        "desc": "恢復 500死亡點數。僅在死亡點數 低於或等於 500 時可使用。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_devour",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "deathKnight"
        ]
    },
    "deadly_ligament_rupture": {
        "id": "deadly_ligament_rupture",
        "name": "致命韌帶撕裂",
        "slug": "deadly_ligament_rupture",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/rupture.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Removes and blocks the following buffs:- Wind Walk- Blade Movement- Fast Run",
        "canonicalCooldown": "15 sec.",
        "canonicalCooldownMs": 15000,
        "desc": "移除並封鎖以下增益：風之疾走、劍刃步伐、快速奔跑。",
        "balance": {
            "mpCost": 43,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_deadly_ligament_rupture",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "deathKnight"
        ]
    },
    "ultimate_death_knight": {
        "id": "ultimate_death_knight",
        "name": "終極死亡騎士",
        "slug": "ultimate_death_knight",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/death_knight_transform.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Transforms into a Death Knight who is cursed by the Fire Dragon Valakas, but pos",
        "canonicalCooldown": "1 min.",
        "canonicalCooldownMs": 60000,
        "desc": "變身為受到火龍 巴拉卡斯 詛咒的 死亡騎士，獲得強大力量：物理攻擊 +2000、+20%，物理防禦 +1000、+10%，物理技能暴擊率 +20%、暴擊傷害 +10%、物理技能傷害 +10%、技能威力 +5%、全武器抗性 +5%、玩家對戰傷害 +6%、攻速 +10%、速度 +10，並提高神聖抗性、降低受到治療懲罰。",
        "balance": {
            "mpCost": 15,
            "pwr": 2,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_ultimate_death_knight",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "deathKnight"
        ]
    },
    "flame_grip": {
        "id": "flame_grip",
        "name": "火焰之握",
        "slug": "flame_grip",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_burning_grip.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Flame Grip effect",
        "canonicalCooldown": "1 min.",
        "canonicalCooldownMs": 60000,
        "desc": "以火焰力量束縛目標。",
        "balance": {
            "mpCost": 70,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_flame_grip",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "deathKnight"
        ]
    },
    "mount_night_mare": {
        "id": "mount_night_mare",
        "name": "騎乘夢魘",
        "slug": "mount_night_mare",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/riding_knightmare.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Mount Night Mare effect",
        "canonicalCooldown": "1s",
        "canonicalCooldownMs": 1000,
        "desc": "召喚並騎乘夢魘。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_mount_night_mare",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [],
        "availableTo": [],
        "disabled": true,
        "removalReason": "cosmetic_mount_purge"
    },
    "howling": {
        "id": "howling",
        "name": "狼嚎",
        "slug": "howling",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/s_werewolf_howling.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Howling effect",
        "canonicalCooldown": ":",
        "canonicalCooldownMs": 1000,
        "desc": "發出強烈狼嚎。",
        "balance": {
            "mpCost": 50,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_howling",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "warg"
        ]
    },
    "moon_s_grace": {
        "id": "moon_s_grace",
        "name": "月之恩典",
        "slug": "moon_s_grace",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/s_werewolf_lv_reward2.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Moon's Grace effect",
        "canonicalCooldown": "2",
        "canonicalCooldownMs": 1000,
        "desc": "獲得月之恩典。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_moon_s_grace",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 60,
        "classes": []
    },
    "young_moon_s_grace": {
        "id": "young_moon_s_grace",
        "name": "新月恩典",
        "slug": "young_moon_s_grace",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/s_werewolf_lv_reward1.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Young Moon's Grace effect",
        "canonicalCooldown": "2",
        "canonicalCooldownMs": 1000,
        "desc": "獲得新月恩典。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_young_moon_s_grace",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 45,
        "classes": []
    },
    "full_moon_s_grace": {
        "id": "full_moon_s_grace",
        "name": "滿月恩典",
        "slug": "full_moon_s_grace",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/s_werewolf_lv_reward3.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Full Moon's Grace effect",
        "canonicalCooldown": "1",
        "canonicalCooldownMs": 1000,
        "desc": "獲得滿月恩典。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_full_moon_s_grace",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 70,
        "classes": []
    },
    "recovery": {
        "id": "recovery",
        "name": "恢復",
        "slug": "recovery",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/skill11770.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Recovery effect",
        "canonicalCooldown": "1 min.",
        "canonicalCooldownMs": 60000,
        "desc": "恢復自身狀態。",
        "balance": {
            "mpCost": 20,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_recovery",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 20,
        "classes": [
            "elfScout",
            "elvenKnight",
            "plainsWalker",
            "silverRanger",
            "swordSinger",
            "templeKnight"
        ]
    },
    "synchro_freedom": {
        "id": "synchro_freedom",
        "name": "同步自由",
        "slug": "synchro_freedom",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_synchro_freedom.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Synchro Freedom effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "啟動同步自由效果。",
        "balance": {
            "mpCost": 50,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_synchro_freedom",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "windRider"
        ]
    },
    "fury_blade": {
        "id": "fury_blade",
        "name": "狂怒之刃",
        "slug": "fury_blade",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/s_fury_blade.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Fury Blade effect",
        "canonicalCooldown": "20 sec.",
        "canonicalCooldownMs": 20000,
        "desc": "以狂怒強化刀刃。",
        "balance": {
            "mpCost": 100,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_fury_blade",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "windRider"
        ]
    },
    "wind_riding": {
        "id": "wind_riding",
        "name": "御風",
        "slug": "wind_riding",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill0769.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Speed +30P. Evasion +15Vital Point Attack Rate +20%P. Skill Evasion +60%Buff Can",
        "canonicalCooldown": "15 min.",
        "canonicalCooldownMs": 900000,
        "desc": "速度 +30、物理迴避 +15、要害攻擊率 +20%、物理技能迴避 +60%、增益解除抗性 +90%。可與 極限迴避 同時使用。",
        "balance": {
            "mpCost": 36,
            "pwr": 2,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_wind_riding",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "windRider"
        ]
    },
    "evasion": {
        "id": "evasion",
        "name": "迴避",
        "slug": "evasion",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill0446.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "P. Skill Evasion +50%M. Skill Evasion +50%(The effect is removed after 5 hit(s).",
        "canonicalCooldown": "5 min.",
        "canonicalCooldownMs": 300000,
        "desc": "物理技能迴避 +50%、魔法技能迴避 +50%。受到 5 次攻擊後效果解除。",
        "balance": {
            "mpCost": 40,
            "pwr": 5,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_evasion",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 79,
        "classes": [
            "windRider"
        ]
    },
    "mount_pegasus": {
        "id": "mount_pegasus",
        "name": "騎乘天馬",
        "slug": "mount_pegasus",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1834.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Mount Pegasus effect",
        "canonicalCooldown": "1s",
        "canonicalCooldownMs": 1000,
        "desc": "召喚並騎乘天馬。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_mount_pegasus",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [],
        "availableTo": [],
        "disabled": true,
        "removalReason": "cosmetic_mount_purge"
    },
    "deadly_rhythm": {
        "id": "deadly_rhythm",
        "name": "致命節奏",
        "slug": "deadly_rhythm",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_deadly_beat_classic1.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks the target. Power 4615.For 10 sec., P. Def. -23%.Ignores Shield Defense.",
        "canonicalCooldown": "1 sec.",
        "canonicalCooldownMs": 1000,
        "desc": "攻擊目標，威力 4615。10 秒內使物理防禦 -23%。無視盾牌防禦與目標 10% 防禦力。",
        "balance": {
            "mpCost": 60,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_deadly_rhythm",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "spectralDancer",
            "swordMuse"
        ]
    },
    "entangle": {
        "id": "entangle",
        "name": "纏繞",
        "slug": "entangle",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/skill0102.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Entangle effect",
        "canonicalCooldown": "20 sec.",
        "canonicalCooldownMs": 20000,
        "desc": "以魔法纏繞限制目標。",
        "balance": {
            "mpCost": 12,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_entangle",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 36,
        "classes": [
            "elvenKnight",
            "evaTemplar",
            "plainsWalker",
            "templeKnight"
        ]
    },
    "symphony": {
        "id": "symphony",
        "name": "交響曲",
        "slug": "symphony",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill0098.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks the target. Power 1737.For 3 sec., blocks the target's M. skills.",
        "canonicalCooldown": "20 sec.",
        "canonicalCooldownMs": 20000,
        "desc": "攻擊目標，威力 1737。3 秒內封鎖目標魔法技能。",
        "balance": {
            "mpCost": 60,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_symphony",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "swordMuse",
            "swordSinger"
        ]
    },
    "wild_beat": {
        "id": "wild_beat",
        "name": "狂野節拍",
        "slug": "wild_beat",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/s_madness_beat.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Charms the target making them move toward you. When the Paralysis effect expires",
        "canonicalCooldown": "25 sec.",
        "canonicalCooldownMs": 25000,
        "desc": "魅惑目標使其向你移動。麻痺效果結束時對目標造成威力 9848 的傷害。無視盾牌防禦與目標 10% 防禦力。",
        "balance": {
            "mpCost": 15,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_wild_beat",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "swordMuse"
        ]
    },
    "slowdown": {
        "id": "slowdown",
        "name": "緩速",
        "slug": "slowdown",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/skill11767.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Slowdown effect",
        "canonicalCooldown": "+200%Terms",
        "canonicalCooldownMs": 1000,
        "desc": "降低目標速度。",
        "balance": {
            "mpCost": 11,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_slowdown",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 20,
        "classes": [
            "elvenKnight",
            "evaTemplar",
            "templeKnight"
        ]
    },
    "song_of_silence": {
        "id": "song_of_silence",
        "name": "寂靜之歌",
        "slug": "song_of_silence",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/s_song_of_silence.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Blocks P./ M. skills of nearby enemies.Duration: 2 sec.Attacks the target. Power",
        "canonicalCooldown": "1 min.",
        "canonicalCooldownMs": 60000,
        "desc": "封鎖附近敵人的物理／魔法技能，持續 2 秒。之後攻擊目標，威力 7579。無視盾牌防禦與目標 10% 防禦力。另有機率解除傷害上限。玩家對戰／對怪物戰鬥最多攻擊 6 個目標。",
        "balance": {
            "mpCost": 95,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_song_of_silence",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 85,
        "classes": [
            "swordMuse"
        ]
    },
    "song_of_wind": {
        "id": "song_of_wind",
        "name": "風之歌",
        "slug": "song_of_wind",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_song_of_wind.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "<Rush skill>Attacks the target. Power 3996.Ignores Shield Defense.Ignores 10% of",
        "canonicalCooldown": "5 sec.",
        "canonicalCooldownMs": 5000,
        "desc": "突進並攻擊目標，威力 3996。無視盾牌防禦與目標 10% 防禦力。",
        "balance": {
            "mpCost": 32,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_song_of_wind",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 80,
        "classes": [
            "swordMuse",
            "swordSinger"
        ]
    },
    "song_of_cosmos": {
        "id": "song_of_cosmos",
        "name": "宇宙之歌",
        "slug": "song_of_cosmos",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_song_of_water.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "P. Atk. +10%P. Skill Critical Rate +5%P. Skill Critical Damage +5%P. Skill damag",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "物理攻擊 +10%、物理技能暴擊率 +5%、暴擊傷害 +5%、對怪物戰鬥 物理技能傷害 +5%、速度 +10。",
        "balance": {
            "mpCost": 160,
            "pwr": 1,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_song_of_cosmos",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 80,
        "classes": [
            "swordMuse",
            "swordSinger"
        ]
    },
    "frantic_pace": {
        "id": "frantic_pace",
        "name": "狂亂步伐",
        "slug": "frantic_pace",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_extreme_tempo1.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks the target. Power 6075.Ignores Shield Defense.Ignores 10% of the target'",
        "canonicalCooldown": "1 sec.",
        "canonicalCooldownMs": 1000,
        "desc": "攻擊目標，威力 6075。無視盾牌防禦與目標 10% 防禦力。",
        "balance": {
            "mpCost": 64,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_frantic_pace",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "spectralDancer",
            "swordMuse"
        ]
    },
    "song_of_hunter": {
        "id": "song_of_hunter",
        "name": "獵人之歌",
        "slug": "song_of_hunter",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_song_of_hunter.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks the target and nearby enemies twice. Power 3154.Ignores Shield Defense.I",
        "canonicalCooldown": "16 sec.",
        "canonicalCooldownMs": 16000,
        "desc": "連續兩次攻擊目標與附近敵人，威力 3154。無視盾牌防禦與目標 10% 防禦力。玩家對戰／對怪物戰鬥最多攻擊 4 個目標。",
        "balance": {
            "mpCost": 76,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_song_of_hunter",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 79,
        "classes": [
            "swordMuse",
            "swordSinger"
        ]
    },
    "song_of_earth": {
        "id": "song_of_earth",
        "name": "大地之歌",
        "slug": "song_of_earth",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_song_of_earth.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Caster:P. Atk. +500P./ M. Def. +500P. Skill Critical Rate +5%Speed +15P. Skill C",
        "canonicalCooldown": "-3%<Skill",
        "canonicalCooldownMs": 1000,
        "desc": "施法者獲得物理攻擊 +500、物理／魔法防禦 +500、物理技能暴擊率 +5%、速度 +15、物理技能冷卻 -3%。並強化 狂亂步伐、風之歌、獵人之歌 等技能；使用強化版 狂亂步伐 時會追加威力 7286 的攻擊。",
        "balance": {
            "mpCost": 97,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_song_of_earth",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 80,
        "classes": [
            "swordMuse",
            "swordSinger"
        ]
    },
    "amadeus": {
        "id": "amadeus",
        "name": "阿瑪迪斯",
        "slug": "amadeus",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/s_amadeus_self.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Amadeus effect",
        "canonicalCooldown": "1 min.",
        "canonicalCooldownMs": 60000,
        "desc": "啟動阿瑪迪斯效果。",
        "balance": {
            "mpCost": 375,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_amadeus",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "swordMuse"
        ]
    },
    "guard_crush": {
        "id": "guard_crush",
        "name": "守衛粉碎",
        "slug": "guard_crush",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill10009.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks the target. Power 811.Ignores Shield Defense.Ignores 10% of the target's",
        "canonicalCooldown": "1 sec.",
        "canonicalCooldownMs": 1000,
        "desc": "攻擊目標，威力 811。無視盾牌防禦與目標 10% 防禦力。",
        "balance": {
            "mpCost": 33,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_guard_crush",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "bladeDancer",
            "swordSinger"
        ]
    },
    "additional_strain": {
        "id": "additional_strain",
        "name": "額外負荷",
        "slug": "additional_strain",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_walkout.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "P./ M. Skill Cooldown +3 sec.",
        "canonicalCooldown": "+3 sec.",
        "canonicalCooldownMs": 3000,
        "desc": "物理／魔法技能冷卻時間 +3 秒。",
        "balance": {
            "mpCost": 45,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_additional_strain",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "swordMuse",
            "swordSinger"
        ]
    },
    "eliminate_obstruction": {
        "id": "eliminate_obstruction",
        "name": "排除障礙",
        "slug": "eliminate_obstruction",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0305.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Eliminate Obstruction effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "清除阻礙並突破敵方防線。",
        "balance": {
            "mpCost": 50,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_eliminate_obstruction",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 74,
        "classes": [
            "swordMuse",
            "swordSinger"
        ]
    },
    "sword_symphony": {
        "id": "sword_symphony",
        "name": "劍之交響曲",
        "slug": "sword_symphony",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/sympony_sword.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Sword Symphony effect",
        "canonicalCooldown": ":",
        "canonicalCooldownMs": 1000,
        "desc": "以劍舞奏出交響攻擊。",
        "balance": {
            "mpCost": 100,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_sword_symphony",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "swordMuse",
            "swordSinger"
        ]
    },
    "aqua_strike": {
        "id": "aqua_strike",
        "name": "水之打擊",
        "slug": "aqua_strike",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/s_aqua_strike_eva.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks the target. Power 619.Ignores Shield Defense.Ignores 10% of the target's",
        "canonicalCooldown": "1 sec.",
        "canonicalCooldownMs": 1000,
        "desc": "攻擊目標，威力 619。無視盾牌防禦與目標 10% 防禦力。",
        "balance": {
            "mpCost": 33,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_aqua_strike",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "evaTemplar",
            "templeKnight"
        ]
    },
    "life_magic_harmony_defense": {
        "id": "life_magic_harmony_defense",
        "name": "生命魔法和聲－防禦",
        "slug": "life_magic_harmony_defense",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/life_return.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "P. Def. +100M. Def. +100Max HP +100Recovers 5 MP on killing an enemy.",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "物理防禦 +100、魔法防禦 +100、最大生命值 +100。擊殺敵人時恢復 5 魔力。",
        "balance": {
            "mpCost": 30,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_life_magic_harmony_defense",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "evaTemplar",
            "templeKnight"
        ]
    },
    "battle_training": {
        "id": "battle_training",
        "name": "戰鬥訓練",
        "slug": "battle_training",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0985.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "P. Atk. +30Atk. Spd. +10",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "物理攻擊 +30、攻擊速度 +10。",
        "balance": {
            "mpCost": 86,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_battle_training",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "evaTemplar",
            "shillienKnight",
            "shillienTemplar",
            "templeKnight"
        ]
    },
    "shield_bash": {
        "id": "shield_bash",
        "name": "盾牌猛擊",
        "slug": "shield_bash",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0352.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks with a shield.Deals P. damage. Power 1970.With a certain chance, stuns f",
        "canonicalCooldown": "20 sec.",
        "canonicalCooldownMs": 20000,
        "desc": "以盾牌攻擊造成物理傷害，威力 1970。有一定機率使目標暈眩 3 秒，不會暴擊。",
        "balance": {
            "mpCost": 52,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_shield_bash",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 66,
        "classes": [
            "evaTemplar",
            "templeKnight"
        ]
    },
    "chameleon_rest": {
        "id": "chameleon_rest",
        "name": "變色龍休息",
        "slug": "chameleon_rest",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0296.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Chameleon Rest effect",
        "canonicalCooldown": "1s",
        "canonicalCooldownMs": 1000,
        "desc": "進入隱蔽休息狀態。",
        "balance": {
            "mpCost": 9,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_chameleon_rest",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 46,
        "classes": [
            "plainsWalker"
        ]
    },
    "enormous_wolf": {
        "id": "enormous_wolf",
        "name": "巨狼之力",
        "slug": "enormous_wolf",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_gigantic_wolf.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "<Fear skill>Unleashes the power of the giant wolf monster.Attacks the target twi",
        "canonicalCooldown": "30 sec.",
        "canonicalCooldownMs": 30000,
        "desc": "釋放巨狼力量，對目標攻擊兩次，威力 5503。3 秒內使附近敵人陷入 焦慮，之後 5 秒內 物理／魔法防禦 -20%。無視盾牌防禦與目標 15% 防禦力。玩家對戰／對怪物戰鬥最多攻擊 8 個目標。施法者獲得 巴格斯特：持續 10 秒，生命值藥水效果 +50、弱化／異常抗性 +20%。",
        "balance": {
            "mpCost": 100,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_enormous_wolf",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "warg"
        ]
    },
    "powerful_fists": {
        "id": "powerful_fists",
        "name": "強力雙拳",
        "slug": "powerful_fists",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill11508.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Makes powerful blows at the target.Attacks twice dealing P. damage. Power 6622.I",
        "canonicalCooldown": "5 sec.",
        "canonicalCooldownMs": 5000,
        "desc": "以強力拳擊連續攻擊目標兩次，威力 6622。無視盾牌防禦與目標 25% 防禦力。狼形態時變為 堅韌之爪，並有機率附加 爪痕印記。",
        "balance": {
            "mpCost": 84,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_powerful_fists",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "warg"
        ]
    },
    "artful_disarm": {
        "id": "artful_disarm",
        "name": "巧妙繳械",
        "slug": "artful_disarm",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_divesting_disarm.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Artful Disarm effect",
        "canonicalCooldown": "30 sec.",
        "canonicalCooldownMs": 30000,
        "desc": "以技巧解除目標武裝。",
        "balance": {
            "mpCost": 50,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_artful_disarm",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "warg"
        ]
    },
    "imminent_piercing": {
        "id": "imminent_piercing",
        "name": "迫近穿刺",
        "slug": "imminent_piercing",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_lethal_piercing.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Imminent Piercing effect",
        "canonicalCooldown": "30 sec.",
        "canonicalCooldownMs": 30000,
        "desc": "準備並施展強力穿刺攻擊。",
        "balance": {
            "mpCost": 80,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_imminent_piercing",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "warg"
        ]
    },
    "devastating_assault": {
        "id": "devastating_assault",
        "name": "毀滅突擊",
        "slug": "devastating_assault",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_down_crush.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "<Pull skill>Attacks nearby enemies with an earth-shaking strike. With a certain ",
        "canonicalCooldown": "20 sec.",
        "canonicalCooldownMs": 20000,
        "desc": "以撼動大地的攻擊打擊附近敵人，並有機率將敵人拉向自己。威力 6535。無視盾牌防禦與目標 10% 防禦力。玩家對戰／對怪物戰鬥最多攻擊 8 個目標。狼形態時變為 爪之漩渦。",
        "balance": {
            "mpCost": 74,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_devastating_assault",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "warg"
        ]
    },
    "moon_influence": {
        "id": "moon_influence",
        "name": "月之影響",
        "slug": "moon_influence",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_lunar_infusion.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Moon Influence effect",
        "canonicalCooldown": "10 min.",
        "canonicalCooldownMs": 600000,
        "desc": "獲得月之力量影響。",
        "balance": {
            "mpCost": 100,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_moon_influence",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "warg"
        ]
    },
    "improved_speed": {
        "id": "improved_speed",
        "name": "強化速度",
        "slug": "improved_speed",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_werewolf_high_speed.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Improved Speed effect",
        "canonicalCooldown": "1 min.",
        "canonicalCooldownMs": 60000,
        "desc": "提高移動速度。",
        "balance": {
            "mpCost": 50,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_improved_speed",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "warg"
        ]
    },
    "confused_mind": {
        "id": "confused_mind",
        "name": "混亂心智",
        "slug": "confused_mind",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_insanity.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Confused Mind effect",
        "canonicalCooldown": "1 min.",
        "canonicalCooldownMs": 60000,
        "desc": "使目標心智混亂。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_confused_mind",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "warg"
        ]
    },
    "tough_skin": {
        "id": "tough_skin",
        "name": "堅韌皮膚",
        "slug": "tough_skin",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill14424.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Tough Skin effect",
        "canonicalCooldown": "10 min.",
        "canonicalCooldownMs": 600000,
        "desc": "強化自身防禦韌性。",
        "balance": {
            "mpCost": 50,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_tough_skin",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "warg"
        ]
    },
    "eva_s_defense": {
        "id": "eva_s_defense",
        "name": "伊娃防護",
        "slug": "eva_s_defense",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1623.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Eva grants her protection.P. Def. +15%M. Def. +10%Speed +4",
        "canonicalCooldown": "7 min.",
        "canonicalCooldownMs": 420000,
        "desc": "獲得伊娃的守護：物理防禦 +15%、魔法防禦 +10%、速度 +4。",
        "balance": {
            "mpCost": 114,
            "pwr": 2,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_eva_s_defense",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "evaTemplar"
        ]
    },
    "water_shield_throwing": {
        "id": "water_shield_throwing",
        "name": "水之盾投擲",
        "slug": "water_shield_throwing",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_shield_throwing_eva.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Throws a shield imbued with water energy striking the target and nearby enemies.",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "投擲注入水元素的盾牌，攻擊目標與附近敵人，威力 7751。玩家對戰：10 秒內物理技能暴擊率 -5%、暴擊傷害 -5%；對怪物戰鬥：10 秒內物理防禦 -25%。無視盾牌防禦與目標 20% 防禦力。玩家對戰／對怪物戰鬥最多攻擊 6 個目標。",
        "balance": {
            "mpCost": 66,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_water_shield_throwing",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "evaTemplar"
        ]
    },
    "templar_s_assault": {
        "id": "templar_s_assault",
        "name": "聖殿騎士突擊",
        "slug": "templar_s_assault",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_knight_assault_eva2.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Makes a powerful attack at the target.Deals P. damage. Power 4907.Ignores Shield",
        "canonicalCooldown": "1 sec.",
        "canonicalCooldownMs": 1000,
        "desc": "對目標發動強力物理攻擊，威力 4907。無視盾牌防禦與目標 10% 防禦力。使用後疊加 水之力量，最高 3 層：等級 1 物理技能暴擊率／傷害 +3%，等級 2 +8%，等級 3 +15% 並使技能威力 +1%，每層持續 8 秒。",
        "balance": {
            "mpCost": 63,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_templar_s_assault",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "evaTemplar",
            "shillienTemplar"
        ]
    },
    "supernova": {
        "id": "supernova",
        "name": "超新星",
        "slug": "supernova",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/s_super_nova.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Supernova effect",
        "canonicalCooldown": "17 sec.",
        "canonicalCooldownMs": 17000,
        "desc": "釋放超新星般的強大能量。",
        "balance": {
            "mpCost": 300,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_supernova",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "evaTemplar"
        ]
    },
    "templar_s_rush": {
        "id": "templar_s_rush",
        "name": "聖殿騎士突進",
        "slug": "templar_s_rush",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill10092.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "<Rush skill>Charges at the enemy and attacks them. Power 2300.Ignores 10% of the",
        "canonicalCooldown": "3 sec.",
        "canonicalCooldownMs": 3000,
        "desc": "突進至敵人並攻擊，威力 2300。無視目標 10% 防禦力。",
        "balance": {
            "mpCost": 55,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_templar_s_rush",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "evaTemplar",
            "shillienTemplar"
        ]
    },
    "blessed_by_eva": {
        "id": "blessed_by_eva",
        "name": "伊娃祝福",
        "slug": "blessed_by_eva",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/keeping_of_eva.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Blessed by Eva effect",
        "canonicalCooldown": "2 min.",
        "canonicalCooldownMs": 120000,
        "desc": "獲得伊娃的祝福。",
        "balance": {
            "mpCost": 146,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_blessed_by_eva",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "evaTemplar"
        ]
    },
    "battle_shield": {
        "id": "battle_shield",
        "name": "戰鬥之盾",
        "slug": "battle_shield",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_aquatic_shield.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "This shield not only protects you in battle, but also helps you deal more damage",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "此盾牌不只保護你，也能提升傷害：物理攻擊 +50、物理防禦 +50。並強化 聖殿騎士衝鋒 為 海洋漩渦衝鋒，提高威力、防禦無視、攻擊範圍，附加暈眩並轉為範圍技能。",
        "balance": {
            "mpCost": 93,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_battle_shield",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "evaTemplar",
            "shillienTemplar"
        ]
    },
    "armor_of_swift_current": {
        "id": "armor_of_swift_current",
        "name": "迅流鎧甲",
        "slug": "armor_of_swift_current",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_rapid_stream_armor3.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Armor of Swift Current effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "獲得迅流鎧甲的防護效果。",
        "balance": {
            "mpCost": 150,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_armor_of_swift_current",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "evaTemplar"
        ]
    },
    "over_the_rainbow": {
        "id": "over_the_rainbow",
        "name": "越過彩虹",
        "slug": "over_the_rainbow",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/s_over_the_rainbow.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Over the Rainbow effect",
        "canonicalCooldown": "2 min.",
        "canonicalCooldownMs": 120000,
        "desc": "啟動越過彩虹效果。",
        "balance": {
            "mpCost": 300,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_over_the_rainbow",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "elementalMaster"
        ]
    },
    "elemental_strike": {
        "id": "elemental_strike",
        "name": "元素打擊",
        "slug": "elemental_strike",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_elemental_strike.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "<Changing skill>Unleashes magic powers to deal damage to the enemy.Range < 400De",
        "canonicalCooldown": "1 sec.",
        "canonicalCooldownMs": 1000,
        "desc": "依距離改變攻擊方式。距離小於 400 時造成魔法傷害，威力 130；距離 400 以上時威力 119。",
        "balance": {
            "mpCost": 43,
            "pwr": 130,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_elemental_strike",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "elementalMaster"
        ]
    },
    "elemental_shackles": {
        "id": "elemental_shackles",
        "name": "元素枷鎖",
        "slug": "elemental_shackles",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_elemental_shackle.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "<Changing/ Suppression skill>Casts a magic spell that slows enemies down.Range <",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "依距離改變攻擊方式並降低敵人速度。距離小於 400 時對目標與附近敵人造成威力 104 的魔法傷害；距離 400 以上時威力 95。附加 元素破壞 等級 1，持續 5 秒，使攻速／施法速度 -23%、物理攻擊／魔法攻擊 -23%。玩家對戰／對怪物戰鬥最多攻擊 2 個目標。",
        "balance": {
            "mpCost": 76,
            "pwr": 104,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_elemental_shackles",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "elementalMaster"
        ]
    },
    "elemental_roots": {
        "id": "elemental_roots",
        "name": "元素根鬚",
        "slug": "elemental_roots",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_elemental_root.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Deals M. damage to several targets. Power 153. For 5 sec., applies Hold.PvP: Hol",
        "canonicalCooldown": "20 sec.",
        "canonicalCooldownMs": 20000,
        "desc": "對多個目標造成魔法傷害，威力 153，並束縛 5 秒。玩家對戰／對怪物戰鬥最多束縛 5 個目標。",
        "balance": {
            "mpCost": 60,
            "pwr": 153,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_elemental_roots",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "elementalMaster"
        ]
    },
    "elemental_discharge": {
        "id": "elemental_discharge",
        "name": "元素釋放",
        "slug": "elemental_discharge",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1441.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Strikes the enemy with unleashed magic powers.Deals M. damage to the target. Pow",
        "canonicalCooldown": "1 sec.",
        "canonicalCooldownMs": 1000,
        "desc": "以釋放的魔力攻擊敵人，對目標造成魔法傷害，威力 110。",
        "balance": {
            "mpCost": 56,
            "pwr": 110,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_elemental_discharge",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "elementalMaster",
            "elementalSummoner"
        ]
    },
    "elemental_vortex": {
        "id": "elemental_vortex",
        "name": "元素漩渦",
        "slug": "elemental_vortex",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_elemental_vortex.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "<Changing skill>Unleashes explosive magic power to deal damage to enemies.Range ",
        "canonicalCooldown": "8 sec.",
        "canonicalCooldownMs": 8000,
        "desc": "依距離改變攻擊方式。距離小於 400 時對目標與附近敵人造成威力 108 的魔法傷害；距離 400 以上時威力 98。附加 元素創傷 等級 1，持續 5 秒，使魔法防禦 -23%。玩家對戰／對怪物戰鬥最多攻擊 3 個目標。",
        "balance": {
            "mpCost": 103,
            "pwr": 108,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_elemental_vortex",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "elementalMaster"
        ]
    },
    "shelter": {
        "id": "shelter",
        "name": "庇護所",
        "slug": "shelter",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/shelter.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Note!When used, removes Shield of Faith.Cancelled if Shelter: Master is applied.",
        "canonicalCooldown": "2 min.",
        "canonicalCooldownMs": 120000,
        "desc": "建立保護附近盟友的安全區。施法者獲得水之屏障，屏障 生命值 = 5000 + 最大生命值的 200%，受到的暴擊傷害 -300；使用 聖殿騎士突擊 時有 30% 機率觸發 水柱。盟友可將 50% 傷害轉移給施法者，最多影響 20 個目標，距離過遠時效果解除。",
        "balance": {
            "mpCost": 101,
            "pwr": 120,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_shelter",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "cardinal",
            "evaTemplar"
        ]
    },
    "elemental_nurture": {
        "id": "elemental_nurture",
        "name": "元素滋養",
        "slug": "elemental_nurture",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/skill8363.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Elemental Nurture effect",
        "canonicalCooldown": "1s",
        "canonicalCooldownMs": 1000,
        "desc": "以元素力量滋養自身。",
        "balance": {
            "mpCost": 100,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_elemental_nurture",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 88,
        "classes": [
            "elementalMaster"
        ]
    },
    "elemental_charm": {
        "id": "elemental_charm",
        "name": "元素魅力",
        "slug": "elemental_charm",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill31173.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Elemental Charm effect",
        "canonicalCooldown": "3 min.",
        "canonicalCooldownMs": 180000,
        "desc": "啟動元素魅力效果。",
        "balance": {
            "mpCost": 150,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_elemental_charm",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "elementalMaster"
        ]
    },
    "elemental_concentration": {
        "id": "elemental_concentration",
        "name": "元素專注",
        "slug": "elemental_concentration",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_elemental_concentration.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Elemental Concentration effect",
        "canonicalCooldown": "-10%MP",
        "canonicalCooldownMs": 1000,
        "desc": "集中元素力量以強化施法。",
        "balance": {
            "mpCost": 150,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_elemental_concentration",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "elementalMaster"
        ]
    },
    "elemental_mastership": {
        "id": "elemental_mastership",
        "name": "元素精通",
        "slug": "elemental_mastership",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_elemental_expert.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Elemental Mastership effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "提升對各種元素力量的掌控。",
        "balance": {
            "mpCost": 120,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_elemental_mastership",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "elementalMaster"
        ]
    },
    "wind_shackles": {
        "id": "wind_shackles",
        "name": "風之枷鎖",
        "slug": "wind_shackles",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1206.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Slows targets down.Atk. Spd. -23%Casting Spd. -23%",
        "canonicalCooldown": "2 sec.",
        "canonicalCooldownMs": 2000,
        "desc": "降低目標速度：攻速 -23%、施法速度 -23%。",
        "balance": {
            "mpCost": 75,
            "pwr": 2,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_wind_shackles",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "darkElfMage",
            "darkWizard",
            "elementalMaster",
            "elementalSummoner",
            "elfElder",
            "elfMage",
            "elfOracle",
            "elvenWizard",
            "evaSaint",
            "mysticMuse",
            "phantomSummoner",
            "sayhaMageBase",
            "sayhaSeeker",
            "sayhaSeer",
            "shillienElder",
            "shillienOracle",
            "shillienSaint",
            "spectralMaster",
            "spellhowler",
            "spellsinger",
            "stormScreamer",
            "windRiderErth"
        ]
    },
    "friendly_walk": {
        "id": "friendly_walk",
        "name": "友善步伐",
        "slug": "friendly_walk",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_friend_escort.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Friendly Walk effect",
        "canonicalCooldown": "2 min.",
        "canonicalCooldownMs": 120000,
        "desc": "以輕盈步伐移動。",
        "balance": {
            "mpCost": 200,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_friendly_walk",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 92,
        "classes": [
            "elementalMaster"
        ]
    },
    "elemental_speed": {
        "id": "elemental_speed",
        "name": "元素疾行",
        "slug": "elemental_speed",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_elemental_shift.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Note!The learnt Summon Elemental Globe skill is required.Teleports you to the su",
        "canonicalCooldown": "20 sec.",
        "canonicalCooldownMs": 20000,
        "desc": "需要先學會 召喚元素球。傳送至已召喚的 元素球，僅在該球體存在時可使用。",
        "balance": {
            "mpCost": 100,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_elemental_speed",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "elementalMaster"
        ]
    },
    "summon_elemental_cubic": {
        "id": "summon_elemental_cubic",
        "name": "召喚元素魔方",
        "slug": "summon_elemental_cubic",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill10073_2.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "<Summon skill>Summons a cubic imbued with the Elemental Unicorn's power.M. Atk. ",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "召喚注入 元素獨角獸 力量的魔方。魔法攻擊 +150、魔法技能魔力消耗 -1%。使用 元素枷鎖、元素漩渦、元素根鬚 時會觸發追加攻擊，並由幽靈 元素獨角獸 協助攻擊。",
        "balance": {
            "mpCost": 78,
            "pwr": 0,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_summon_elemental_cubic",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "elementalMaster"
        ]
    },
    "summon_elemental_globe": {
        "id": "summon_elemental_globe",
        "name": "召喚元素球",
        "slug": "summon_elemental_globe",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/s_elemental_sphere.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Summon Elemental Globe effect",
        "canonicalCooldown": "3 min.",
        "canonicalCooldownMs": 180000,
        "desc": "召喚元素球體。",
        "balance": {
            "mpCost": 100,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_summon_elemental_globe",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "elementalMaster"
        ]
    },
    "summon_elemental_unicorn": {
        "id": "summon_elemental_unicorn",
        "name": "召喚元素獨角獸",
        "slug": "summon_elemental_unicorn",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/s_elemental_unicorn_1st.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Summon Elemental Unicorn effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "召喚元素獨角獸。",
        "balance": {
            "mpCost": 50,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_summon_elemental_unicorn",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "elementalSummoner"
        ]
    },
    "freezing_shot": {
        "id": "freezing_shot",
        "name": "冰凍射擊",
        "slug": "freezing_shot",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_freezing_shot.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Shoots an arrow imbued with cold energy.Attacks the target and nearby enemies. P",
        "canonicalCooldown": "15 sec.",
        "canonicalCooldownMs": 15000,
        "desc": "射出注入寒氣的箭矢，攻擊目標與附近敵人，威力 8800。附加 冰冷氣息，使受到技能暴擊率 +5%，持續 5 秒。無視目標 10% 防禦力，玩家對戰／對怪物戰鬥最多攻擊 8 個目標。",
        "balance": {
            "mpCost": 110,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_freezing_shot",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "moonlightSentinel"
        ]
    },
    "water_arrow_rain": {
        "id": "water_arrow_rain",
        "name": "水之箭雨",
        "slug": "water_arrow_rain",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_aqua_arrow_rain.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Shoots an arrow imbued with water power.Attacks the target and nearby enemies. P",
        "canonicalCooldown": "30 sec.",
        "canonicalCooldownMs": 30000,
        "desc": "射出注入水元素的箭矢，攻擊目標與附近敵人，威力 10800。無視目標 10% 防禦力，並有機率附加 箭矢破壞，使物理攻擊、魔法攻擊、物理防禦、魔法防禦 各 -20%，持續 10 秒。玩家對戰／對怪物戰鬥最多攻擊 15 個目標。",
        "balance": {
            "mpCost": 170,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_water_arrow_rain",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "moonlightSentinel"
        ]
    },
    "rapid_fire": {
        "id": "rapid_fire",
        "name": "快速射擊",
        "slug": "rapid_fire",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill0413.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "P. Atk. +250Atk. Spd. +20%",
        "canonicalCooldown": "30 sec.",
        "canonicalCooldownMs": 30000,
        "desc": "物理攻擊 +250、攻速 +20%。",
        "balance": {
            "mpCost": 80,
            "pwr": 2,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_rapid_fire",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 80,
        "classes": [
            "moonlightSentinel",
            "silverRanger"
        ]
    },
    "alacrity": {
        "id": "alacrity",
        "name": "迅捷",
        "slug": "alacrity",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_swift.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Swift movements let you attack more effectively.P. Atk. +1000Atk. Spd. +10%P. Sk",
        "canonicalCooldown": "-5%Speed",
        "canonicalCooldownMs": 1000,
        "desc": "敏捷動作強化攻擊：物理攻擊 +1000、攻速 +10%、物理技能暴擊率 +3%、暴擊傷害 +3%、技能冷卻 -5%、速度 +7。使用 雙重射擊 時有 50% 機率追加攻擊。",
        "balance": {
            "mpCost": 80,
            "pwr": 1,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_alacrity",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "moonlightSentinel"
        ]
    },
    "aqua_swirl": {
        "id": "aqua_swirl",
        "name": "水流旋渦",
        "slug": "aqua_swirl",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/skill1175.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Releases a water vortex.Deals M. damage to the target. Power 31.",
        "canonicalCooldown": "1 sec.",
        "canonicalCooldownMs": 1000,
        "desc": "釋放水之漩渦，對目標造成魔法傷害，威力 31。",
        "balance": {
            "mpCost": 18,
            "pwr": 31,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_aqua_swirl",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 20,
        "classes": [
            "elvenWizard",
            "sayhaSeer"
        ]
    },
    "freezing_skin": {
        "id": "freezing_skin",
        "name": "冰凍之膚",
        "slug": "freezing_skin",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1238.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Reflects 3% of received damage.",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "反射受到傷害的 3%。",
        "balance": {
            "mpCost": 35,
            "pwr": 0,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_freezing_skin",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "spellsinger",
            "windRiderErth"
        ]
    },
    "aqua_splash": {
        "id": "aqua_splash",
        "name": "水浪爆發",
        "slug": "aqua_splash",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1295.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Creates a powerful wave.Deals M. damage to the target and nearby enemies. Power ",
        "canonicalCooldown": "7 sec.",
        "canonicalCooldownMs": 7000,
        "desc": "創造強力波浪，對目標與附近敵人造成魔法傷害，威力 60。玩家對戰／對怪物戰鬥最多攻擊 5 個目標。",
        "balance": {
            "mpCost": 49,
            "pwr": 60,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_aqua_splash",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 58,
        "classes": [
            "mysticMuse",
            "sayhaSeeker",
            "spellsinger",
            "windRiderErth"
        ]
    },
    "hydro_blast": {
        "id": "hydro_blast",
        "name": "水流爆破",
        "slug": "hydro_blast",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1235.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Strikes with the rush of water.Deals M. damage to the target. Power 76.",
        "canonicalCooldown": "1 sec.",
        "canonicalCooldownMs": 1000,
        "desc": "以猛烈水流攻擊目標，造成魔法傷害，威力 76。",
        "balance": {
            "mpCost": 32,
            "pwr": 76,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_hydro_blast",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "mysticMuse",
            "sayhaSeeker",
            "spellsinger",
            "windRiderErth"
        ]
    },
    "blizzard": {
        "id": "blizzard",
        "name": "暴風雪",
        "slug": "blizzard",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1290.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Blizzard effect",
        "canonicalCooldown": "30 sec.",
        "canonicalCooldownMs": 30000,
        "desc": "召喚暴風雪攻擊敵人。",
        "balance": {
            "mpCost": 100,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_blizzard",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 70,
        "classes": [
            "mysticMuse",
            "sayhaSeeker",
            "spellsinger",
            "windRiderErth"
        ]
    },
    "mystic_freeze": {
        "id": "mystic_freeze",
        "name": "神秘冰封",
        "slug": "mystic_freeze",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/s_mystic_frozen.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Deals M. damage to the target and nearby enemies. Power 144.Creates Ice Field in",
        "canonicalCooldown": "30 sec.",
        "canonicalCooldownMs": 30000,
        "desc": "對目標與附近敵人造成魔法傷害，威力 144，並在區域形成 冰霜領域。區域內敵人受到 燃燒寒氣：麻痺／壓制／睡眠抗性 -30%；對怪物戰鬥額外魔法防禦 -10%；玩家對戰額外速度 -90% 且禁止突進技能。玩家對戰／對怪物戰鬥最多攻擊 10 個目標。",
        "balance": {
            "mpCost": 232,
            "pwr": 144,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_mystic_freeze",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 80,
        "classes": [
            "mysticMuse",
            "sayhaSeeker"
        ]
    },
    "winter_shackles": {
        "id": "winter_shackles",
        "name": "寒冬枷鎖",
        "slug": "winter_shackles",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/s_winter_shackle.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Winter Shackles effect",
        "canonicalCooldown": "45 sec.",
        "canonicalCooldownMs": 45000,
        "desc": "以寒冬力量束縛並削弱敵人。",
        "balance": {
            "mpCost": 100,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_winter_shackles",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 85,
        "classes": [
            "mysticMuse",
            "sayhaSeeker"
        ]
    },
    "mystic_explosion": {
        "id": "mystic_explosion",
        "name": "神秘爆發",
        "slug": "mystic_explosion",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_mystic_burst.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Deals M. damage to the target. Power 152.Enemies with debuffs/ anomalies receive",
        "canonicalCooldown": "1 sec.",
        "canonicalCooldownMs": 1000,
        "desc": "對目標造成魔法傷害，威力 152。帶有弱化／異常狀態的敵人會受到更多傷害。",
        "balance": {
            "mpCost": 56,
            "pwr": 152,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_mystic_explosion",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "mysticMuse",
            "sayhaSeeker"
        ]
    },
    "mystic_spiral": {
        "id": "mystic_spiral",
        "name": "神秘螺旋",
        "slug": "mystic_spiral",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_mystic_helix.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Deals M. damage to enemies. Power 229.Applies the Mystic Decrease debuff.Mystic ",
        "canonicalCooldown": "30 sec.",
        "canonicalCooldownMs": 30000,
        "desc": "對敵人造成魔法傷害，威力 229，並附加 神秘衰弱，使 力量／智力／敏捷／智慧／體質／精神 中隨機一項 -4，持續 5 秒。玩家對戰／對怪物戰鬥最多攻擊 12 個目標。",
        "balance": {
            "mpCost": 116,
            "pwr": 229,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_mystic_spiral",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "mysticMuse",
            "sayhaSeeker"
        ]
    },
    "ice_vortex": {
        "id": "ice_vortex",
        "name": "冰霜漩渦",
        "slug": "ice_vortex",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1340.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Creates a vortex that draws its strength from the Water dimension.Deals M. damag",
        "canonicalCooldown": "30 sec.",
        "canonicalCooldownMs": 30000,
        "desc": "召喚來自水之次元的漩渦，對目標造成魔法傷害，威力 158。",
        "balance": {
            "mpCost": 116,
            "pwr": 158,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_ice_vortex",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "mysticMuse",
            "sayhaSeeker"
        ]
    },
    "eva_s_serenade": {
        "id": "eva_s_serenade",
        "name": "伊娃小夜曲",
        "slug": "eva_s_serenade",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1273.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Removes debuffs and anomalies from the target.With a 70% chance, protects from d",
        "canonicalCooldown": "15 min.",
        "canonicalCooldownMs": 900000,
        "desc": "移除目標的弱化與異常狀態，並有 70% 機率使其受到保護。",
        "balance": {
            "mpCost": 225,
            "pwr": 7,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_eva_s_serenade",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 44,
        "classes": [
            "elfElder",
            "evaSaint"
        ]
    },
    "party_return": {
        "id": "party_return",
        "name": "隊伍返回",
        "slug": "party_return",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1255.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Teleports to the nearest village together with a party member.Cannot be used in ",
        "canonicalCooldown": "2 min.",
        "canonicalCooldownMs": 120000,
        "desc": "與隊伍成員一起傳送至最近村莊。在 管理員 諮詢服務等特殊地點無法使用。",
        "balance": {
            "mpCost": 257,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_party_return",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 48,
        "classes": [
            "elfElder"
        ]
    },
    "winter_frostbite": {
        "id": "winter_frostbite",
        "name": "寒冬凍傷",
        "slug": "winter_frostbite",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "5★",
        "starRank": 5,
        "icon": "/icons/s_winter_icicle.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Winter Frostbite effect",
        "canonicalCooldown": "30 sec.",
        "canonicalCooldownMs": 30000,
        "desc": "以寒冬力量造成凍傷。",
        "balance": {
            "mpCost": 80,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_winter_frostbite",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 90,
        "classes": [
            "mysticMuse",
            "sayhaSeeker"
        ]
    },
    "winter_skin": {
        "id": "winter_skin",
        "name": "寒冬之膚",
        "slug": "winter_skin",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_winter_skin.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Reflects 12% of received damage.Max HP +600P. Def. +600With a certain chance whe",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "反射受到傷害的 12%，最大生命值 +600、物理防禦 +600。受到攻擊時有機率對攻擊者附加 霜露，使其麻痺 3 秒。",
        "balance": {
            "mpCost": 100,
            "pwr": 1,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_winter_skin",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 82,
        "classes": [
            "mysticMuse",
            "sayhaSeeker"
        ]
    },
    "faery_shield": {
        "id": "faery_shield",
        "name": "妖精護盾",
        "slug": "faery_shield",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/s_mystic_shield.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Faery Shield effect",
        "canonicalCooldown": "-4%Knockback/",
        "canonicalCooldownMs": 1000,
        "desc": "展開妖精護盾。",
        "balance": {
            "mpCost": 200,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_faery_shield",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "mysticMuse",
            "sayhaSeeker"
        ]
    },
    "riot": {
        "id": "riot",
        "name": "暴動",
        "slug": "riot",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/s_repel.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Riot effect",
        "canonicalCooldown": "1 min.",
        "canonicalCooldownMs": 60000,
        "desc": "引發猛烈騷動。",
        "balance": {
            "mpCost": 100,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_riot",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 89,
        "classes": [
            "mysticMuse",
            "sayhaSeeker"
        ]
    },
    "winter_slumber": {
        "id": "winter_slumber",
        "name": "寒冬沉眠",
        "slug": "winter_slumber",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_winter_dreamer.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Inflicts Sleep on the target.For some time the target cannot wake up even if hit",
        "canonicalCooldown": "15 sec.",
        "canonicalCooldownMs": 15000,
        "desc": "使目標陷入深度睡眠，一段時間內即使受到攻擊也不會醒來。",
        "balance": {
            "mpCost": 74,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_winter_slumber",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "mysticMuse",
            "sayhaSeeker"
        ]
    },
    "frozen_field": {
        "id": "frozen_field",
        "name": "冰凍領域",
        "slug": "frozen_field",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_freezing_area_new.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks the enemies thrice. Power 7500.Ignores Shield Defense.Ignores 15% of the",
        "canonicalCooldown": "30 sec.",
        "canonicalCooldownMs": 30000,
        "desc": "連續攻擊敵人 3 次，威力 7500，之後追加一次威力 7500 的物理攻擊並附加 冰凍沼澤。無視盾牌防禦與目標 15% 防禦力。玩家對戰／對怪物戰鬥最多攻擊 6 個目標。冰凍沼澤 持續 5 秒，禁止突進技能。",
        "balance": {
            "mpCost": 15,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_frozen_field",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": []
    },
    "frost_grip": {
        "id": "frost_grip",
        "name": "冰霜之握",
        "slug": "frost_grip",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_freezing_grip.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Frost Grip effect",
        "canonicalCooldown": "1 min.",
        "canonicalCooldownMs": 60000,
        "desc": "以冰霜力量抓住目標。",
        "balance": {
            "mpCost": 70,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_frost_grip",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": []
    },
    "call_of_frost": {
        "id": "call_of_frost",
        "name": "冰霜召喚",
        "slug": "call_of_frost",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1493.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "P. Atk. +5%P. Skill Damage bonus +2%Imprisonment Atk. Rate +10%When using Punish",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "物理攻擊 +5%、物理技能傷害 +2%、禁錮成功率 +10%。使用 懲戒 時有 10% 機率附加 死亡凍傷，持續 5 秒；玩家對戰 使禁錮抗性 -10%，對怪物戰鬥 使物理防禦／魔法防禦 -15%。",
        "balance": {
            "mpCost": 15,
            "pwr": 1,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_call_of_frost",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": []
    },
    "drain_hp": {
        "id": "drain_hp",
        "name": "生命吸取",
        "slug": "drain_hp",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0070.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Deals M. damage to the target. Power 60.Absorbs 20% of inflicted damage as HP.",
        "canonicalCooldown": "4 sec.",
        "canonicalCooldownMs": 4000,
        "desc": "對目標造成魔法傷害，威力 60，並將造成傷害的 20% 轉為 生命值。",
        "balance": {
            "mpCost": 19,
            "pwr": 60,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_drain_hp",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 20,
        "classes": [
            "palusKnight",
            "shillienKnight",
            "shillienTemplar"
        ]
    },
    "life_leech": {
        "id": "life_leech",
        "name": "生命汲取",
        "slug": "life_leech",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1511.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Deals M. damage to the target. Power 26.Absorbs 80% of inflicted damage as HP.",
        "canonicalCooldown": "2 sec.",
        "canonicalCooldownMs": 2000,
        "desc": "對目標造成魔法傷害，威力 26，並將造成傷害的 80% 轉為 生命值。",
        "balance": {
            "mpCost": 47,
            "pwr": 26,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_life_leech",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "shillienKnight",
            "shillienTemplar"
        ]
    },
    "abyss_strike": {
        "id": "abyss_strike",
        "name": "深淵打擊",
        "slug": "abyss_strike",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0401.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks the target. Power 619.Ignores Shield Defense.Ignores 10% of the target's",
        "canonicalCooldown": "1 sec.",
        "canonicalCooldownMs": 1000,
        "desc": "攻擊目標，威力 619。無視盾牌防禦與目標 10% 防禦力。",
        "balance": {
            "mpCost": 33,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_abyss_strike",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "shillienKnight",
            "shillienTemplar"
        ]
    },
    "lightning_strike": {
        "id": "lightning_strike",
        "name": "雷電打擊",
        "slug": "lightning_strike",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0279.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "A powerful lightning strike.Attacks the target. Power 2927.Paralyzes for 3 sec.",
        "canonicalCooldown": "20 sec.",
        "canonicalCooldownMs": 20000,
        "desc": "以強力雷擊攻擊目標，威力 2927，並麻痺 3 秒。",
        "balance": {
            "mpCost": 46,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_lightning_strike",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 58,
        "classes": [
            "shillienKnight",
            "shillienTemplar"
        ]
    },
    "confusion": {
        "id": "confusion",
        "name": "混亂",
        "slug": "confusion",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/iconskill30522.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Confuses the target and makes them unable to move.",
        "canonicalCooldown": "5 sec.",
        "canonicalCooldownMs": 5000,
        "desc": "使目標陷入混亂並無法移動。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_confusion",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "palusKnight",
            "shillienKnight",
            "shillienTemplar"
        ]
    },
    "power_break": {
        "id": "power_break",
        "name": "力量削弱",
        "slug": "power_break",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0115.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Target's P. Atk. --23%",
        "canonicalCooldown": "2 sec.",
        "canonicalCooldownMs": 2000,
        "desc": "使目標物理攻擊 -23%。",
        "balance": {
            "mpCost": 16,
            "pwr": 2,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_power_break",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "abyssWalker",
            "palusKnight",
            "shillienKnight",
            "shillienTemplar"
        ]
    },
    "life_magic_harmony_attack": {
        "id": "life_magic_harmony_attack",
        "name": "生命魔法和聲－攻擊",
        "slug": "life_magic_harmony_attack",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/life_steal.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "P. Def. +100M. Def. +100Max HP +100Recovers 5 MP on killing an enemy.",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "物理防禦 +100、魔法防禦 +100、最大生命值 +100。擊殺敵人時恢復 5 魔力。",
        "balance": {
            "mpCost": 30,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_life_magic_harmony_attack",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "shillienKnight",
            "shillienTemplar"
        ]
    },
    "hex": {
        "id": "hex",
        "name": "詛咒破甲",
        "slug": "hex",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0122.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Decreases target's P. Def.P. Def. -23%",
        "canonicalCooldown": "2 sec.",
        "canonicalCooldownMs": 2000,
        "desc": "降低目標物理防禦 23%。",
        "balance": {
            "mpCost": 15,
            "pwr": 2,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_hex",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "abyssWalker",
            "shillienKnight",
            "shillienTemplar"
        ]
    },
    "saint_s_burst_sea": {
        "id": "saint_s_burst_sea",
        "name": "聖者爆發：海",
        "slug": "saint_s_burst_sea",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_aquatic_buster.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Saint's Burst: Sea effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "釋放海洋之力的聖者爆發。",
        "balance": {
            "mpCost": 120,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_saint_s_burst_sea",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "evaSaint"
        ]
    },
    "prophecy_of_water": {
        "id": "prophecy_of_water",
        "name": "水之預言",
        "slug": "prophecy_of_water",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1355.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "M. Atk. +5%Casting Spd. +10%P. Def. +10%M. Def. +10%Max MP +15%MP Recovery Rate ",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "魔法攻擊 +5%、施法速度 +10%、物理防禦 +10%、魔法防禦 +10%、最大魔力 +15%、魔力 恢復率 +10%、技能 魔力 消耗 -10%、對怪物戰鬥傷害 +20%、速度 +2、弱化／異常抗性 +10%。",
        "balance": {
            "mpCost": 72,
            "pwr": 1,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_prophecy_of_water",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "evaSaint"
        ]
    },
    "eva_s_grail": {
        "id": "eva_s_grail",
        "name": "伊娃聖杯",
        "slug": "eva_s_grail",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_evas_grail.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Eva's Grail effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "獲得伊娃聖杯的力量。",
        "balance": {
            "mpCost": 150,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_eva_s_grail",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "evaSaint"
        ]
    },
    "mass_vitalize": {
        "id": "mass_vitalize",
        "name": "群體活力恢復",
        "slug": "mass_vitalize",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1552.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Recovers party members' HP. Power 800.Removes debuffs from party members.P. Atk.",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "恢復隊伍成員 生命值，威力 800，並移除降低物理攻擊、魔法攻擊、物理防禦、魔法防禦 的弱化效果。",
        "balance": {
            "mpCost": 80,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_mass_vitalize",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "evaSaint"
        ]
    },
    "eva_s_nocturne": {
        "id": "eva_s_nocturne",
        "name": "伊娃夜曲",
        "slug": "eva_s_nocturne",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_nocturn_of_eva.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Eva's Nocturne effect",
        "canonicalCooldown": "4 min.",
        "canonicalCooldownMs": 240000,
        "desc": "奏響伊娃夜曲並獲得祝福。",
        "balance": {
            "mpCost": 297,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_eva_s_nocturne",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "evaSaint"
        ]
    },
    "mass_cure": {
        "id": "mass_cure",
        "name": "群體治癒",
        "slug": "mass_cure",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1550.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Removes Infection/ Suppression debuffs from party members.",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "移除隊伍成員的感染／壓制類弱化效果。",
        "balance": {
            "mpCost": 80,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_mass_cure",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "evaSaint",
            "shillienSaint"
        ]
    },
    "exclusion": {
        "id": "exclusion",
        "name": "排除",
        "slug": "exclusion",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/expel.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Exclusion effect",
        "canonicalCooldown": "+5%Can",
        "canonicalCooldownMs": 1000,
        "desc": "排除不利效果。",
        "balance": {
            "mpCost": 105,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_exclusion",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "evaSaint",
            "hierophant"
        ]
    },
    "remove": {
        "id": "remove",
        "name": "移除",
        "slug": "remove",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_refine.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Remove effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "移除指定效果。",
        "balance": {
            "mpCost": 152,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_remove",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "evaSaint"
        ]
    },
    "eva_s_blessing": {
        "id": "eva_s_blessing",
        "name": "伊娃祝福",
        "slug": "eva_s_blessing",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/skill1506.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Eva's Blessing effect",
        "canonicalCooldown": "20 min.",
        "canonicalCooldownMs": 1200000,
        "desc": "獲得伊娃的祝福。",
        "balance": {
            "mpCost": 73,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_eva_s_blessing",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 80,
        "classes": [
            "evaSaint"
        ]
    },
    "dance_of_fury": {
        "id": "dance_of_fury",
        "name": "狂怒之舞",
        "slug": "dance_of_fury",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/s_dance_of_fury.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "<Rush skill>Attacks the target. Power 1308.Ignores Shield Defense.Ignores 10% of",
        "canonicalCooldown": "5 sec.",
        "canonicalCooldownMs": 5000,
        "desc": "突進並攻擊目標，威力 1308。無視盾牌防禦與目標 10% 防禦力。",
        "balance": {
            "mpCost": 20,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_dance_of_fury",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "bladeDancer",
            "spectralDancer"
        ]
    },
    "poison_blade_dance": {
        "id": "poison_blade_dance",
        "name": "毒刃之舞",
        "slug": "poison_blade_dance",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0084.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Your swords emit a toxic cloud.Attacks nearby targets. Power 1039.Applies Poison",
        "canonicalCooldown": "15 sec.",
        "canonicalCooldownMs": 15000,
        "desc": "雙劍釋放毒霧，攻擊附近目標，威力 1039，並附加中毒。不會暴擊。玩家對戰／對怪物戰鬥最多攻擊 5 個目標。",
        "balance": {
            "mpCost": 66,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_poison_blade_dance",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 55,
        "classes": [
            "bladeDancer",
            "spectralDancer"
        ]
    },
    "dance_of_blood": {
        "id": "dance_of_blood",
        "name": "鮮血之舞",
        "slug": "dance_of_blood",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/s_blood_dance.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Dance of Blood effect",
        "canonicalCooldown": ":",
        "canonicalCooldownMs": 1000,
        "desc": "以鮮血之舞強化戰鬥能力。",
        "balance": {
            "mpCost": 100,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_dance_of_blood",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "bladeDancer"
        ]
    },
    "dance_of_fire": {
        "id": "dance_of_fire",
        "name": "火焰之舞",
        "slug": "dance_of_fire",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/s_dance_of_fire.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks the target and nearby enemies twice. Power 1149.Ignores Shield Defense.I",
        "canonicalCooldown": "16 sec.",
        "canonicalCooldownMs": 16000,
        "desc": "連續兩次攻擊目標與附近敵人，威力 1149。無視盾牌防禦與目標 10% 防禦力。玩家對戰／對怪物戰鬥最多攻擊 4 個目標。",
        "balance": {
            "mpCost": 40,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_dance_of_fire",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "bladeDancer",
            "spectralDancer"
        ]
    },
    "moving_shadows": {
        "id": "moving_shadows",
        "name": "流動暗影",
        "slug": "moving_shadows",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0366.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Moving Shadows effect",
        "canonicalCooldown": "1s",
        "canonicalCooldownMs": 1000,
        "desc": "讓自身如流動暗影般移動。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_moving_shadows",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 62,
        "classes": [
            "bladeDancer"
        ]
    },
    "dance_of_cosmos": {
        "id": "dance_of_cosmos",
        "name": "宇宙之舞",
        "slug": "dance_of_cosmos",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/s_dance_of_mystic.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "P. Atk. +10%",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "物理攻擊 +10%。",
        "balance": {
            "mpCost": 10,
            "pwr": 1,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_dance_of_cosmos",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 42,
        "classes": [
            "bladeDancer",
            "spectralDancer"
        ]
    },
    "dance_of_warrior": {
        "id": "dance_of_warrior",
        "name": "戰士之舞",
        "slug": "dance_of_warrior",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/s_dance_of_warrior.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Caster:P. Atk. +150P./ M. Def. +50P. Skill Critical Damage +1%Atk. Spd. +10P. Sk",
        "canonicalCooldown": "-1%<Skill",
        "canonicalCooldownMs": 1000,
        "desc": "施法者獲得物理攻擊 +150、物理防禦／魔法防禦 +50、物理技能暴擊傷害 +1%、攻速 +10、物理技能冷卻 -1%。並強化 狂亂步伐、狂怒之舞、火焰之舞；使用強化版 狂亂步伐 時會追加威力 7286 的攻擊。",
        "balance": {
            "mpCost": 51,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_dance_of_warrior",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "bladeDancer",
            "spectralDancer"
        ]
    },
    "break": {
        "id": "break",
        "name": "破壞",
        "slug": "break",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/s_break.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "PvP effectP. Atk. -740, P. Def. -740PvE effectP. Atk. -300, P. Def. -23%",
        "canonicalCooldown": "40 sec.",
        "canonicalCooldownMs": 40000,
        "desc": "玩家對戰：物理攻擊 -740、物理防禦 -740；對怪物戰鬥：物理攻擊 -300、物理防禦 -23%。",
        "balance": {
            "mpCost": 10,
            "pwr": 2,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_break",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "bladeDancer",
            "spectralDancer"
        ]
    },
    "mass_lightning_strike": {
        "id": "mass_lightning_strike",
        "name": "群體雷電打擊",
        "slug": "mass_lightning_strike",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skll10094.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "A powerful lightning strike.Attacks the target and nearby enemies. Power 2311.Pa",
        "canonicalCooldown": "1 min.",
        "canonicalCooldownMs": 60000,
        "desc": "以強力雷擊攻擊目標與附近敵人，威力 2311，並麻痺 3 秒。玩家對戰／對怪物戰鬥最多攻擊 5 個目標。",
        "balance": {
            "mpCost": 94,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_mass_lightning_strike",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "shillienTemplar"
        ]
    },
    "shillien_s_curse": {
        "id": "shillien_s_curse",
        "name": "席琳詛咒",
        "slug": "shillien_s_curse",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_curse_of_shilen.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Shillien's Curse effect",
        "canonicalCooldown": "30 sec.",
        "canonicalCooldownMs": 30000,
        "desc": "施加席琳的詛咒。",
        "balance": {
            "mpCost": 112,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_shillien_s_curse",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "shillienTemplar"
        ]
    },
    "dark_shield_throwing": {
        "id": "dark_shield_throwing",
        "name": "黑暗盾牌投擲",
        "slug": "dark_shield_throwing",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_shield_throwing_shilen2.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Throws a shield imbued with dark energy striking the target and nearby enemies.D",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "投擲注入黑暗能量的盾牌，攻擊目標與附近敵人，威力 7751。玩家對戰：10 秒內物理技能暴擊率 -5%、暴擊傷害 -5%；對怪物戰鬥：10 秒內物理防禦 -25%。無視盾牌防禦與目標 20% 防禦力。玩家對戰／對怪物戰鬥最多攻擊 6 個目標。",
        "balance": {
            "mpCost": 66,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_dark_shield_throwing",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "shillienTemplar"
        ]
    },
    "apocalypse": {
        "id": "apocalypse",
        "name": "末日",
        "slug": "apocalypse",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_debuff_apocalypse.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Apocalypse effect",
        "canonicalCooldown": "3 sec.",
        "canonicalCooldownMs": 3000,
        "desc": "釋放末日之力。",
        "balance": {
            "mpCost": 300,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_apocalypse",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "shillienTemplar"
        ]
    },
    "condemnation": {
        "id": "condemnation",
        "name": "天譴",
        "slug": "condemnation",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/condemn.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "<Rush/ Paralysis/ skill with fixed cast time>Base effectReleases the energy of l",
        "canonicalCooldown": "1 min.",
        "canonicalCooldownMs": 60000,
        "desc": "釋放雷電能量，連續兩次攻擊目標與附近敵人，威力 10500，使目標戰鬥點數 -50%。無視盾牌防禦與目標 20% 防禦力。玩家對戰／對怪物戰鬥最多攻擊 12 個目標。另使受到治療 -30%、弱化／異常抗性 -30%，並麻痺 3 秒。",
        "balance": {
            "mpCost": 82,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_condemnation",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "hellKnight",
            "shillienTemplar"
        ]
    },
    "lightning_wave_break": {
        "id": "lightning_wave_break",
        "name": "雷電波破",
        "slug": "lightning_wave_break",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_lightning_break_wave2.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Lightning Wave Break effect",
        "canonicalCooldown": "1 min.",
        "canonicalCooldownMs": 60000,
        "desc": "以雷電波動擊破敵人。",
        "balance": {
            "mpCost": 112,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_lightning_wave_break",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "shillienTemplar"
        ]
    },
    "mount_saber_toothed_cougar": {
        "id": "mount_saber_toothed_cougar",
        "name": "騎乘劍齒美洲獅",
        "slug": "mount_saber_toothed_cougar",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1835.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Mount Saber-toothed Cougar effect",
        "canonicalCooldown": "1s",
        "canonicalCooldownMs": 1000,
        "desc": "召喚並騎乘劍齒美洲獅。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_mount_saber_toothed_cougar",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [],
        "availableTo": [],
        "disabled": true,
        "removalReason": "cosmetic_mount_purge"
    },
    "dead_eye": {
        "id": "dead_eye",
        "name": "死亡之眼",
        "slug": "dead_eye",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0414.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "P. Accuracy +1P. Atk. +140Basic Critical Damage +20%Atk. Spd. -5%",
        "canonicalCooldown": "30 sec.",
        "canonicalCooldownMs": 30000,
        "desc": "物理命中 +1、物理攻擊 +140、基礎暴擊傷害 +20%、攻速 -5%。",
        "balance": {
            "mpCost": 60,
            "pwr": 2,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_dead_eye",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 60,
        "classes": [
            "ghostSentinel",
            "phantomRanger"
        ]
    },
    "mortal_piercing": {
        "id": "mortal_piercing",
        "name": "致命穿刺",
        "slug": "mortal_piercing",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/skill0321.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Mortal Piercing effect",
        "canonicalCooldown": "20 sec.",
        "canonicalCooldownMs": 20000,
        "desc": "施展致命穿刺攻擊。",
        "balance": {
            "mpCost": 100,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_mortal_piercing",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "ghostHunter"
        ]
    },
    "counterattack": {
        "id": "counterattack",
        "name": "反擊",
        "slug": "counterattack",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill0447.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Reflects 10% of received damage.",
        "canonicalCooldown": "5 min.",
        "canonicalCooldownMs": 300000,
        "desc": "反射受到傷害的 10%。",
        "balance": {
            "mpCost": 40,
            "pwr": 1,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_counterattack",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 79,
        "classes": [
            "ghostHunter"
        ]
    },
    "ethereal_blood": {
        "id": "ethereal_blood",
        "name": "乙太之血",
        "slug": "ethereal_blood",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill31402.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Ethereal Blood effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "獲得乙太之血強化。",
        "balance": {
            "mpCost": 50,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_ethereal_blood",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "ghostHunter"
        ]
    },
    "ghost_walking": {
        "id": "ghost_walking",
        "name": "幽靈步伐",
        "slug": "ghost_walking",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill0770.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Speed +30P. Evasion +15Vital Point Attack Rate +20%P. Skill Evasion +60%Buff Can",
        "canonicalCooldown": "15 min.",
        "canonicalCooldownMs": 900000,
        "desc": "速度 +30、物理迴避 +15、要害攻擊率 +20%、物理技能迴避 +60%、增益解除抗性 +90%。可與 極限迴避 同時使用。",
        "balance": {
            "mpCost": 36,
            "pwr": 2,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_ghost_walking",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "ghostHunter"
        ]
    },
    "hurricane": {
        "id": "hurricane",
        "name": "颶風",
        "slug": "hurricane",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1239.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Summons a devastating tornado.Deals M. damage to the target. Power 132.",
        "canonicalCooldown": "1 sec.",
        "canonicalCooldownMs": 1000,
        "desc": "召喚毀滅性龍捲風，對目標造成魔法傷害，威力 132。",
        "balance": {
            "mpCost": 56,
            "pwr": 132,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_hurricane",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "spellhowler",
            "stormScreamer"
        ]
    },
    "tempest": {
        "id": "tempest",
        "name": "暴風",
        "slug": "tempest",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1176.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Creates a furious storm.Deals M. damage to the target and nearby enemies. Power ",
        "canonicalCooldown": "7 sec.",
        "canonicalCooldownMs": 7000,
        "desc": "創造狂暴風暴，對目標與附近敵人造成魔法傷害，威力 78。玩家對戰／對怪物戰鬥最多攻擊 5 個目標。",
        "balance": {
            "mpCost": 68,
            "pwr": 78,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_tempest",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "spellhowler",
            "stormScreamer"
        ]
    },
    "wind_vortex": {
        "id": "wind_vortex",
        "name": "風之漩渦",
        "slug": "wind_vortex",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1341.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Creates a vortex that draws its strength from the Wind dimension.Deals M. damage",
        "canonicalCooldown": "30 sec.",
        "canonicalCooldownMs": 30000,
        "desc": "召喚來自風之次元的漩渦，對目標造成魔法傷害，威力 158。",
        "balance": {
            "mpCost": 116,
            "pwr": 158,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_wind_vortex",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "stormScreamer"
        ]
    },
    "demon_wind": {
        "id": "demon_wind",
        "name": "惡魔之風",
        "slug": "demon_wind",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1291.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Deals Wind damage to the target. Power 155.For 20 sec., Received Healing -52%.",
        "canonicalCooldown": "30 sec.",
        "canonicalCooldownMs": 30000,
        "desc": "對目標造成風屬性傷害，威力 155。20 秒內使受到治療 -52%。",
        "balance": {
            "mpCost": 110,
            "pwr": 155,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_demon_wind",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "spellhowler",
            "stormScreamer"
        ]
    },
    "storm_screamer_s_shield": {
        "id": "storm_screamer_s_shield",
        "name": "暴風狂嘯者護盾",
        "slug": "storm_screamer_s_shield",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/s_mystic_shield_original.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Storm Screamer's Shield effect",
        "canonicalCooldown": "5 min.",
        "canonicalCooldownMs": 300000,
        "desc": "展開暴風狂嘯者專屬護盾。",
        "balance": {
            "mpCost": 125,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_storm_screamer_s_shield",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "stormScreamer"
        ]
    },
    "wind_spiral": {
        "id": "wind_spiral",
        "name": "風之螺旋",
        "slug": "wind_spiral",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/s_wind_spiral.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks with a wind spiral.Deals M. damage to the target and nearby enemies. Pow",
        "canonicalCooldown": "30 sec.",
        "canonicalCooldownMs": 30000,
        "desc": "以風之螺旋攻擊目標與附近敵人，威力 229。5 秒內使 敏捷／智慧 -2。玩家對戰／對怪物戰鬥最多攻擊 6 個目標。",
        "balance": {
            "mpCost": 116,
            "pwr": 229,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_wind_spiral",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "stormScreamer"
        ]
    },
    "master_of_thunder": {
        "id": "master_of_thunder",
        "name": "雷霆大師",
        "slug": "master_of_thunder",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_gale_expert1.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Master of Thunder effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "強化對雷電力量的掌控。",
        "balance": {
            "mpCost": 120,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_master_of_thunder",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "stormScreamer"
        ]
    },
    "thunder_explosion": {
        "id": "thunder_explosion",
        "name": "雷霆爆發",
        "slug": "thunder_explosion",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/gale_burst.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Makes an explosion imbued with the Wind energy.Deals M. damage to the target. Po",
        "canonicalCooldown": "1 sec.",
        "canonicalCooldownMs": 1000,
        "desc": "引發注入風元素能量的爆炸，對目標造成魔法傷害，威力 152。帶有弱化／異常狀態的敵人會受到更多傷害。",
        "balance": {
            "mpCost": 56,
            "pwr": 152,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_thunder_explosion",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "stormScreamer"
        ]
    },
    "silence": {
        "id": "silence",
        "name": "沉默",
        "slug": "silence",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1064.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Blocks target's M. skills.",
        "canonicalCooldown": "20 sec.",
        "canonicalCooldownMs": 20000,
        "desc": "封鎖目標的魔法技能。",
        "balance": {
            "mpCost": 75,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_silence",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "spellhowler",
            "stormScreamer"
        ]
    },
    "flamenco": {
        "id": "flamenco",
        "name": "佛朗明哥",
        "slug": "flamenco",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/s_flamenco_self.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Flamenco effect",
        "canonicalCooldown": "1 min.",
        "canonicalCooldownMs": 60000,
        "desc": "以舞步啟動佛朗明哥效果。",
        "balance": {
            "mpCost": 375,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_flamenco",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "bladeDancer",
            "spectralDancer"
        ]
    },
    "crazy_waltz": {
        "id": "crazy_waltz",
        "name": "瘋狂華爾滋",
        "slug": "crazy_waltz",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_madness_waltz.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Puts the target to sleep making them dance. When the Sleep effect expires, deals",
        "canonicalCooldown": "25 sec.",
        "canonicalCooldownMs": 25000,
        "desc": "使目標沉睡並強迫其舞動。睡眠結束時造成威力 9848 的傷害。無視盾牌防禦與目標 10% 防禦力。",
        "balance": {
            "mpCost": 15,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_crazy_waltz",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "spectralDancer"
        ]
    },
    "dance_of_medusa": {
        "id": "dance_of_medusa",
        "name": "梅杜莎之舞",
        "slug": "dance_of_medusa",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/s_dance_of_medusa.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Inflicts Paralysis on nearby enemies.Duration: 2 sec.Attacks the target. Power 7",
        "canonicalCooldown": "1 min.",
        "canonicalCooldownMs": 60000,
        "desc": "使附近敵人麻痺 2 秒，之後攻擊目標，威力 7579。無視盾牌防禦與目標 10% 防禦力，並有機率解除屏障。玩家對戰／對怪物戰鬥最多攻擊 6 個目標。",
        "balance": {
            "mpCost": 95,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_dance_of_medusa",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 85,
        "classes": [
            "spectralDancer"
        ]
    },
    "twister": {
        "id": "twister",
        "name": "旋風",
        "slug": "twister",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/skill1178.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Summons a wind blade.Deals M. damage to the target. Power 31.",
        "canonicalCooldown": "1 sec.",
        "canonicalCooldownMs": 1000,
        "desc": "召喚風刃，對目標造成魔法傷害，威力 31。",
        "balance": {
            "mpCost": 18,
            "pwr": 31,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_twister",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 20,
        "classes": [
            "darkElfMage",
            "darkWizard",
            "phantomSummoner",
            "spectralMaster"
        ]
    },
    "storm_arrow_rain": {
        "id": "storm_arrow_rain",
        "name": "暴風箭雨",
        "slug": "storm_arrow_rain",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_storm_arrow_rain.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Shoots an arrow imbued with storm power.Attacks the target and nearby enemies. P",
        "canonicalCooldown": "30 sec.",
        "canonicalCooldownMs": 30000,
        "desc": "射出注入暴風力量的箭矢，攻擊目標與附近敵人，威力 10800。無視目標 10% 防禦力，並有機率附加 箭矢破壞，使物理攻擊、魔法攻擊、物理防禦、魔法防禦 各 -20%，持續 10 秒。玩家對戰／對怪物戰鬥最多攻擊 15 個目標。",
        "balance": {
            "mpCost": 170,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_storm_arrow_rain",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "ghostSentinel"
        ]
    },
    "wind_shot": {
        "id": "wind_shot",
        "name": "風之射擊",
        "slug": "wind_shot",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_wind_shot.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Shoots an arrow imbued with wind energy.Attacks the target and nearby enemies. P",
        "canonicalCooldown": "15 sec.",
        "canonicalCooldownMs": 15000,
        "desc": "射出注入風能量的箭矢，攻擊目標與附近敵人，威力 8800。附加 微風，使技能暴擊傷害 -5%，持續 5 秒。無視目標 10% 防禦力，玩家對戰／對怪物戰鬥最多攻擊 8 個目標。",
        "balance": {
            "mpCost": 110,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_wind_shot",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "ghostSentinel"
        ]
    },
    "eagle_eye": {
        "id": "eagle_eye",
        "name": "鷹眼",
        "slug": "eagle_eye",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_disaster.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Accurate shooting increases your lethal attack damage.P. Atk. +1000Atk. Spd. +3%",
        "canonicalCooldown": "-3%Speed",
        "canonicalCooldownMs": 1000,
        "desc": "精準射擊提高致命攻擊傷害：物理攻擊 +1000、攻速 +3%、物理技能暴擊傷害 +5%、技能冷卻 -3%、速度 +5。使用 雙重射擊 時有 50% 機率追加攻擊。",
        "balance": {
            "mpCost": 80,
            "pwr": 0,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_eagle_eye",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "ghostSentinel"
        ]
    },
    "summon_lord_raise": {
        "id": "summon_lord_raise",
        "name": "召喚領主萊茲",
        "slug": "summon_lord_raise",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/s_wraith_lord.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Summon Lord Raise effect",
        "canonicalCooldown": ".-",
        "canonicalCooldownMs": 1000,
        "desc": "召喚領主萊茲。",
        "balance": {
            "mpCost": 186,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_summon_lord_raise",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "spectralMaster"
        ]
    },
    "summon_nightshade": {
        "id": "summon_nightshade",
        "name": "召喚夜影",
        "slug": "summon_nightshade",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1333.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Summons Nightshade.",
        "canonicalCooldown": "5 sec.",
        "canonicalCooldownMs": 5000,
        "desc": "召喚 夜影。",
        "balance": {
            "mpCost": 144,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_summon_nightshade",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 77,
        "classes": [
            "spectralMaster"
        ]
    },
    "summon_spectral_lord": {
        "id": "summon_spectral_lord",
        "name": "召喚幽靈領主",
        "slug": "summon_spectral_lord",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1408.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Summons Spectral Lord.",
        "canonicalCooldown": "5 sec.",
        "canonicalCooldownMs": 5000,
        "desc": "召喚 幽靈領主。",
        "balance": {
            "mpCost": 144,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_summon_spectral_lord",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 77,
        "classes": [
            "spectralMaster"
        ]
    },
    "assassin_servitor": {
        "id": "assassin_servitor",
        "name": "刺客召喚獸",
        "slug": "assassin_servitor",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1348.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Makes the servitor possessed with an ancient assassin's spirit.Max HP +20%Max MP",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "讓召喚獸獲得古代刺客之魂：最大生命值 +20%、最大魔力 +20%、物理攻擊 +20%、魔法攻擊 +20%、物理防禦 +20%、魔法防禦 +20%、攻速 +15%、施法速度 +15%、速度 +10%、基礎暴擊率 +50 與 +35%、魔法技能暴擊率 +50 與 +35%、基礎／魔法技能暴擊傷害 +40%、弱化／異常抗性 +40%。",
        "balance": {
            "mpCost": 70,
            "pwr": 2,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_assassin_servitor",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 78,
        "classes": [
            "assassinS2",
            "spectralMaster"
        ]
    },
    "summon_phantom_cubic": {
        "id": "summon_phantom_cubic",
        "name": "召喚幽靈魔方",
        "slug": "summon_phantom_cubic",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill10079_3.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Summons Phantom Cubic.M. Atk. +150When using magic skills (attack/ anomaly/ debu",
        "canonicalCooldown": ":",
        "canonicalCooldownMs": 1000,
        "desc": "召喚幽靈魔方。魔法攻擊 +150。使用攻擊／異常／弱化魔法時會追加攻擊，由召喚師的幻影攻擊目標；觸發冷卻 15 秒。",
        "balance": {
            "mpCost": 78,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_summon_phantom_cubic",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "spectralMaster"
        ]
    },
    "call_of_lightning": {
        "id": "call_of_lightning",
        "name": "雷電召喚",
        "slug": "call_of_lightning",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1494.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Call of Lightning effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "呼喚雷電之力。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_call_of_lightning",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 60,
        "classes": []
    },
    "vampiric_rage": {
        "id": "vampiric_rage",
        "name": "吸血狂怒",
        "slug": "vampiric_rage",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/skill1268.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Vampiric Rage effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "獲得吸血狂怒效果。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_vampiric_rage",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 30,
        "classes": [
            "shillienElder",
            "shillienOracle"
        ]
    },
    "lightning_storm": {
        "id": "lightning_storm",
        "name": "雷電風暴",
        "slug": "lightning_storm",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/s_lightning_storm_new.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks the enemies thrice. Power 7500.Ignores Shield Defense.Ignores 15% of the",
        "canonicalCooldown": "30 sec.",
        "canonicalCooldownMs": 30000,
        "desc": "連續攻擊敵人 3 次，威力 7500，之後追加一次威力 7500 的物理攻擊並附加 暴風沼澤。無視盾牌防禦與目標 15% 防禦力。玩家對戰／對怪物戰鬥最多攻擊 6 個目標。暴風沼澤 持續 5 秒，禁止突進技能。",
        "balance": {
            "mpCost": 15,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_lightning_storm",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": []
    },
    "lightning_grip": {
        "id": "lightning_grip",
        "name": "雷電之握",
        "slug": "lightning_grip",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_lightning_grip.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Lightning Grip effect",
        "canonicalCooldown": "1 min.",
        "canonicalCooldownMs": 60000,
        "desc": "以雷電力量束縛目標。",
        "balance": {
            "mpCost": 70,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_lightning_grip",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": []
    },
    "shillien_s_mass_stigma": {
        "id": "shillien_s_mass_stigma",
        "name": "席琳群體烙印",
        "slug": "shillien_s_mass_stigma",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1897.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Marks nearby enemies with Shillien's stigma.Melee Weapon Resistance -20%M. Def. ",
        "canonicalCooldown": "3 sec.",
        "canonicalCooldownMs": 3000,
        "desc": "對附近敵人施加席琳烙印，使近戰武器抗性 -20%、魔法防禦 -10%。",
        "balance": {
            "mpCost": 45,
            "pwr": 2,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_shillien_s_mass_stigma",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "shillienElder",
            "shillienSaint"
        ]
    },
    "shillien_s_stigma": {
        "id": "shillien_s_stigma",
        "name": "席琳烙印",
        "slug": "shillien_s_stigma",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1539.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Marks the target with Shillien's stigma.Melee Weapon Resistance -20%M. Def. -10%",
        "canonicalCooldown": "3 sec.",
        "canonicalCooldownMs": 3000,
        "desc": "對目標施加席琳烙印，使近戰武器抗性 -20%、魔法防禦 -10%。",
        "balance": {
            "mpCost": 15,
            "pwr": 2,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_shillien_s_stigma",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "shillienElder",
            "shillienSaint"
        ]
    },
    "rapid_recharge": {
        "id": "rapid_recharge",
        "name": "快速充能",
        "slug": "rapid_recharge",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1428.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Recovers 263 MP.",
        "canonicalCooldown": "1 min.",
        "canonicalCooldownMs": 60000,
        "desc": "恢復 263 魔力。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_rapid_recharge",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 66,
        "classes": [
            "shillienElder",
            "shillienSaint"
        ]
    },
    "mighty_servitor": {
        "id": "mighty_servitor",
        "name": "強力召喚獸",
        "slug": "mighty_servitor",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1146.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "The effect is applied on servitors.P. Atk. +8%P. Def. +8%M. Def. +10%Atk. Spd. +",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "效果作用於召喚獸：物理攻擊 +8%、物理防禦 +8%、魔法防禦 +10%、攻速 +10%。",
        "balance": {
            "mpCost": 30,
            "pwr": 1,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_mighty_servitor",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 48,
        "classes": [
            "phantomSummoner"
        ]
    },
    "summon_soulless": {
        "id": "summon_soulless",
        "name": "召喚無魂者",
        "slug": "summon_soulless",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1278.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Summons Soulless.",
        "canonicalCooldown": "5 sec.",
        "canonicalCooldownMs": 5000,
        "desc": "召喚 無魂者。",
        "balance": {
            "mpCost": 70,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_summon_soulless",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "phantomSummoner"
        ]
    },
    "summon_silhouette": {
        "id": "summon_silhouette",
        "name": "召喚幻影",
        "slug": "summon_silhouette",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1228.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Summons Silhouette.",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "召喚 幻影。",
        "balance": {
            "mpCost": 39,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_summon_silhouette",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "phantomSummoner"
        ]
    },
    "summon_shadow": {
        "id": "summon_shadow",
        "name": "召喚暗影",
        "slug": "summon_shadow",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1128.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Summons Shadow.",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "召喚 暗影。",
        "balance": {
            "mpCost": 39,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_summon_shadow",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "phantomSummoner"
        ]
    },
    "saint_s_burst_dark": {
        "id": "saint_s_burst_dark",
        "name": "聖者爆發：暗",
        "slug": "saint_s_burst_dark",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_darkness_buster.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Saint's Burst: Dark effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "釋放黑暗之力的聖者爆發。",
        "balance": {
            "mpCost": 120,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_saint_s_burst_dark",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "shillienSaint"
        ]
    },
    "prophecy_of_wind": {
        "id": "prophecy_of_wind",
        "name": "風之預言",
        "slug": "prophecy_of_wind",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1357.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "M. Skill Critical Rate +15%M. Skill Critical Damage +25%Speed +10%PvE damage bon",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "魔法技能暴擊率 +15%、暴擊傷害 +25%、速度 +10%、對怪物戰鬥傷害 +20%、受到技能暴擊率 -6%、受到技能暴擊傷害 -6%、弱化／異常抗性 +10%。",
        "balance": {
            "mpCost": 72,
            "pwr": 2,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_prophecy_of_wind",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "shillienSaint"
        ]
    },
    "nemesis": {
        "id": "nemesis",
        "name": "復仇女神",
        "slug": "nemesis",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_nemesis.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Brings punishment to enemies.Deals M. damage to the target. Power 195.Received H",
        "canonicalCooldown": "1 min.",
        "canonicalCooldownMs": 60000,
        "desc": "懲罰敵人，對目標造成魔法傷害，威力 195。受到治療 -70%，持續 10 秒。玩家對戰／對怪物戰鬥最多攻擊 6 個目標。",
        "balance": {
            "mpCost": 91,
            "pwr": 195,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_nemesis",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "shillienSaint"
        ]
    },
    "shillien_s_power": {
        "id": "shillien_s_power",
        "name": "席琳之力",
        "slug": "shillien_s_power",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill19167.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Shillien's Power effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "獲得席琳之力。",
        "balance": {
            "mpCost": 58,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_shillien_s_power",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "shillienSaint"
        ]
    },
    "shillien_s_touch": {
        "id": "shillien_s_touch",
        "name": "席琳之觸",
        "slug": "shillien_s_touch",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1912.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Shillien's Touch effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "施展席琳之觸。",
        "balance": {
            "mpCost": 37,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_shillien_s_touch",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "shillienSaint"
        ]
    },
    "shillien_s_grail": {
        "id": "shillien_s_grail",
        "name": "席琳聖杯",
        "slug": "shillien_s_grail",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_shilen_grail.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Shillien's Grail effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "獲得席琳聖杯的力量。",
        "balance": {
            "mpCost": 150,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_shillien_s_grail",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "shillienSaint"
        ]
    },
    "mass_wind_shackles": {
        "id": "mass_wind_shackles",
        "name": "群體風之枷鎖",
        "slug": "mass_wind_shackles",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill30523.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Slows targets and nearby enemies down.Atk. Spd. -23%Casting Spd. -23%",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "降低目標與附近敵人的速度：攻速 -23%、施法速度 -23%。",
        "balance": {
            "mpCost": 140,
            "pwr": 2,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_mass_wind_shackles",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "shillienSaint"
        ]
    },
    "duress": {
        "id": "duress",
        "name": "脅迫",
        "slug": "duress",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_imprison.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Inflicts Hold on the target and nearby enemies. P./ M. Atk. -30%.",
        "canonicalCooldown": "20 sec.",
        "canonicalCooldownMs": 20000,
        "desc": "束縛目標與附近敵人，並使物理攻擊／魔法攻擊 -30%。",
        "balance": {
            "mpCost": 150,
            "pwr": 3,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_duress",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "shillienSaint"
        ]
    },
    "mass_dryad_root": {
        "id": "mass_dryad_root",
        "name": "群體樹精之根",
        "slug": "mass_dryad_root",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1508.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Inflicts Hold on the target and nearby enemies.",
        "canonicalCooldown": "30 sec.",
        "canonicalCooldownMs": 30000,
        "desc": "使目標與附近敵人陷入束縛。",
        "balance": {
            "mpCost": 140,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_mass_dryad_root",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "shillienSaint"
        ]
    },
    "parasite_rose": {
        "id": "parasite_rose",
        "name": "寄生玫瑰",
        "slug": "parasite_rose",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_parasite_rose.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Parasite Rose effect",
        "canonicalCooldown": "3 sec.",
        "canonicalCooldownMs": 3000,
        "desc": "施加寄生玫瑰效果。",
        "balance": {
            "mpCost": 100,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_parasite_rose",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "bloodRose",
            "bloodRoseS2"
        ]
    },
    "enchanted_rose_s_assault": {
        "id": "enchanted_rose_s_assault",
        "name": "魔法玫瑰突襲",
        "slug": "enchanted_rose_s_assault",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_rose_assault.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Rushes to the target and attacks them.Deals M. damage to the target twice. Power",
        "canonicalCooldown": "5 sec.",
        "canonicalCooldownMs": 5000,
        "desc": "突進至目標並連續造成兩次魔法傷害，威力 151。使用時消耗 1 個 靈魂礦石。",
        "balance": {
            "mpCost": 59,
            "pwr": 151,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_enchanted_rose_s_assault",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "bloodRose",
            "bloodRoseS2"
        ]
    },
    "rose_attack": {
        "id": "rose_attack",
        "name": "玫瑰攻擊",
        "slug": "rose_attack",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_rose_spine.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "<Changing skill>The attack type is based on the distance to the target.Distance ",
        "canonicalCooldown": "0.5 sec.",
        "canonicalCooldownMs": 500,
        "desc": "依距離改變攻擊方式。距離 250 以內時以附魔荊棘藤攻擊，造成魔法傷害，威力 126；距離 250 以上時召喚附魔匕首，威力 110。",
        "balance": {
            "mpCost": 47,
            "pwr": 126,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_rose_attack",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "bloodRose",
            "bloodRoseBase",
            "bloodRoseS1",
            "bloodRoseS2"
        ]
    },
    "dark_disruption": {
        "id": "dark_disruption",
        "name": "黑暗擾亂",
        "slug": "dark_disruption",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/banish.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Dark Disruption effect",
        "canonicalCooldown": "1 min.",
        "canonicalCooldownMs": 60000,
        "desc": "以黑暗力量擾亂敵人。",
        "balance": {
            "mpCost": 105,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_dark_disruption",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "shillienSaint"
        ]
    },
    "mass_purify": {
        "id": "mass_purify",
        "name": "群體淨化",
        "slug": "mass_purify",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1551.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Mass Purify effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "淨化附近盟友的負面狀態。",
        "balance": {
            "mpCost": 80,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_mass_purify",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "shillienSaint"
        ]
    },
    "aroma_of_death": {
        "id": "aroma_of_death",
        "name": "死亡芳香",
        "slug": "aroma_of_death",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_purpume_of_death.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Teleports to the target poisoning them with deadly fumes.Deals M. damage to the ",
        "canonicalCooldown": "1 min.",
        "canonicalCooldownMs": 60000,
        "desc": "傳送至目標並以致命毒霧攻擊目標與附近敵人，威力 237。帶有弱化／異常狀態的敵人會受到更多傷害，弱化／異常抗性 -5%，持續 7 秒，並附加 寄生玫瑰 等級 3：最大生命值 -30%、束縛抗性 -15%。之後可再次造成威力 257 的魔法傷害。玩家對戰／對怪物戰鬥最多攻擊 5 個目標。",
        "balance": {
            "mpCost": 109,
            "pwr": 237,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_aroma_of_death",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "bloodRose",
            "bloodRoseS2"
        ]
    },
    "briar_vortex": {
        "id": "briar_vortex",
        "name": "荊棘漩渦",
        "slug": "briar_vortex",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_bloody_spiral.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "<Changing skill>The attack type is based on the distance to the target.Distance ",
        "canonicalCooldown": "15 sec.",
        "canonicalCooldownMs": 15000,
        "desc": "依距離改變攻擊方式。距離 250 以內時荊棘化為撕裂風暴，造成魔法傷害，威力 166，魔法防禦 -30%，持續 4 秒；距離 250 以上時荊棘穿刺敵人，威力 156。玩家對戰／對怪物戰鬥最多攻擊 3 個目標。",
        "balance": {
            "mpCost": 60,
            "pwr": 166,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_briar_vortex",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "bloodRose",
            "bloodRoseS1"
        ]
    },
    "vine_embrace": {
        "id": "vine_embrace",
        "name": "藤蔓纏抱",
        "slug": "vine_embrace",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_ensnare.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "<Hold/ changing skill>Damage is based on the distance to the target.With a certa",
        "canonicalCooldown": "15 sec.",
        "canonicalCooldownMs": 15000,
        "desc": "束縛／距離變化技能。有一定機率使目標束縛 3 秒。距離 250 以內時造成魔法傷害，威力 60；距離 250 以上時威力 45。",
        "balance": {
            "mpCost": 61,
            "pwr": 60,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_vine_embrace",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "bloodRose",
            "bloodRoseS1",
            "bloodRoseS2"
        ]
    },
    "reflecting_illusion": {
        "id": "reflecting_illusion",
        "name": "反射幻象",
        "slug": "reflecting_illusion",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_reflect_illusion2.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Reflecting Illusion effect",
        "canonicalCooldown": "2 min.",
        "canonicalCooldownMs": 120000,
        "desc": "反射幻象。",
        "balance": {
            "mpCost": 75,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_reflecting_illusion",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "bloodRose"
        ]
    },
    "crimson_rose": {
        "id": "crimson_rose",
        "name": "緋紅玫瑰",
        "slug": "crimson_rose",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_crimson_rose.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Crimson Rose effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "緋紅玫瑰。",
        "balance": {
            "mpCost": 85,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_crimson_rose",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "bloodRose",
            "bloodRoseS2"
        ]
    },
    "kingdom_of_plants": {
        "id": "kingdom_of_plants",
        "name": "植物王國",
        "slug": "kingdom_of_plants",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_floral_monarch.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Kingdom of Plants effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "植物王國。",
        "balance": {
            "mpCost": 120,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_kingdom_of_plants",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "bloodRose"
        ]
    },
    "blooming_nightmare": {
        "id": "blooming_nightmare",
        "name": "綻放夢魘",
        "slug": "blooming_nightmare",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/s_nightmare_blossom2.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Summons an enchanted dagger to shred all enemies.Deals M. damage to the target a",
        "canonicalCooldown": "30 sec.",
        "canonicalCooldownMs": 30000,
        "desc": "召喚附魔匕首撕裂所有敵人，對目標與附近敵人造成魔法傷害，威力 195。附加夢魘玫瑰：技能暴擊傷害 -10%、受到魔法技能暴擊傷害 +5%、魔法防禦 -30%，持續 8 秒。玩家對戰／對怪物戰鬥最多攻擊 3 個目標。",
        "balance": {
            "mpCost": 60,
            "pwr": 195,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_blooming_nightmare",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "bloodRose",
            "bloodRoseS2"
        ]
    },
    "bleeding_rose": {
        "id": "bleeding_rose",
        "name": "流血玫瑰",
        "slug": "bleeding_rose",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_bloody_rose.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Bleeding Rose effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "流血玫瑰。",
        "balance": {
            "mpCost": 120,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_bleeding_rose",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "bloodRose"
        ]
    },
    "rose_thorns": {
        "id": "rose_thorns",
        "name": "玫瑰荊棘",
        "slug": "rose_thorns",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/s_briar_lance.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Rose Thorns effect",
        "canonicalCooldown": "20 sec.",
        "canonicalCooldownMs": 20000,
        "desc": "玫瑰荊棘。",
        "balance": {
            "mpCost": 150,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_rose_thorns",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "bloodRose"
        ]
    },
    "excruciating_strike": {
        "id": "excruciating_strike",
        "name": "劇痛打擊",
        "slug": "excruciating_strike",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill11513.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks nearby targets. Power 465.Ignores Shield Defense.Ignores 30% of the targ",
        "canonicalCooldown": "3 sec.",
        "canonicalCooldownMs": 3000,
        "desc": "攻擊附近目標，威力 465。無視盾牌防禦與目標 30% 防禦力。不會暴擊。玩家對戰／對怪物戰鬥最多攻擊 5 個目標。",
        "balance": {
            "mpCost": 46,
            "pwr": 465,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_excruciating_strike",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "destroyer",
            "titan"
        ]
    },
    "queen_s_garden": {
        "id": "queen_s_garden",
        "name": "女王花園",
        "slug": "queen_s_garden",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/s_queens_garden.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "<Hold skill>Summons a royal garden with giant flowers.Attacks the target and nea",
        "canonicalCooldown": "4 min.",
        "canonicalCooldownMs": 240000,
        "desc": "<束縛技能>召喚長滿巨大花朵的皇家花園，連續 3 次攻擊目標與附近敵人，威力 380。弱化效果持續 4 秒：受到治療 -50%、生命值 恢復藥水效果上限 1000，禁止突進技能。異常狀態會施加束縛，持續 10 秒。玩家對戰／對怪物戰鬥最多攻擊 8 個目標。使用時附加 女王 效果，持續 30 秒：魔法技能威力 +5%、對怪物戰鬥傷害 +5%。使用攻擊技能時有一定機率召喚荊棘追加傷害；傷害依魔法攻擊 計算，威力 200，觸發率 30%。",
        "balance": {
            "mpCost": 330,
            "pwr": 380,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_queen_s_garden",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "bloodRose"
        ]
    },
    "iron_punch": {
        "id": "iron_punch",
        "name": "鐵拳",
        "slug": "iron_punch",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/skill0029.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Delivers a powerful fist strike.Power 45.No critical.",
        "canonicalCooldown": "1 sec.",
        "canonicalCooldownMs": 1000,
        "desc": "施展強力拳擊，威力 45。不會暴擊。",
        "balance": {
            "mpCost": 5,
            "pwr": 45,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_iron_punch",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 3,
        "classes": [
            "dwarfFighter",
            "marauderBase",
            "monk",
            "orcFighter"
        ]
    },
    "chant_of_wild_magic": {
        "id": "chant_of_wild_magic",
        "name": "野性魔力頌歌",
        "slug": "chant_of_wild_magic",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1413.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Chant of Wild Magic effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "野性魔力頌歌。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_chant_of_wild_magic",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "destroyer",
            "dominator",
            "doomcryer",
            "dragoon",
            "grandKhavatari",
            "grandVanguard",
            "monk",
            "overlord",
            "raider",
            "shaman",
            "titan",
            "tyrant",
            "vanguardRider",
            "warcryer"
        ]
    },
    "zealot": {
        "id": "zealot",
        "name": "狂信者",
        "slug": "zealot",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0420.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Atk. Spd. +10%Basic Critical Rate +22Basic Critical Damage +22%P. Skill Critical",
        "canonicalCooldown": "5 min.",
        "canonicalCooldownMs": 300000,
        "desc": "攻速 +10%、基礎暴擊率 +22、基礎暴擊傷害 +22%、物理技能暴擊率 +11%、物理技能暴擊傷害 +11%、物理命中 +6、速度 +10、物理技能魔力消耗 -50%、增益解除抗性 +40%、受到治療 -50%、擊退／擊倒成功率 +3%。使用技能時恢復 80 魔力。",
        "balance": {
            "mpCost": 15,
            "pwr": 1,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_zealot",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 58,
        "classes": [
            "destroyer",
            "titan",
            "tyrant"
        ]
    },
    "chant_of_berserker_spirit": {
        "id": "chant_of_berserker_spirit",
        "name": "狂戰士之魂頌歌",
        "slug": "chant_of_berserker_spirit",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill19207.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Chant of Berserker Spirit effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "狂戰士之魂頌歌。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_chant_of_berserker_spirit",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "destroyer",
            "dominator",
            "doomcryer",
            "dragoon",
            "grandKhavatari",
            "grandVanguard",
            "monk",
            "overlord",
            "raider",
            "shaman",
            "titan",
            "tyrant",
            "vanguardRider",
            "warcryer"
        ]
    },
    "guts": {
        "id": "guts",
        "name": "毅力",
        "slug": "guts",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0139.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "P. Def. +400P. Def. +35%Speed +10Debuff Resistance +25%Anomaly Resistance +25%Ca",
        "canonicalCooldown": "5 min.",
        "canonicalCooldownMs": 300000,
        "desc": "物理防禦 +400、物理防禦 +35%、速度 +10、弱化抗性 +25%、異常抗性 +25%。無法與 狂暴 同時使用。",
        "balance": {
            "mpCost": 30,
            "pwr": 4,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_guts",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 43,
        "classes": [
            "destroyer",
            "raider",
            "titan"
        ]
    },
    "chant_of_haste": {
        "id": "chant_of_haste",
        "name": "加速頌歌",
        "slug": "chant_of_haste",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1251.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Chant of Haste effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "加速頌歌。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_chant_of_haste",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "destroyer",
            "dominator",
            "doomcryer",
            "dragoon",
            "grandKhavatari",
            "grandVanguard",
            "monk",
            "overlord",
            "raider",
            "shaman",
            "titan",
            "tyrant",
            "vanguardRider",
            "warcryer"
        ]
    },
    "rage": {
        "id": "rage",
        "name": "狂怒",
        "slug": "rage",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0094.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Rage effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "狂怒。",
        "balance": {
            "mpCost": 30,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_rage",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 55,
        "classes": [
            "destroyer",
            "raider",
            "titan"
        ]
    },
    "chant_of_magic_barrier": {
        "id": "chant_of_magic_barrier",
        "name": "魔法屏障頌歌",
        "slug": "chant_of_magic_barrier",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1549.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Chant of Magic Barrier effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "魔法屏障頌歌。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_chant_of_magic_barrier",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "destroyer",
            "dominator",
            "doomcryer",
            "dragoon",
            "grandKhavatari",
            "grandVanguard",
            "monk",
            "overlord",
            "raider",
            "shaman",
            "titan",
            "tyrant",
            "vanguardRider",
            "warcryer"
        ]
    },
    "frenzy": {
        "id": "frenzy",
        "name": "狂暴",
        "slug": "frenzy",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0176.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "P. Atk. +9%Atk. Spd. +8%Basic Critical Rate +5Basic Critical Damage +5%P. Skill ",
        "canonicalCooldown": "5 min.",
        "canonicalCooldownMs": 300000,
        "desc": "物理攻擊 +9%、攻速 +8%、基礎暴擊率 +5、基礎暴擊傷害 +5%、物理技能暴擊率 +4%、物理技能暴擊傷害 +4%、速度 +9。使用雙手劍時：物理攻擊 +10%、物理命中 +8。無法與 毅力 同時使用。",
        "balance": {
            "mpCost": 20,
            "pwr": 1,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_frenzy",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 46,
        "classes": [
            "destroyer",
            "raider",
            "titan"
        ]
    },
    "chant_of_acumen": {
        "id": "chant_of_acumen",
        "name": "靈活思緒頌歌",
        "slug": "chant_of_acumen",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1006.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Chant of Acumen effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "靈活思緒頌歌。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_chant_of_acumen",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "destroyer",
            "dominator",
            "doomcryer",
            "dragoon",
            "grandKhavatari",
            "grandVanguard",
            "monk",
            "overlord",
            "raider",
            "shaman",
            "titan",
            "tyrant",
            "vanguardRider",
            "warcryer"
        ]
    },
    "chant_of_wind_walk": {
        "id": "chant_of_wind_walk",
        "name": "風之疾走頌歌",
        "slug": "chant_of_wind_walk",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/skill1535.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Speed +20",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "速度 +20。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_chant_of_wind_walk",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 20,
        "classes": [
            "dominator",
            "doomcryer",
            "dragoon",
            "grandKhavatari",
            "grandVanguard",
            "monk",
            "raider",
            "shaman",
            "titan"
        ]
    },
    "enchanted_rose_s_gentle_magic": {
        "id": "enchanted_rose_s_gentle_magic",
        "name": "附魔玫瑰的柔和魔法",
        "slug": "enchanted_rose_s_gentle_magic",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/s_rose_lv_reward2.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Enchanted Rose's Gentle Magic effect",
        "canonicalCooldown": "2",
        "canonicalCooldownMs": 1000,
        "desc": "附魔玫瑰的柔和魔法。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_enchanted_rose_s_gentle_magic",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 60,
        "classes": [
            "bloodRoseS2"
        ]
    },
    "enchanted_rose_s_weak_magic": {
        "id": "enchanted_rose_s_weak_magic",
        "name": "附魔玫瑰的微弱魔法",
        "slug": "enchanted_rose_s_weak_magic",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/s_rose_lv_reward1.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Enchanted Rose's Weak Magic effect",
        "canonicalCooldown": "2",
        "canonicalCooldownMs": 1000,
        "desc": "附魔玫瑰的微弱魔法。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_enchanted_rose_s_weak_magic",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 45,
        "classes": [
            "bloodRoseS2"
        ]
    },
    "enchanted_rose_s_strong_magic": {
        "id": "enchanted_rose_s_strong_magic",
        "name": "附魔玫瑰的強力魔法",
        "slug": "enchanted_rose_s_strong_magic",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/s_rose_lv_reward3.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Enchanted Rose's Strong Magic effect",
        "canonicalCooldown": "1",
        "canonicalCooldownMs": 1000,
        "desc": "附魔玫瑰的強力魔法。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_enchanted_rose_s_strong_magic",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 70,
        "classes": [
            "bloodRoseS2"
        ]
    },
    "titanic_break": {
        "id": "titanic_break",
        "name": "泰坦破壞",
        "slug": "titanic_break",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_gigantic_crush.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks the target and nearby enemies. Power 5544.With a certain chance, knocks ",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "攻擊目標與附近敵人，威力 5544。有一定機率擊倒目標。無視盾牌防禦與目標 5% 防禦力。玩家對戰／對怪物戰鬥最多攻擊 5 個目標。",
        "balance": {
            "mpCost": 64,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_titanic_break",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "titan"
        ]
    },
    "demolition_formula": {
        "id": "demolition_formula",
        "name": "破壞公式",
        "slug": "demolition_formula",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/s_formula_of_demolition.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks the target. Power 6625.Ignores Shield Defense.Ignores 10% of the target'",
        "canonicalCooldown": "1 sec.",
        "canonicalCooldownMs": 1000,
        "desc": "攻擊目標，威力 6625。無視盾牌防禦與目標 10% 防禦力。",
        "balance": {
            "mpCost": 68,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_demolition_formula",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "titan"
        ]
    },
    "armor_crush": {
        "id": "armor_crush",
        "name": "護甲粉碎",
        "slug": "armor_crush",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill0362.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks the target. Power 4275.Stuns for 3 sec.P./ M. Def. -30%Ignores Shield De",
        "canonicalCooldown": "20 sec.",
        "canonicalCooldownMs": 20000,
        "desc": "攻擊目標，威力 4275，並暈眩 3 秒。物理防禦／魔法防禦 -30%。無視盾牌防禦。",
        "balance": {
            "mpCost": 60,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_armor_crush",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 77,
        "classes": [
            "fortuneSeeker",
            "maestro",
            "shinemaker",
            "titan"
        ]
    },
    "demolition_impact": {
        "id": "demolition_impact",
        "name": "破壞衝擊",
        "slug": "demolition_impact",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill0777.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Unleashes a destructive shockwave.Attacks the target. Power 4907.Ignores Shield ",
        "canonicalCooldown": "1 sec.",
        "canonicalCooldownMs": 1000,
        "desc": "釋放毀滅性衝擊波攻擊目標，威力 4907。無視盾牌防禦與目標 10% 防禦力。",
        "balance": {
            "mpCost": 60,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_demolition_impact",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "titan"
        ]
    },
    "titanic_sweep": {
        "id": "titanic_sweep",
        "name": "泰坦橫掃",
        "slug": "titanic_sweep",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_gigantic_swing.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks nearby enemies. Power 3772.When Giant's Ring is triggered, attacks nearb",
        "canonicalCooldown": "5 sec.",
        "canonicalCooldownMs": 5000,
        "desc": "攻擊附近敵人，威力 3772。觸發 巨人之環 時，連續 3 次攻擊附近敵人，威力 3960。無視盾牌防禦與目標 10% 防禦力。玩家對戰／對怪物戰鬥最多攻擊 12 個目標。",
        "balance": {
            "mpCost": 72,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_titanic_sweep",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "titan"
        ]
    },
    "full_sweep": {
        "id": "full_sweep",
        "name": "全力橫掃",
        "slug": "full_sweep",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill0814.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Makes a powerful strike with a two-handed sword.Attacks nearby targets. Power 24",
        "canonicalCooldown": "3 sec.",
        "canonicalCooldownMs": 3000,
        "desc": "以雙手劍施展強力揮擊，攻擊附近目標，威力 2424。無視盾牌防禦。玩家對戰／對怪物戰鬥最多攻擊 5 個目標。",
        "balance": {
            "mpCost": 85,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_full_sweep",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "titan"
        ]
    },
    "titans_secrets": {
        "id": "titans_secrets",
        "name": "泰坦秘訣",
        "slug": "titans_secrets",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/s_secret_of_titan.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "When used, removes Suppression anomalies.For 3 sec., Suppression Resistance +20%",
        "canonicalCooldown": "2 min.",
        "canonicalCooldownMs": 120000,
        "desc": "使用時移除壓制異常狀態。3 秒內壓制抗性 +20%。恢復 1000戰鬥點數，並恢復 15% 生命值。",
        "balance": {
            "mpCost": 45,
            "pwr": 2,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_titans_secrets",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "titan"
        ]
    },
    "hyper_rush": {
        "id": "hyper_rush",
        "name": "極速突進",
        "slug": "hyper_rush",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/titan_hyper_rush.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Hyper Rush effect",
        "canonicalCooldown": "3 sec.",
        "canonicalCooldownMs": 3000,
        "desc": "極速突進。",
        "balance": {
            "mpCost": 50,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_hyper_rush",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 80,
        "classes": [
            "titan"
        ]
    },
    "destroyer_s_roar": {
        "id": "destroyer_s_roar",
        "name": "破壞者咆哮",
        "slug": "destroyer_s_roar",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1481.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Destroyer's Roar effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "破壞者咆哮。",
        "balance": {
            "mpCost": 57,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_destroyer_s_roar",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 77,
        "classes": [
            "titan"
        ]
    },
    "titanic_weapon": {
        "id": "titanic_weapon",
        "name": "泰坦武器",
        "slug": "titanic_weapon",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_gigantic_weapon.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "P. Atk. +55%Dagger Resistance -20%P. Accuracy +3Max HP +300Atk. Spd. +100Increas",
        "canonicalCooldown": "1 min.",
        "canonicalCooldownMs": 60000,
        "desc": "物理攻擊 +55%、匕首抗性 -20%、物理命中 +3、最大生命值 +300、攻速 +100，並增加攻擊距離。使用 破壞衝擊／破壞公式 時有 20% 機率追加攻擊。",
        "balance": {
            "mpCost": 63,
            "pwr": 6,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_titanic_weapon",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "titan"
        ]
    },
    "destructive_shriek": {
        "id": "destructive_shriek",
        "name": "毀滅尖嘯",
        "slug": "destructive_shriek",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "5★",
        "starRank": 5,
        "icon": "/icons/s_destroy_shout.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Destructive Shriek effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "毀滅尖嘯。",
        "balance": {
            "mpCost": 62,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_destructive_shriek",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 90,
        "classes": [
            "titan"
        ]
    },
    "full_knockdown": {
        "id": "full_knockdown",
        "name": "全力擊倒",
        "slug": "full_knockdown",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1896.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Delivers a powerful blow to the target knocking them down.Attacks the target. Po",
        "canonicalCooldown": "15 sec.",
        "canonicalCooldownMs": 15000,
        "desc": "對目標施展強力一擊並將其擊倒。攻擊目標，威力 4937；有一定機率擊倒 3 秒。無視盾牌防禦。",
        "balance": {
            "mpCost": 107,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_full_knockdown",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 81,
        "classes": [
            "grandKhavatari",
            "titan"
        ]
    },
    "over_the_body": {
        "id": "over_the_body",
        "name": "超越肉體",
        "slug": "over_the_body",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill0536.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Over the Body effect",
        "canonicalCooldown": "15 min.",
        "canonicalCooldownMs": 900000,
        "desc": "超越肉體。",
        "balance": {
            "mpCost": 65,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_over_the_body",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 78,
        "classes": [
            "titan"
        ]
    },
    "mount_black_bear": {
        "id": "mount_black_bear",
        "name": "騎乘黑熊",
        "slug": "mount_black_bear",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1837.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Mount Black Bear effect",
        "canonicalCooldown": "1s",
        "canonicalCooldownMs": 1000,
        "desc": "騎乘黑熊。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_mount_black_bear",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [],
        "availableTo": [],
        "disabled": true,
        "removalReason": "cosmetic_mount_purge"
    },
    "inferno_strike": {
        "id": "inferno_strike",
        "name": "煉獄打擊",
        "slug": "inferno_strike",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/skill31389.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Inferno Strike effect",
        "canonicalCooldown": "1 sec.",
        "canonicalCooldownMs": 1000,
        "desc": "煉獄打擊。",
        "balance": {
            "mpCost": 43,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_inferno_strike",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 20,
        "classes": [
            "dominator",
            "doomcryer",
            "orcMage",
            "overlord",
            "shaman",
            "warcryer"
        ]
    },
    "vortex_of_fire": {
        "id": "vortex_of_fire",
        "name": "火焰漩渦",
        "slug": "vortex_of_fire",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/skill11024.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Vortex of Fire effect",
        "canonicalCooldown": "4 sec.",
        "canonicalCooldownMs": 4000,
        "desc": "火焰漩渦。",
        "balance": {
            "mpCost": 20,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_vortex_of_fire",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 20,
        "classes": [
            "dominator",
            "doomcryer",
            "orcMage",
            "overlord",
            "shaman",
            "warcryer"
        ]
    },
    "titan_champion": {
        "id": "titan_champion",
        "name": "泰坦勇士",
        "slug": "titan_champion",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/s_champion_of_titan.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Max HP +50%(Momentarily recovers bonus HP.)P. Atk. +35%P. Def. +15%M. Def. +15%C",
        "canonicalCooldown": "1 min.",
        "canonicalCooldownMs": 60000,
        "desc": "最大生命值 +50%（立即恢復增加的 生命值）、物理攻擊 +35%、物理防禦 +15%、魔法防禦 +15%、基礎攻擊與物理技能暴擊率 +65%、基礎攻擊與物理技能暴擊傷害 +65%、物理技能傷害 +20%、擊退／擊倒成功率 +55%、恐懼成功率 +55%、擊退／擊倒抗性 +20%、束縛抗性 +15%、速度 +20。使用技能時對附近敵人附加 泰坦魅力，持續 3 秒；有一定機率使附近敵人陷入 焦慮（恐懼）。",
        "balance": {
            "mpCost": 300,
            "pwr": 5,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_titan_champion",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "titan"
        ]
    },
    "shining_prison": {
        "id": "shining_prison",
        "name": "光輝牢籠",
        "slug": "shining_prison",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/skill1452.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Inflicts Hold on the target and nearby enemies.",
        "canonicalCooldown": "30 sec.",
        "canonicalCooldownMs": 30000,
        "desc": "使目標與附近敵人陷入束縛。",
        "balance": {
            "mpCost": 34,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_shining_prison",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 25,
        "classes": [
            "dominator",
            "doomcryer",
            "overlord",
            "shaman",
            "warcryer"
        ]
    },
    "life_rescue": {
        "id": "life_rescue",
        "name": "生命救援",
        "slug": "life_rescue",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/skill1610.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Recovers 127 of the target's HP.",
        "canonicalCooldown": "3 sec.",
        "canonicalCooldownMs": 3000,
        "desc": "恢復目標 127 生命值。",
        "balance": {
            "mpCost": 32,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_life_rescue",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 20,
        "classes": [
            "dominator",
            "doomcryer",
            "overlord",
            "shaman",
            "warcryer"
        ]
    },
    "fear": {
        "id": "fear",
        "name": "恐懼",
        "slug": "fear",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/skill1092.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Inflicts Fear on the target.",
        "canonicalCooldown": "20 sec.",
        "canonicalCooldownMs": 20000,
        "desc": "使目標陷入恐懼。",
        "balance": {
            "mpCost": 20,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_fear",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 20,
        "classes": [
            "dominator",
            "doomcryer",
            "orcMage",
            "overlord",
            "shaman",
            "warcryer"
        ]
    },
    "madness": {
        "id": "madness",
        "name": "瘋狂",
        "slug": "madness",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/skill1105.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Swaps the enemy's target for a random one.",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "將敵人的目標隨機切換為其他目標。",
        "balance": {
            "mpCost": 20,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_madness",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 20,
        "classes": [
            "overlord",
            "shaman",
            "warcryer"
        ]
    },
    "dreaming_spirit": {
        "id": "dreaming_spirit",
        "name": "夢之精靈",
        "slug": "dreaming_spirit",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/skill1097.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Inflicts Sleep on the target.",
        "canonicalCooldown": "20 sec.",
        "canonicalCooldownMs": 20000,
        "desc": "使目標陷入睡眠。",
        "balance": {
            "mpCost": 20,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_dreaming_spirit",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 20,
        "classes": [
            "dominator",
            "doomcryer",
            "orcMage",
            "overlord",
            "shaman",
            "warcryer"
        ]
    },
    "frost_flame": {
        "id": "frost_flame",
        "name": "霜凍火焰",
        "slug": "frost_flame",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/skill1107.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Deadly freezing flame strikes the target.Deals continuous damage for 15 sec.",
        "canonicalCooldown": "2 sec.",
        "canonicalCooldownMs": 2000,
        "desc": "致命的冰凍火焰襲擊目標，持續造成傷害 15 秒。",
        "balance": {
            "mpCost": 29,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_frost_flame",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 20,
        "classes": [
            "shaman"
        ]
    },
    "cold_flames": {
        "id": "cold_flames",
        "name": "寒冰火焰",
        "slug": "cold_flames",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/cold_flame.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Cold Flames effect",
        "canonicalCooldown": "4 sec.",
        "canonicalCooldownMs": 4000,
        "desc": "寒冰火焰。",
        "balance": {
            "mpCost": 95,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_cold_flames",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "doomcryer"
        ]
    },
    "blood_bond": {
        "id": "blood_bond",
        "name": "血之羈絆",
        "slug": "blood_bond",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/blood_link.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Uses blood magic to attack the target bound to the target.Deals M. damage. Power",
        "canonicalCooldown": "15 sec.",
        "canonicalCooldownMs": 15000,
        "desc": "使用血魔法攻擊與目標相連的敵人，造成魔法傷害，威力 234。有 30% 機率觸發追加攻擊；帶有弱化／異常狀態的敵人受到更多傷害。另有 30% 機率恢復 1000 生命值。玩家對戰／對怪物戰鬥最多攻擊 6 個目標。",
        "balance": {
            "mpCost": 97,
            "pwr": 234,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_blood_bond",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "doomcryer"
        ]
    },
    "crimson_flame": {
        "id": "crimson_flame",
        "name": "緋紅火焰",
        "slug": "crimson_flame",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_crimson_flame.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Crimson Flame effect",
        "canonicalCooldown": ".-",
        "canonicalCooldownMs": 1000,
        "desc": "緋紅火焰。",
        "balance": {
            "mpCost": 150,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_crimson_flame",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "dominator",
            "doomcryer"
        ]
    },
    "bloody_strike": {
        "id": "bloody_strike",
        "name": "血腥打擊",
        "slug": "bloody_strike",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_bloody_strike.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Unleashes a black-red energy ball at the enemy making it explode.Deals M. damage",
        "canonicalCooldown": "1 sec.",
        "canonicalCooldownMs": 1000,
        "desc": "向敵人釋放黑紅色能量球並引爆，對目標造成魔法傷害，威力 152。帶有弱化／異常狀態的敵人受到更多傷害。",
        "balance": {
            "mpCost": 56,
            "pwr": 152,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_bloody_strike",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "dominator",
            "doomcryer"
        ]
    },
    "chant_of_prophecy": {
        "id": "chant_of_prophecy",
        "name": "預言頌歌",
        "slug": "chant_of_prophecy",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1363.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Casting Spd. +10%Speed +2M. Skill Critical Rate +15%M. Skill Critical Damage +15",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "施法速度 +10%、速度 +2、魔法技能暴擊率 +15%、魔法技能暴擊傷害 +15%、受到基礎暴擊傷害 -30%、受到基礎暴擊率 -30%、生命值 恢復率 +20%、魔力 恢復率 +20%、對怪物戰鬥傷害 +23%，並反射受到傷害的 10%。",
        "balance": {
            "mpCost": 172,
            "pwr": 1,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_chant_of_prophecy",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "doomcryer"
        ]
    },
    "chant_of_healing": {
        "id": "chant_of_healing",
        "name": "治癒頌歌",
        "slug": "chant_of_healing",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/cure_of_paagrio.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Chant of Healing effect",
        "canonicalCooldown": "2 sec.",
        "canonicalCooldownMs": 2000,
        "desc": "治癒頌歌。",
        "balance": {
            "mpCost": 282,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_chant_of_healing",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "doomcryer"
        ]
    },
    "chant_of_glory": {
        "id": "chant_of_glory",
        "name": "榮耀頌歌",
        "slug": "chant_of_glory",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/skill1002_new.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Chant of Glory effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "榮耀頌歌。",
        "balance": {
            "mpCost": 100,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_chant_of_glory",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 85,
        "classes": [
            "doomcryer",
            "warcryer"
        ]
    },
    "chant_of_gate": {
        "id": "chant_of_gate",
        "name": "傳送門頌歌",
        "slug": "chant_of_gate",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1429.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Chant of Gate effect",
        "canonicalCooldown": "5 min.",
        "canonicalCooldownMs": 300000,
        "desc": "傳送門頌歌。",
        "balance": {
            "mpCost": 383,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_chant_of_gate",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 78,
        "classes": [
            "doomcryer"
        ]
    },
    "freezing_flame": {
        "id": "freezing_flame",
        "name": "冰凍火焰",
        "slug": "freezing_flame",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill11598.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "A wave of freezing flame strikes the enemy.Deals damage for 10 sec.",
        "canonicalCooldown": "5 sec.",
        "canonicalCooldownMs": 5000,
        "desc": "冰凍火焰之波襲擊敵人，持續造成傷害 10 秒。",
        "balance": {
            "mpCost": 67,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_freezing_flame",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "doomcryer",
            "warcryer"
        ]
    },
    "team_building": {
        "id": "team_building",
        "name": "團隊合作",
        "slug": "team_building",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/party_unite.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Team Building effect",
        "canonicalCooldown": "5 min.",
        "canonicalCooldownMs": 300000,
        "desc": "團隊合作。",
        "balance": {
            "mpCost": 550,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_team_building",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "doomcryer"
        ]
    },
    "hurricane_assault": {
        "id": "hurricane_assault",
        "name": "颶風突擊",
        "slug": "hurricane_assault",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0284.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks the target. Power 728.Ignores Shield Defense.Ignores 10% of the target's",
        "canonicalCooldown": "1 sec.",
        "canonicalCooldownMs": 1000,
        "desc": "攻擊目標，威力 728。無視盾牌防禦與目標 10% 防禦力。",
        "balance": {
            "mpCost": 30,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_hurricane_assault",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "grandKhavatari",
            "monk",
            "tyrant"
        ]
    },
    "iron_fist": {
        "id": "iron_fist",
        "name": "鐵拳",
        "slug": "iron_fist",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0281.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Delivers a powerful blow to an enemy.Attacks the target. Power 586.With a certai",
        "canonicalCooldown": "20 sec.",
        "canonicalCooldownMs": 20000,
        "desc": "對敵人施展強力一擊，攻擊目標，威力 586。有一定機率使目標暈眩 3 秒。無視盾牌防禦。",
        "balance": {
            "mpCost": 31,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_iron_fist",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "grandKhavatari",
            "tyrant"
        ]
    },
    "force_burst": {
        "id": "force_burst",
        "name": "氣勁爆發",
        "slug": "force_burst",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0017.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks the target and nearby enemies. Power 129.Ignores Shield Defense.Ignores ",
        "canonicalCooldown": "3 sec.",
        "canonicalCooldownMs": 3000,
        "desc": "攻擊目標與附近敵人，威力 129。無視盾牌防禦與目標 10% 防禦力。玩家對戰／對怪物戰鬥最多攻擊 5 個目標。",
        "balance": {
            "mpCost": 49,
            "pwr": 129,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_force_burst",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 41,
        "classes": [
            "grandKhavatari",
            "tyrant"
        ]
    },
    "punch_of_doom": {
        "id": "punch_of_doom",
        "name": "末日之拳",
        "slug": "punch_of_doom",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0081.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Strikes at the target with the iron fist. Power 4580.Ignores Shield Defense.",
        "canonicalCooldown": "1 min.",
        "canonicalCooldownMs": 60000,
        "desc": "以鐵拳重擊目標，威力 4580。無視盾牌防禦。",
        "balance": {
            "mpCost": 44,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_punch_of_doom",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 55,
        "classes": [
            "tyrant"
        ]
    },
    "burning_fist": {
        "id": "burning_fist",
        "name": "燃燒之拳",
        "slug": "burning_fist",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0280.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Strikes at the enemy with the flaming fist.Attacks the target. Power 694.Ignores",
        "canonicalCooldown": "1 sec.",
        "canonicalCooldownMs": 1000,
        "desc": "以燃燒的拳頭攻擊敵人，威力 694。無視盾牌防禦與目標 5% 防禦力。",
        "balance": {
            "mpCost": 30,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_burning_fist",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "grandKhavatari",
            "tyrant"
        ]
    },
    "force_storm": {
        "id": "force_storm",
        "name": "氣勁風暴",
        "slug": "force_storm",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0035.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks the target and nearby enemies. Power 181.Ignores Shield Defense.Ignores ",
        "canonicalCooldown": "3 sec.",
        "canonicalCooldownMs": 3000,
        "desc": "攻擊目標與附近敵人，威力 181。無視盾牌防禦與目標 10% 防禦力。玩家對戰／對怪物戰鬥最多攻擊 5 個目標。",
        "balance": {
            "mpCost": 57,
            "pwr": 181,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_force_storm",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 47,
        "classes": [
            "grandKhavatari",
            "tyrant"
        ]
    },
    "force_blaster": {
        "id": "force_blaster",
        "name": "氣勁砲",
        "slug": "force_blaster",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0054.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks the target. Power 431.Ignores Shield Defense.Ignores 10% of the target's",
        "canonicalCooldown": "4 sec.",
        "canonicalCooldownMs": 4000,
        "desc": "攻擊目標，威力 431。無視盾牌防禦與目標 10% 防禦力。",
        "balance": {
            "mpCost": 31,
            "pwr": 431,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_force_blaster",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "grandKhavatari",
            "monk",
            "tyrant"
        ]
    },
    "break_duress": {
        "id": "break_duress",
        "name": "解除壓制",
        "slug": "break_duress",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0461.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Removes Paralysis/ Shock/ Sleep.",
        "canonicalCooldown": "10 min.",
        "canonicalCooldownMs": 600000,
        "desc": "移除麻痺／休克／睡眠狀態。",
        "balance": {
            "mpCost": 29,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_break_duress",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 52,
        "classes": [
            "tyrant"
        ]
    },
    "bison_spirit_totem": {
        "id": "bison_spirit_totem",
        "name": "野牛之魂圖騰",
        "slug": "bison_spirit_totem",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill11834.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Bison Spirit Totem effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "野牛之魂圖騰。",
        "balance": {
            "mpCost": 32,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_bison_spirit_totem",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 68,
        "classes": [
            "tyrant"
        ]
    },
    "ogre_s_essence": {
        "id": "ogre_s_essence",
        "name": "食人魔精華",
        "slug": "ogre_s_essence",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/totem_spirit_ogre.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Ogre's Essence effect",
        "canonicalCooldown": "4 min.",
        "canonicalCooldownMs": 240000,
        "desc": "食人魔精華。",
        "balance": {
            "mpCost": 50,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_ogre_s_essence",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 45,
        "classes": [
            "grandKhavatari",
            "tyrant"
        ]
    },
    "cripple": {
        "id": "cripple",
        "name": "衰弱",
        "slug": "cripple",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill19156.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Slows the enemy down.Atk. Spd. -15%Casting Spd. -15%Speed -15%",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "使敵人減速：攻速 -15%、施法速度 -15%、速度 -15%。",
        "balance": {
            "mpCost": 30,
            "pwr": 2,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_cripple",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "grandKhavatari",
            "monk",
            "tyrant"
        ]
    },
    "chant_of_vampire": {
        "id": "chant_of_vampire",
        "name": "吸血鬼頌歌",
        "slug": "chant_of_vampire",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1007.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Speed +2Debuff/ Anomaly Resistance +10%With a 80% chance, absorbs 7% of damage a",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "速度 +2、弱化／異常抗性 +10%。有 80% 機率將造成傷害的 7% 吸收為 生命值。",
        "balance": {
            "mpCost": 40,
            "pwr": 1,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_chant_of_vampire",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "warcryer"
        ]
    },
    "swap_defense": {
        "id": "swap_defense",
        "name": "防禦轉換",
        "slug": "swap_defense",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill19560.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "M. Def. -60%P. Def. +10%Note!Cannot be used with Convert.",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "魔法防禦 -60%、物理防禦 +10%。注意：無法與 轉換 同時使用。",
        "balance": {
            "mpCost": 54,
            "pwr": 6,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_swap_defense",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "overlord",
            "warcryer"
        ]
    },
    "convert": {
        "id": "convert",
        "name": "轉換",
        "slug": "convert",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0458.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Max MP -50%Max HP +10%Note!Cannot be used with Swap Defense.",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "最大魔力 -50%、最大生命值 +10%。注意：無法與 防禦轉換 同時使用。",
        "balance": {
            "mpCost": 54,
            "pwr": 5,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_convert",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "overlord",
            "warcryer"
        ]
    },
    "swap_attack": {
        "id": "swap_attack",
        "name": "攻擊轉換",
        "slug": "swap_attack",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0982.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "M. Atk. -70%P. Atk. +100%",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "魔法攻擊 -70%、物理攻擊 +100%。",
        "balance": {
            "mpCost": 65,
            "pwr": 7,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_swap_attack",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "overlord",
            "warcryer"
        ]
    },
    "flame_burst": {
        "id": "flame_burst",
        "name": "火焰爆發",
        "slug": "flame_burst",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/flame_blast.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Deals M. damage to the target and nearby enemies. Power 234.With a 30% chance, t",
        "canonicalCooldown": "15 sec.",
        "canonicalCooldownMs": 15000,
        "desc": "對目標與附近敵人造成魔法傷害，威力 234。有 30% 機率觸發追加攻擊。10 秒內降低 生命值／魔力；生命值 降低量依自身魔法攻擊 計算。對單一目標使用時效果會強化（僅一次）。玩家對戰／對怪物戰鬥最多攻擊 6 個目標。",
        "balance": {
            "mpCost": 97,
            "pwr": 234,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_flame_burst",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "dominator"
        ]
    },
    "debuff_master": {
        "id": "debuff_master",
        "name": "弱化大師",
        "slug": "debuff_master",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/debuff_master.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Debuff Master effect",
        "canonicalCooldown": ".Terms",
        "canonicalCooldownMs": 1000,
        "desc": "弱化大師。",
        "balance": {
            "mpCost": 200,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_debuff_master",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "dominator"
        ]
    },
    "prophecy_of_pa_agrio": {
        "id": "prophecy_of_pa_agrio",
        "name": "帕格立歐預言",
        "slug": "prophecy_of_pa_agrio",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1414.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Max CP +20%Speed +12Casting Spd. +10%Casting Spd. +50M. Skill Critical Rate +15%",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "最大戰鬥點數 +20%、速度 +12、施法速度 +10%、施法速度 +50、魔法技能暴擊率 +15%、魔法技能暴擊傷害 +15%、戰鬥點數 恢復率 +20%、對怪物戰鬥傷害 +23%。",
        "balance": {
            "mpCost": 172,
            "pwr": 2,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_prophecy_of_pa_agrio",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "dominator"
        ]
    },
    "pa_agrio_s_glory": {
        "id": "pa_agrio_s_glory",
        "name": "帕格立歐榮耀",
        "slug": "pa_agrio_s_glory",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/skill1004_new.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Pa'agrio's Glory effect",
        "canonicalCooldown": "-15%M.",
        "canonicalCooldownMs": 1000,
        "desc": "帕格立歐榮耀。",
        "balance": {
            "mpCost": 100,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_pa_agrio_s_glory",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 85,
        "classes": [
            "dominator",
            "overlord"
        ]
    },
    "seal_of_suspension": {
        "id": "seal_of_suspension",
        "name": "停滯封印",
        "slug": "seal_of_suspension",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1248.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "P./ M. Skill Cooldown +200%",
        "canonicalCooldown": "+200%Terms",
        "canonicalCooldownMs": 1000,
        "desc": "物理／魔法技能冷卻時間 +200%。",
        "balance": {
            "mpCost": 113,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_seal_of_suspension",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "dominator",
            "overlord"
        ]
    },
    "seal_of_silence": {
        "id": "seal_of_silence",
        "name": "沉默封印",
        "slug": "seal_of_silence",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1246.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Blocks target's M. skills.",
        "canonicalCooldown": "1 min.",
        "canonicalCooldownMs": 60000,
        "desc": "封鎖目標的魔法技能。",
        "balance": {
            "mpCost": 113,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_seal_of_silence",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "dominator",
            "overlord"
        ]
    },
    "pa_agrio_s_honor": {
        "id": "pa_agrio_s_honor",
        "name": "帕格立歐榮譽",
        "slug": "pa_agrio_s_honor",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1305.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Recovers nearby allies' CP. Power 690.Number of targets: up to 5 (caster include",
        "canonicalCooldown": "5 sec.",
        "canonicalCooldownMs": 5000,
        "desc": "恢復附近盟友的戰鬥點數，威力 690。最多影響 5 個目標（包含施法者）。",
        "balance": {
            "mpCost": 263,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_pa_agrio_s_honor",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "dominator",
            "overlord"
        ]
    },
    "seal_of_blockade": {
        "id": "seal_of_blockade",
        "name": "封鎖封印",
        "slug": "seal_of_blockade",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1462.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Blocks basic attacks to the target and nearby enemies.",
        "canonicalCooldown": "2 min.",
        "canonicalCooldownMs": 120000,
        "desc": "封鎖目標與附近敵人的基礎攻擊。",
        "balance": {
            "mpCost": 100,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_seal_of_blockade",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 79,
        "classes": [
            "dominator"
        ]
    },
    "pa_agrio_s_touch": {
        "id": "pa_agrio_s_touch",
        "name": "帕格立歐之觸",
        "slug": "pa_agrio_s_touch",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/touch_of_paagrio1.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Pa'agrio's Touch effect",
        "canonicalCooldown": "5 min.",
        "canonicalCooldownMs": 300000,
        "desc": "帕格立歐之觸。",
        "balance": {
            "mpCost": 550,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_pa_agrio_s_touch",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "dominator"
        ]
    },
    "seal_of_despair": {
        "id": "seal_of_despair",
        "name": "絕望封印",
        "slug": "seal_of_despair",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1376.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "P. Atk. -10%Atk. Spd. -30%Basic Critical Rate -30%Basic Critical Damage -30%M. D",
        "canonicalCooldown": "30 sec.",
        "canonicalCooldownMs": 30000,
        "desc": "物理攻擊 -10%、攻速 -30%、基礎暴擊率 -30%、基礎暴擊傷害 -30%、魔法防禦 -30%、物理命中 -6、速度 -20%。",
        "balance": {
            "mpCost": 107,
            "pwr": 1,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_seal_of_despair",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 79,
        "classes": [
            "dominator"
        ]
    },
    "seal_of_infection": {
        "id": "seal_of_infection",
        "name": "感染封印",
        "slug": "seal_of_infection",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill0776.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Seal of Infection effect",
        "canonicalCooldown": "15 sec.",
        "canonicalCooldownMs": 15000,
        "desc": "感染封印。",
        "balance": {
            "mpCost": 125,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_seal_of_infection",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 77,
        "classes": [
            "dominator"
        ]
    },
    "seal_of_disease": {
        "id": "seal_of_disease",
        "name": "疾病封印",
        "slug": "seal_of_disease",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1367.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Seal of Disease effect",
        "canonicalCooldown": "15 sec.",
        "canonicalCooldownMs": 15000,
        "desc": "疾病封印。",
        "balance": {
            "mpCost": 105,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_seal_of_disease",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 77,
        "classes": [
            "dominator"
        ]
    },
    "ritual_of_life": {
        "id": "ritual_of_life",
        "name": "生命儀式",
        "slug": "ritual_of_life",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill11775.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Restores target's CP by 863.",
        "canonicalCooldown": "5 sec.",
        "canonicalCooldownMs": 5000,
        "desc": "恢復目標 863戰鬥點數。",
        "balance": {
            "mpCost": 132,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_ritual_of_life",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "dominator",
            "overlord"
        ]
    },
    "pa_agrio_s_cure": {
        "id": "pa_agrio_s_cure",
        "name": "帕格立歐治癒",
        "slug": "pa_agrio_s_cure",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/remedy_of_paagrio.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Pa'agrio's Cure effect",
        "canonicalCooldown": "3.5 sec.",
        "canonicalCooldownMs": 3500,
        "desc": "帕格立歐治癒。",
        "balance": {
            "mpCost": 282,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_pa_agrio_s_cure",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "dominator"
        ]
    },
    "jump_attack": {
        "id": "jump_attack",
        "name": "跳躍攻擊",
        "slug": "jump_attack",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/jump_dash.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "<Rush/ Knockdown skill>Deals P. damage to the target and nearby enemies. Power 6",
        "canonicalCooldown": "5 sec.",
        "canonicalCooldownMs": 5000,
        "desc": "<突進／擊倒技能>對目標與附近敵人造成物理傷害，威力 6800。有一定機率擊倒 2 秒。無視盾牌防禦與目標 10% 防禦力。玩家對戰／對怪物戰鬥最多攻擊 5 個目標。使用技能時，5 秒內增加 急速撤退 的施放距離與後退距離。",
        "balance": {
            "mpCost": 60,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_jump_attack",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "grandKhavatari"
        ]
    },
    "burning_strike": {
        "id": "burning_strike",
        "name": "燃燒打擊",
        "slug": "burning_strike",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_new_burning_strike.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "<Knockdown skill>Attacks the target twice. Power 6534.With a 30% chance, attacks",
        "canonicalCooldown": "3 sec.",
        "canonicalCooldownMs": 3000,
        "desc": "<擊倒技能>連續攻擊目標 2 次，威力 6534。另有 30% 機率再次連續攻擊 2 次。有一定機率擊倒目標。無視盾牌防禦與目標 15% 防禦力。",
        "balance": {
            "mpCost": 64,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_burning_strike",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "grandKhavatari"
        ]
    },
    "burning_assault": {
        "id": "burning_assault",
        "name": "燃燒突擊",
        "slug": "burning_assault",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/burning_assault.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks the target and nearby enemies. Power 6243.For 5 sec., the target's P. De",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "攻擊目標與附近敵人，威力 6243。5 秒內目標物理防禦 -23%、擊退／擊倒抗性 -10%。無視盾牌防禦與目標 10% 防禦力。玩家對戰／對怪物戰鬥最多攻擊 5 個目標。",
        "balance": {
            "mpCost": 64,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_burning_assault",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "grandKhavatari"
        ]
    },
    "raging_force": {
        "id": "raging_force",
        "name": "狂暴氣勁",
        "slug": "raging_force",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill0346.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks the target. Power 195.For 20 sec., P. Def. -30%.",
        "canonicalCooldown": "1s",
        "canonicalCooldownMs": 1000,
        "desc": "攻擊目標，威力 195。20 秒內物理防禦 -30%。",
        "balance": {
            "mpCost": 60,
            "pwr": 195,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_raging_force",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "grandKhavatari"
        ]
    },
    "blazing_strike": {
        "id": "blazing_strike",
        "name": "烈焰打擊",
        "slug": "blazing_strike",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/burning_blow.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Makes an empowered attack after a strike with a fire-imbued fist.Critical Rate i",
        "canonicalCooldown": "30 sec.",
        "canonicalCooldownMs": 30000,
        "desc": "以灌注火焰的拳擊後接強化攻擊，暴擊率極高。攻擊目標，威力 7182，無視盾牌防禦與目標 10% 防禦力；再追加一次攻擊，威力 9690，無視盾牌防禦與目標 25% 防禦力。",
        "balance": {
            "mpCost": 62,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_blazing_strike",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "grandKhavatari"
        ]
    },
    "wondrous_power": {
        "id": "wondrous_power",
        "name": "奇蹟之力",
        "slug": "wondrous_power",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill0443.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Wondrous Power effect",
        "canonicalCooldown": "10 min.",
        "canonicalCooldownMs": 600000,
        "desc": "奇蹟之力。",
        "balance": {
            "mpCost": 50,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_wondrous_power",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 77,
        "classes": [
            "grandKhavatari"
        ]
    },
    "rushing_retreat": {
        "id": "rushing_retreat",
        "name": "急速撤退",
        "slug": "rushing_retreat",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/back_dash.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Rushing Retreat effect",
        "canonicalCooldown": "30 sec.",
        "canonicalCooldownMs": 30000,
        "desc": "急速撤退。",
        "balance": {
            "mpCost": 65,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_rushing_retreat",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "grandKhavatari"
        ]
    },
    "final_secret_grand_khavatari": {
        "id": "final_secret_grand_khavatari",
        "name": "最終秘技：大卡瓦塔里",
        "slug": "final_secret_grand_khavatari",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/final_secret_khavatari.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Final Secret: Grand Khavatari effect",
        "canonicalCooldown": "2 min.",
        "canonicalCooldownMs": 120000,
        "desc": "最終秘技：大卡瓦塔里。",
        "balance": {
            "mpCost": 41,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_final_secret_grand_khavatari",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "grandKhavatari"
        ]
    },
    "cacophony_of_war": {
        "id": "cacophony_of_war",
        "name": "戰爭狂響",
        "slug": "cacophony_of_war",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/war_roar.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "<Base effect>Max HP +20%Momentarily recovers bonus HP.Atk. Spd. +30%Skill Power ",
        "canonicalCooldown": "5 min.",
        "canonicalCooldownMs": 300000,
        "desc": "<基礎效果>最大生命值 +20%（立即恢復增加的 生命值）、攻速 +30%、技能威力 +5%、對怪物戰鬥傷害 +5%、速度 +30、物理技能魔力消耗 -50%、增益解除抗性 +40%、擊退／擊倒成功率 +20%，持續 1 分鐘。<玩家對戰效果><恐懼技能>攻擊附近敵人並施加 焦慮，持續 3 秒，威力 12000；無視盾牌防禦與目標 10% 防禦力。有 40% 機率使目前生命值 -30%、目前魔力 -30%。<對怪物戰鬥效果>連續 2 次攻擊附近敵人，威力 12000；無視盾牌防禦與目標 10% 防禦力。玩家對戰 最多攻擊 5 個目標，對怪物戰鬥 最多攻擊 12 個目標。",
        "balance": {
            "mpCost": 300,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_cacophony_of_war",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "doomcryer",
            "grandKhavatari"
        ]
    },
    "pa_agrio_s_immunity": {
        "id": "pa_agrio_s_immunity",
        "name": "帕格立歐免疫",
        "slug": "pa_agrio_s_immunity",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1260.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "M. Atk. +100P. Def. +300M. Skill Critical Rate +1%Received M. Skill Critical Dam",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "魔法攻擊 +100、物理防禦 +300、魔法技能暴擊率 +1%、受到魔法技能暴擊傷害 -10%、弱化／異常抗性 +20%。",
        "balance": {
            "mpCost": 40,
            "pwr": 0,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_pa_agrio_s_immunity",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "overlord"
        ]
    },
    "seal_of_mirage": {
        "id": "seal_of_mirage",
        "name": "幻象封印",
        "slug": "seal_of_mirage",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill1213.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Switches the enemy's target to a random one from all nearby targets.",
        "canonicalCooldown": "5 sec.",
        "canonicalCooldownMs": 5000,
        "desc": "將敵人的目標隨機切換為附近其他目標。",
        "balance": {
            "mpCost": 59,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_seal_of_mirage",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 44,
        "classes": [
            "overlord"
        ]
    },
    "spoil": {
        "id": "spoil",
        "name": "搜刮",
        "slug": "spoil",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/skill0254.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Spoil effect",
        "canonicalCooldown": "3 sec.",
        "canonicalCooldownMs": 3000,
        "desc": "搜刮。",
        "balance": {
            "mpCost": 10,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_spoil",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 5,
        "classes": [
            "bountyHunter",
            "dwarfFighter",
            "fortuneSeeker",
            "scavenger"
        ]
    },
    "sweeper": {
        "id": "sweeper",
        "name": "掃蕩",
        "slug": "sweeper",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/skill0042.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Sweeper effect",
        "canonicalCooldown": "0.5 sec.",
        "canonicalCooldownMs": 500,
        "desc": "掃蕩。",
        "balance": {
            "mpCost": 3,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_sweeper",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 5,
        "classes": [
            "dwarfFighter"
        ]
    },
    "earthquake": {
        "id": "earthquake",
        "name": "地震",
        "slug": "earthquake",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill0347.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Swings the spear to attack nearby targets. Power 3878.Ignores Shield Defense.PvP",
        "canonicalCooldown": "8 sec.",
        "canonicalCooldownMs": 8000,
        "desc": "揮動長槍攻擊附近目標，威力 3878。無視盾牌防禦。玩家對戰／對怪物戰鬥最多攻擊 5 個目標。",
        "balance": {
            "mpCost": 90,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_earthquake",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "fortuneSeeker",
            "maestro",
            "shinemaker"
        ]
    },
    "golden_stone": {
        "id": "golden_stone",
        "name": "黃金石化",
        "slug": "golden_stone",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/gold_stone.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "For 5 sec., inflicts Imprisonment on the target and nearby enemies turning them ",
        "canonicalCooldown": "20 sec.",
        "canonicalCooldownMs": 20000,
        "desc": "使目標與附近敵人陷入禁錮 5 秒並化為黃金雕像。被禁錮的目標無法移動、攻擊、傳送、使用技能或恢復生命值／魔力／戰鬥點數。禁錮結束時攻擊敵人，威力 3256；並使物理防禦／魔法防禦／速度 -10%，持續 5 秒。玩家對戰／對怪物戰鬥最多攻擊 9 個目標。",
        "balance": {
            "mpCost": 60,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_golden_stone",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "fortuneSeeker"
        ]
    },
    "maphr_s_acumen": {
        "id": "maphr_s_acumen",
        "name": "瑪芙爾的靈活思緒",
        "slug": "maphr_s_acumen",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill11184.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Maphr’s Acumen effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "瑪芙爾的靈活思緒。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_maphr_s_acumen",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "artisanDwarf",
            "bountyHunter",
            "fortuneSeeker",
            "maestro",
            "scavenger",
            "shineMakerS1",
            "shineMakerS2",
            "shinemaker",
            "warsmith"
        ]
    },
    "adena_stun": {
        "id": "adena_stun",
        "name": "亞丁幣暈眩",
        "slug": "adena_stun",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/coin_stun.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Throws some adena at the target.Attacks the target. Power 5018. Stuns for 3 sec.",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "向目標投擲亞丁幣並攻擊目標，威力 5018，使其暈眩 3 秒。有一定機率移除目標最多 3 個增益效果。無視盾牌防禦。不會暴擊。",
        "balance": {
            "mpCost": 54,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_adena_stun",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "fortuneSeeker"
        ]
    },
    "maphr_s_magic_barrier": {
        "id": "maphr_s_magic_barrier",
        "name": "瑪芙爾的魔法屏障",
        "slug": "maphr_s_magic_barrier",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill19121.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Maphr's Magic Barrier effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "瑪芙爾的魔法屏障。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_maphr_s_magic_barrier",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "artisanDwarf",
            "bountyHunter",
            "fortuneSeeker",
            "maestro",
            "scavenger",
            "shineMakerS1",
            "shineMakerS2",
            "shinemaker",
            "warsmith"
        ]
    },
    "maphr_s_haste": {
        "id": "maphr_s_haste",
        "name": "瑪芙爾的加速",
        "slug": "maphr_s_haste",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill0820.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Maphr's Haste effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "瑪芙爾的加速。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_maphr_s_haste",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "artisanDwarf",
            "bountyHunter",
            "fortuneSeeker",
            "maestro",
            "scavenger",
            "shineMakerS1",
            "shineMakerS2",
            "shinemaker",
            "warsmith"
        ]
    },
    "trophy_thief": {
        "id": "trophy_thief",
        "name": "戰利品竊賊",
        "slug": "trophy_thief",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_spoil_destroy.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks the target. Power 6373.Ignores Shield Defense.Ignores 20% of the target'",
        "canonicalCooldown": "8 sec.",
        "canonicalCooldownMs": 8000,
        "desc": "攻擊目標，威力 6373。無視盾牌防禦與目標 20% 防禦力。有一定機率從怪物身上竊取物品。",
        "balance": {
            "mpCost": 72,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_trophy_thief",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "fortuneSeeker"
        ]
    },
    "maphr_s_wild_magic": {
        "id": "maphr_s_wild_magic",
        "name": "瑪芙爾的野性魔力",
        "slug": "maphr_s_wild_magic",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill19160.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Maphr's Wild Magic effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "瑪芙爾的野性魔力。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_maphr_s_wild_magic",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "artisanDwarf",
            "bountyHunter",
            "fortuneSeeker",
            "maestro",
            "scavenger",
            "shineMakerS1",
            "shineMakerS2",
            "shinemaker",
            "warsmith"
        ]
    },
    "spoil_crush": {
        "id": "spoil_crush",
        "name": "搜刮重擊",
        "slug": "spoil_crush",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill0348.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks the target. Power 4907.With a certain chance, steals items from monsters",
        "canonicalCooldown": "1 sec.",
        "canonicalCooldownMs": 1000,
        "desc": "攻擊目標，威力 4907。有一定機率從怪物身上竊取物品。無視盾牌防禦與目標 10% 防禦力。",
        "balance": {
            "mpCost": 60,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_spoil_crush",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "fortuneSeeker"
        ]
    },
    "crushing_leap": {
        "id": "crushing_leap",
        "name": "粉碎跳躍",
        "slug": "crushing_leap",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/little_big_attack.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks the target. Power 5545.When using the skill, increases its power with a ",
        "canonicalCooldown": "1 sec.",
        "canonicalCooldownMs": 1000,
        "desc": "攻擊目標，威力 5545。使用技能時有一定機率提高技能威力。無視盾牌防禦與目標 10% 防禦力。",
        "balance": {
            "mpCost": 65,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_crushing_leap",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "fortuneSeeker"
        ]
    },
    "maphr_s_wind_walk": {
        "id": "maphr_s_wind_walk",
        "name": "瑪芙爾的風之疾走",
        "slug": "maphr_s_wind_walk",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill10504.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Maphr's Wind Walk effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "瑪芙爾的風之疾走。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_maphr_s_wind_walk",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "artisanDwarf",
            "fortuneSeeker",
            "maestro",
            "scavenger",
            "shineMakerS1",
            "shinemaker"
        ]
    },
    "body_crush": {
        "id": "body_crush",
        "name": "身體粉碎",
        "slug": "body_crush",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1613.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks the target. Power 4129.With a certain chance, stuns for 3 sec.With a cer",
        "canonicalCooldown": "20 sec.",
        "canonicalCooldownMs": 20000,
        "desc": "攻擊目標，威力 4129。有一定機率使目標暈眩 3 秒，並有一定機率移除目標最多 2 個增益效果。無視盾牌防禦。不會暴擊。",
        "balance": {
            "mpCost": 61,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_body_crush",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 78,
        "classes": [
            "bountyHunter",
            "fortuneSeeker",
            "maestro",
            "shineMakerS2",
            "shinemaker",
            "warsmith"
        ]
    },
    "maphr_s_berserker_spirit": {
        "id": "maphr_s_berserker_spirit",
        "name": "瑪芙爾的狂戰士之魂",
        "slug": "maphr_s_berserker_spirit",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill19192.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Maphr's Berserker Spirit effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "瑪芙爾的狂戰士之魂。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_maphr_s_berserker_spirit",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "artisanDwarf",
            "bountyHunter",
            "fortuneSeeker",
            "maestro",
            "scavenger",
            "shineMakerS1",
            "shineMakerS2",
            "shinemaker",
            "warsmith"
        ]
    },
    "rolling_dice": {
        "id": "rolling_dice",
        "name": "擲骰子",
        "slug": "rolling_dice",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/rolling_dice.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Rolling Dice effect",
        "canonicalCooldown": "7 sec.",
        "canonicalCooldownMs": 7000,
        "desc": "擲骰子。",
        "balance": {
            "mpCost": 150,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_rolling_dice",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "fortuneSeeker"
        ]
    },
    "spoil_festival": {
        "id": "spoil_festival",
        "name": "搜刮祭典",
        "slug": "spoil_festival",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill0302.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Curses the target and nearby enemies.",
        "canonicalCooldown": "3 sec.",
        "canonicalCooldownMs": 3000,
        "desc": "詛咒目標與附近敵人。",
        "balance": {
            "mpCost": 89,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_spoil_festival",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "bountyHunter",
            "fortuneSeeker",
            "scavenger"
        ]
    },
    "weapon_reinforcement": {
        "id": "weapon_reinforcement",
        "name": "武器強化",
        "slug": "weapon_reinforcement",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1615.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "When using a sword, a blunt weapon, or a spear:P. Atk. +12%With a certain chance",
        "canonicalCooldown": ":",
        "canonicalCooldownMs": 1000,
        "desc": "使用劍、鈍器或長槍時：物理攻擊 +12%。基礎攻擊時有一定機率附加 物理傷害 弱化效果：攻速／施法速度 -10%，持續 15 秒；觸發冷卻 15 秒。",
        "balance": {
            "mpCost": 40,
            "pwr": 1,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_weapon_reinforcement",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 77,
        "classes": [
            "bountyHunter",
            "fortuneSeeker",
            "maestro",
            "shineMakerS2",
            "shinemaker",
            "warsmith"
        ]
    },
    "tenacity": {
        "id": "tenacity",
        "name": "堅毅",
        "slug": "tenacity",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1617.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Tenacity effect",
        "canonicalCooldown": ":",
        "canonicalCooldownMs": 1000,
        "desc": "堅毅。",
        "balance": {
            "mpCost": 72,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_tenacity",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 80,
        "classes": [
            "bountyHunter",
            "fortuneSeeker",
            "maestro",
            "shineMakerS2",
            "shinemaker",
            "warsmith"
        ]
    },
    "mount_kukuru": {
        "id": "mount_kukuru",
        "name": "騎乘庫庫魯",
        "slug": "mount_kukuru",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1836.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Mount Kukuru effect",
        "canonicalCooldown": "1s",
        "canonicalCooldownMs": 1000,
        "desc": "騎乘庫庫魯。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_mount_kukuru",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [],
        "availableTo": [],
        "disabled": true,
        "removalReason": "cosmetic_mount_purge"
    },
    "fortune_time": {
        "id": "fortune_time",
        "name": "幸運時刻",
        "slug": "fortune_time",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/fortune_time.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Max HP +20%Max HP +2500P. Skill Power +5%With a certain chance when attacking a ",
        "canonicalCooldown": ":",
        "canonicalCooldownMs": 1000,
        "desc": "最大生命值 +20%、最大生命值 +2500、物理技能威力 +5%。攻擊團體首領與部分怪物時有一定機率附加 尋寶者印記，持續 2 分鐘（觸發冷卻 10 秒）。標記生效時，擊殺團體首領或部分怪物可掉落額外獎勵。<額外獎勵與掉落率>幸運時刻箱－首領團體：1%：稀有飾品強化卷軸、改良稀有飾品強化卷軸、超越升級石。幸運時刻箱－指揮頻道團體：0.8%：改良／詛咒 A 級武器／防具強化卷軸、祝福卷軸－武器／防具、充能石：技能強化－3 星（10%）、超越升級石。幸運時刻箱－特殊怪物：1%：火／水／地／風精靈進化石、A 級武器／防具強化卷軸、改良／詛咒 A 級武器／防具強化卷軸、祝福卷軸－武器／防具。幸運時刻箱－守護團體：0.8%：守護墜飾 等級 1、守護之書兌換券－1～3 星、守護戒指／耳環、守護飾品強化卷軸。<注意>額外獎勵只能從特定首領與怪物取得；對已帶有 尋寶者印記 的首領或怪物再次施放，不會刷新標記持續時間。",
        "balance": {
            "mpCost": 102,
            "pwr": 2,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_fortune_time",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "fortuneSeeker"
        ]
    },
    "blazing_fury": {
        "id": "blazing_fury",
        "name": "烈焰狂怒",
        "slug": "blazing_fury",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/skill1434.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Blazing Fury effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "烈焰狂怒。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_blazing_fury",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 1,
        "classes": [
            "grandVanguard",
            "rider"
        ]
    },
    "blazing_beast": {
        "id": "blazing_beast",
        "name": "烈焰野獸",
        "slug": "blazing_beast",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/s_burning_beast.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Blazing Beast effect",
        "canonicalCooldown": "for",
        "canonicalCooldownMs": 1000,
        "desc": "烈焰野獸。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_blazing_beast",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 1,
        "classes": [
            "rider"
        ]
    },
    "summon_mechanic_golem": {
        "id": "summon_mechanic_golem",
        "name": "召喚機械魔像",
        "slug": "summon_mechanic_golem",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/skill0025.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Summons a Mechanical Golem.",
        "canonicalCooldown": "5 sec.",
        "canonicalCooldownMs": 5000,
        "desc": "召喚機械魔像。",
        "balance": {
            "mpCost": 49,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_summon_mechanic_golem",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 28,
        "classes": [
            "artisanDwarf",
            "maestro",
            "shineMakerS1",
            "shineMakerS2",
            "shinemaker",
            "warsmith"
        ]
    },
    "wild_rush": {
        "id": "wild_rush",
        "name": "野性突進",
        "slug": "wild_rush",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/s_wild_rush.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Wild Rush effect",
        "canonicalCooldown": ".Terms",
        "canonicalCooldownMs": 1000,
        "desc": "野性突進。",
        "balance": {
            "mpCost": 30,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_wild_rush",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 20,
        "classes": [
            "dragoon",
            "grandVanguard",
            "rider",
            "vanguardRider"
        ]
    },
    "piercing": {
        "id": "piercing",
        "name": "穿刺",
        "slug": "piercing",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/s_impale.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "<Spear skill>Attacks the target with a spear.Deals P. damage. Power 419.Ignores ",
        "canonicalCooldown": "1 sec.",
        "canonicalCooldownMs": 1000,
        "desc": "<長槍技能>使用長槍攻擊目標，造成物理傷害，威力 419。無視盾牌防禦。",
        "balance": {
            "mpCost": 18,
            "pwr": 419,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_piercing",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 20,
        "classes": [
            "dragoon",
            "vanguardRider"
        ]
    },
    "wild_scratch": {
        "id": "wild_scratch",
        "name": "野性抓擊",
        "slug": "wild_scratch",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_wild_scratch.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "<Beast skill>The beast attacks the target with its claws.Deals damage based on y",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "<野獸技能>野獸以利爪攻擊目標，傷害依自身物理攻擊 計算，威力 8087。無視盾牌防禦與目標 10% 防禦力。",
        "balance": {
            "mpCost": 61,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_wild_scratch",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 78,
        "classes": [
            "grandVanguard",
            "vanguardRider"
        ]
    },
    "amazing_piercing": {
        "id": "amazing_piercing",
        "name": "驚人穿刺",
        "slug": "amazing_piercing",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_arc_impale.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "<Spear skill>Attacks the target with a spear.Deals P. damage. Power 7276.Ignores",
        "canonicalCooldown": "1 sec.",
        "canonicalCooldownMs": 1000,
        "desc": "<長槍技能>使用長槍攻擊目標，造成物理傷害，威力 7276。無視盾牌防禦與目標 15% 防禦力。",
        "balance": {
            "mpCost": 60,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_amazing_piercing",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "grandVanguard"
        ]
    },
    "sweeper_festival": {
        "id": "sweeper_festival",
        "name": "掃蕩祭典",
        "slug": "sweeper_festival",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/br_skill0444_a.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Sweeper Festival effect",
        "canonicalCooldown": "3 sec.",
        "canonicalCooldownMs": 3000,
        "desc": "掃蕩祭典。",
        "balance": {
            "mpCost": 12,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_sweeper_festival",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 28,
        "classes": [
            "scavenger"
        ]
    },
    "wild_charge": {
        "id": "wild_charge",
        "name": "野性衝鋒",
        "slug": "wild_charge",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_wild_charge.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "<Beast skill>The beast's materialized soul strikes at the target.Attacks the tar",
        "canonicalCooldown": "30 sec.",
        "canonicalCooldownMs": 30000,
        "desc": "<野獸技能>野獸實體化的靈魂打擊目標，連續 2 次攻擊目標與附近敵人，傷害依自身物理攻擊 計算，威力 3773。無視盾牌防禦與目標 10% 防禦力。",
        "balance": {
            "mpCost": 88,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_wild_charge",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "grandVanguard"
        ]
    },
    "wide_threatening_swing": {
        "id": "wide_threatening_swing",
        "name": "廣域威嚇揮擊",
        "slug": "wide_threatening_swing",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_brandish_wave.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "<Spear skill>Attacks nearby targets. Power 6198.Ignores Shield Defense.Ignores 1",
        "canonicalCooldown": "3 sec.",
        "canonicalCooldownMs": 3000,
        "desc": "<長槍技能>攻擊附近目標，威力 6198。無視盾牌防禦與目標 10% 防禦力。",
        "balance": {
            "mpCost": 68,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_wide_threatening_swing",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "grandVanguard"
        ]
    },
    "giant_s_stomp": {
        "id": "giant_s_stomp",
        "name": "巨人踐踏",
        "slug": "giant_s_stomp",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/skill33915.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Giant's Stomp effect",
        "canonicalCooldown": "1 min.",
        "canonicalCooldownMs": 60000,
        "desc": "巨人踐踏。",
        "balance": {
            "mpCost": 120,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_giant_s_stomp",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "grandVanguard"
        ]
    },
    "wild_growl": {
        "id": "wild_growl",
        "name": "野性咆哮",
        "slug": "wild_growl",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/s_wild_roar.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "<Suppression skill>With a certain chance, suppresses nearby enemies for 3 sec.Su",
        "canonicalCooldown": "45 sec.",
        "canonicalCooldownMs": 45000,
        "desc": "<壓制技能>有一定機率壓制附近敵人 3 秒。壓制：速度 -70%。",
        "balance": {
            "mpCost": 90,
            "pwr": 7,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_wild_growl",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 78,
        "classes": [
            "grandVanguard"
        ]
    },
    "threatening_swing": {
        "id": "threatening_swing",
        "name": "威嚇揮擊",
        "slug": "threatening_swing",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/s_brandish.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "<Spear skill>Attacks nearby targets. Power 1136.Ignores Shield Defense.",
        "canonicalCooldown": "3 sec.",
        "canonicalCooldownMs": 3000,
        "desc": "<長槍技能>攻擊附近目標，威力 1136。無視盾牌防禦。",
        "balance": {
            "mpCost": 38,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_threatening_swing",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "vanguardRider"
        ]
    },
    "wild_assault": {
        "id": "wild_assault",
        "name": "野性突擊",
        "slug": "wild_assault",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/s_wild_assault.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Wild Assault effect",
        "canonicalCooldown": ".Terms",
        "canonicalCooldownMs": 1000,
        "desc": "野性突擊。",
        "balance": {
            "mpCost": 100,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_wild_assault",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "vanguardRider"
        ]
    },
    "machine_assistance": {
        "id": "machine_assistance",
        "name": "機械支援",
        "slug": "machine_assistance",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0025.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Machine Assistance effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "機械支援。",
        "balance": {
            "mpCost": 50,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_machine_assistance",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 50,
        "classes": [
            "shineMakerS2",
            "warsmith"
        ]
    },
    "golem_reinforcement": {
        "id": "golem_reinforcement",
        "name": "魔像強化",
        "slug": "golem_reinforcement",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0824.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "The effect is applied on servitors.P. Def. +8%Max HP +5%",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "效果套用於召喚獸：物理防禦 +8%、最大生命值 +5%。",
        "balance": {
            "mpCost": 35,
            "pwr": 1,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_golem_reinforcement",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "shineMakerS2",
            "warsmith"
        ]
    },
    "repair_golem": {
        "id": "repair_golem",
        "name": "修復魔像",
        "slug": "repair_golem",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0822.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Repairs the summoned golem to recover the golem's HP by 8%.",
        "canonicalCooldown": "2 sec.",
        "canonicalCooldownMs": 2000,
        "desc": "修復召喚出的魔像，恢復魔像 8% 生命值。",
        "balance": {
            "mpCost": 61,
            "pwr": 1,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_repair_golem",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "shineMakerS2",
            "warsmith"
        ]
    },
    "strengthen_golem": {
        "id": "strengthen_golem",
        "name": "強化魔像",
        "slug": "strengthen_golem",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0823.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "The effect is applied on servitors.P. Atk. +8%Atk. Spd. +3%",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "效果套用於召喚獸：物理攻擊 +8%、攻速 +3%。",
        "balance": {
            "mpCost": 24,
            "pwr": 1,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_strengthen_golem",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 43,
        "classes": [
            "shineMakerS2",
            "warsmith"
        ]
    },
    "spirit_weakness": {
        "id": "spirit_weakness",
        "name": "靈魂弱點",
        "slug": "spirit_weakness",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_soul_weakness.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Spirit Weakness effect",
        "canonicalCooldown": "30 sec.",
        "canonicalCooldownMs": 30000,
        "desc": "靈魂弱點。",
        "balance": {
            "mpCost": 120,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_spirit_weakness",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "soulHound"
        ]
    },
    "summon_siege_golem": {
        "id": "summon_siege_golem",
        "name": "召喚攻城魔像",
        "slug": "summon_siege_golem",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0013.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Summon Siege Golem effect",
        "canonicalCooldown": "5 sec.",
        "canonicalCooldownMs": 5000,
        "desc": "召喚攻城魔像。",
        "balance": {
            "mpCost": 530,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_summon_siege_golem",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 49,
        "classes": [
            "shineMakerS2",
            "warsmith"
        ]
    },
    "summon_wild_hog_cannon": {
        "id": "summon_wild_hog_cannon",
        "name": "召喚野豬加農砲",
        "slug": "summon_wild_hog_cannon",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0299.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Summon Wild Hog Cannon effect",
        "canonicalCooldown": "5 sec.",
        "canonicalCooldownMs": 5000,
        "desc": "召喚野豬加農砲。",
        "balance": {
            "mpCost": 530,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_summon_wild_hog_cannon",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 58,
        "classes": [
            "shineMakerS2",
            "warsmith"
        ]
    },
    "soul_wild_magic": {
        "id": "soul_wild_magic",
        "name": "靈魂野性魔力",
        "slug": "soul_wild_magic",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1479.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Soul Wild Magic effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "靈魂野性魔力。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_soul_wild_magic",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "berserker",
            "doombringer",
            "ertheiaWarrior",
            "eviscerator",
            "marauder",
            "soulBreakerKamael",
            "soulFinder",
            "soulHound",
            "soulRanger",
            "trickster",
            "trooper",
            "warder"
        ]
    },
    "rapier_slash": {
        "id": "rapier_slash",
        "name": "細劍斬擊",
        "slug": "rapier_slash",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_rapier_slash.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Swings a rapier.Attacks 2 times ignoring 20% of P. Def. Power 5500.With a 50% ch",
        "canonicalCooldown": "25 sec.",
        "canonicalCooldownMs": 25000,
        "desc": "揮動細劍連續攻擊 2 次，無視目標 20%物理防禦，威力 5500。有 50% 機率觸發 劍擊 追加魔法傷害，威力 152。若目標帶有 弱點偵測（靈魂弱點），則觸發 閃電 追加攻擊。玩家對戰／對怪物戰鬥最多攻擊 6 個目標。劍擊：玩家對戰 降低麻痺抗性與物理防禦／魔法防禦；對怪物戰鬥 降低物理防禦／魔法防禦。閃電：玩家對戰 施加麻痺並降低物理防禦／魔法防禦；對怪物戰鬥 降低物理防禦／魔法防禦。",
        "balance": {
            "mpCost": 100,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_rapier_slash",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "soulHound"
        ]
    },
    "excess_charge": {
        "id": "excess_charge",
        "name": "過載充能",
        "slug": "excess_charge",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/s_overcharge.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "P. Atk. +30%Skill Critical Rate +10%Skill Critical Damage +10%Skill Power +5%Par",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "物理攻擊 +30%、技能暴擊率 +10%、技能暴擊傷害 +10%、技能威力 +5%、麻痺成功率 +20%、麻痺抗性 +20%。可使用 雷奧波德。",
        "balance": {
            "mpCost": 100,
            "pwr": 3,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_excess_charge",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "soulHound"
        ]
    },
    "soul_haste": {
        "id": "soul_haste",
        "name": "靈魂加速",
        "slug": "soul_haste",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill0427.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Soul Haste effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "靈魂加速。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_soul_haste",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "berserker",
            "doombringer",
            "ertheiaWarrior",
            "eviscerator",
            "marauder",
            "soulBreakerKamael",
            "soulFinder",
            "soulHound",
            "soulRanger",
            "trickster",
            "trooper",
            "warder"
        ]
    },
    "flash_dash": {
        "id": "flash_dash",
        "name": "閃光突進",
        "slug": "flash_dash",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_fleche.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "<Rush skill>Rushes to the target.Makes 1 attack ignoring 10% of P. Def. Power 60",
        "canonicalCooldown": "5 sec.",
        "canonicalCooldownMs": 5000,
        "desc": "<突進技能>快速衝向目標，進行 1 次攻擊並無視目標 10%物理防禦，威力 6013。有 30% 機率觸發 閃電 追加魔法傷害，威力 78。閃電：玩家對戰 施加麻痺並降低物理防禦／魔法防禦；對怪物戰鬥 降低物理防禦／魔法防禦。",
        "balance": {
            "mpCost": 72,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_flash_dash",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "soulBreakerKamael",
            "soulHound"
        ]
    },
    "soul_magic_barrier": {
        "id": "soul_magic_barrier",
        "name": "靈魂魔法屏障",
        "slug": "soul_magic_barrier",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill34119.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Soul Magic Barrier effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "靈魂魔法屏障。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_soul_magic_barrier",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "berserker",
            "doombringer",
            "ertheiaWarrior",
            "eviscerator",
            "marauder",
            "soulBreakerKamael",
            "soulFinder",
            "soulHound",
            "soulRanger",
            "trickster",
            "trooper",
            "warder"
        ]
    },
    "soul_berserker_spirit": {
        "id": "soul_berserker_spirit",
        "name": "靈魂狂戰士之魂",
        "slug": "soul_berserker_spirit",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill30520.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Soul Berserker Spirit effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "靈魂狂戰士之魂。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_soul_berserker_spirit",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "berserker",
            "doombringer",
            "ertheiaWarrior",
            "eviscerator",
            "marauder",
            "soulBreakerKamael",
            "soulFinder",
            "soulHound",
            "soulRanger",
            "trickster",
            "trooper",
            "warder"
        ]
    },
    "crescent_swing": {
        "id": "crescent_swing",
        "name": "新月揮擊",
        "slug": "crescent_swing",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_cresent_arc.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Swings a rapier in a half-circle.Makes 1 attack ignoring 15% of P. Def. Power 48",
        "canonicalCooldown": "12 sec.",
        "canonicalCooldownMs": 12000,
        "desc": "以半圓軌跡揮動細劍，進行 1 次攻擊並無視目標 15%物理防禦，威力 4820。有 30% 機率觸發 劍擊 追加魔法傷害，威力 100。玩家對戰／對怪物戰鬥最多攻擊 4 個目標。劍擊：玩家對戰 降低麻痺抗性與物理防禦／魔法防禦；對怪物戰鬥 降低物理防禦／魔法防禦。",
        "balance": {
            "mpCost": 91,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_crescent_swing",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "soulHound"
        ]
    },
    "through_strike": {
        "id": "through_strike",
        "name": "貫穿打擊",
        "slug": "through_strike",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/s_thrust.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Makes a rapier attack.Makes 1 attack ignoring 15% of P. Def. Power 4715.With a 3",
        "canonicalCooldown": "1 sec.",
        "canonicalCooldownMs": 1000,
        "desc": "以細劍攻擊，進行 1 次攻擊並無視目標 15%物理防禦，威力 4715。有 30% 機率觸發 劍擊 追加魔法傷害，威力 120。劍擊：玩家對戰 降低麻痺抗性與物理防禦／魔法防禦；對怪物戰鬥 降低物理防禦／魔法防禦。",
        "balance": {
            "mpCost": 84,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_through_strike",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 80,
        "classes": [
            "soulBreakerKamael",
            "soulFinder",
            "soulHound"
        ]
    },
    "soul_wind_walk": {
        "id": "soul_wind_walk",
        "name": "靈魂風之疾走",
        "slug": "soul_wind_walk",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill11823.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Soul Wind Walk effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "靈魂風之疾走。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_soul_wind_walk",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "doombringer",
            "eviscerator",
            "marauder",
            "soulFinder",
            "soulHound",
            "trickster",
            "trooper",
            "warder"
        ]
    },
    "soul_acumen": {
        "id": "soul_acumen",
        "name": "靈魂靈活思緒",
        "slug": "soul_acumen",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1520.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Soul Acumen effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "靈魂靈活思緒。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_soul_acumen",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "berserker",
            "doombringer",
            "ertheiaWarrior",
            "eviscerator",
            "marauder",
            "soulBreakerKamael",
            "soulFinder",
            "soulHound",
            "soulRanger",
            "trickster",
            "trooper",
            "warder"
        ]
    },
    "light_soul_connection": {
        "id": "light_soul_connection",
        "name": "光之靈魂連結",
        "slug": "light_soul_connection",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_shine_soul_affinity.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Light Soul Connection effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "光之靈魂連結。",
        "balance": {
            "mpCost": 50,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_light_soul_connection",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "doombringer",
            "eviscerator",
            "soulHound",
            "trickster"
        ]
    },
    "shadow_soul_connection": {
        "id": "shadow_soul_connection",
        "name": "暗影靈魂連結",
        "slug": "shadow_soul_connection",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_shadow_soul_affinity.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Shadow Soul Connection effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "暗影靈魂連結。",
        "balance": {
            "mpCost": 50,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_shadow_soul_connection",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "doombringer",
            "eviscerator",
            "soulHound",
            "trickster"
        ]
    },
    "chain_lightning": {
        "id": "chain_lightning",
        "name": "連鎖閃電",
        "slug": "chain_lightning",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_chain_lightning_hound.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Strikes the target with intertwining lightnings.Incessantly attacks the target i",
        "canonicalCooldown": "8 sec.",
        "canonicalCooldownMs": 8000,
        "desc": "以交織閃電打擊目標，連續攻擊並無視目標 15%物理防禦，威力 5060。有 40% 機率觸發 閃電 追加魔法傷害，威力 152。玩家對戰／對怪物戰鬥最多攻擊 4 個目標。閃電：玩家對戰 施加麻痺並降低物理防禦／魔法防禦；對怪物戰鬥 降低物理防禦／魔法防禦。",
        "balance": {
            "mpCost": 100,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_chain_lightning",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "soulHound"
        ]
    },
    "leopold": {
        "id": "leopold",
        "name": "雷奧波德",
        "slug": "leopold",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/s_leopold.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Leopold effect",
        "canonicalCooldown": "45 sec.",
        "canonicalCooldownMs": 45000,
        "desc": "雷奧波德。",
        "balance": {
            "mpCost": 400,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_leopold",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "maestro",
            "soulHound"
        ]
    },
    "resonance_shield": {
        "id": "resonance_shield",
        "name": "共鳴護盾",
        "slug": "resonance_shield",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/s_resonance_shield.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Resonance Shield effect",
        "canonicalCooldown": "5 min.",
        "canonicalCooldownMs": 300000,
        "desc": "共鳴護盾。",
        "balance": {
            "mpCost": 125,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_resonance_shield",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "soulHound"
        ]
    },
    "mount_griffin": {
        "id": "mount_griffin",
        "name": "騎乘獅鷲",
        "slug": "mount_griffin",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/griffin_vehicle.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Mount Griffin effect",
        "canonicalCooldown": "1s",
        "canonicalCooldownMs": 1000,
        "desc": "騎乘獅鷲。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_mount_griffin",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [],
        "availableTo": [],
        "disabled": true,
        "removalReason": "cosmetic_mount_purge"
    },
    "soul_reinforcement": {
        "id": "soul_reinforcement",
        "name": "靈魂強化",
        "slug": "soul_reinforcement",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill32617.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Strengthens yourself with the power of your soul.Max HP +30%P. Atk. +1500PvE dam",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "以靈魂力量強化自身：最大生命值 +30%、物理攻擊 +1500、對怪物戰鬥傷害 +20%。",
        "balance": {
            "mpCost": 50,
            "pwr": 3,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_soul_reinforcement",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 80,
        "classes": [
            "soulBreakerKamael",
            "soulHound"
        ]
    },
    "collect_light_souls": {
        "id": "collect_light_souls",
        "name": "收集光之靈魂",
        "slug": "collect_light_souls",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill55474.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Collect Light Souls effect",
        "canonicalCooldown": "15 min.",
        "canonicalCooldownMs": 900000,
        "desc": "收集光之靈魂。",
        "balance": {
            "mpCost": 50,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_collect_light_souls",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "berserker",
            "doombringer",
            "ertheiaWarrior",
            "eviscerator",
            "soulBreakerKamael",
            "soulHound",
            "soulRanger",
            "trickster"
        ]
    },
    "soul_blade": {
        "id": "soul_blade",
        "name": "靈魂之刃",
        "slug": "soul_blade",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1473.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Soul Blade effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "靈魂之刃。",
        "balance": {
            "mpCost": 120,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_soul_blade",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "soulHound"
        ]
    },
    "collect_shadow_souls": {
        "id": "collect_shadow_souls",
        "name": "收集暗影靈魂",
        "slug": "collect_shadow_souls",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill55475.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Collect Shadow Souls effect",
        "canonicalCooldown": "15 min.",
        "canonicalCooldownMs": 900000,
        "desc": "收集暗影靈魂。",
        "balance": {
            "mpCost": 50,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_collect_shadow_souls",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "berserker",
            "doombringer",
            "ertheiaWarrior",
            "eviscerator",
            "soulBreakerKamael",
            "soulHound",
            "soulRanger",
            "trickster"
        ]
    },
    "fragarach": {
        "id": "fragarach",
        "name": "弗拉格拉赫",
        "slug": "fragarach",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_fragarach.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Fragarach effect",
        "canonicalCooldown": ":",
        "canonicalCooldownMs": 1000,
        "desc": "弗拉格拉赫。",
        "balance": {
            "mpCost": 100,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_fragarach",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "soulHound"
        ]
    },
    "way_back": {
        "id": "way_back",
        "name": "返程",
        "slug": "way_back",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill10774.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Way Back effect",
        "canonicalCooldown": "30 sec.",
        "canonicalCooldownMs": 30000,
        "desc": "返程。",
        "balance": {
            "mpCost": 100,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_way_back",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 80,
        "classes": [
            "soulHound"
        ]
    },
    "enuma_elish": {
        "id": "enuma_elish",
        "name": "艾努瑪艾利什",
        "slug": "enuma_elish",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/s_enuma_elish2.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Enuma Elish's energy drives you forward.Attacks the target and nearby enemies. P",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "艾努瑪艾利什的能量推動你向前，攻擊目標與附近敵人，威力 5077。無視盾牌防禦與目標 15% 防禦力。玩家對戰／對怪物戰鬥最多攻擊 3 個目標。",
        "balance": {
            "mpCost": 59,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_enuma_elish",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 80,
        "classes": [
            "berserker",
            "doombringer",
            "ertheiaWarrior",
            "eviscerator"
        ]
    },
    "soul_mark": {
        "id": "soul_mark",
        "name": "靈魂印記",
        "slug": "soul_mark",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_soul_sign.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Swings your ancient sword to attack the enemy. Applies Soul Stamp on the target ",
        "canonicalCooldown": "40 sec.",
        "canonicalCooldownMs": 40000,
        "desc": "揮動古代劍攻擊敵人，對目標附加 靈魂烙印，封鎖物理技能與吸取能力。攻擊目標與附近敵人，威力 6072。無視盾牌防禦與目標 15% 防禦力。靈魂烙印：生命值 吸收率 -20%、魔力 吸收率 -20%，持續 7 秒；物理防禦 -30%，持續 7 秒。玩家對戰／對怪物戰鬥最多攻擊 7 個目標。",
        "balance": {
            "mpCost": 86,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_soul_mark",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "doombringer",
            "eviscerator"
        ]
    },
    "rush_impact": {
        "id": "rush_impact",
        "name": "突進衝擊",
        "slug": "rush_impact",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/s_rush_impact.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "<Rush/ Shock skill>Charges at the target and nearby enemies attacking them. Powe",
        "canonicalCooldown": "3 sec.",
        "canonicalCooldownMs": 3000,
        "desc": "<突進／休克技能>衝向目標與附近敵人並攻擊，威力 2800。有一定機率暈眩 2 秒。玩家對戰 中有 2% 機率中斷目標施法。與目標距離超過 100 時可使用。玩家對戰／對怪物戰鬥最多攻擊 3 個目標。注意：連續施放 3 秒不中斷時會施加失能效果；失能生效期間使用此技能，會降低自身 魔力，並降低受擊時的 魔力 吸取。",
        "balance": {
            "mpCost": 45,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_rush_impact",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 80,
        "classes": [
            "berserker",
            "doombringer",
            "ertheiaWarrior",
            "eviscerator"
        ]
    },
    "soul_guard": {
        "id": "soul_guard",
        "name": "靈魂守護",
        "slug": "soul_guard",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1515.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Soul Guard effect",
        "canonicalCooldown": "2 min.",
        "canonicalCooldownMs": 120000,
        "desc": "靈魂守護。",
        "balance": {
            "mpCost": 100,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_soul_guard",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "berserker",
            "doombringer",
            "ertheiaWarrior",
            "eviscerator",
            "marauder",
            "trooper"
        ]
    },
    "soul_impulse": {
        "id": "soul_impulse",
        "name": "靈魂衝擊",
        "slug": "soul_impulse",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_soul_impact2.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Deals massive damage with the soul power.Attacks the target. Power 8017.Ignores ",
        "canonicalCooldown": "7 sec.",
        "canonicalCooldownMs": 7000,
        "desc": "以靈魂力量造成巨大傷害。攻擊目標，威力 8017。無視盾牌防禦與目標 15% 防禦力。",
        "balance": {
            "mpCost": 61,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_soul_impulse",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "berserker",
            "doombringer",
            "ertheiaWarrior",
            "eviscerator"
        ]
    },
    "soul_smash": {
        "id": "soul_smash",
        "name": "靈魂粉碎",
        "slug": "soul_smash",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_ark_soul_smash.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Crushes enemies with the soul power.Attacks the target twice. Power 5694.Ignores",
        "canonicalCooldown": "1 sec.",
        "canonicalCooldownMs": 1000,
        "desc": "以靈魂力量粉碎敵人，連續攻擊目標 2 次，威力 5694。無視盾牌防禦與目標 15% 防禦力。",
        "balance": {
            "mpCost": 55,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_soul_smash",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "berserker",
            "doombringer",
            "ertheiaWarrior",
            "eviscerator",
            "marauder",
            "trooper"
        ]
    },
    "increase_power": {
        "id": "increase_power",
        "name": "力量提升",
        "slug": "increase_power",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1432.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Unravels potential and increases M. Atk.P. Atk. Shock Atk. Rate +30%",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "釋放潛能並提高魔法攻擊力。物理攻擊、休克成功率 +30%。",
        "balance": {
            "mpCost": 45,
            "pwr": 3,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_increase_power",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "berserker",
            "doombringer",
            "ertheiaWarrior",
            "eviscerator",
            "marauder",
            "trooper"
        ]
    },
    "soul_prison": {
        "id": "soul_prison",
        "name": "靈魂監獄",
        "slug": "soul_prison",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/s_soul_prison2.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Soul Prison effect",
        "canonicalCooldown": "1 min.",
        "canonicalCooldownMs": 60000,
        "desc": "靈魂監獄。",
        "balance": {
            "mpCost": 85,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_soul_prison",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "doombringer",
            "eviscerator"
        ]
    },
    "magic_break": {
        "id": "magic_break",
        "name": "魔力削弱",
        "slug": "magic_break",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/iconskill30517.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Weakens the target's magic abilities.M. Atk. -23%",
        "canonicalCooldown": "2 sec.",
        "canonicalCooldownMs": 2000,
        "desc": "削弱目標的魔法能力：魔法攻擊 -23%。",
        "balance": {
            "mpCost": 40,
            "pwr": 2,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_magic_break",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "doombringer",
            "eviscerator"
        ]
    },
    "soul_weapon": {
        "id": "soul_weapon",
        "name": "靈魂武器",
        "slug": "soul_weapon",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_ark_soul_sword.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Soul Weapon effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "靈魂武器。",
        "balance": {
            "mpCost": 75,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_soul_weapon",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "doombringer",
            "eviscerator"
        ]
    },
    "overwhelming_power": {
        "id": "overwhelming_power",
        "name": "壓倒性力量",
        "slug": "overwhelming_power",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/skill0494.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Awakens Kamael's true powers.P. Atk. +35%P. Atk. +300Atk. Spd. +30%P. Skill Crit",
        "canonicalCooldown": ":",
        "canonicalCooldownMs": 1000,
        "desc": "喚醒卡麥爾真正力量：物理攻擊 +35%、物理攻擊 +300、攻速 +30%、物理技能暴擊率 +30%、物理技能暴擊傷害 +30%、物理技能威力 +15%、最大生命值 +20%（立即恢復增加的 生命值）、休克成功率 +20%、牽引成功率 +20%。攻擊時追加相當於自身戰鬥力 30% 的額外傷害，計入傷害上限。攻擊時有 2% 機率附加 壓倒性力量：物理技能暴擊率 +20%、物理技能暴擊傷害 +20%、受到技能暴擊傷害 -50%、速度 +20；使用 靈魂粉碎／靈魂衝擊 時觸發追加攻擊，持續 10 秒，觸發冷卻 11 秒。並將 靈魂粉碎、靈魂衝擊 強化為 強化版本：提高威力，對屏障額外傷害 +50%。靈魂監獄：提高威力、牽引成功率、強化 玩家對戰弱化效果，並增加 對怪物戰鬥目標數。",
        "balance": {
            "mpCost": 100,
            "pwr": 4,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_overwhelming_power",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "doombringer",
            "eviscerator",
            "titan"
        ]
    },
    "kamael_symbol": {
        "id": "kamael_symbol",
        "name": "卡麥爾象徵",
        "slug": "kamael_symbol",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_symbol_of_kamael.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Kamael Symbol effect",
        "canonicalCooldown": "2 min.",
        "canonicalCooldownMs": 120000,
        "desc": "卡麥爾象徵。",
        "balance": {
            "mpCost": 41,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_kamael_symbol",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "doombringer",
            "eviscerator"
        ]
    },
    "powerful_rush": {
        "id": "powerful_rush",
        "name": "強力突進",
        "slug": "powerful_rush",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_inhanced_rush.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Powerful Rush effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "強力突進。",
        "balance": {
            "mpCost": 65,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_powerful_rush",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "doombringer",
            "eviscerator"
        ]
    },
    "disarm": {
        "id": "disarm",
        "name": "繳械",
        "slug": "disarm",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill0485.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Disarms a hostile character for 2 sec.The skill can only be used on a mutually h",
        "canonicalCooldown": "1 min.",
        "canonicalCooldownMs": 60000,
        "desc": "使敵對角色被繳械 2 秒。此技能只能對相互敵對的角色使用。",
        "balance": {
            "mpCost": 50,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_disarm",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "doombringer",
            "eviscerator"
        ]
    },
    "cunning_throw": {
        "id": "cunning_throw",
        "name": "狡詐投擲",
        "slug": "cunning_throw",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/s_trickal_dart.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Cunning Throw effect",
        "canonicalCooldown": "3 sec.",
        "canonicalCooldownMs": 3000,
        "desc": "狡詐投擲。",
        "balance": {
            "mpCost": 20,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_cunning_throw",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 20,
        "classes": [
            "soulRanger",
            "trickster",
            "warder"
        ]
    },
    "cunning_shot": {
        "id": "cunning_shot",
        "name": "狡詐射擊",
        "slug": "cunning_shot",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/s_trickal_shot_basic.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks the enemy. Power 1000. With a 20% chance, makes 1 additional attack.Igno",
        "canonicalCooldown": "2 sec.",
        "canonicalCooldownMs": 2000,
        "desc": "攻擊敵人，威力 1000。有 20% 機率追加 1 次攻擊。無視目標 10% 防禦力。",
        "balance": {
            "mpCost": 10,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_cunning_shot",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 20,
        "classes": [
            "soulRanger",
            "trickster",
            "warder"
        ]
    },
    "earth_tremor": {
        "id": "earth_tremor",
        "name": "大地震擊",
        "slug": "earth_tremor",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/skill1922.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "A powerful earth strike.Attacks the target. Power 4907.Ignores Shield Defense.Ig",
        "canonicalCooldown": "1 sec.",
        "canonicalCooldownMs": 1000,
        "desc": "以強力大地打擊攻擊目標，威力 4907。無視盾牌防禦與目標 10% 防禦力。",
        "balance": {
            "mpCost": 60,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_earth_tremor",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "maestro",
            "shinemaker"
        ]
    },
    "blacksmith_s_attack": {
        "id": "blacksmith_s_attack",
        "name": "鐵匠突擊",
        "slug": "blacksmith_s_attack",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/forge_attack.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "<Rush/ Shock skill>Rushes forward.Attacks the target. Power 4588.With a certain ",
        "canonicalCooldown": "3 sec.",
        "canonicalCooldownMs": 3000,
        "desc": "<突進／休克技能>向前突進並攻擊目標，威力 4588。有一定機率暈眩 3 秒。與目標距離超過 80 時可使用。",
        "balance": {
            "mpCost": 50,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_blacksmith_s_attack",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "maestro",
            "shinemaker"
        ]
    },
    "hammer_rumble": {
        "id": "hammer_rumble",
        "name": "戰鎚震擊",
        "slug": "hammer_rumble",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/s_broken_match.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks the target. Power 6049.Ignores Shield Defense.Ignores 20% of the target'",
        "canonicalCooldown": "8 sec.",
        "canonicalCooldownMs": 8000,
        "desc": "攻擊目標，威力 6049。無視盾牌防禦與目標 20% 防禦力。",
        "balance": {
            "mpCost": 72,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_hammer_rumble",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "maestro",
            "shinemaker"
        ]
    },
    "prime_master": {
        "id": "prime_master",
        "name": "至尊大師",
        "slug": "prime_master",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/s_maestro_hero.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Prime Master effect",
        "canonicalCooldown": "1 min.",
        "canonicalCooldownMs": 60000,
        "desc": "至尊大師。",
        "balance": {
            "mpCost": 200,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_prime_master",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "maestro",
            "shinemaker"
        ]
    },
    "mechanical_masterpiece": {
        "id": "mechanical_masterpiece",
        "name": "機械傑作",
        "slug": "mechanical_masterpiece",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/mechanic_masterpiece.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Mechanical Masterpiece effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "機械傑作。",
        "balance": {
            "mpCost": 70,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_mechanical_masterpiece",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "maestro",
            "shinemaker"
        ]
    },
    "death_mark": {
        "id": "death_mark",
        "name": "死亡印記",
        "slug": "death_mark",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/skill1435.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Death Mark effect",
        "canonicalCooldown": ":",
        "canonicalCooldownMs": 1000,
        "desc": "死亡印記。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_death_mark",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 15,
        "classes": [
            "kamaelSoldier",
            "marauderBase"
        ]
    },
    "might": {
        "id": "might",
        "name": "力量強化",
        "slug": "might",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/skill1068.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Might effect",
        "canonicalCooldown": "1s",
        "canonicalCooldownMs": 1000,
        "desc": "力量強化。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_might",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 3,
        "classes": [
            "cleric",
            "highElfBase",
            "kamaelSoldier",
            "marauderBase"
        ]
    },
    "expand_head_accessory_slots": {
        "id": "expand_head_accessory_slots",
        "name": "擴充頭部飾品欄位",
        "slug": "expand_head_accessory_slots",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/lv_benefit_hair.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Expand Head Accessory Slots effect",
        "canonicalCooldown": "1s",
        "canonicalCooldownMs": 1000,
        "desc": "擴充頭部飾品欄位。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_expand_head_accessory_slots",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 1,
        "classes": [
            "highElfBase",
            "kamaelSoldier",
            "marauderBase"
        ]
    },
    "kamael_s_dignity": {
        "id": "kamael_s_dignity",
        "name": "卡麥爾尊嚴",
        "slug": "kamael_s_dignity",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/skill1923.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Kamael's Dignity effect",
        "canonicalCooldown": "1s",
        "canonicalCooldownMs": 1000,
        "desc": "卡麥爾尊嚴。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_kamael_s_dignity",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 1,
        "classes": [
            "kamaelSoldier",
            "marauderBase"
        ]
    },
    "expand_world_trade_l_coins": {
        "id": "expand_world_trade_l_coins",
        "name": "擴充世界交易（L 金幣）",
        "slug": "expand_world_trade_l_coins",
        "type": "passive",
        "rawType": "Passivo",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/s_expand_trade.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "World Trade sales slots (L-Coins) +1World Trade operations slots (L-Coins) +1",
        "canonicalCooldown": "1s",
        "canonicalCooldownMs": 1000,
        "desc": "世界交易販售欄位（L 金幣）+1，世界交易操作欄位（L 金幣）+1。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_expand_world_trade_l_coins",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 1,
        "classes": [
            "highElfBase",
            "kamaelSoldier",
            "marauderBase"
        ]
    },
    "expand_warehouse": {
        "id": "expand_warehouse",
        "name": "擴充倉庫",
        "slug": "expand_warehouse",
        "type": "passive",
        "rawType": "Passivo",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/s_expand_storage.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Increases the number of items that can be stored in private warehouses and freig",
        "canonicalCooldown": "1s",
        "canonicalCooldownMs": 1000,
        "desc": "增加私人倉庫與貨運可存放的物品數量。欄位 +6。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_expand_warehouse",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 1,
        "classes": [
            "highElfBase",
            "kamaelSoldier",
            "marauderBase"
        ]
    },
    "shield": {
        "id": "shield",
        "name": "護盾",
        "slug": "shield",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/skill1040.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Shield effect",
        "canonicalCooldown": "1s",
        "canonicalCooldownMs": 1000,
        "desc": "護盾。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_shield",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 2,
        "classes": [
            "highElfBase",
            "kamaelSoldier",
            "marauderBase"
        ]
    },
    "soul_roar": {
        "id": "soul_roar",
        "name": "靈魂咆哮",
        "slug": "soul_roar",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/skill20006.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Soul Roar effect",
        "canonicalCooldown": "2 min.",
        "canonicalCooldownMs": 120000,
        "desc": "靈魂咆哮。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_soul_roar",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 28,
        "classes": [
            "berserker",
            "ertheiaWarrior",
            "marauder",
            "trooper"
        ]
    },
    "pride_of_kamael": {
        "id": "pride_of_kamael",
        "name": "卡麥爾榮耀",
        "slug": "pride_of_kamael",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/skill1925.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Pride of Kamael effect",
        "canonicalCooldown": "1s",
        "canonicalCooldownMs": 1000,
        "desc": "卡麥爾榮耀。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_pride_of_kamael",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 1,
        "classes": [
            "kamaelSoldier",
            "marauderBase"
        ]
    },
    "expand_inventory": {
        "id": "expand_inventory",
        "name": "擴充背包",
        "slug": "expand_inventory",
        "type": "passive",
        "rawType": "Passivo",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/s_expand_inventory.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Increases the number of items you can carry.Slots: +10.",
        "canonicalCooldown": "1s",
        "canonicalCooldownMs": 1000,
        "desc": "增加可攜帶的物品數量。欄位 +10。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_expand_inventory",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 1,
        "classes": [
            "highElfBase",
            "kamaelSoldier",
            "marauderBase"
        ]
    },
    "magic_lamp": {
        "id": "magic_lamp",
        "name": "魔法神燈",
        "slug": "magic_lamp",
        "type": "passive",
        "rawType": "Passivo",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/s_magiclamp_info.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Magic Lamp effect",
        "canonicalCooldown": "1s",
        "canonicalCooldownMs": 1000,
        "desc": "魔法神燈。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_magic_lamp",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 1,
        "classes": [
            "highElfBase",
            "kamaelSoldier",
            "marauderBase"
        ]
    },
    "scorn": {
        "id": "scorn",
        "name": "嘲諷",
        "slug": "scorn",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0503.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Provokes nearby monsters with 11,000 power.",
        "canonicalCooldown": "4 sec.",
        "canonicalCooldownMs": 4000,
        "desc": "以 11000 威力挑釁附近怪物。",
        "balance": {
            "mpCost": 50,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_scorn",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 43,
        "classes": [
            "berserker",
            "ertheiaWarrior"
        ]
    },
    "freezing_wound": {
        "id": "freezing_wound",
        "name": "冰凍創傷",
        "slug": "freezing_wound",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/freezing_wound.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Uses spirit powers to attack.Power 120%.Duration: 3 sec.Atk. Spd. -200Casting Sp",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "使用靈魂力量攻擊，威力 120%。持續 3 秒：攻速 -200、施法速度 -200、速度 -50。",
        "balance": {
            "mpCost": 40,
            "pwr": 120,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_freezing_wound",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "stormBlaster",
            "windSniper"
        ]
    },
    "elemental_magic_barrier": {
        "id": "elemental_magic_barrier",
        "name": "元素魔法屏障",
        "slug": "elemental_magic_barrier",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/elemental_magic_barrier.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Elemental Magic Barrier effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "元素魔法屏障。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_elemental_magic_barrier",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "sharpshooter",
            "stormBlaster",
            "sylphGunner",
            "windSniper"
        ]
    },
    "elemental_haste": {
        "id": "elemental_haste",
        "name": "元素加速",
        "slug": "elemental_haste",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/elemental_haste.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Elemental Haste effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "元素加速。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_elemental_haste",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "sharpshooter",
            "stormBlaster",
            "sylphGunner",
            "windSniper"
        ]
    },
    "blessing_of_winds": {
        "id": "blessing_of_winds",
        "name": "風之祝福",
        "slug": "blessing_of_winds",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/elemental_affinity_wind.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Blessing of Winds effect",
        "canonicalCooldown": "1 min.",
        "canonicalCooldownMs": 60000,
        "desc": "風之祝福。",
        "balance": {
            "mpCost": 30,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_blessing_of_winds",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "stormBlaster",
            "windSniper"
        ]
    },
    "elemental_berserker_spirit": {
        "id": "elemental_berserker_spirit",
        "name": "元素狂戰士之魂",
        "slug": "elemental_berserker_spirit",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/elemental_berserker_spirit.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Elemental Berserker Spirit effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "元素狂戰士之魂。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_elemental_berserker_spirit",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "sharpshooter",
            "stormBlaster",
            "sylphGunner",
            "windSniper"
        ]
    },
    "elemental_wild_magic": {
        "id": "elemental_wild_magic",
        "name": "元素野性魔力",
        "slug": "elemental_wild_magic",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/elemental_wild_magic.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Elemental Wild Magic effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "元素野性魔力。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_elemental_wild_magic",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "sharpshooter",
            "stormBlaster",
            "sylphGunner",
            "windSniper"
        ]
    },
    "elemental_insight": {
        "id": "elemental_insight",
        "name": "元素洞察",
        "slug": "elemental_insight",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/elemental_acumen.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Elemental Insight effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "元素洞察。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_elemental_insight",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "sharpshooter",
            "stormBlaster",
            "sylphGunner",
            "windSniper"
        ]
    },
    "dual_blow": {
        "id": "dual_blow",
        "name": "雙重打擊",
        "slug": "dual_blow",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/double_impact.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "<Changing skill>Uses spirit powers in combat.Attacks the target twice. Power 100",
        "canonicalCooldown": ":",
        "canonicalCooldownMs": 1000,
        "desc": "<變化技能>運用靈魂力量戰鬥，連續攻擊目標 2 次，威力 1001。遠程攻擊時有一定機率恢復 1% 魔力（觸發冷卻 30 秒）。此觸發冷卻同樣套用於 隱密迅捷、強化風之射擊。",
        "balance": {
            "mpCost": 32,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_dual_blow",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "sharpshooter",
            "stormBlaster",
            "sylphGunner",
            "windSniper"
        ]
    },
    "fire_explosion": {
        "id": "fire_explosion",
        "name": "火焰爆炸",
        "slug": "fire_explosion",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/s_flame_explosion.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Fire Explosion effect",
        "canonicalCooldown": "20 sec.",
        "canonicalCooldownMs": 20000,
        "desc": "火焰爆炸。",
        "balance": {
            "mpCost": 84,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_fire_explosion",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "stormBlaster",
            "windSniper"
        ]
    },
    "pursuit": {
        "id": "pursuit",
        "name": "追擊",
        "slug": "pursuit",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/s_crow_pursuit.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Charges to the target and attacks them. Power 1650. Applies Rush Jump on the cas",
        "canonicalCooldown": "5 sec.",
        "canonicalCooldownMs": 5000,
        "desc": "衝向目標並攻擊，威力 1650。對自身附加 衝刺跳躍。無視盾牌防禦與目標 10% 防禦力。衝刺跳躍 持續 4 秒：物理攻擊 +300、攻速 +120、速度 +20、物理命中 +10。",
        "balance": {
            "mpCost": 40,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_pursuit",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "hatamoto",
            "ronin"
        ]
    },
    "strike": {
        "id": "strike",
        "name": "打擊",
        "slug": "strike",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/s_crow_strike.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks the target and nearby enemies. Power 2731.Ignores Shield Defense.Ignores",
        "canonicalCooldown": "30 sec.",
        "canonicalCooldownMs": 30000,
        "desc": "攻擊目標與附近敵人，威力 2731。無視盾牌防禦與目標 15% 防禦力。玩家對戰／對怪物戰鬥最多攻擊 2 個目標。",
        "balance": {
            "mpCost": 52,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_strike",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 45,
        "classes": [
            "ronin",
            "samurai"
        ]
    },
    "atsumori": {
        "id": "atsumori",
        "name": "敦盛",
        "slug": "atsumori",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/s_crow_atsumori.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Atsumori effect",
        "canonicalCooldown": "50 sec.",
        "canonicalCooldownMs": 50000,
        "desc": "敦盛。",
        "balance": {
            "mpCost": 40,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_atsumori",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 70,
        "classes": [
            "ronin",
            "samurai"
        ]
    },
    "forest": {
        "id": "forest",
        "name": "林",
        "slug": "forest",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/s_crow_forest.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Forest effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "林。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_forest",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "hatamoto",
            "ronin",
            "samurai"
        ]
    },
    "fire": {
        "id": "fire",
        "name": "火",
        "slug": "fire",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/s_crow_fire.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Fire effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "火。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_fire",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "hatamoto",
            "ronin",
            "samurai"
        ]
    },
    "mountain": {
        "id": "mountain",
        "name": "山",
        "slug": "mountain",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/s_crow_mountain.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Mountain effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "山。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_mountain",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "hatamoto",
            "ronin",
            "samurai"
        ]
    },
    "elemental_care": {
        "id": "elemental_care",
        "name": "元素守護",
        "slug": "elemental_care",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill11027.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Elemental Care effect",
        "canonicalCooldown": "15 sec.",
        "canonicalCooldownMs": 15000,
        "desc": "元素守護。",
        "balance": {
            "mpCost": 83,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_elemental_care",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 58,
        "classes": [
            "stormBlaster",
            "sylphGunner",
            "windSniper"
        ]
    },
    "wind": {
        "id": "wind",
        "name": "風",
        "slug": "wind",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/s_crow_wind.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Wind effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "風。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_wind",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "hatamoto",
            "ronin",
            "samurai"
        ]
    },
    "single_flash": {
        "id": "single_flash",
        "name": "一閃",
        "slug": "single_flash",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/s_crow_slash.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks the target. Power 985.Ignores Shield Defense.Ignores 15% of the target's",
        "canonicalCooldown": "1 sec.",
        "canonicalCooldownMs": 1000,
        "desc": "攻擊目標，威力 985。無視盾牌防禦與目標 15% 防禦力。",
        "balance": {
            "mpCost": 30,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_single_flash",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "hatamoto",
            "ronin",
            "samurai",
            "samuraiBase"
        ]
    },
    "elemental_wind_walk": {
        "id": "elemental_wind_walk",
        "name": "元素風之疾走",
        "slug": "elemental_wind_walk",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/elemental_wind_walk.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Speed +20",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "速度 +20。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_elemental_wind_walk",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 20,
        "classes": [
            "sharpshooter",
            "stormBlaster",
            "sylphGunner"
        ]
    },
    "greater_wind_shot": {
        "id": "greater_wind_shot",
        "name": "強化風之射擊",
        "slug": "greater_wind_shot",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_hyper_wind_shot.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "<Changing skill>The spirit power makes your attacks as swift as the wind.Attacks",
        "canonicalCooldown": ":",
        "canonicalCooldownMs": 1000,
        "desc": "<變化技能>以靈魂力量讓攻擊如疾風般迅速，連續攻擊目標 3 次，威力 5052。無視盾牌防禦與目標 10% 防禦力。遠程攻擊時有一定機率恢復 1% 魔力。觸發冷卻 30 秒；此觸發冷卻同樣套用於 雙重打擊、三重打擊。",
        "balance": {
            "mpCost": 75,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_greater_wind_shot",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "stormBlaster"
        ]
    },
    "goring_charge": {
        "id": "goring_charge",
        "name": "猛角衝鋒",
        "slug": "goring_charge",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/piercing_thrust.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "<Changing/ Knockdown skill>Shoots enemies with bullets enhanced by fearsome wind",
        "canonicalCooldown": "1 min.",
        "canonicalCooldownMs": 60000,
        "desc": "<變化／擊倒技能>發射由強風精靈強化的彈丸。距離小於 700：攻擊目標與附近敵人並擊倒，威力 7107，無視盾牌防禦與目標 15% 防禦力，玩家對戰／對怪物戰鬥最多攻擊 8 個目標。距離 700 以上：攻擊目標與攻擊路徑上的敵人並擊倒，威力 8440，無視盾牌防禦與目標 20% 防禦力，玩家對戰／對怪物戰鬥最多攻擊 5 個目標。",
        "balance": {
            "mpCost": 121,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_goring_charge",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "stormBlaster"
        ]
    },
    "frosty_sting": {
        "id": "frosty_sting",
        "name": "冰霜刺擊",
        "slug": "frosty_sting",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill11136.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Frosty Sting effect",
        "canonicalCooldown": "40 sec.",
        "canonicalCooldownMs": 40000,
        "desc": "冰霜刺擊。",
        "balance": {
            "mpCost": 120,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_frosty_sting",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "stormBlaster"
        ]
    },
    "stealthy_swiftness": {
        "id": "stealthy_swiftness",
        "name": "隱密迅捷",
        "slug": "stealthy_swiftness",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/silent_blitz.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "<Changing skill>Strikes so swiftly that the target does not see you.Attacks the ",
        "canonicalCooldown": ":",
        "canonicalCooldownMs": 1000,
        "desc": "<變化技能>以快到目標無法察覺的速度連續攻擊 3 次，威力 4971。無視盾牌防禦與目標 10% 防禦力。遠程攻擊時有一定機率恢復 1% 魔力（觸發冷卻 30 秒）。此觸發冷卻同樣套用於 雙重打擊、三重打擊。",
        "balance": {
            "mpCost": 110,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_stealthy_swiftness",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "stormBlaster"
        ]
    },
    "wild_dance": {
        "id": "wild_dance",
        "name": "野性之舞",
        "slug": "wild_dance",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill10028.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Wild Dance effect",
        "canonicalCooldown": "15 sec.",
        "canonicalCooldownMs": 15000,
        "desc": "野性之舞。",
        "balance": {
            "mpCost": 200,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_wild_dance",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "stormBlaster"
        ]
    },
    "extra_hit": {
        "id": "extra_hit",
        "name": "追加打擊",
        "slug": "extra_hit",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill32111.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Extra Hit effect",
        "canonicalCooldown": ".Fire",
        "canonicalCooldownMs": 1000,
        "desc": "追加打擊。",
        "balance": {
            "mpCost": 120,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_extra_hit",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "stormBlaster"
        ]
    },
    "mount_elemental_lyn_draco": {
        "id": "mount_elemental_lyn_draco",
        "name": "騎乘元素林德拉科",
        "slug": "mount_elemental_lyn_draco",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/etc_lind_vehicle.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Mount Elemental Lyn Draco effect",
        "canonicalCooldown": "1s",
        "canonicalCooldownMs": 1000,
        "desc": "騎乘元素林德拉科。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_mount_elemental_lyn_draco",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [],
        "availableTo": [],
        "disabled": true,
        "removalReason": "cosmetic_mount_purge"
    },
    "elemental_change": {
        "id": "elemental_change",
        "name": "元素轉換",
        "slug": "elemental_change",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill1996.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Elemental Change effect",
        "canonicalCooldown": "30 sec.",
        "canonicalCooldownMs": 30000,
        "desc": "元素轉換。",
        "balance": {
            "mpCost": 100,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_elemental_change",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "stormBlaster"
        ]
    },
    "cunning_arrow": {
        "id": "cunning_arrow",
        "name": "狡詐箭矢",
        "slug": "cunning_arrow",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_trickal_arrow_basic.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks the target and nearby enemies. Power 5124.Ignores 10% of the target's de",
        "canonicalCooldown": "20 sec.",
        "canonicalCooldownMs": 20000,
        "desc": "攻擊目標與附近敵人，威力 5124。無視目標 10% 防禦力。玩家對戰／對怪物戰鬥最多攻擊 4 個目標。",
        "balance": {
            "mpCost": 40,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_cunning_arrow",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "soulRanger",
            "trickster"
        ]
    },
    "dragon_strike": {
        "id": "dragon_strike",
        "name": "龍之打擊",
        "slug": "dragon_strike",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/elemental_dragon.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Dragon Strike effect",
        "canonicalCooldown": "2 min.",
        "canonicalCooldownMs": 120000,
        "desc": "龍之打擊。",
        "balance": {
            "mpCost": 400,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_dragon_strike",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "stormBlaster"
        ]
    },
    "soul_wound": {
        "id": "soul_wound",
        "name": "靈魂創傷",
        "slug": "soul_wound",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill0792.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks the target and nearby enemies twice. Power 10,200. With a certain chance",
        "canonicalCooldown": "15 sec.",
        "canonicalCooldownMs": 15000,
        "desc": "連續攻擊目標與附近敵人 2 次，威力 10200。有一定機率附加 靈魂裂隙。無視目標 15% 防禦力。靈魂裂隙 持續 12 秒：物理攻擊／魔法攻擊 -4%、物理技能暴擊傷害 -2%、物理技能威力 -1%、弓抗性 -10%。玩家對戰／對怪物戰鬥最多攻擊 8 個目標。",
        "balance": {
            "mpCost": 30,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_soul_wound",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "soulRanger",
            "trickster"
        ]
    },
    "cunning_arrest": {
        "id": "cunning_arrest",
        "name": "狡詐拘束",
        "slug": "cunning_arrest",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_trickal_arrest.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Applies Cunning Method (triggered in 6 sec.) on the enemy. Attacks the enemy twi",
        "canonicalCooldown": "30 sec.",
        "canonicalCooldownMs": 30000,
        "desc": "對敵人附加 狡詐手段（6 秒後觸發），並連續攻擊 2 次，威力 10179。無視目標 20% 防禦力。玩家對戰／對怪物戰鬥最多攻擊 8 個目標。狡詐手段 持續 6 秒：若目標為角色／守護者／召喚獸，效果結束時攻擊敵人，威力 6000，並有一定機率觸發以下其中一種效果：牽引（40%）將敵人拉近；束縛（40%，持續 3 秒）使敵人無法移動；牽引＋束縛（20%，持續 3 秒）拉近並定身。玩家對戰／對怪物戰鬥最多攻擊 20 個目標。若目標為 非玩家角色／怪物，則不施加異常狀態，改為引發爆炸造成威力 6000 的傷害。",
        "balance": {
            "mpCost": 100,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_cunning_arrest",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "trickster"
        ]
    },
    "expert_advantage": {
        "id": "expert_advantage",
        "name": "專家優勢",
        "slug": "expert_advantage",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_tricky_advance.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Max HP +25%P. Atk. +20%Atk. Spd. +100P. Skill Power +10%Bow damage bonus +5%<Ski",
        "canonicalCooldown": ".-",
        "canonicalCooldownMs": 1000,
        "desc": "最大生命值 +25%、物理攻擊 +20%、攻速 +100、物理技能威力 +10%、弓傷害 +5%。<技能強化>狡詐射擊 → 狡詐射擊：優勢：縮短施放時間、縮短冷卻時間、增加攻擊次數。",
        "balance": {
            "mpCost": 150,
            "pwr": 3,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_expert_advantage",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "trickster"
        ]
    },
    "expert_focus": {
        "id": "expert_focus",
        "name": "專家專注",
        "slug": "expert_focus",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_tricky_focus.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "P. Atk. +1100P. Accuracy +10P. Skill Critical Rate +15%P. Skill Critical Damage ",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "物理攻擊 +1100、物理命中 +10、物理技能暴擊率 +15%、物理技能暴擊傷害 +10%、對怪物戰鬥傷害 +2%。<技能強化>狡詐投擲 → 狡詐投擲：專注：縮短施放時間，並追加 射擊支配 效果。",
        "balance": {
            "mpCost": 100,
            "pwr": 2,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_expert_focus",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "trickster"
        ]
    },
    "force_unleashed": {
        "id": "force_unleashed",
        "name": "力量解放",
        "slug": "force_unleashed",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/s_unleashed.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Absolute skillMax HP +1200; momentarily recovers HP.P./ M. Def. +1000Debuff/ Ano",
        "canonicalCooldown": "4 min.",
        "canonicalCooldownMs": 240000,
        "desc": "絕對技能：最大生命值 +1200，並立即恢復增加的 生命值；物理防禦／魔法防禦 +1000、弱化／異常抗性 +30%、技能迴避 +20%、速度 +30、最大速度 +20、受到物理技能暴擊傷害 -20%。",
        "balance": {
            "mpCost": 100,
            "pwr": 3,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_force_unleashed",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "trickster"
        ]
    },
    "expert_sense": {
        "id": "expert_sense",
        "name": "專家感知",
        "slug": "expert_sense",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_tricky_sense.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Expert Sense effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "專家感知。",
        "balance": {
            "mpCost": 50,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_expert_sense",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "trickster"
        ]
    },
    "ruse": {
        "id": "ruse",
        "name": "詭計",
        "slug": "ruse",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_deception.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Manual useSteps back from the target and applies Relocation on yourself.Auto-use",
        "canonicalCooldown": "45 sec.",
        "canonicalCooldownMs": 45000,
        "desc": "手動使用：向後跳離目標，並對自身附加 重新定位。自動使用：使用突進技能後向後跳離目標，並對自身附加 重新定位。重新定位：觸發率 50%，持續 3 秒；技能距離 +20、玩家對戰傷害 +1%。",
        "balance": {
            "mpCost": 15,
            "pwr": 5,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_ruse",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "trickster"
        ]
    },
    "ability_to_protect": {
        "id": "ability_to_protect",
        "name": "守護能力",
        "slug": "ability_to_protect",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/skill10065.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Ability to Protect effect",
        "canonicalCooldown": "1s",
        "canonicalCooldownMs": 1000,
        "desc": "守護能力。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_ability_to_protect",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 5,
        "classes": []
    },
    "ability_to_attack": {
        "id": "ability_to_attack",
        "name": "攻擊能力",
        "slug": "ability_to_attack",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/skill10069_2.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Ability to Attack effect",
        "canonicalCooldown": "1s",
        "canonicalCooldownMs": 1000,
        "desc": "攻擊能力。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_ability_to_attack",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 10,
        "classes": []
    },
    "elemental_spirit": {
        "id": "elemental_spirit",
        "name": "元素之靈",
        "slug": "elemental_spirit",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/skill30401.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Elemental Spirit effect",
        "canonicalCooldown": "1s",
        "canonicalCooldownMs": 1000,
        "desc": "元素之靈。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_elemental_spirit",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 1,
        "classes": []
    },
    "elemental_connection": {
        "id": "elemental_connection",
        "name": "元素連結",
        "slug": "elemental_connection",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/skill30400.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Elemental Connection effect",
        "canonicalCooldown": "1s",
        "canonicalCooldownMs": 1000,
        "desc": "元素連結。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_elemental_connection",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 1,
        "classes": []
    },
    "armor_mastery": {
        "id": "armor_mastery",
        "name": "護甲精通",
        "slug": "armor_mastery",
        "type": "passive",
        "rawType": "Passive",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/skill0142.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "P. Def. +10, M. Def. +20",
        "canonicalCooldown": "N/A",
        "canonicalCooldownMs": 0,
        "desc": "永久提高物理防禦 與魔法防禦。",
        "balance": {
            "mpCost": 0,
            "pwr": 0,
            "pveMultiplier": 1,
            "pvpMultiplier": 1
        },
        "vfxId": null,
        "vfxGap": false,
        "sfxId": null,
        "sfxGap": false,
        "minLevel": 1,
        "classes": [
            "darkElfFighter",
            "elfFighter",
            "fighter"
        ]
    },
    "weapon_mastery": {
        "id": "weapon_mastery",
        "name": "武器精通",
        "slug": "weapon_mastery",
        "type": "passive",
        "rawType": "Passive",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/skill0141.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "P. Atk. +10",
        "canonicalCooldown": "N/A",
        "canonicalCooldownMs": 0,
        "desc": "永久提高物理攻擊。",
        "balance": {
            "mpCost": 0,
            "pwr": 0,
            "pveMultiplier": 1,
            "pvpMultiplier": 1
        },
        "vfxId": null,
        "vfxGap": false,
        "sfxId": null,
        "sfxGap": false,
        "minLevel": 1,
        "classes": [
            "darkElfFighter",
            "dwarfFighter",
            "elfFighter",
            "fighter",
            "kamaelSoldier",
            "orcFighter"
        ]
    },
    "lord_knight": {
        "id": "lord_knight",
        "name": "領主騎士",
        "slug": "lord_knight",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/s_st_lord_knight.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Lord Knight effect",
        "canonicalCooldown": ":",
        "canonicalCooldownMs": 1000,
        "desc": "領主騎士。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_lord_knight",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 1,
        "classes": [
            "highElfBase"
        ]
    },
    "maneuver": {
        "id": "maneuver",
        "name": "機動突擊",
        "slug": "maneuver",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_crow_dash.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks the enemy. Power 6200.Ignores Shield Defense.Ignores 10% of the target's",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "攻擊敵人，威力 6200。無視盾牌防禦與目標 10% 防禦力。使用技能時對自身附加 衝刺跳躍，持續 4 秒：物理攻擊 +300、攻速 +120、速度 +20、物理命中 +10。",
        "balance": {
            "mpCost": 40,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_maneuver",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "samurai"
        ]
    },
    "destiny": {
        "id": "destiny",
        "name": "命運",
        "slug": "destiny",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/s_st_destiny.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Destiny effect",
        "canonicalCooldown": "1s",
        "canonicalCooldownMs": 1000,
        "desc": "命運。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_destiny",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 1,
        "classes": [
            "highElfBase"
        ]
    },
    "take_life": {
        "id": "take_life",
        "name": "奪取生命",
        "slug": "take_life",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_crow_strong_slash.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks the target twice. Power 7850.Ignores Shield Defense.Ignores 20% of the t",
        "canonicalCooldown": "5 sec.",
        "canonicalCooldownMs": 5000,
        "desc": "連續攻擊目標 2 次，威力 7850。無視盾牌防禦與目標 20% 防禦力。",
        "balance": {
            "mpCost": 50,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_take_life",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "samurai"
        ]
    },
    "thousand_wounds": {
        "id": "thousand_wounds",
        "name": "千重創傷",
        "slug": "thousand_wounds",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_crow_grinding.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks the enemies twice. Power 6240.Ignores Shield Defense.Ignores 15% of the ",
        "canonicalCooldown": "45 sec.",
        "canonicalCooldownMs": 45000,
        "desc": "連續攻擊敵人 2 次，威力 6240。無視盾牌防禦與目標 15% 防禦力。玩家對戰／對怪物戰鬥最多攻擊 8 個目標。",
        "balance": {
            "mpCost": 108,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_thousand_wounds",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 78,
        "classes": [
            "samurai"
        ]
    },
    "crippling_strike": {
        "id": "crippling_strike",
        "name": "致殘打擊",
        "slug": "crippling_strike",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_crow_shredding_weapon.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "PvP effectDeals P. damage to the target. Power 5740. With a certain chance, appl",
        "canonicalCooldown": "30 sec.",
        "canonicalCooldownMs": 30000,
        "desc": "玩家對戰效果：對目標造成物理傷害，威力 5740。有一定機率附加 武器破壞。無視盾牌防禦與目標 25% 防禦力。武器破壞：目標武器失去所有效果並自動卸下；修理費用為 靈魂礦石 100 個（不含護甲修理費）。此技能只能對相互敵對的角色使用。對怪物戰鬥效果：連續攻擊目標 2 次造成物理傷害，威力 5740。無視盾牌防禦與目標 25% 防禦力。",
        "balance": {
            "mpCost": 84,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_crippling_strike",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 78,
        "classes": [
            "samurai"
        ]
    },
    "twist": {
        "id": "twist",
        "name": "扭轉斬擊",
        "slug": "twist",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_crow_range_basic.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks the enemies twice. Power 5101.Ignores Shield Defense.Ignores 10% of the ",
        "canonicalCooldown": "20 sec.",
        "canonicalCooldownMs": 20000,
        "desc": "連續攻擊敵人 2 次，威力 5101。無視盾牌防禦與目標 10% 防禦力。對目標附加 弱點偵測。玩家對戰／對怪物戰鬥最多攻擊 2 個目標。弱點偵測 持續 8 秒：受到物理技能暴擊率 +20%、受到物理技能暴擊傷害 +15%、擊退／擊倒抗性 -30%。使用 扭轉斬擊／完整扭轉 時，災厄尖嘯 的攻擊次數 +1。",
        "balance": {
            "mpCost": 84,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_twist",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 77,
        "classes": [
            "samurai"
        ]
    },
    "walk_in_monster_woods": {
        "id": "walk_in_monster_woods",
        "name": "漫步魔物森林",
        "slug": "walk_in_monster_woods",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_crow_oni.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks the enemies twice. Power 6480. Applies the Scratch effect.Ignores Shield",
        "canonicalCooldown": "20 sec.",
        "canonicalCooldownMs": 20000,
        "desc": "連續攻擊敵人 2 次，威力 6480，並附加 抓傷。無視盾牌防禦與目標 15% 防禦力。抓傷 持續 5 秒：生命值 吸收率 -10%。玩家對戰／對怪物戰鬥最多攻擊 8 個目標。若目標為一般怪物，會將其拉向自己。",
        "balance": {
            "mpCost": 96,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_walk_in_monster_woods",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 78,
        "classes": [
            "samurai"
        ]
    },
    "determination": {
        "id": "determination",
        "name": "決心",
        "slug": "determination",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_crow_survival.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "P. Atk. +5%P. Skill Critical Rate +5%P. Def. +5000Debuff/ Anomaly Resistance +15",
        "canonicalCooldown": ":",
        "canonicalCooldownMs": 1000,
        "desc": "物理攻擊 +5%、物理技能暴擊率 +5%、物理防禦 +5000、弱化／異常抗性 +15%、強化肉體 5%。有 20% 機率觸發 挑釁抗性。受擊時有 10% 機率附加 血之吐息（決心 結束時一併解除）。血之吐息：持續 5 秒，冷卻 10 秒；每 2 秒恢復 10000 生命值，攻擊時吸收造成傷害的 30% 為 生命值，吸收率 80%。",
        "balance": {
            "mpCost": 15,
            "pwr": 1,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_determination",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 78,
        "classes": [
            "samurai"
        ]
    },
    "battojutsu": {
        "id": "battojutsu",
        "name": "拔刀術",
        "slug": "battojutsu",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_drawing1.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Battojutsu effect",
        "canonicalCooldown": ".-",
        "canonicalCooldownMs": 1000,
        "desc": "拔刀術。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_battojutsu",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "samurai"
        ]
    },
    "omen": {
        "id": "omen",
        "name": "徵兆",
        "slug": "omen",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_oni_is_coming1.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "You feel the presence of an alien power.Nearby enemies' CP -35%Max HP +20% (mome",
        "canonicalCooldown": "1 min.",
        "canonicalCooldownMs": 60000,
        "desc": "感受到異界力量的存在。附近敵人戰鬥點數 -35%。自身最大生命值 +20%（立即恢復增加的 生命值）、最大魔力 +20%、物理防禦 +10%、魔法防禦 +10%、物理攻擊 +30%、物理技能暴擊率 +20%、物理技能暴擊傷害 +20%、物理技能傷害 +10%、物理技能威力 +5%、擊退／擊倒成功率 +15%、弱化／異常抗性 +10%。可使用 拔刀術。",
        "balance": {
            "mpCost": 300,
            "pwr": 4,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_omen",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "samurai"
        ]
    },
    "adamant_will": {
        "id": "adamant_will",
        "name": "鋼鐵意志",
        "slug": "adamant_will",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_decision.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "P. Atk. +20%P. Skill damage bonus +10%One-handed sword damage bonus +10%Skill up",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "物理攻擊 +20%、物理技能傷害 +10%、單手劍傷害 +10%。技能強化：一閃 → 雙重閃擊。使用 雙重閃擊 時追加攻擊。三重斬擊：觸發率 40%，追加 1 次攻擊。",
        "balance": {
            "mpCost": 80,
            "pwr": 2,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_adamant_will",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "samurai"
        ]
    },
    "summon_kagami": {
        "id": "summon_kagami",
        "name": "召喚鏡靈",
        "slug": "summon_kagami",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_umemaru_mount.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Summon Kagami effect",
        "canonicalCooldown": "1s",
        "canonicalCooldownMs": 1000,
        "desc": "召喚鏡靈。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_summon_kagami",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "samurai"
        ]
    },
    "fire_sphere": {
        "id": "fire_sphere",
        "name": "火焰球",
        "slug": "fire_sphere",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/s_sp_flame_orb.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Fire Sphere effect",
        "canonicalCooldown": "1 sec.",
        "canonicalCooldownMs": 1000,
        "desc": "火焰球。",
        "balance": {
            "mpCost": 25,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_fire_sphere",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 30,
        "classes": [
            "elementWeaver",
            "elementWeaverS1",
            "elementWeaverS2"
        ]
    },
    "ice_sphere": {
        "id": "ice_sphere",
        "name": "冰霜球",
        "slug": "ice_sphere",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/s_sp_freezing_orb.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Ice Sphere effect",
        "canonicalCooldown": "1 sec.",
        "canonicalCooldownMs": 1000,
        "desc": "冰霜球。",
        "balance": {
            "mpCost": 25,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_ice_sphere",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 30,
        "classes": [
            "elementWeaver",
            "elementWeaverS1",
            "elementWeaverS2"
        ]
    },
    "sacral_strike": {
        "id": "sacral_strike",
        "name": "聖潔打擊",
        "slug": "sacral_strike",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/s_st_sacred_bash.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Sacral Strike effect",
        "canonicalCooldown": "1 sec.",
        "canonicalCooldownMs": 1000,
        "desc": "聖潔打擊。",
        "balance": {
            "mpCost": 18,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_sacral_strike",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 20,
        "classes": [
            "divineTemplar",
            "holyTemplar",
            "lightTemplar"
        ]
    },
    "bright_dance": {
        "id": "bright_dance",
        "name": "光明之舞",
        "slug": "bright_dance",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_sp_bright_dance.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Light magicA sphere containing the power of light attacks the enemy.The sphere h",
        "canonicalCooldown": "15 sec.",
        "canonicalCooldownMs": 15000,
        "desc": "光系魔法。以蘊含光之力的球體攻擊敵人，球體形態會依攻擊距離改變。連續造成 2 次魔法傷害，威力 228。若目標受到 纏繞植物 影響，則觸發 米斯特汀。米斯特汀：造成魔法傷害，威力 228。",
        "balance": {
            "mpCost": 84,
            "pwr": 228,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_bright_dance",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "elementWeaver",
            "elementWeaverS2"
        ]
    },
    "florescence": {
        "id": "florescence",
        "name": "繁花綻放",
        "slug": "florescence",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_sp_bloom.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Nature magicDeals M. damage to the target. Power 120.Plants grow on the target.A",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "自然魔法。對目標造成魔法傷害，威力 120，並使植物在目標身上生長。2 秒後 植物 轉為 纏繞植物。植物：全武器抗性 -30%、魔法防禦 -15%。纏繞植物：全武器抗性 -60%、魔法防禦 -30%，並有一定機率施加束縛。若被 纏繞植物 束縛的目標受到光系魔法攻擊，會移除 纏繞植物 並觸發 米斯特汀 造成額外傷害。米斯特汀：造成強力魔法傷害，並使全武器抗性 -60%、魔法防禦 -30%，持續 2 秒。",
        "balance": {
            "mpCost": 93,
            "pwr": 120,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_florescence",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "elementWeaver"
        ]
    },
    "nature_spirit": {
        "id": "nature_spirit",
        "name": "自然之靈",
        "slug": "nature_spirit",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_sp_natural_spirit.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Nature Spirit effect",
        "canonicalCooldown": ":",
        "canonicalCooldownMs": 1000,
        "desc": "自然之靈。",
        "balance": {
            "mpCost": 150,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_nature_spirit",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "elementWeaver"
        ]
    },
    "glacier_strike": {
        "id": "glacier_strike",
        "name": "冰河打擊",
        "slug": "glacier_strike",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/s_sp_glacial_strike.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Ice magicSummons a frosty glacier.Deals M. damage to the target and nearby enemi",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "冰系魔法。召喚冰霜冰河，對目標與附近敵人造成魔法傷害，威力 137。對目標附加 火焰弱點 與 冰冷之風。火焰弱點：降低火屬性抗性；若目標已有 冰霜弱點，則移除 冰霜弱點。冰冷之風：若目標已有 灼熱旋風，則移除 灼熱旋風 並觸發 環形爆炸。環形爆炸：造成魔法傷害，威力 137，魔法防禦 -23%。玩家對戰／對怪物戰鬥最多攻擊 8 個目標。",
        "balance": {
            "mpCost": 124,
            "pwr": 137,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_glacier_strike",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "elementWeaver"
        ]
    },
    "blazing_tempest": {
        "id": "blazing_tempest",
        "name": "烈焰暴風",
        "slug": "blazing_tempest",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/s_sp_blazing_tempest.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Fire magicSummons a blazing whirlwind.Deals M. damage to the target and nearby e",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "火系魔法。召喚燃燒旋風，對目標與附近敵人造成魔法傷害，威力 137。對目標附加 冰霜弱點 與 灼熱旋風。冰霜弱點：降低冰屬性抗性；若目標已有 火焰弱點，則移除 火焰弱點。灼熱旋風：若目標已有 冰冷之風，則移除 冰冷之風 並觸發 環形爆炸。環形爆炸：造成魔法傷害，威力 137，魔法防禦 -23%。玩家對戰／對怪物戰鬥最多攻擊 8 個目標。",
        "balance": {
            "mpCost": 124,
            "pwr": 137,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_blazing_tempest",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "elementWeaver"
        ]
    },
    "claidheamh_soluis": {
        "id": "claidheamh_soluis",
        "name": "光之劍",
        "slug": "claidheamh_soluis",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/s_sp_claiomh_solais.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Light magicSummons a shining sword that attacks the target and nearby enemies.De",
        "canonicalCooldown": "30 sec.",
        "canonicalCooldownMs": 30000,
        "desc": "光系魔法。召喚閃耀之劍攻擊目標與附近敵人，連續造成 2 次魔法傷害，威力 174。若目標受到 纏繞植物 影響，則觸發 米斯特汀。米斯特汀：造成魔法傷害，威力 174。玩家對戰／對怪物戰鬥最多攻擊 8 個目標。",
        "balance": {
            "mpCost": 106,
            "pwr": 174,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_claidheamh_soluis",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "elementWeaver"
        ]
    },
    "mount_unicorn": {
        "id": "mount_unicorn",
        "name": "騎乘獨角獸",
        "slug": "mount_unicorn",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_highelf_unicon_riding.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Mount Unicorn effect",
        "canonicalCooldown": "1s",
        "canonicalCooldownMs": 1000,
        "desc": "騎乘獨角獸。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_mount_unicorn",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [],
        "availableTo": [],
        "disabled": true,
        "removalReason": "cosmetic_mount_purge"
    },
    "sephiroth": {
        "id": "sephiroth",
        "name": "賽菲羅斯",
        "slug": "sephiroth",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/s_sp_sephiroth.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Light magicSummons the light of the universe.Deals M. damage to the target and n",
        "canonicalCooldown": "3 min.",
        "canonicalCooldownMs": 180000,
        "desc": "光系魔法。召喚宇宙之光，連續 3 次對目標與附近敵人造成魔法傷害，威力 450。若目標受到 纏繞植物 影響，則觸發 米斯特汀。米斯特汀：造成魔法傷害，威力 450。玩家對戰／對怪物戰鬥最多攻擊 16 個目標。特殊效果：對施法者附加 光之影響，持續 10 秒：魔法技能距離 +200、魔法技能威力 +5%、沉默成功率 +25%。使用魔法技能時以 強光 攻擊；若攻擊次數尚未用完，會刷新持續時間（最多攻擊 2 次）。",
        "balance": {
            "mpCost": 330,
            "pwr": 450,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_sephiroth",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "elementWeaver",
            "hierophant"
        ]
    },
    "sacral_power": {
        "id": "sacral_power",
        "name": "聖潔之力",
        "slug": "sacral_power",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill30931.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Sacral Power effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "聖潔之力。",
        "balance": {
            "mpCost": 50,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_sacral_power",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "divineTemplar",
            "holyTemplar"
        ]
    },
    "large_protection_of_light": {
        "id": "large_protection_of_light",
        "name": "大型光之守護",
        "slug": "large_protection_of_light",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/s_highelf_lv_reward3.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Large Protection of Light effect",
        "canonicalCooldown": "1",
        "canonicalCooldownMs": 1000,
        "desc": "大型光之守護。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_large_protection_of_light",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 70,
        "classes": [
            "elementWeaverS2",
            "holyTemplar"
        ]
    },
    "flying_leap": {
        "id": "flying_leap",
        "name": "飛躍突擊",
        "slug": "flying_leap",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_st_wing_leap.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "<Rush/ Knockdown skill>Encouraged by the power of light, leaps to the target.Att",
        "canonicalCooldown": "5 sec.",
        "canonicalCooldownMs": 5000,
        "desc": "<突進／擊倒技能>受到光之力量鼓舞，飛躍至目標身邊。攻擊目標與附近敵人，威力 4514。有一定機率使敵人物理防禦 -30%，並有一定機率將敵人擊倒。無視目標 10% 防禦力。玩家對戰／對怪物戰鬥最多攻擊 6 個目標。學習 神聖守護者 後：對敵人使用技能時獲得光之點數；無論戰鬥力多少，固定獲得 50 點光之點數。",
        "balance": {
            "mpCost": 84,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_flying_leap",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "divineTemplar"
        ]
    },
    "small_protection_of_light": {
        "id": "small_protection_of_light",
        "name": "小型光之守護",
        "slug": "small_protection_of_light",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/s_highelf_lv_reward1.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Small Protection of Light effect",
        "canonicalCooldown": "2",
        "canonicalCooldownMs": 1000,
        "desc": "小型光之守護。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_small_protection_of_light",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 45,
        "classes": [
            "elementWeaverS2",
            "holyTemplar",
            "lightTemplar"
        ]
    },
    "judgment": {
        "id": "judgment",
        "name": "審判",
        "slug": "judgment",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_st_justice.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Attacks enemies with the sword filled with the power of light.Deals damage twice",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "以灌注光之力量的劍攻擊敵人，連續造成 2 次傷害，威力 8124。無視目標 20% 防禦力。玩家對戰／對怪物戰鬥最多攻擊 6 個目標。學習 神聖守護者 後：對敵人使用技能時獲得光之點數。戰鬥力 1～500：600 點；500～999：750 點；1000～1499：850 點；1500～1999：1550 點；2000 以上：2300 點。",
        "balance": {
            "mpCost": 116,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_judgment",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "divineTemplar"
        ]
    },
    "protection_of_light": {
        "id": "protection_of_light",
        "name": "光之守護",
        "slug": "protection_of_light",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/s_highelf_lv_reward2.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Protection of Light effect",
        "canonicalCooldown": "2",
        "canonicalCooldownMs": 1000,
        "desc": "光之守護。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_protection_of_light",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 60,
        "classes": [
            "elementWeaverS2",
            "holyTemplar"
        ]
    },
    "noblesse_circlet": {
        "id": "noblesse_circlet",
        "name": "貴族頭冠",
        "slug": "noblesse_circlet",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_st_noblesse_oblige2.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Noblesse Circlet effect",
        "canonicalCooldown": "10 sec.",
        "canonicalCooldownMs": 10000,
        "desc": "貴族頭冠。",
        "balance": {
            "mpCost": 120,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_noblesse_circlet",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "divineTemplar"
        ]
    },
    "light_counter": {
        "id": "light_counter",
        "name": "光之反擊",
        "slug": "light_counter",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_st_light_counter.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Light Counter effect",
        "canonicalCooldown": ":",
        "canonicalCooldownMs": 1000,
        "desc": "光之反擊。",
        "balance": {
            "mpCost": 100,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_light_counter",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "divineTemplar"
        ]
    },
    "decoy": {
        "id": "decoy",
        "name": "誘餌",
        "slug": "decoy",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_st_decoy.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Decoy effect",
        "canonicalCooldown": "3 min.",
        "canonicalCooldownMs": 180000,
        "desc": "誘餌。",
        "balance": {
            "mpCost": 150,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_decoy",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "divineTemplar"
        ]
    },
    "light_discharge": {
        "id": "light_discharge",
        "name": "光能釋放",
        "slug": "light_discharge",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/s_st_light_discharge.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "The skill effect is changed based on its target.Fires an accumulated light charg",
        "canonicalCooldown": "30 sec.",
        "canonicalCooldownMs": 30000,
        "desc": "技能效果依目標類型改變，釋放累積的光能。對敵人：連續攻擊目標與附近敵人 2 次，並有一定機率使其暈眩，威力 5632。無視目標 20% 防禦力。玩家對戰／對怪物戰鬥最多攻擊 6 個目標。對盟友：依自身魔法攻擊 恢復生命值，威力 1900，最多影響 6 個目標，之後持續恢復生命值。學習 神聖守護者 後：對敵人使用技能時獲得光之點數。戰鬥力 1～500：720 點；500～999：900 點；1000～1499：1050 點；1500～1999：2050 點；2000 以上：3000 點。",
        "balance": {
            "mpCost": 103,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_light_discharge",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "divineTemplar"
        ]
    },
    "divine_guardian": {
        "id": "divine_guardian",
        "name": "神聖守護者",
        "slug": "divine_guardian",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/s_st_sacred_guardian.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Summons Divine Guardian.Divine Guardian uses skills that match Divine Templar's ",
        "canonicalCooldown": "1 sec.",
        "canonicalCooldownMs": 1000,
        "desc": "召喚 神聖守護者。神聖守護者 會使用與 神聖聖殿騎士 對應的技能：聖潔打擊 → 聖潔打擊、淨化 → 淨化。更多雙生技能資訊可查看 神聖聖殿騎士 的增益圖示。若 神聖守護者 與 神聖聖殿騎士 距離過遠，神聖守護者 會傳送至 神聖聖殿騎士 身邊。",
        "balance": {
            "mpCost": 15,
            "pwr": 20,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_divine_guardian",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "divineTemplar"
        ]
    },
    "holy_circle": {
        "id": "holy_circle",
        "name": "神聖法陣",
        "slug": "holy_circle",
        "type": "active",
        "rawType": "Ativo",
        "rarity": "4★",
        "starRank": 4,
        "icon": "/icons/s_st_sacrosanctity.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "On using the skill, its effect affects both enemies and allies.Creates a circle ",
        "canonicalCooldown": "3 min.",
        "canonicalCooldownMs": 180000,
        "desc": "使用技能時同時影響敵人與盟友，創造光之法陣。敵人：光之審判，連續攻擊附近敵人 2 次，威力 20000；10 秒內使被攻擊目標的弱化／異常抗性 -4%；無視目標 30% 防禦力。玩家對戰／對怪物戰鬥最多攻擊 16 個目標。盟友：光之守護，持續 10 秒；每 2 秒恢復 3000 生命值、受到傷害 -10%，最多影響 16 個目標。學習 神聖守護者 後：對敵人使用技能時獲得光之點數；無論戰鬥力多少，固定獲得 50000 點光之點數。",
        "balance": {
            "mpCost": 200,
            "pwr": 500,
            "pveMultiplier": 1,
            "pvpMultiplier": 0.85
        },
        "vfxId": "vfx_holy_circle",
        "vfxGap": false,
        "sfxId": "sfx_action",
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "divineTemplar",
            "phoenixKnight"
        ]
    },
    "sword_blunt_mastery": {
        "id": "sword_blunt_mastery",
        "name": "劍／鈍器精通",
        "slug": "sword_blunt_mastery",
        "type": "passive",
        "rawType": "Passive",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0257.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "P. Atk. +15%",
        "canonicalCooldown": "N/A",
        "canonicalCooldownMs": 0,
        "desc": "使用劍或鈍器時提高物理攻擊。",
        "balance": {
            "mpCost": 0,
            "pwr": 0,
            "pveMultiplier": 1,
            "pvpMultiplier": 1
        },
        "vfxId": null,
        "vfxGap": false,
        "sfxId": null,
        "sfxGap": false,
        "minLevel": 20,
        "classes": [
            "artisanDwarf",
            "deathPilgrim",
            "elvenKnight",
            "knight",
            "lightTemplar",
            "palusKnight",
            "samuraiBase",
            "shineMakerS1",
            "trooper",
            "warrior"
        ]
    },
    "dual_weapon_mastery": {
        "id": "dual_weapon_mastery",
        "name": "雙武器精通",
        "slug": "dual_weapon_mastery",
        "type": "passive",
        "rawType": "Passive",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0144.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "P. Atk. +20%",
        "canonicalCooldown": "N/A",
        "canonicalCooldownMs": 0,
        "desc": "使用雙劍或雙匕首時提高物理攻擊。",
        "balance": {
            "mpCost": 0,
            "pwr": 0,
            "pveMultiplier": 1,
            "pvpMultiplier": 1
        },
        "vfxId": null,
        "vfxGap": false,
        "sfxId": null,
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "bladeDancer",
            "gladiator"
        ]
    },
    "bow_mastery": {
        "id": "bow_mastery",
        "name": "弓精通",
        "slug": "bow_mastery",
        "type": "passive",
        "rawType": "Passive",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0208.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "P. Atk. +25%",
        "canonicalCooldown": "N/A",
        "canonicalCooldownMs": 0,
        "desc": "使用弓時提高物理攻擊。",
        "balance": {
            "mpCost": 0,
            "pwr": 0,
            "pveMultiplier": 1,
            "pvpMultiplier": 1
        },
        "vfxId": null,
        "vfxGap": false,
        "sfxId": null,
        "sfxGap": false,
        "minLevel": 20,
        "classes": [
            "elfScout",
            "hawkeye",
            "phantomRanger",
            "sylphGunner",
            "warder"
        ]
    },
    "dagger_mastery": {
        "id": "dagger_mastery",
        "name": "匕首精通",
        "slug": "dagger_mastery",
        "type": "passive",
        "rawType": "Passive",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0209.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "P. Atk. +20%, Critical Rate +10",
        "canonicalCooldown": "N/A",
        "canonicalCooldownMs": 0,
        "desc": "使用匕首時提高物理攻擊 與暴擊率。",
        "balance": {
            "mpCost": 0,
            "pwr": 0,
            "pveMultiplier": 1,
            "pvpMultiplier": 1
        },
        "vfxId": null,
        "vfxGap": false,
        "sfxId": null,
        "sfxGap": false,
        "minLevel": 20,
        "classes": [
            "assassinDE",
            "assassinS0",
            "bloodRoseBase",
            "rogue",
            "scavenger",
            "soulFinder"
        ]
    },
    "polearm_mastery": {
        "id": "polearm_mastery",
        "name": "長柄武器精通",
        "slug": "polearm_mastery",
        "type": "passive",
        "rawType": "Passive",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0216.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "P. Atk. +20%, Max targets +3",
        "canonicalCooldown": "N/A",
        "canonicalCooldownMs": 0,
        "desc": "使用長槍或長柄武器時提高物理攻擊 與攻擊目標數。",
        "balance": {
            "mpCost": 0,
            "pwr": 0,
            "pveMultiplier": 1,
            "pvpMultiplier": 1
        },
        "vfxId": null,
        "vfxGap": false,
        "sfxId": null,
        "sfxGap": false,
        "minLevel": 20,
        "classes": [
            "rider",
            "warlord"
        ]
    },
    "fist_mastery": {
        "id": "fist_mastery",
        "name": "拳套精通",
        "slug": "fist_mastery",
        "type": "passive",
        "rawType": "Passive",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0210.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "P. Atk. +20%",
        "canonicalCooldown": "N/A",
        "canonicalCooldownMs": 0,
        "desc": "使用拳套武器時提高物理攻擊。",
        "balance": {
            "mpCost": 0,
            "pwr": 0,
            "pveMultiplier": 1,
            "pvpMultiplier": 1
        },
        "vfxId": null,
        "vfxGap": false,
        "sfxId": null,
        "sfxGap": false,
        "minLevel": 20,
        "classes": [
            "marauderBase",
            "monk"
        ]
    },
    "two_handed_weapon_mastery": {
        "id": "two_handed_weapon_mastery",
        "name": "雙手武器精通",
        "slug": "two_handed_weapon_mastery",
        "type": "passive",
        "rawType": "Passive",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0293.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "P. Atk. +25%, Critical Damage +15%",
        "canonicalCooldown": "N/A",
        "canonicalCooldownMs": 0,
        "desc": "使用雙手武器時提高物理攻擊 與暴擊傷害。",
        "balance": {
            "mpCost": 0,
            "pwr": 0,
            "pveMultiplier": 1,
            "pvpMultiplier": 1
        },
        "vfxId": null,
        "vfxGap": false,
        "sfxId": null,
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "berserker",
            "deathMessenger",
            "raider"
        ]
    },
    "magic_mastery": {
        "id": "magic_mastery",
        "name": "魔法精通",
        "slug": "magic_mastery",
        "type": "passive",
        "rawType": "Passive",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0249.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "M. Atk. +20%, Magic Critical Rate +10",
        "canonicalCooldown": "N/A",
        "canonicalCooldownMs": 0,
        "desc": "提高魔法攻擊 與魔法暴擊率。",
        "balance": {
            "mpCost": 0,
            "pwr": 0,
            "pveMultiplier": 1,
            "pvpMultiplier": 1
        },
        "vfxId": null,
        "vfxGap": false,
        "sfxId": null,
        "sfxGap": false,
        "minLevel": 20,
        "classes": [
            "darkElfMage",
            "elfMage",
            "highElfBase",
            "mage",
            "orcMage",
            "sayhaMageBase"
        ]
    },
    "heavy_armor_mastery": {
        "id": "heavy_armor_mastery",
        "name": "重甲精通",
        "slug": "heavy_armor_mastery",
        "type": "passive",
        "rawType": "Passive",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0259.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "P. Def. +60, Received Critical Damage -10%",
        "canonicalCooldown": "N/A",
        "canonicalCooldownMs": 0,
        "desc": "裝備重甲時提高物理防禦。",
        "balance": {
            "mpCost": 0,
            "pwr": 0,
            "pveMultiplier": 1,
            "pvpMultiplier": 1
        },
        "vfxId": null,
        "vfxGap": false,
        "sfxId": null,
        "sfxGap": false,
        "minLevel": 20,
        "classes": [
            "deathPilgrim",
            "dwarfFighter",
            "elvenKnight",
            "knight",
            "lightTemplar",
            "orcFighter",
            "palusKnight",
            "rider",
            "warrior"
        ]
    },
    "light_armor_mastery": {
        "id": "light_armor_mastery",
        "name": "輕甲精通",
        "slug": "light_armor_mastery",
        "type": "passive",
        "rawType": "Passive",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0258.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "P. Def. +40, Evasion +6, Atk. Spd +5%",
        "canonicalCooldown": "N/A",
        "canonicalCooldownMs": 0,
        "desc": "裝備輕甲時提高物理防禦 與迴避。",
        "balance": {
            "mpCost": 0,
            "pwr": 0,
            "pveMultiplier": 1,
            "pvpMultiplier": 1
        },
        "vfxId": null,
        "vfxGap": false,
        "sfxId": null,
        "sfxGap": false,
        "minLevel": 20,
        "classes": [
            "assassinDE",
            "assassinS0",
            "bloodRoseBase",
            "elfScout",
            "kamaelSoldier",
            "marauderBase",
            "monk",
            "rogue",
            "samuraiBase",
            "scavenger",
            "shineMakerS1",
            "sylphGunner"
        ]
    },
    "robe_mastery": {
        "id": "robe_mastery",
        "name": "法袍精通",
        "slug": "robe_mastery",
        "type": "passive",
        "rawType": "Passive",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0251.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "P. Def. +30, Cast. Spd +10%",
        "canonicalCooldown": "N/A",
        "canonicalCooldownMs": 0,
        "desc": "裝備法袍時提高物理防禦 與施法速度。",
        "balance": {
            "mpCost": 0,
            "pwr": 0,
            "pveMultiplier": 1,
            "pvpMultiplier": 1
        },
        "vfxId": null,
        "vfxGap": false,
        "sfxId": null,
        "sfxGap": false,
        "minLevel": 20,
        "classes": [
            "darkElfMage",
            "elfMage",
            "highElfBase",
            "mage",
            "orcMage",
            "sayhaMageBase"
        ]
    },
    "shield_mastery": {
        "id": "shield_mastery",
        "name": "盾牌精通",
        "slug": "shield_mastery",
        "type": "passive",
        "rawType": "Passive",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0153.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Shield Def +100, Block Rate +20%",
        "canonicalCooldown": "N/A",
        "canonicalCooldownMs": 0,
        "desc": "提高盾牌防禦力與盾牌格擋率。",
        "balance": {
            "mpCost": 0,
            "pwr": 0,
            "pveMultiplier": 1,
            "pvpMultiplier": 1
        },
        "vfxId": null,
        "vfxGap": false,
        "sfxId": null,
        "sfxGap": false,
        "minLevel": 20,
        "classes": [
            "darkAvenger",
            "holyTemplar",
            "paladin",
            "shillienKnight",
            "templeKnight"
        ]
    },
    "sigil_mastery": {
        "id": "sigil_mastery",
        "name": "符印精通",
        "slug": "sigil_mastery",
        "type": "passive",
        "rawType": "Passive",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0153.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Max MP +200, M. Atk. +10%",
        "canonicalCooldown": "N/A",
        "canonicalCooldownMs": 0,
        "desc": "裝備符印時提高最大魔力 與魔法攻擊。",
        "balance": {
            "mpCost": 0,
            "pwr": 0,
            "pveMultiplier": 1,
            "pvpMultiplier": 1
        },
        "vfxId": null,
        "vfxGap": false,
        "sfxId": null,
        "sfxGap": false,
        "minLevel": 20,
        "classes": [
            "elementalSummoner",
            "phantomSummoner",
            "warlock"
        ]
    },
    "anti_magic": {
        "id": "anti_magic",
        "name": "抗魔",
        "slug": "anti_magic",
        "type": "passive",
        "rawType": "Passive",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0146.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "M. Def. +90",
        "canonicalCooldown": "N/A",
        "canonicalCooldownMs": 0,
        "desc": "提高魔法防禦 與魔法傷害抗性。",
        "balance": {
            "mpCost": 0,
            "pwr": 0,
            "pveMultiplier": 1,
            "pvpMultiplier": 1
        },
        "vfxId": null,
        "vfxGap": false,
        "sfxId": null,
        "sfxGap": false,
        "minLevel": 20,
        "classes": [
            "cleric",
            "darkWizard",
            "dragoon",
            "elementWeaverS1",
            "elfOracle",
            "elvenWizard",
            "sayhaSeer",
            "shaman",
            "shillienOracle",
            "wizard"
        ]
    },
    "magic_resistance": {
        "id": "magic_resistance",
        "name": "魔法抗性",
        "slug": "magic_resistance",
        "type": "passive",
        "rawType": "Passive",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0147.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "M. Def. +50, Magic Resistance +10%",
        "canonicalCooldown": "N/A",
        "canonicalCooldownMs": 0,
        "desc": "提高對魔法攻擊與異常狀態的抗性。",
        "balance": {
            "mpCost": 0,
            "pwr": 0,
            "pveMultiplier": 1,
            "pvpMultiplier": 1
        },
        "vfxId": null,
        "vfxGap": false,
        "sfxId": null,
        "sfxGap": false,
        "minLevel": 20,
        "classes": []
    },
    "boost_hp": {
        "id": "boost_hp",
        "name": "生命值強化",
        "slug": "boost_hp",
        "type": "passive",
        "rawType": "Passive",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/skill0211.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Max HP +500",
        "canonicalCooldown": "N/A",
        "canonicalCooldownMs": 0,
        "desc": "永久提高最大生命值。",
        "balance": {
            "mpCost": 0,
            "pwr": 0,
            "pveMultiplier": 1,
            "pvpMultiplier": 1
        },
        "vfxId": null,
        "vfxGap": false,
        "sfxId": null,
        "sfxGap": false,
        "minLevel": 20,
        "classes": [
            "artisanDwarf",
            "assassinS0",
            "deathPilgrim",
            "dragoon",
            "raider"
        ]
    },
    "boost_mana": {
        "id": "boost_mana",
        "name": "魔力強化",
        "slug": "boost_mana",
        "type": "passive",
        "rawType": "Passive",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/skill0213.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Max MP +300",
        "canonicalCooldown": "N/A",
        "canonicalCooldownMs": 0,
        "desc": "永久提高最大魔力。",
        "balance": {
            "mpCost": 0,
            "pwr": 0,
            "pveMultiplier": 1,
            "pvpMultiplier": 1
        },
        "vfxId": null,
        "vfxGap": false,
        "sfxId": null,
        "sfxGap": false,
        "minLevel": 20,
        "classes": []
    },
    "fast_hp_recovery": {
        "id": "fast_hp_recovery",
        "name": "快速生命值恢復",
        "slug": "fast_hp_recovery",
        "type": "passive",
        "rawType": "Passive",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/skill0212.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "HP Recovery Rate +15%",
        "canonicalCooldown": "N/A",
        "canonicalCooldownMs": 0,
        "desc": "提高自然生命值恢復速度。",
        "balance": {
            "mpCost": 0,
            "pwr": 0,
            "pveMultiplier": 1,
            "pvpMultiplier": 1
        },
        "vfxId": null,
        "vfxGap": false,
        "sfxId": null,
        "sfxGap": false,
        "minLevel": 20,
        "classes": []
    },
    "fast_mana_recovery": {
        "id": "fast_mana_recovery",
        "name": "快速魔力恢復",
        "slug": "fast_mana_recovery",
        "type": "passive",
        "rawType": "Passive",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/skill0229.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "MP Recovery Rate +15%",
        "canonicalCooldown": "N/A",
        "canonicalCooldownMs": 0,
        "desc": "提高自然魔力恢復速度。",
        "balance": {
            "mpCost": 0,
            "pwr": 0,
            "pveMultiplier": 1,
            "pvpMultiplier": 1
        },
        "vfxId": null,
        "vfxGap": false,
        "sfxId": null,
        "sfxGap": false,
        "minLevel": 20,
        "classes": []
    },
    "vital_force": {
        "id": "vital_force",
        "name": "生命之力",
        "slug": "vital_force",
        "type": "passive",
        "rawType": "Passive",
        "rarity": "1★",
        "starRank": 1,
        "icon": "/icons/skill0148.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "HP/MP Recovery +30%",
        "canonicalCooldown": "N/A",
        "canonicalCooldownMs": 0,
        "desc": "坐下或休息時提高 生命值 與 魔力 恢復率。",
        "balance": {
            "mpCost": 0,
            "pwr": 0,
            "pveMultiplier": 1,
            "pvpMultiplier": 1
        },
        "vfxId": null,
        "vfxGap": false,
        "sfxId": null,
        "sfxGap": false,
        "minLevel": 20,
        "classes": [
            "deathBlade"
        ]
    },
    "quick_step": {
        "id": "quick_step",
        "name": "迅捷步伐",
        "slug": "quick_step",
        "type": "passive",
        "rawType": "Passive",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0169.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Speed +15",
        "canonicalCooldown": "N/A",
        "canonicalCooldownMs": 0,
        "desc": "提高移動速度。",
        "balance": {
            "mpCost": 0,
            "pwr": 0,
            "pveMultiplier": 1,
            "pvpMultiplier": 1
        },
        "vfxId": null,
        "vfxGap": false,
        "sfxId": null,
        "sfxGap": false,
        "minLevel": 20,
        "classes": [
            "assassinS1",
            "bloodRoseS1",
            "elfScout",
            "hatamoto",
            "rogue",
            "soulFinder",
            "swordSinger",
            "warder"
        ]
    },
    "long_shot": {
        "id": "long_shot",
        "name": "遠距射擊",
        "slug": "long_shot",
        "type": "passive",
        "rawType": "Passive",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0113.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Bow Range +200",
        "canonicalCooldown": "N/A",
        "canonicalCooldownMs": 0,
        "desc": "提高弓與弩的射程。",
        "balance": {
            "mpCost": 0,
            "pwr": 0,
            "pveMultiplier": 1,
            "pvpMultiplier": 1
        },
        "vfxId": null,
        "vfxGap": false,
        "sfxId": null,
        "sfxGap": false,
        "minLevel": 20,
        "classes": [
            "hawkeye",
            "phantomRanger",
            "sharpshooter",
            "silverRanger",
            "soulRanger"
        ]
    },
    "critical_power": {
        "id": "critical_power",
        "name": "暴擊威力",
        "slug": "critical_power",
        "type": "passive",
        "rawType": "Passive",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill4085.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Critical Damage +25%",
        "canonicalCooldown": "N/A",
        "canonicalCooldownMs": 0,
        "desc": "提高物理技能與基礎攻擊的暴擊傷害。",
        "balance": {
            "mpCost": 0,
            "pwr": 0,
            "pveMultiplier": 1,
            "pvpMultiplier": 1
        },
        "vfxId": null,
        "vfxGap": false,
        "sfxId": null,
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "abyssWalker",
            "assassinS1",
            "bloodRoseS1",
            "bountyHunter",
            "deathBlade",
            "destroyer",
            "ertheiaWarrior",
            "hatamoto",
            "plainsWalker",
            "sharpshooter",
            "shineMakerS2",
            "silverRanger",
            "soulRanger",
            "treasureHunter",
            "vanguardRider"
        ]
    },
    "critical_chance": {
        "id": "critical_chance",
        "name": "暴擊機率",
        "slug": "critical_chance",
        "type": "passive",
        "rawType": "Passive",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill4086.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Critical Rate +30",
        "canonicalCooldown": "N/A",
        "canonicalCooldownMs": 0,
        "desc": "提高基礎攻擊與物理技能攻擊的暴擊率。",
        "balance": {
            "mpCost": 0,
            "pwr": 0,
            "pveMultiplier": 1,
            "pvpMultiplier": 1
        },
        "vfxId": null,
        "vfxGap": false,
        "sfxId": null,
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "adventurer",
            "assassinS2",
            "ghostHunter"
        ]
    },
    "boost_attack_speed": {
        "id": "boost_attack_speed",
        "name": "攻擊速度強化",
        "slug": "boost_attack_speed",
        "type": "passive",
        "rawType": "Passive",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0168.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Atk. Spd. +10%",
        "canonicalCooldown": "N/A",
        "canonicalCooldownMs": 0,
        "desc": "永久提高攻速。",
        "balance": {
            "mpCost": 0,
            "pwr": 0,
            "pveMultiplier": 1,
            "pvpMultiplier": 1
        },
        "vfxId": null,
        "vfxGap": false,
        "sfxId": null,
        "sfxGap": false,
        "minLevel": 20,
        "classes": [
            "dragoon",
            "marauder",
            "tyrant",
            "windSniper"
        ]
    },
    "focus_mind": {
        "id": "focus_mind",
        "name": "精神集中",
        "slug": "focus_mind",
        "type": "passive",
        "rawType": "Passive",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0191.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "MP Recovery +10 per tick",
        "canonicalCooldown": "N/A",
        "canonicalCooldownMs": 0,
        "desc": "戰鬥中持續恢復魔力。",
        "balance": {
            "mpCost": 0,
            "pwr": 0,
            "pveMultiplier": 1,
            "pvpMultiplier": 1
        },
        "vfxId": null,
        "vfxGap": false,
        "sfxId": null,
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "arcanaLord",
            "archmage",
            "cardinal",
            "dominator",
            "doomcryer",
            "elementalMaster",
            "evaSaint",
            "hierophant",
            "shillienSaint",
            "spectralMaster",
            "spellhowler",
            "spellsinger"
        ]
    },
    "spellcraft": {
        "id": "spellcraft",
        "name": "法術工藝",
        "slug": "spellcraft",
        "type": "passive",
        "rawType": "Passive",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0163.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "M. Critical Damage +20%",
        "canonicalCooldown": "N/A",
        "canonicalCooldownMs": 0,
        "desc": "提高魔法暴擊傷害並縮短冷卻時間。",
        "balance": {
            "mpCost": 0,
            "pwr": 0,
            "pveMultiplier": 1,
            "pvpMultiplier": 1
        },
        "vfxId": null,
        "vfxGap": false,
        "sfxId": null,
        "sfxGap": false,
        "minLevel": 40,
        "classes": [
            "elementWeaverS2",
            "necromancer",
            "sorcerer",
            "soulBreakerKamael",
            "spellhowler",
            "spellsinger",
            "windRiderErth"
        ]
    },
    "fast_spell_casting": {
        "id": "fast_spell_casting",
        "name": "快速施法",
        "slug": "fast_spell_casting",
        "type": "passive",
        "rawType": "Passive",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0228.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Cast. Spd. +15%",
        "canonicalCooldown": "N/A",
        "canonicalCooldownMs": 0,
        "desc": "永久提高施法速度。",
        "balance": {
            "mpCost": 0,
            "pwr": 0,
            "pveMultiplier": 1,
            "pvpMultiplier": 1
        },
        "vfxId": null,
        "vfxGap": false,
        "sfxId": null,
        "sfxGap": false,
        "minLevel": 20,
        "classes": [
            "cleric",
            "darkWizard",
            "elementWeaverS1",
            "elfOracle",
            "elvenWizard",
            "sayhaSeer",
            "shaman",
            "shillienOracle",
            "wizard"
        ]
    },
    "higher_mana_gain": {
        "id": "higher_mana_gain",
        "name": "高效魔力恢復",
        "slug": "higher_mana_gain",
        "type": "passive",
        "rawType": "Passive",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0244.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "MP Gain +20%",
        "canonicalCooldown": "N/A",
        "canonicalCooldownMs": 0,
        "desc": "提高從藥水與恢復魔法獲得的 魔力。",
        "balance": {
            "mpCost": 0,
            "pwr": 0,
            "pveMultiplier": 1,
            "pvpMultiplier": 1
        },
        "vfxId": null,
        "vfxGap": false,
        "sfxId": null,
        "sfxGap": false,
        "minLevel": 20,
        "classes": [
            "bishop",
            "elementWeaverS1",
            "elementalSummoner",
            "elfElder",
            "overlord",
            "phantomSummoner",
            "prophet",
            "shillienElder",
            "warcryer"
        ]
    },
    "master_of_combat": {
        "id": "master_of_combat",
        "name": "戰鬥大師",
        "slug": "master_of_combat",
        "type": "passive",
        "rawType": "Passive",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill0430.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "P. Atk. +150, Max HP +1000, Max CP +1500",
        "canonicalCooldown": "N/A",
        "canonicalCooldownMs": 0,
        "desc": "超越級戰鬥精通，提高物理攻擊 與最大戰鬥點數／生命值。",
        "balance": {
            "mpCost": 0,
            "pwr": 0,
            "pveMultiplier": 1,
            "pvpMultiplier": 1
        },
        "vfxId": null,
        "vfxGap": false,
        "sfxId": null,
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "deathKnight",
            "doombringer",
            "dreadnought",
            "duelist",
            "evaTemplar",
            "eviscerator",
            "fortuneSeeker",
            "grandKhavatari",
            "grandVanguard",
            "hellKnight",
            "phoenixKnight",
            "shillienTemplar",
            "shinemaker",
            "spectralDancer",
            "swordMuse",
            "titan",
            "warg"
        ]
    },
    "eye_of_slayer": {
        "id": "eye_of_slayer",
        "name": "屠戮之眼",
        "slug": "eye_of_slayer",
        "type": "passive",
        "rawType": "Passive",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill0358.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Damage to Beasts +15%",
        "canonicalCooldown": "N/A",
        "canonicalCooldownMs": 0,
        "desc": "提高對野獸與怪物造成的傷害。",
        "balance": {
            "mpCost": 0,
            "pwr": 0,
            "pveMultiplier": 1,
            "pvpMultiplier": 1
        },
        "vfxId": null,
        "vfxGap": false,
        "sfxId": null,
        "sfxGap": false,
        "minLevel": 76,
        "classes": [
            "assassinS3",
            "ghostSentinel",
            "moonlightSentinel",
            "sagittarius"
        ]
    },
    "eye_of_hunter": {
        "id": "eye_of_hunter",
        "name": "獵人之眼",
        "slug": "eye_of_hunter",
        "type": "passive",
        "rawType": "Passive",
        "rarity": "3★",
        "starRank": 3,
        "icon": "/icons/skill0359.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Damage to Insects +15%",
        "canonicalCooldown": "N/A",
        "canonicalCooldownMs": 0,
        "desc": "提高對昆蟲與植物造成的傷害。",
        "balance": {
            "mpCost": 0,
            "pwr": 0,
            "pveMultiplier": 1,
            "pvpMultiplier": 1
        },
        "vfxId": null,
        "vfxGap": false,
        "sfxId": null,
        "sfxGap": false,
        "minLevel": 76,
        "classes": []
    },
    "boost_evasion": {
        "id": "boost_evasion",
        "name": "迴避強化",
        "slug": "boost_evasion",
        "type": "passive",
        "rawType": "Passive",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0198.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Evasion +6",
        "canonicalCooldown": "N/A",
        "canonicalCooldownMs": 0,
        "desc": "永久提高物理迴避。",
        "balance": {
            "mpCost": 0,
            "pwr": 0,
            "pveMultiplier": 1,
            "pvpMultiplier": 1
        },
        "vfxId": null,
        "vfxGap": false,
        "sfxId": null,
        "sfxGap": false,
        "minLevel": 20,
        "classes": [
            "assassinS1",
            "bloodRoseS2",
            "plainsWalker",
            "ronin",
            "soulBreakerKamael",
            "vanguardRider",
            "windSniper"
        ]
    },
    "boost_morale": {
        "id": "boost_morale",
        "name": "士氣強化",
        "slug": "boost_morale",
        "type": "passive",
        "rawType": "Passive",
        "rarity": "2★",
        "starRank": 2,
        "icon": "/icons/skill0287.webp",
        "iconGap": false,
        "iconGapReason": null,
        "canonicalEffect": "Anomaly Resistance +20%",
        "canonicalCooldown": "N/A",
        "canonicalCooldownMs": 0,
        "desc": "提高對束縛、睡眠與精神異常狀態的抗性。",
        "balance": {
            "mpCost": 0,
            "pwr": 0,
            "pveMultiplier": 1,
            "pvpMultiplier": 1
        },
        "vfxId": null,
        "vfxGap": false,
        "sfxId": null,
        "sfxGap": false,
        "minLevel": 40,
        "classes": []
    },
    "legendary_archer_master": {
        "id": "legendary_archer_master",
        "name": "傳說弓手：大師",
        "slug": "legendary_archer_master",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "5★",
        "starRank": 5,
        "icon": "/icons/s_legendary_archer_hu1.webp",
        "canonicalEffect": "Transforms into a Legendary Archer Master. Max HP +10%, P. Atk. +20%, P. Skill Critical Rate +30%, P. Skill Critical Damage +30%, PvE Damage +10%, Melee Resistance +10%. Triggers Carpet Bombing & Tactical Error.",
        "canonicalCooldown": "1 min.",
        "canonicalCooldownMs": 60000,
        "desc": "變身為傳說弓手大師。最大生命值 +10%、物理攻擊 +20%、物理技能暴擊率 +30%、物理技能暴擊傷害 +30%、對怪物戰鬥傷害 +10%、近戰抗性 +10%。可使用 傳說射擊，並觸發 地毯式轟炸（範圍威力 15000）與 戰術失誤。",
        "balance": {
            "mpCost": 250,
            "pwr": 1,
            "pveMultiplier": 1.2,
            "pvpMultiplier": 1
        },
        "vfxId": "vfx_legendary_archer",
        "sfxId": "sfx_action",
        "minLevel": 90,
        "classes": [
            "ghostSentinel",
            "moonlightSentinel",
            "sagittarius",
            "trickster"
        ],
        "iconGap": false,
        "iconGapReason": null,
        "vfxGap": false,
        "sfxGap": false
    },
    "indestructible_blade_master": {
        "id": "indestructible_blade_master",
        "name": "不滅之刃：大師",
        "slug": "indestructible_blade_master",
        "type": "active",
        "rawType": "Physical Skills",
        "rarity": "5★",
        "starRank": 5,
        "icon": "/icons/unbreakable_sonic.webp",
        "canonicalEffect": "Master ultimate strike with dual swords. Ignores defense and deals colossal critical damage.",
        "canonicalCooldown": "30s",
        "canonicalCooldownMs": 30000,
        "desc": "釋放不滅雙刃的至高力量，以壓倒性威力攻擊前方敵人，無視目標 30% 防禦力。",
        "balance": {
            "mpCost": 180,
            "pwr": 250,
            "pveMultiplier": 1.3,
            "pvpMultiplier": 1
        },
        "vfxId": "vfx_indestructible_blade",
        "sfxId": "sfx_action",
        "minLevel": 90,
        "classes": [
            "duelist"
        ],
        "iconGap": false,
        "iconGapReason": null,
        "vfxGap": false,
        "sfxGap": false
    },
    "titan_champion_master": {
        "id": "titan_champion_master",
        "name": "泰坦勇士：大師",
        "slug": "titan_champion_master",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "5★",
        "starRank": 5,
        "icon": "/icons/s_champion_of_titan.webp",
        "canonicalEffect": "Master stance of the Titan Champion. Max HP +40%, P. Atk. +50%, Critical Damage +35%.",
        "canonicalCooldown": "2 min.",
        "canonicalCooldownMs": 120000,
        "desc": "喚醒遠古泰坦之怒。生命值 低於 60% 時獲得強大耐久力與無可阻擋的破壞力。",
        "balance": {
            "mpCost": 200,
            "pwr": 1,
            "pveMultiplier": 1.25,
            "pvpMultiplier": 1
        },
        "vfxId": "vfx_titan_champion",
        "sfxId": "sfx_action",
        "minLevel": 90,
        "classes": [
            "titan"
        ],
        "iconGap": false,
        "iconGapReason": null,
        "vfxGap": false,
        "sfxGap": false
    },
    "overwhelming_power_master": {
        "id": "overwhelming_power_master",
        "name": "壓倒性力量：大師",
        "slug": "overwhelming_power_master",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "5★",
        "starRank": 5,
        "icon": "/icons/skill0494.webp",
        "canonicalEffect": "Master destructive surge. P. Atk. +40%, P. Skill Power +25%.",
        "canonicalCooldown": "1 min.",
        "canonicalCooldownMs": 60000,
        "desc": "引導壓倒性的原始力量，粉碎任何防禦。",
        "balance": {
            "mpCost": 180,
            "pwr": 1,
            "pveMultiplier": 1.2,
            "pvpMultiplier": 1
        },
        "vfxId": "vfx_overwhelming_power",
        "sfxId": "sfx_action",
        "minLevel": 90,
        "classes": [
            "titan"
        ],
        "iconGap": false,
        "iconGapReason": null,
        "vfxGap": false,
        "sfxGap": false
    },
    "cacophony_of_war_master": {
        "id": "cacophony_of_war_master",
        "name": "戰爭狂響：大師",
        "slug": "cacophony_of_war_master",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "5★",
        "starRank": 5,
        "icon": "/icons/war_roar.webp",
        "canonicalEffect": "Supreme battle chant. P. Atk. +35%, Atk. Spd. +25%, P. Critical Damage +30%.",
        "canonicalCooldown": "2 min.",
        "canonicalCooldownMs": 120000,
        "desc": "釋放震耳欲聾的戰歌，激勵盟友並使所有敵人陷入恐懼。",
        "balance": {
            "mpCost": 220,
            "pwr": 1,
            "pveMultiplier": 1.25,
            "pvpMultiplier": 1
        },
        "vfxId": "vfx_cacophony_of_war",
        "sfxId": "sfx_action",
        "minLevel": 90,
        "classes": [
            "doomcryer"
        ],
        "iconGap": false,
        "iconGapReason": null,
        "vfxGap": false,
        "sfxGap": false
    },
    "mystic_meteor_master": {
        "id": "mystic_meteor_master",
        "name": "秘法隕星：大師",
        "slug": "mystic_meteor_master",
        "type": "active",
        "rawType": "Magic Skills",
        "rarity": "5★",
        "starRank": 5,
        "icon": "/icons/meteor.webp",
        "canonicalEffect": "Supreme celestial meteor. Falls from the stratosphere dealing catastrophic AoE damage.",
        "canonicalCooldown": "30s",
        "canonicalCooldownMs": 30000,
        "desc": "從高空召喚巨大的燃燒星體摧毀戰場，並留下持續 10 秒的燃燒地面。",
        "balance": {
            "mpCost": 260,
            "pwr": 300,
            "pveMultiplier": 1.35,
            "pvpMultiplier": 1
        },
        "vfxId": "vfx_meteor",
        "sfxId": "sfx_action",
        "minLevel": 90,
        "classes": [
            "archmage",
            "soultaker"
        ],
        "iconGap": false,
        "iconGapReason": null,
        "vfxGap": false,
        "sfxGap": false
    },
    "ultimate_death_knight_master": {
        "id": "ultimate_death_knight_master",
        "name": "終極死亡騎士：大師",
        "slug": "ultimate_death_knight_master",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "5★",
        "starRank": 5,
        "icon": "/icons/death_knight_transform.webp",
        "canonicalEffect": "Transcends into the Sovereign of Death. All attacks gain vampiric drain and hellfire burst.",
        "canonicalCooldown": "2 min.",
        "canonicalCooldownMs": 120000,
        "desc": "喚醒真正的深淵領主。將造成傷害的 20% 轉為 生命值，並以黑色火焰點燃敵人。",
        "balance": {
            "mpCost": 240,
            "pwr": 1,
            "pveMultiplier": 1.3,
            "pvpMultiplier": 1
        },
        "vfxId": "vfx_ultimate_death_knight",
        "sfxId": "sfx_action",
        "minLevel": 90,
        "classes": [
            "deathKnight"
        ],
        "iconGap": false,
        "iconGapReason": null,
        "vfxGap": false,
        "sfxGap": false
    },
    "holy_circle_master": {
        "id": "holy_circle_master",
        "name": "神聖法陣：大師",
        "slug": "holy_circle_master",
        "type": "active",
        "rawType": "Physical Skills",
        "rarity": "5★",
        "starRank": 5,
        "icon": "/icons/s_st_sacrosanctity.webp",
        "canonicalEffect": "Supreme holy barrier and consecrated ground. Deals holy damage and grants invulnerability.",
        "canonicalCooldown": "45s",
        "canonicalCooldownMs": 45000,
        "desc": "聖化聖騎士周圍的地面，使敵人失明並形成難以突破的神聖防線。",
        "balance": {
            "mpCost": 200,
            "pwr": 220,
            "pveMultiplier": 1.25,
            "pvpMultiplier": 1
        },
        "vfxId": "vfx_holy_circle",
        "sfxId": "sfx_action",
        "minLevel": 90,
        "classes": [
            "phoenixKnight"
        ],
        "iconGap": false,
        "iconGapReason": null,
        "vfxGap": false,
        "sfxGap": false
    },
    "condemnation_master": {
        "id": "condemnation_master",
        "name": "天譴：大師",
        "slug": "condemnation_master",
        "type": "active",
        "rawType": "Physical Skills",
        "rarity": "5★",
        "starRank": 5,
        "icon": "/icons/condemn.webp",
        "canonicalEffect": "Damns the souls of all nearby enemies with abyssal chains.",
        "canonicalCooldown": "40s",
        "canonicalCooldownMs": 40000,
        "desc": "以黑暗審判之刃重擊敵人，封鎖目標移動並汲取其生命力。",
        "balance": {
            "mpCost": 200,
            "pwr": 240,
            "pveMultiplier": 1.3,
            "pvpMultiplier": 1
        },
        "vfxId": "vfx_condemnation",
        "sfxId": "sfx_action",
        "minLevel": 90,
        "classes": [
            "hellKnight"
        ],
        "iconGap": false,
        "iconGapReason": null,
        "vfxGap": false,
        "sfxGap": false
    },
    "sephiroth_master": {
        "id": "sephiroth_master",
        "name": "賽菲羅斯：大師",
        "slug": "sephiroth_master",
        "type": "active",
        "rawType": "Magic Skills",
        "rarity": "5★",
        "starRank": 5,
        "icon": "/icons/s_sp_sephiroth.webp",
        "canonicalEffect": "Divine inquisitor strike. Deals pure sacred damage that ignores attribute defenses.",
        "canonicalCooldown": "30s",
        "canonicalCooldownMs": 30000,
        "desc": "召喚生命之樹的神聖光芒，以無法減免的神聖光輝懲擊敵人。",
        "balance": {
            "mpCost": 220,
            "pwr": 260,
            "pveMultiplier": 1.3,
            "pvpMultiplier": 1
        },
        "vfxId": "vfx_sephiroth",
        "sfxId": "sfx_action",
        "minLevel": 90,
        "classes": [
            "hierophant"
        ],
        "iconGap": false,
        "iconGapReason": null,
        "vfxGap": false,
        "sfxGap": false
    },
    "time_distortion_master": {
        "id": "time_distortion_master",
        "name": "時間扭曲：大師",
        "slug": "time_distortion_master",
        "type": "active",
        "rawType": "Special skills - Active",
        "rarity": "5★",
        "starRank": 5,
        "icon": "/icons/s_time_distortion_new.webp",
        "canonicalEffect": "Distorts the fabric of spacetime. Freezes enemy actions and resets caster cooldowns.",
        "canonicalCooldown": "1 min.",
        "canonicalCooldownMs": 60000,
        "desc": "扭曲靈魂獵犬周圍的時空，使敵人減速 80%，同時可施展瞬間傳送打擊。",
        "balance": {
            "mpCost": 250,
            "pwr": 240,
            "pveMultiplier": 1.3,
            "pvpMultiplier": 1
        },
        "vfxId": "vfx_time_distortion",
        "sfxId": "sfx_action",
        "minLevel": 90,
        "classes": [
            "soulHound"
        ],
        "iconGap": false,
        "iconGapReason": null,
        "vfxGap": false,
        "sfxGap": false
    },
    "shelter_master": {
        "id": "shelter_master",
        "name": "庇護：大師",
        "slug": "shelter_master",
        "type": "buff",
        "rawType": "Buff",
        "rarity": "5★",
        "starRank": 5,
        "icon": "/icons/shelter.webp",
        "canonicalEffect": "Absolute divine sanctuary. Complete immunity to damage and debuffs for 10 seconds.",
        "canonicalCooldown": "3 min.",
        "canonicalCooldownMs": 180000,
        "desc": "以無法破壞的神聖屏障包覆聖者，同時將所有隊伍成員 生命值 恢復至滿。",
        "balance": {
            "mpCost": 300,
            "pwr": 1,
            "pveMultiplier": 1,
            "pvpMultiplier": 1
        },
        "vfxId": "vfx_shelter",
        "sfxId": "sfx_action",
        "minLevel": 90,
        "classes": [
            "evaSaint",
            "shillienSaint"
        ],
        "iconGap": false,
        "iconGapReason": null,
        "vfxGap": false,
        "sfxGap": false
    },
    "exclusion_master": {
        "id": "exclusion_master",
        "name": "排除：大師",
        "slug": "exclusion_master",
        "type": "buff",
        "rawType": "Debuff/ Anomaly skill",
        "rarity": "5★",
        "starRank": 5,
        "icon": "/icons/expel.webp",
        "canonicalEffect": "Banishes the target to an alternate dimension, removing them from combat for 15 seconds.",
        "canonicalCooldown": "2 min.",
        "canonicalCooldownMs": 120000,
        "desc": "將目標敵人排除出戰場，送入隔離的虛空空間。",
        "balance": {
            "mpCost": 250,
            "pwr": 1,
            "pveMultiplier": 1,
            "pvpMultiplier": 1
        },
        "vfxId": "vfx_exclusion",
        "sfxId": "sfx_action",
        "minLevel": 90,
        "classes": [
            "hierophant"
        ],
        "iconGap": false,
        "iconGapReason": null,
        "vfxGap": false,
        "sfxGap": false
    },
    "dark_disruption_master": {
        "id": "dark_disruption_master",
        "name": "黑暗擾亂：大師",
        "slug": "dark_disruption_master",
        "type": "buff",
        "rawType": "Debuff/ Anomaly skill",
        "rarity": "5★",
        "starRank": 5,
        "icon": "/icons/banish.webp",
        "canonicalEffect": "Mass dark curse. Corrupts all enemy buffs and slashes P. Def and M. Def by 40%.",
        "canonicalCooldown": "1 min.",
        "canonicalCooldownMs": 60000,
        "desc": "施放席琳的大範圍擾亂，消除敵人的防禦型增益並大幅削弱防禦。",
        "balance": {
            "mpCost": 240,
            "pwr": 1,
            "pveMultiplier": 1.2,
            "pvpMultiplier": 1
        },
        "vfxId": "vfx_dark_disruption",
        "sfxId": "sfx_action",
        "minLevel": 90,
        "classes": [
            "shillienSaint"
        ],
        "iconGap": false,
        "iconGapReason": null,
        "vfxGap": false,
        "sfxGap": false
    },
    "paagrios_touch_master": {
        "id": "paagrios_touch_master",
        "name": "帕格立歐之觸：大師",
        "slug": "paagrios_touch_master",
        "type": "buff",
        "rawType": "Recovery Skills",
        "rarity": "5★",
        "starRank": 5,
        "icon": "/icons/touch_of_paagrio1.webp",
        "canonicalEffect": "Restores 100% HP and CP and grants Pa'agrio's divine flame protection.",
        "canonicalCooldown": "2 min.",
        "canonicalCooldownMs": 120000,
        "desc": "呼喚火神的至高祝福，移除所有弱化效果，並將生命值與戰鬥點數恢復至最大。",
        "balance": {
            "mpCost": 280,
            "pwr": 1,
            "pveMultiplier": 1,
            "pvpMultiplier": 1
        },
        "vfxId": "vfx_paagrios_touch",
        "sfxId": "sfx_action",
        "minLevel": 90,
        "classes": [
            "dominator"
        ],
        "iconGap": false,
        "iconGapReason": null,
        "vfxGap": false,
        "sfxGap": false
    },
    "leopold_master": {
        "id": "leopold_master",
        "name": "雷奧波德：大師",
        "slug": "leopold_master",
        "type": "active",
        "rawType": "Physical Skills",
        "rarity": "5★",
        "starRank": 5,
        "icon": "/icons/s_leopold.webp",
        "canonicalEffect": "Fires the colossal Leopold siege cannon, obliterating everything in a massive forward radius.",
        "canonicalCooldown": "45s",
        "canonicalCooldownMs": 45000,
        "desc": "部署終極矮人攻城火砲，發射高爆砲彈，對所有敵人造成威力 35000 的傷害。",
        "balance": {
            "mpCost": 250,
            "pwr": 350,
            "pveMultiplier": 1.4,
            "pvpMultiplier": 1
        },
        "vfxId": "vfx_leopold",
        "sfxId": "sfx_action",
        "minLevel": 90,
        "classes": [
            "maestro"
        ],
        "iconGap": false,
        "iconGapReason": null,
        "vfxGap": false,
        "sfxGap": false
    }
});
